...
TL;DR: V-Agent是一个用于高级视频搜索和交互式对话的多智能体平台，通过微调视觉语言模型和使用检索向量增强，克服了传统文本检索在多媒体场景中的限制。


<details>
  <summary>Details</summary>
Motivation: 传统基于文本的检索系统在多模态场景中存在局限性，无法有效处理视频中的视觉和语音内容，需要开发能够同时理解视觉和语音内容的高级视频搜索系统。

Method: 1. 使用小规模视频偏好数据集微调视觉语言模型；2. 通过图像-文本检索模型添加检索向量增强；3. 构建包含路由、搜索和聊天三个智能体的协作系统；4. 搜索智能体使用VLM检索模型和重排序模块；5. 将视频帧和ASR转录嵌入到共享多模态表示空间。

Result: 在MultiVENT 2.0基准测试中实现了最先进的零样本性能，展示了在学术研究和实际应用中的潜力。

Conclusion: V-Agent通过多智能体协作和先进的视觉语言模型，有效解决了多模态视频搜索的挑战，为高级视频搜索和交互式对话系统提供了有前景的解决方案。
...
TL;DR: 该研究提出了一种新方法，将Finer-CAM解释与TLS投影中的结构树特征片段联系起来，系统评估哪些特征驱动树种区分，在七种欧洲树种上达到96%的准确率，发现模型主要依赖树冠特征进行分类。


<details>
  <summary>Details</summary>
Motivation: 尽管新型传感器和深度学习方法在树种分类中取得了最先进的准确率，但其决策过程仍不明确。现有方法如Finer-CAM可以突出显示TLS投影中影响目标树种分类的特征，但在相似树种对比中不常见。需要改进对树种分类模型决策过程的理解，以揭示数据集和模型的局限性、偏见，并建立对模型预测的信心。

Method: 提出了一种新颖方法，将Finer-CAM解释与TLS投影中代表结构树特征的片段联系起来，系统评估哪些特征驱动树种区分。使用来自7种欧洲树种的2,445棵树的TLS数据，训练和验证了五个YOLOv8模型，采用交叉验证。分析了630个显著性图来理解模型决策过程。

Result: 模型达到平均96%的准确率（标准差0.24%）。显著性图分析显示模型主要依赖TLS投影中的树冠特征进行树种分类。这一结果在白桦、欧洲山毛榉、英国橡树和挪威云杉中尤为明显，而欧洲白蜡树、苏格兰松和花旗松的区分则更多依赖树干特征。特别是细枝的表示对模型决策有贡献。模型认为相似的树种与人类专家的判断一致。

Conclusion: 研究结果强调了改进树种分类模型决策过程理解的重要性，有助于揭示数据集和模型的局限性、偏见，并建立对模型预测的信心。提出的方法为系统评估驱动树种区分的特征提供了新途径。
...
Method: 采用电影制作式的分阶段流程：1) 大语言模型生成详细制作剧本；2) 文本到图像模型为每个角色创建一致的视觉锚点；3) 视频生成模型基于视觉锚点逐场景合成视频。

Result: 视觉锚点机制对角色一致性至关重要，移除该机制会导致角色一致性评分从7.99骤降至0.55。同时发现当前模型存在文化差异，印度与西方主题生成在主体一致性和动态程度方面表现出明显偏差。

Conclusion: 分阶段视频生成方法通过视觉锚点机制有效解决了角色一致性问题，为长视频故事生成提供了可行方案，同时揭示了当前模型存在的文化偏差需要进一步研究。
...
TL;DR: InfoTok是一个基于信息理论的自适应视频标记化框架，通过根据信息密度动态分配标记，实现更高效的视频压缩表示。


<details>
  <summary>Details</summary>
Motivation: 当前视频标记化方法对所有内容采用固定压缩率，忽略了视频固有的复杂性和可变信息密度，导致冗余或信息丢失。需要一种能够根据信息丰富度自适应分配标记的方法。

Method: 基于香农信息理论，提出InfoTok框架，证明现有数据无关训练方法在表示长度上不是最优的，并提出基于证据下界（ELBO）的新算法接近理论最优性。开发基于Transformer的自适应压缩器实现自适应标记化。

Result: 实现了最先进的压缩性能：节省20%标记而不影响性能，达到2.3倍压缩率，同时优于先前的启发式自适应方法。通过根据信息丰富度分配标记，实现了更压缩且准确的视频表示。

Conclusion: InfoTok为自适应视频标记化提供了理论框架和实用方法，通过信息理论指导的标记分配实现了更高效的视频表示，为未来研究提供了有价值的见解。
...
TL;DR: 提出LongShOTBench基准测试，用于诊断长视频多模态理解能力，包含开放式问题、多轮对话和多模态推理任务，并开发LongShOTAgent系统进行评估。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试要么关注时间长度，要么关注多模态丰富性，很少同时兼顾两者，且大多依赖单一准确率分数，掩盖了失败模式。需要更全面的评估框架来推动长视频多模态理解的发展。

Method: 1) 创建LongShOTBench基准：包含开放式意图驱动问题、单轮/多轮对话、多模态推理和工具使用任务，每个项目都有参考答案和分级评分标准；2) 开发LongShOTAgent系统：通过预处理、搜索和迭代优化分析长视频；3) 使用可扩展的人工验证流水线确保覆盖范围和可重复性。

Result: 在LongShOTBench上，最先进的多模态大模型表现不佳：Gemini-2.5-Flash达到52.95%，开源模型低于30%，LongShOTAgent达到44.66%。结果显示现实世界长视频理解的困难性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在3D结构推理和时间动态理解方面存在局限，现有3D和4D视频问答基准主要关注静态场景且缺乏区域级提示，需要更好的4D感知能力。

Method: 提出三个核心贡献：1) 4D-RGPT专门化多模态大语言模型，增强时间感知；2) P4D训练框架，从冻结专家模型蒸馏4D表示；3) R4D-Bench基准，包含深度感知动态场景和区域级提示。

Result: 4D-RGPT在现有4D视频问答基准和新提出的R4D-Bench基准上都取得了显著改进。

Conclusion: 通过专门设计的模型架构、训练框架和评估基准，成功提升了多模态大语言模型在4D感知和推理方面的能力，解决了现有方法的局限性。
...
TL;DR: FORMSpoT是一个基于SPOT卫星时间序列的法国森林监测系统，提供1.5米分辨率的树冠高度图和年度扰动多边形，显著优于现有产品，特别适合检测小尺度森林变化。
...
<details>
  <summary>Details</summary>
Motivation: 现有相机控制视频生成方法存在两个主要问题：基于重投影的方法容易受深度估计误差影响，以及现有数据集相机轨迹多样性有限，导致生成的视频在相机姿态保真度和质量方面表现不佳。

Method: InfCam包含两个关键组件：1）无限单应性扭曲，直接在视频扩散模型的2D潜在空间中编码3D相机旋转；2）数据增强流程，将现有合成多视角数据集转换为具有多样轨迹和焦距的序列。

Result: 实验结果表明，InfCam在相机姿态准确性和视觉保真度方面优于基线方法，并且能够很好地从合成数据泛化到真实世界数据。

Conclusion: InfCam通过深度无关的方法和数据增强策略，解决了相机控制视频生成中的姿态保真度和数据多样性问题，为动态场景的新视角视频生成提供了有效的解决方案。
...
TL;DR: 提出一种可解释的效用相似性(IUS)度量方法，用于评估合成医学图像数据集与真实数据集在深度学习临床决策支持系统中的效用相似性，相比传统方法更具解释性且能提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 当前评估合成医学图像与真实图像相似性的方法主要依赖用户评估、基于Inception的度量和分类性能，缺乏可解释性且无法解释为什么一个合成数据集比另一个更有用。需要一种能够量化评估合成图像集在深度学习临床决策支持系统中实用性的可解释度量方法。

Method: 提出基于广义神经可加模型的可解释效用相似性(IUS)度量方法，通过临床相关图像特征来解释合成数据集在临床决策支持系统中的效用。该方法能够量化合成图像集与真实图像集在深度学习模型训练中的效用相似性。

Result: 在多种彩色医学成像模态（内窥镜、皮肤镜、眼底成像）的公开数据集上实验表明，使用IUS选择高效用相似性的合成图像可以使分类性能相对提升高达54.6%。在灰度X射线和超声成像模态上也验证了IUS的通用性。

Conclusion: IUS提供了一种可解释的方法来评估合成医学图像数据集在深度学习临床决策支持系统中的效用相似性，能够有效指导合成数据选择并提升分类性能，为合成医学图像质量评估提供了新的量化工具。
...
Method: 1) 从粗到细的模型学习时间一致的头发运动动态；2) 发丝引导的优化模块学习动态3D高斯表示，支持可微分渲染

Result: DGH在几何和外观上取得良好效果，能泛化到不同发型和头部运动序列，可集成到3D高斯化身框架中

Conclusion: 提供了一种可扩展的数据驱动替代方案，相比传统物理模拟和渲染方法更具优势
...
TL;DR: 本文系统综述了海事雷达帧预测的建模方法，重点分析了基于transformer架构的时空序列预测技术，发现虽然已有transformer用于声纳帧预测的研究，但海事雷达帧预测领域仍存在明显研究空白。


<details>
  <summary>Details</summary>
Motivation: 海事自主系统需要强大的预测能力来预判船舶运动和动态环境。虽然transformer架构已革新了基于AIS的轨迹预测并展示了声纳帧预测的可行性，但将其应用于海事雷达帧预测仍未被探索，考虑到雷达在导航中的全天候可靠性，这形成了一个关键的研究空白。

Method: 本文采用系统综述方法，分析海事雷达预测建模的相关方法，重点关注基于transformer架构的时空序列预测技术。按照数据类型、架构和预测时间范围对现有代表性方法进行分类分析。

Result: 综述显示，虽然文献已展示了基于transformer的声纳传感帧预测，但尚无先前工作涉及基于transformer的海事雷达帧预测，从而明确了具体的研究空白。

Conclusion: 本文确定了海事雷达帧预测领域存在明确的研究缺口，为未来研究提供了具体的研究方向，即开发基于transformer架构的海事雷达帧预测方法。
...
TL;DR: PathBench-MIL是一个用于组织病理学中多实例学习的开源AutoML和基准测试框架，自动化端到端MIL流程构建并提供可复现的模型评估


<details>
  <summary>Details</summary>
Motivation: 解决组织病理学中多实例学习流程构建复杂、缺乏标准化基准测试工具的问题，为研究人员提供统一的实验平台

Method: 开发开源AutoML框架，自动化预处理、特征提取和MIL聚合流程，集成可视化工具、统一配置系统和模块化扩展架构

Result: 成功构建了可复现的基准测试框架，支持数十种MIL模型和特征提取器的评估，提供快速实验和跨数据集标准化能力

Conclusion: PathBench-MIL为组织病理学MIL研究提供了强大的开源工具，促进了实验标准化和可复现性，加速该领域的研究进展
...
TL;DR: 该论文提出了一种将普通人物照片转换为专业肖像照的方法，通过UV空间转换和重姿态技术，在保持人物身份特征的同时提升照片质量。


<details>
  <summary>Details</summary>
Motivation: 专业摄影师拍摄的人物照片通常具有优美的光线、有趣的姿态和高质量效果，而普通人自拍的照片则达不到这种水平。现有的挑战在于缺乏同一人物在"野外"拍摄和专业拍摄的配对数据集。
...
Conclusion: 提出的方法成功解决了在缺乏配对数据的情况下，将普通人物照片转换为专业肖像照的问题，通过UV空间操作和多图像微调实现了身份保持和高质量输出。
...
TL;DR: 提出文档中心背景生成框架，支持多页面编辑和主题连续性，通过潜在掩码保持文本可读性，自动优化可读性，确保多页面一致性


<details>
  <summary>Details</summary>
Motivation: 解决文档背景生成中保持文本可读性、多页面主题一致性的问题，将生成模型与自然设计工作流程相结合

Method: 采用潜在掩码公式在扩散空间中软衰减更新；引入自动可读性优化（ARO）放置半透明背景形状；通过摘要-指令过程保持多页面一致性；将文档视为结构化组合

Result: 训练免费框架产生视觉连贯、文本保留和主题对齐的文档，在保持可读性的同时实现美学和谐

Conclusion: 该框架成功桥接了生成建模与自然设计工作流程，实现了文档背景的自动化生成与编辑，同时确保文本可读性和多页面主题连续性
...
<details>
  <summary>Details</summary>
Motivation: 当前火灾预测方法主要局限于二元掩模建模，信号稀疏且无法捕捉火灾复杂动态；而世界模型在视频生成中表现出潜力，但其物理不一致性对火灾预测构成挑战。

Method: 提出PhysFire-WM模型：1）从物理模拟器编码结构化先验来内化燃烧动力学并纠正物理差异；2）采用跨任务协作训练策略（CC-Train），通过参数共享和梯度协调整合热辐射动态和空间边界信息。

Result: 在细粒度多模态火灾数据集上的广泛实验表明，PhysFire-WM在火灾蔓延预测方面具有优越的准确性。
...
TL;DR: LGCLIP是一种无需人工标注图像-文本对的轻量级视觉语言模型，利用大语言模型生成类别提示，指导扩散模型合成参考图像作为视觉原型，仅需类别标签即可实现零样本图像分类。
...
Conclusion: LGCLIP通过大语言模型生成和扩散模型合成，仅需类别标签即可实现零样本图像分类，无需人工标注图像-文本对，提供了一种轻量高效的视觉语言模型新范式。
...
<details>
  <summary>Details</summary>
Motivation: CLIP在组合式图像-文本匹配任务中表现不佳，特别是在准确关联对象与其属性方面，因为其固有的全局表示往往忽略了细粒度语义。现有方法需要额外训练或大量负采样，但对新组合概念的泛化能力有限，且未能从根本上解决全局表示的缺陷。
...
### [18] [Adversarial Robustness of Vision in Open Foundation Models](https://arxiv.org/abs/2512.17902)
...
<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型复杂度增加，理解AI系统识别物体的机制变得困难，攻击者可能通过添加不可见元素来混淆AI识别。本研究旨在评估当代视觉语言模型在对抗攻击下的鲁棒性，特别是针对视觉输入模态的攻击。

Method: 研究采用无目标PGD（投影梯度下降）攻击针对视觉输入模态，在VQA v2数据集子集上对LLaVA-1.5-13B和Meta Llama 3.2 Vision-8B-2进行实证评估。使用标准VQA准确率指标量化攻击效果，并比较两种模型的准确率下降情况。

Result: Llama 3.2 Vision虽然在此设置下基准准确率较低，但在攻击下表现出更小的性能下降，特别是在高扰动水平下。视觉模态被证实是降低当代开放权重视觉语言模型性能的可行攻击向量。
...
TL;DR: 研究量化了城市绿化客观测量与主观感知之间的差异，发现这种差异在全球范围内具有可比性，且人口统计和人格因素对感知影响不大，而居住地是解释感知差异的最重要因素。


<details>
  <summary>Details</summary>
Motivation: 城市绿化评估通常分为客观测量（如植被数量）和主观感知（如人们感受）两种方法，但两者可能存在差异。本研究旨在测量这种差异并通过人类、地理和空间维度来解释这些差异。

Method: 研究基于街景图像提取的上下文信息，以及来自5个国家1000人的全面城市视觉感知调查数据（包含详细的人口统计和人格信息）。分析客观测量（如绿色视野指数GVI）与主观评分（如成对评分）之间的差异，检验这些差异是否能通过年龄组、场景中绿化的空间变化等多种人类和视觉因素来解释。

Result: 研究发现：1）客观与主观绿化评估之间的差异在全球范围内具有可比性；2）人口统计和人格因素在感知中不起显著作用；3）感知和测量的绿化在地理上具有一致性相关性；4）居住地是解释感知差异的最重要因素（在七个影响因素中排名前二），表明文化、环境和经验因素显著影响个体对城市绿化的观察方式。

Conclusion: 城市绿化感知差异主要受居住地相关因素（文化、环境、经验）的影响，而非人口统计或人格特征。这一发现对城市规划具有重要意义，强调了考虑当地背景和文化因素在绿化评估中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 移动设备（如自动驾驶汽车）配备多相机系统和惯性测量单元（IMU）的应用日益广泛，视觉和惯性信息的相对位姿估计在多个领域具有重要应用价值。现有方法在精度方面有待提升。

Method: 1. 在解耦旋转矩阵和平移向量后，基于仿射对应的几何约束建立关于相对旋转角的成本函数；2. 将全局优化问题转化为基于特征方程及其一阶导数为零的两个二元多项式；3. 使用多项式特征值求解器求解相对旋转角，从特征向量获取平移向量；4. 针对小相对旋转情况提出新的线性解。

Result: 在合成数据和真实世界数据集上的实验结果表明，该方法在精度方面优于可比的最新方法。
...
Method: 提出解剖区域引导对比解码（ARCD），利用解剖掩码指导三层对比解码过程：在token、注意力和logits层面动态重新加权，可验证地将模型注意力引导到指定区域，增强解剖理解并抑制事实错误输出。

Result: 在胸部X光、CT、脑部MRI和眼部超声等多种数据集上的广泛实验表明，该方法能有效改善区域理解、减少幻觉并提高整体诊断准确性。

Conclusion: ARCD作为一种即插即用策略，通过提供有针对性的区域特定指导来缓解医学视觉语言模型的幻觉问题，在复杂临床环境中表现出可靠效果。
...
TL;DR: 本文提出Reasoning Palette框架，通过引入随机潜变量来指导大语言模型的推理策略，在生成前进行内部规划，从而提升探索能力和推理性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的探索能力限制了其在推理时的表现和强化学习训练效果，随机采样往往产生冗余的推理路径而缺乏高层次多样性。

Method: 提出Reasoning Palette框架，使用变分自编码器从问题-答案对的平均池化嵌入中推断潜变量，每个采样的潜变量编码不同的推理上下文。在推理时，潜变量被解码为可学习的token前缀并附加到输入提示前，调制模型的内部推理轨迹。

Result: 在多个推理基准测试中，该方法实现了对（视觉）语言模型战略行为的可解释和可控控制，相比标准强化学习方法获得了持续的性能提升。

Conclusion: Reasoning Palette框架通过潜变量调制实现了结构化的探索，显著提升了探索效率和持续学习能力，为大语言模型的推理策略控制提供了有效方法。
...
Method: 提出CheXPO-v2对齐框架，核心创新是知识图谱一致性奖励机制，通过实体关系匹配驱动。该方法将推理步骤明确解析为结构化的"疾病、关系、解剖"三元组，提供细粒度监督，在原子级别惩罚不连贯逻辑和幻觉。结合硬样本挖掘策略，显著提升性能。

Result: CheXPO-v2在MIMIC-CXR-VQA等基准测试中显著优于GRPO和最先进模型。仅使用5k样本就达到了新的最先进准确率，展示了卓越的数据效率，同时产生临床可靠且可验证的推理。

Conclusion: CheXPO-v2通过过程监督和知识图谱一致性奖励机制，有效解决了医学视觉语言模型的幻觉问题，提高了临床可靠性和推理可验证性，为医学AI安全提供了重要解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感基础模型通常基于固定的波段配置和分辨率进行预训练，在实际应用中面临波段缺失、跨传感器融合和未知空间尺度等挑战，限制了模型的泛化能力和实际部署。

Method: 1. 频谱独立标记器：为每个通道分配专用波段嵌入，显式编码光谱身份；2. 多尺度自适应补丁嵌入机制：动态调节感受野；3. 多尺度语义对齐机制：保持跨分辨率全局语义一致性；4. 通道级自监督掩码重建预训练策略：联合建模光谱-空间关系。

Result: 在超过10个公共数据集（包括Sentinel-2、Landsat和HLS）上的广泛实验表明，AOM在波段缺失、跨传感器和跨分辨率等挑战性条件下始终达到最先进的性能。

Conclusion: AOM通过创新的频谱独立标记器、多尺度自适应嵌入和语义对齐机制，成功解决了遥感基础模型在实际应用中的泛化问题，为处理任意波段组合和分辨率尺度的遥感任务提供了通用解决方案。
...
TL;DR: 提出了一种基于视觉相似性和几何结构一致性的全局描述符学习方法，通过聚合模块纠正重叠分数不可靠导致的错误关联，无需人工标注即可训练，在大规模环境中显著提升定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的视觉定位方法通常仅从几何线索（如共视图）推导全局描述符，限制了其判别能力，且在存在噪声几何约束时鲁棒性降低。需要一种能同时考虑几何结构和视觉相似性的全局描述符学习方法。

Method: 提出聚合模块，学习与几何结构和视觉相似性都一致的全局描述符，确保图像在描述符空间中接近仅当它们视觉相似且空间连接。使用基于重叠分数的批挖掘策略和修改的对比损失，无需人工位置标签即可训练。

Result: 在具有挑战性的基准测试中，该方法在大规模环境中实现了显著的定位性能提升，同时保持了计算和内存效率。

Conclusion: 通过同时考虑几何结构和视觉相似性学习全局描述符，能够纠正不可靠重叠分数导致的错误关联，提高视觉定位的鲁棒性和准确性，且无需人工标注即可跨不同环境泛化。
...
TL;DR: 该论文提出了一种课程学习框架来解决多模态大语言模型在长链视觉推理任务中的"视觉遗忘"问题，通过解耦抽象逻辑推理和策略性视觉感知两种认知技能，并引入强化学习来学习何时进行视觉感知。
...
Method: 提出两阶段课程学习框架：1）解耦的监督微调课程，先在纯文本数据上构建稳健的抽象推理骨干，然后通过感知基础思维链范式将其锚定到视觉；2）将感知时机制定为强化学习问题，设计关键感知奖励，通过将感知动作与认知不确定性语言标记（如"等待"、"验证"）耦合，教会模型何时进行视觉感知。

Result: 该方法将模型从启发式驱动的观察者转变为策略性的、有基础的推理者，解决了视觉遗忘问题，提升了模型在复杂长链视觉推理任务中的性能。

Conclusion: 通过正式化两种认知缺陷并开发原则性的两阶段框架来解决它们，该研究为多模态大语言模型提供了从启发式观察到策略性基础推理的转变路径，显著改善了模型在复杂视觉推理任务中的表现。
...
TL;DR: VideoDetective：一种用于长视频问答的高效问题感知记忆机制，通过迭代处理视频子片段，使用特殊记忆令牌进行有目的压缩，使MLLMs能够从海量信息中有效寻找关键线索。


<details>
  <summary>Details</summary>
Motivation: 长视频问答对多模态大语言模型构成重大挑战，因为巨大的上下文和过载信息会导致内存消耗过高。现有方法通过减少视觉令牌或扩展上下文长度，可能丢失有用信息或需要大量计算。实际上，回答问题时只需要少量关键信息。

Method: 提出VideoDetective方法：1）迭代处理视频子片段；2）每个子片段使用问题感知压缩策略，引入特殊记忆令牌进行有目的压缩；3）递归聚合和存储这些记忆令牌以更新历史上下文，供后续子片段重用。

Result: 实验表明，该方法使上下文长度仅为32K的MLLMs能够高效处理100K令牌（3600帧，1小时视频以1fps采样），仅需2分钟和37GB GPU内存。在多个长视频基准测试中，该方法能更有效地从海量信息中寻找关键线索。

Conclusion: VideoDetective通过问题感知记忆机制，使MLLMs能够高效处理长视频问答任务，在有限上下文长度下处理大量视觉信息，同时保持对关键线索的敏感性和准确性。
...
Method: 基于预训练视频扩散模型构建的Diffusion Transformer，通过视频上下文学习实现端到端Human2Robot视频生成。演示视频被压缩为条件token，在扩散过程中通过双向注意力与机器人去噪token融合。为解决配对数据稀缺问题，开发了从大型第一人称数据集自动合成高质量人机配对数据的流程。

Result: 在Human2Robot和EPIC-Kitchens数据集上的实验表明，Mitty取得了最先进的结果，对未见环境表现出强大的泛化能力，并为从人类观察中实现可扩展机器人学习提供了新的见解。

Conclusion: Mitty通过端到端的人类到机器人视频生成，避免了中间表示的信息损失，利用强大的视觉-时间先验，为从人类演示中学习机器人技能提供了有前景的方法。
...
TL;DR: AnyCXR是一个仅使用合成监督就能在任意胸部X光投影角度实现多器官分割的统一框架，通过多阶段域随机化生成合成数据，结合条件联合标注正则化学习策略，在真实数据集上实现零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 胸部X光解剖分割面临两大挑战：1）全面标注数据稀缺；2）真实世界采集条件变化大。需要开发一种能够减少标注负担、提高跨不同成像条件鲁棒性的解决方案。

Method: 提出AnyCXR框架，包含两个核心组件：1）多阶段域随机化引擎，从3D CT体积生成超过10万张解剖学上忠实且高度多样化的合成X光片；2）条件联合标注正则化学习策略，通过强制潜在空间中的解剖一致性来利用部分和不完美的标签。
...
Conclusion: AnyCXR为解剖感知的胸部X光分析建立了可扩展且可靠的基础，提供了一条减少标注负担、提高跨不同成像条件鲁棒性的实用途径。
...
TL;DR: RadImageNet-VQA是一个大规模放射学视觉问答数据集，包含75万张CT/MRI图像和750万个问答样本，涵盖异常检测、解剖识别和病理识别三大任务，旨在解决现有医学VQA数据集规模小、偏向X射线图像和存在文本捷径的问题。
...
Method: 构建了大规模放射学VQA数据集RadImageNet-VQA，包含专家标注的75万张CT和MRI图像，配以750万个问答样本，涵盖8个解剖区域和97种病理类别，支持开放式、封闭式和多项选择题型。

Result: 实验表明，最先进的视觉语言模型在细粒度病理识别任务上表现不佳，特别是在开放式设置中，即使经过微调也难以达到理想效果。纯文本分析显示，没有图像输入时模型性能接近随机水平，证明数据集不存在语言捷径。

Conclusion: RadImageNet-VQA是一个无语言捷径的大规模放射学VQA数据集，揭示了当前模型在细粒度病理识别方面的局限性，为医学视觉问答研究提供了重要基准。
...
Method: 提出两阶段VLMIR框架：1）VLM特征提取阶段，通过LoRA微调对齐低质量/高质量图像描述的嵌入，使用退化预测器分解退化和干净图像内容嵌入；2）扩散模型修复阶段，通过交叉注意力机制整合视觉和文本嵌入。

Result: 大量实验和消融研究表明，VLMIR在通用和特定退化图像修复任务中均取得优越性能，证明了VLM中视觉和语言知识整合对提升图像修复能力的关键作用。

Conclusion: VLMIR框架通过有效利用视觉语言模型的视觉-语言先验知识，显著提升了图像修复的语义一致性和细节恢复能力，为图像修复领域提供了新的有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: PCB组装件的自动缺陷检测面临三大挑战：1）标记数据不足；2）缺陷微小（仅几个像素）；3）图像视觉复杂且分辨率高。现有方法难以在4K分辨率图像上实现准确的像素级缺陷定位。

Method: 提出HiSIR-Net框架，包含两个轻量级模块：1）选择性输入重建门（SIR-Gate），让模型决定在何处信任重建结果还是原始输入，减少无关重建伪影和误报；2）区域级优化补丁选择（ROPS）方案，利用位置线索选择重叠补丁重建，确保在不同分辨率下的连贯性。

Result: 在自收集的SIPCBA-500数据集和公共基准测试上进行广泛实验，证明了该方法在定位性能上的优越性，同时保持实用运行速度。能够生成干净的高分辨率异常图，误报率低。

Conclusion: HiSIR-Net通过创新的自监督重建框架，有效解决了高分辨率PCB组装件缺陷检测的挑战，为工业检测提供了实用的解决方案，并贡献了高质量的数据集资源。
...
TL;DR: ProCache：一种无需训练的动态特征缓存框架，通过非均匀缓存模式和选择性计算解决DiT加速中的误差累积问题，实现近2-3倍加速且质量损失可忽略。
...
Result: 在PixArt-alpha和DiT上的广泛实验表明，ProCache实现最高1.96倍和2.90倍加速，质量退化可忽略，显著优于先前基于缓存的方法。

Conclusion: ProCache通过分析DiT特征在去噪过程中的演变，揭示了特征变化和误差传播的高度时间和深度变化性，并据此设计了动态特征缓存框架，有效解决了现有缓存方法的局限性，为DiT实时部署提供了高效解决方案。
...
TL;DR: RSHR-Bench是一个超高分辨率遥感视觉理解基准，包含5,329张长边至少4000像素的全场景图像，设计了四种任务类型来评估多模态大语言模型在超高分辨率场景下的视觉理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有遥感基准大多依赖低分辨率图像，而一些高分辨率基准存在推理任务设计缺陷。研究发现纯文本LLM无需访问图像就能在遥感推理任务上与多模态视觉语言模型竞争，揭示了当前基准与视觉理解评估意图之间的严重不匹配。

Method: 从广泛使用的遥感语料库和无人机采集数据中收集5,329张超高分辨率全场景图像（长边≥4000像素，每张图像最多约3×10^8像素）。设计四种任务家族：多项选择VQA、开放式VQA、图像描述和单图像评估，涵盖九个感知类别和四种推理类型。采用强LLM进行对抗性过滤，然后进行严格的人工验证以减少语言先验依赖。

Result: 构建了3,864个VQA任务、3,913个图像描述任务和500个完全人工编写或验证的单图像评估VQA对。对开源、闭源和遥感专用VLM的评估显示，在超高分辨率场景下存在持续的性能差距。

Conclusion: RSHR-Bench为遥感视觉理解提供了可靠的评估基准，揭示了当前多模态大语言模型在超高分辨率场景下的局限性，为未来研究提供了重要的评估工具。
...
TL;DR: EMAG是一种无需训练的注意力修改机制，通过指数移动平均生成更难的负样本，提升扩散模型的生成质量和人类偏好评分
...
Method: 提出指数移动平均引导（EMAG），在推理时修改扩散变换器中的注意力机制，使用基于统计的自适应层选择规则，生成更难的语义忠实负样本

Result: EMAG相比CFG提升人类偏好评分（HPS）+0.46，能发现困难的失败模式，让去噪器能细化细微伪影，且能与APG、CADS等先进引导技术自然组合进一步提升HPS

Conclusion: EMAG通过生成更难的负样本和自适应层选择，有效提升了扩散模型的生成质量和控制能力，为引导技术提供了新的改进方向
...
TL;DR: DRIM模型通过深度可靠的多轮推理，在视觉语言模型中实现自我反思和纠错能力，提升复杂视觉任务的解决能力。
...
Method: 采用三阶段流程：1) 基于高分辨率图像构建高难度可验证的视觉问答对；2) 冷启动SFT阶段收集工具轨迹；3) RL阶段引入冗余惩罚策略优化，鼓励多尺度探索和自我反思。

Result: DRIM在视觉理解基准测试中表现出优越性能，验证了其深度可靠多轮推理的有效性。

Conclusion: DRIM通过自我反思的多轮推理机制，显著提升了视觉语言模型在复杂任务中的可靠性和准确性。
...
TL;DR: CodeDance使用可执行代码作为视觉推理的通用求解器，通过代码定义、组合和执行来协调多个工具，实现透明、可自检的推理，在多个基准测试中超越了GPT-4o等先进模型。


<details>
  <summary>Details</summary>
Motivation: 当前开源方法主要依赖纯文本链、固定视觉模式或单步流程，限制了在复杂任务上的灵活性、可解释性和可迁移性。需要一种更通用的视觉推理方法。

Method: 引入CodeDance，使用可执行代码作为通用求解器，通过代码定义、组合和执行来协调多个工具，计算中间结果，渲染视觉元素。引入平衡自适应工具调用的奖励机制，平衡探索与效率。

Result: 在RL训练中观察到新颖的涌现行为：未见过的工具调用、组合和跨任务迁移。在视觉搜索、数学、图表问答等基准测试中，不仅优于模式驱动和纯文本基线，还超越了GPT-4o和更大的开源模型。

Conclusion: CodeDance展示了可执行代码作为视觉推理通用求解器的有效性，无需任务特定微调即可实现新颖的涌现行为和跨任务迁移，提供了一种通用且可扩展的视觉推理机制。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在分布偏移的下游任务中表现不佳，现有的参数高效微调方法受限于固定的人工提示，而图像诱导提示方法又带来过高计算开销

Method: 提出辅助描述知识框架：1) 利用大语言模型离线生成丰富的类别描述提示；2) 作为组合知识提供平均表示；3) 通过轻量非参数注意力机制动态选择实例相关描述

Result: ADK显著提升了多种PEFT基线的性能，在各种场景下达到了新的最先进水平

Conclusion: ADK作为参数免费的即插即用组件，有效丰富了文本表示，促进了跨域类别区分，同时保持了推理效率
...
TL;DR: InsertAnywhere是一个新的视频对象插入框架，通过4D感知掩码生成和扩散模型联合合成，实现几何一致的对象放置和外观逼真的视频合成，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 尽管基于扩散的视频生成技术取得了进展，但真实的视频对象插入仍然面临挑战，主要原因是有限的4D场景理解以及对遮挡和光照效果处理不足。
...
Conclusion: InsertAnywhere框架通过结合4D场景理解和扩散模型合成，成功解决了视频对象插入中的几何一致性和外观逼真性问题，为可控视频编辑提供了新的可能性。
...
TL;DR: EMMA是一个评估概念擦除技术的新基准，测试五个关键维度和12个指标，发现现有方法在间接提示和视觉相似概念方面表现不佳，某些方法甚至加剧了性别和种族偏见。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成的广泛应用引发了隐私、偏见和版权问题，概念擦除技术提供了有前景的解决方案，但现有评估方法有限，主要依赖简单直接的提示，无法全面测试概念是否真正从模型表示中移除。

Method: 提出EMMA基准，评估概念擦除技术的五个关键维度：1) 间接描述下的鲁棒性 2) 视觉相似非目标概念的区分能力 3) 性别和种族偏见分析 4) 图像质量 5) 时间效率。包含12个指标，涵盖五个领域：物体、名人、艺术风格、NSFW内容和版权材料。

Result: 对五种概念擦除方法的分析显示：现有方法在处理间接提示（间接引用擦除概念时仍能生成）和视觉相似非目标概念（无法生成与擦除概念相似的非目标概念）方面存在困难；某些方法相比原始模型反而加剧了性别和种族偏见。

Conclusion: 概念擦除技术需要更全面的评估框架，EMMA基准揭示了现有方法的局限性，特别是在处理间接描述和视觉相似概念时的不足，以及潜在的偏见放大问题，为未来改进提供了重要方向。
...
Method: 提出Robust-R1框架：1）监督微调建立退化感知推理基础；2）奖励驱动对齐准确感知退化参数；3）动态推理深度缩放适应退化强度。引入包含11K真实退化数据的专用数据集，涵盖四个关键视觉处理阶段
...
Method: 从纵向鹿特丹研究中采样彩色眼底图像，涵盖不同年龄、设备和拍摄条件。使用自定义标注界面，让标注者在初始血管分割掩码基础上，在独立图层上标注动脉、静脉和未知血管，并通过连通分量可视化工具验证和校正连通性。

Result: 数据集包含1024x1024像素PNG图像，有三种模态：原始RGB眼底图像、对比度增强版本和RGB编码的动静脉掩码。图像质量差异大，包含通常被自动质量评估系统排除但具有宝贵血管信息的挑战性样本。

Conclusion: 该数据集提供了丰富、异质的彩色眼底图像资源，包含高质量分割标注，支持在真实世界图像质量和采集条件变化下进行机器学习模型的稳健基准测试和训练。
...
TL;DR: DESSERT：基于扩散模型的事件驱动单帧合成框架，通过残差训练实现更清晰、时间一致性的视频帧预测，解决了传统方法因像素位移不准确导致的空洞和模糊问题。


<details>
  <summary>Details</summary>
Motivation: 传统视频帧预测在动态场景中因缺乏下一帧信息而产生预测误差。事件相机通过异步捕获像素亮度变化提供高时间分辨率信息，但现有事件驱动方法通过预测光流和像素扭曲会导致空洞和模糊问题。

Method: 提出DESSERT框架：1) ER-VAE对齐事件帧与残差；2) 基于预训练Stable Diffusion的扩散模型，以事件数据为条件对残差潜在表示进行去噪；3) 引入DLT增强，在不同时间长度帧段上进行训练以提高鲁棒性。

Result: 实验结果表明，该方法在事件重建、基于图像的视频帧预测、事件视频帧预测和单边事件视频帧插值等方面均优于现有方法，能生成更清晰、时间一致性更好的帧合成结果。

Conclusion: DESSERT通过残差训练和扩散模型有效解决了事件驱动视频帧预测中的空洞和模糊问题，实现了更高质量的帧合成，为动态场景的视频预测提供了新思路。
...
TL;DR: 使用Attention U-Net架构训练统一FLAIR高信号分割模型，在5000多张不同肿瘤类型和采集时间点的FLAIR图像上验证，模型性能与数据集特定模型相当，并能泛化到不同肿瘤类型和时间点。


<details>
  <summary>Details</summary>
Motivation: FLAIR MRI扫描对脑肿瘤的诊断、治疗规划和监测很重要。FLAIR高信号体积是评估肿瘤体积或周围水肿的重要指标，自动分割在临床中很有用。需要开发一个统一的模型来处理不同肿瘤类型和采集时间点。

Method: 使用约5000张来自不同中心、不同肿瘤类型和采集时间点的FLAIR图像，训练基于Attention U-Net架构的统一FLAIR高信号分割模型。与数据集特定模型进行比较，并在不同肿瘤类型、采集时间点和BraTS数据集上进行验证。

Result: 统一模型在不同肿瘤类型上取得良好性能：术前脑膜瘤平均Dice分数88.65%，术前转移瘤80.08%，术前胶质瘤90.92%，术后胶质瘤84.60%（BraTS），术前低级别胶质瘤84.47%，术后低级别胶质瘤61.27%。模型性能与数据集特定模型相当，并能泛化到不同肿瘤类型和采集时间点。

Conclusion: 统一FLAIR高信号分割模型在多种脑肿瘤类型和采集时间点上表现出良好性能，与数据集特定模型性能相当，具有临床部署的泛化能力。模型已集成到开源软件Raidionics中，便于临床使用。
...
TL;DR: 开发了Polysome工具生成合成指令，创建了HISTAI-Instruct大规模病理切片指令调优数据集，并训练了ANTONI-α视觉语言模型，在病理切片视觉问答任务上表现优于MedGemma。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在病理学应用中存在局限：要么只关注切片中的小区域，要么只提供静态切片级输出，或者依赖非公开数据限制了可复现性。同时，包含详细临床报告的全切片图像训练数据稀缺，阻碍了透明且可泛化的视觉语言模型发展。
...
Result: ANTONI-α在组织识别、肿瘤检测和鉴别诊断等全切片视觉问答任务上表现优于MedGemma。同时比较了使用不同数据量训练的多个ANTONI-α版本性能。所有方法、数据和代码均已公开。

Conclusion: 通过开发标准化合成指令生成工具、创建大规模公开病理切片指令数据集，成功训练出在病理学视觉问答任务上表现优异的视觉语言模型，为病理学辅助诊断提供了透明、可复现的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统显式变形方法在准确运动传递和缺失区域恢复方面存在困难，而基于注意力的变形方法虽然有效但复杂度高且几何基础薄弱。需要一种既能保持几何准确性又能处理遮挡和变形区域的方法。

Method: 提出三阶段渐进式框架：1）显式变形模块使用3D密集光流进行粗空间对齐；2）参考增强校正模块利用多参考图像的3D关键点和纹理特征的交叉注意力语义补全遮挡/变形区域；3）置信度引导融合模块通过学习的置信度图自适应融合变形输出
...
TL;DR: MGRegBench是一个公开的乳腺X光配准基准数据集，包含5000多对图像和100对人工标注的解剖标志点与分割掩码，用于标准化评估不同配准方法。


<details>
  <summary>Details</summary>
Motivation: 乳腺X光配准在临床应用中至关重要，但缺乏公开数据集和标准化基准限制了该领域的发展。现有研究使用私有数据和不同评估框架，难以直接比较。

Method: 创建MGRegBench公开基准数据集，包含5000多对图像，其中100对包含人工标注的解剖标志点和分割掩码。使用该数据集评估了多种配准方法，包括经典方法（ANTs）、基于学习的方法（VoxelMorph、TransMorph）、隐式神经表示（IDIR）、经典乳腺X光专用方法以及最新的深度学习方法MammoRegNet。

Result: MGRegBench成为最大的公开2D配准数据集之一，提供了首个大规模人工标注的乳腺X光配准数据集。通过该基准实现了不同方法的首次公平比较，并进行了深度学习配准的深入分析。

Conclusion: 该研究提供了首个大规模公开乳腺X光配准基准数据集，建立了公平比较的基础资源，将促进未来研究。代码和数据已公开，为标准化评估和推动该领域发展提供了重要工具。
...
TL;DR: 提出一种像素级映射预处理方法，通过打乱图像像素值分布来破坏检测器过度依赖的语义线索，迫使检测器关注更本质的生成痕迹，从而提升AI生成图像检测器的跨生成器泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成图像检测器存在泛化能力不足的问题，它们往往过度拟合特定生成模型的语义线索，而非学习通用的生成痕迹，导致在面对未见过的生成模型时性能下降。

Method: 引入简单而有效的像素级映射预处理步骤，通过打乱图像的像素值分布来破坏检测器常用的脆弱、非本质的语义模式，迫使检测器关注图像生成过程中更基础、可泛化的高频痕迹。

Result: 在GAN和基于扩散的生成器上的综合实验表明，该方法显著提升了最先进检测器的跨生成器性能。深入分析验证了破坏语义线索是提升泛化能力的关键。

Conclusion: 通过像素级映射预处理破坏语义线索，能够有效提升AI生成图像检测器的跨模型泛化能力，为解决检测器过度拟合特定生成模型的问题提供了简单而有效的解决方案。
...
TL;DR: 提出情感图像过滤（AIF）任务，将文本中的抽象情感转化为具象图像，创建情感丰富的视觉内容


<details>
  <summary>Details</summary>
Motivation: 社交媒体用户通过文本和图像表达情感，但现有方法难以将文本中的抽象情感有效转化为视觉具象的图像表达

Method: 首先构建AIF数据集并定义模型框架，提出基于多模态transformer的AIF-B作为初步尝试，然后扩展为AIF-D，利用预训练大规模扩散模型的生成先验实现更深层的情感反映

Result: 定量和定性实验表明AIF模型在内容一致性和情感保真度方面优于现有方法，用户研究证实AIF模型能更有效地唤起特定情感

Conclusion: AIF模型具有重要价值和潜力，能够将文本中的抽象情感有效转化为视觉具象表达，为情感驱动的图像生成提供了新方向
...
TL;DR: 本文通过实证消融研究发现，在预训练扩散模型应用条件化方法时，条件化步长对性能的影响远大于扩散步数，最佳步长范围为[2.0, 3.0]


<details>
  <summary>Details</summary>
Motivation: 扩散模型在解决单图像超分辨率等逆问题上表现出强大潜力，但现有的条件化方法（如DPS和MCG）引入了需要仔细调优的额外超参数，这影响了实际应用效果

Method: 在FFHQ超分辨率任务上进行实证消融研究，分析条件化方法应用于预训练扩散模型时影响性能的主要因素，特别关注条件化步长和扩散步数的相对重要性

Result: 研究发现条件化步长对重建质量的影响显著大于扩散步数，在实验中步长范围[2.0, 3.0]能获得最佳整体性能

Conclusion: 条件化步长是影响扩散模型条件化性能的关键因素，这一发现有助于简化超参数调优过程，提高扩散模型在逆问题求解中的实用性和效率
...
TL;DR: AIFloodSense是一个包含470张高分辨率航拍图像的新洪水检测数据集，覆盖64个国家230次洪水事件，支持图像分类、语义分割和视觉问答三种任务，旨在提升AI在气候韧性领域的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 洪水检测对于灾害响应和风险评估至关重要，但现有数据集在地理范围和标注细节方面有限，阻碍了鲁棒计算机视觉方法的发展。需要更全面、多样化的数据集来支持泛化AI工具的开发。

Method: 构建了AIFloodSense数据集，包含470张高分辨率航拍图像，来自64个国家230次洪水事件（2022-2024年）。数据集支持三个任务：1）图像分类（环境类型、相机角度、大洲识别）；2）语义分割（洪水、天空、建筑物的像素级掩码）；3）视觉问答（支持自然语言推理的灾害评估）。

Result: 建立了所有任务的最先进架构基准测试，展示了数据集的复杂性和价值。数据集具有全球多样性和时间相关性，填补了现有洪水分割数据集的空白。

Conclusion: AIFloodSense为推进领域泛化AI工具在气候韧性方面的应用提供了重要资源，通过支持多种互补任务，有助于开发更鲁棒的洪水检测和灾害评估方法。
...
### [52] [LangDriveCTRL: Natural Language Controllable Driving Scene Editing with Multi-modal Agents](https://arxiv.org/abs/2512.17445)
*Yun He,Francesco Pittaluga,Ziyu Jiang,Matthias Zwicker,Manmohan Chandraker,Zaid Tasneem*

Main category: cs.CV

TL;DR: LangDriveCTRL是一个基于自然语言控制的框架，用于编辑真实驾驶视频以合成多样化的交通场景。它通过3D场景分解将驾驶视频表示为包含静态背景和动态对象的场景图，并采用智能体管道实现细粒度编辑。


<details>
  <summary>Details</summary>
Motivation: 现有的驾驶视频编辑方法在细粒度控制、真实感和多对象行为编辑方面存在局限。需要一种能够通过自然语言指令灵活编辑驾驶视频，同时保持高度真实性和交通合理性的框架。

Method: 1) 使用3D场景分解将驾驶视频表示为场景图；2) 采用智能体管道：编排器将用户指令转换为执行图，协调专门智能体：对象定位智能体建立文本描述与场景图节点对应关系，行为编辑智能体从语言指令生成多对象轨迹，行为审查智能体迭代审查和优化轨迹；3) 编辑后的场景图经过渲染和视频扩散工具细化处理。

Result: LangDriveCTRL支持对象节点编辑（移除、插入、替换）和多对象行为编辑。在定量评估中，比之前的最先进方法实现了近2倍的指令对齐度提升，同时在结构保持、照片真实感和交通真实性方面表现优异。

Conclusion: LangDriveCTRL提供了一个强大且灵活的自然语言控制框架，能够高质量地编辑真实驾驶视频，生成多样化的交通场景，在指令对齐、真实性和实用性方面显著优于现有方法。
...
TL;DR: 3D-RE-GEN是一个从单图像重建可编辑3D纹理网格场景的框架，通过组合多个先进模型解决现有方法在物体分解、空间关系和背景缺失方面的问题，满足影视游戏行业艺术家的工作流程需求。


<details>
  <summary>Details</summary>
Motivation: 当前3D场景生成方法虽然视觉效果出色，但无法提供艺术家工作流程所需的可编辑3D纹理网格场景。现有纹理网格场景重建方法存在物体分解错误、空间关系不准确、背景缺失等问题，远未达到艺术家可用的标准。

Method: 提出组合式框架3D-RE-GEN，集成资产检测、重建和放置模型，将某些模型扩展到其原始领域之外。将遮挡物体获取视为图像编辑任务，使用生成模型在一致光照和几何条件下进行场景级推理。采用新颖的4自由度可微分优化对齐重建物体与估计的地平面，生成全面背景以在优化过程中空间约束物体。

Result: 3D-RE-GEN在单图像3D场景重建中达到最先进性能，通过精确相机恢复和空间优化的组合生成，产生连贯、可修改的场景，为视觉特效和游戏中的真实光照和模拟任务提供基础。

Conclusion: 通过组合特定领域的最先进模型，3D-RE-GEN解决了艺术家对可编辑3D纹理网格场景的需求，在场景重建性能上达到最先进水平，为视觉特效和游戏开发提供了实用的解决方案。
...
TL;DR: TwinSegNet：一种结合ViT-UNet混合模型与个性化数字孪生的隐私保护联邦学习框架，用于多机构脑肿瘤分割，在保持数据隐私的同时实现高性能。


<details>
  <summary>Details</summary>
Motivation: 当前脑肿瘤分割的深度学习方法依赖集中式数据收集，存在隐私问题且难以在不同机构间泛化。需要一种既能保护数据隐私又能实现准确分割的解决方案。

Method: 提出TwinSegNet框架，集成混合ViT-UNet模型（卷积编码器+Vision Transformer瓶颈）和个性化数字孪生。采用联邦学习，各机构在私有数据上微调全局模型形成自己的数字孪生。

Result: 在9个异构MRI数据集（包括BraTS 2019-2021）上评估，Dice分数高达0.90%，敏感性和特异性超过90%，在非独立同分布数据上表现稳健。相比集中式模型如TumorVisNet，在保护隐私的同时不牺牲性能。

Conclusion: TwinSegNet为多机构临床环境提供了可扩展、个性化的分割解决方案，同时严格遵守数据保密要求，在隐私保护和性能之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在创意图像生成方面取得了显著进展，但缺乏对场景照明的精确控制，而照明控制是内容设计师操纵图像情绪、氛围和视觉美感的关键因素。

Method: LumiCtrl包含三个基本组件：1) 基于物理的照明增强，沿普朗克轨迹创建标准照明下的微调变体；2) 边缘引导的提示解耦，使用冻结的ControlNet确保提示专注于照明而非结构；3) 掩码重建损失，专注于前景对象学习，同时允许背景进行上下文适应（上下文光适应）。

Result: 与现有T2I定制方法相比，LumiCtrl在光照保真度、美学质量和场景一致性方面表现显著更好。人类偏好研究进一步证实用户强烈偏好LumiCtrl的输出结果。

Conclusion: LumiCtrl提供了一种有效的照明个性化方法，能够精确控制生成图像中的场景照明，为内容设计师提供了更好的工具来操纵图像的情绪和视觉美感。
...
TL;DR: 提出基于注意力机制的CBAM-VGG16模型用于植物叶片病害检测，通过集成卷积块注意力模块提升特征提取和病害定位能力，在多个数据集上达到高精度并具有良好的可解释性。
...
### [57] [MMLANDMARKS: a Cross-View Instance-Level Benchmark for Geo-Spatial Understanding](https://arxiv.org/abs/2512.17492)
...
Result: 该数据集支持多种地理空间任务，包括跨视角地面到卫星检索、地面和卫星地理定位、文本到图像和文本到GPS检索。基线模型展现出广泛泛化能力和与现成基础模型及专门SOTA模型竞争的性能。

Conclusion: 多模态数据集对于实现广泛的地理空间理解是必要的，MMLANDMARKS为地理空间分析提供了统一的多模态基准。
...
TL;DR: RadarGen是一个扩散模型，能够从多视角相机图像生成逼真的汽车雷达点云，通过BEV表示雷达测量数据，并利用预训练基础模型提取的深度、语义和运动线索来指导生成过程。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够从视觉数据生成逼真雷达点云的方法，以实现跨传感模态的统一生成式仿真，解决雷达数据获取困难的问题，并为感知模型提供更多训练数据。

Method: 1. 将雷达测量数据表示为鸟瞰图形式，编码空间结构、雷达截面积和速度属性；2. 采用高效的图像-潜在扩散模型适配到雷达领域；3. 引入轻量级恢复步骤从生成的地图重建点云；4. 利用预训练基础模型提取BEV对齐的深度、语义和运动线索来指导生成过程。

Result: 在大规模驾驶数据上的评估表明，RadarGen能够捕捉特征性的雷达测量分布，并缩小了在真实数据上训练的感知模型之间的差距，标志着跨传感模态统一生成式仿真的进步。

Conclusion: RadarGen通过从视觉图像生成逼真雷达点云，提供了一种可扩展的多模态生成式仿真方向，该方法与现有视觉数据集和仿真框架兼容，有助于推动自动驾驶感知系统的发展。
...
TL;DR: GroundingME是一个新的视觉定位基准，系统性地挑战多模态大语言模型在四个关键维度上的能力：区分性、空间理解、有限视觉信息和拒绝能力，揭示了当前模型与人类水平之间的显著差距。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在现有基准上取得了令人印象深刻的分数，但一个根本问题仍然存在：它们是否真正能够像人类一样复杂地实现语言到视觉的定位，还是仅仅在简化数据集上进行模式匹配？现有基准未能捕捉现实世界的复杂性，而人类能够轻松处理模糊引用并识别何时无法进行定位。

Method: 研究者引入了GroundingME基准，通过结合自动生成和人工验证的方式精心策划了1,005个具有挑战性的示例，系统性地评估模型在四个关键维度上的能力：区分性（区分高度相似的对象）、空间性（理解复杂的关系描述）、有限性（处理遮挡或微小对象）和拒绝性（识别不可定位的查询）。

Result: 评估25个最先进的多模态大语言模型揭示了显著的能力差距：最佳模型仅达到45.1%的准确率，而大多数模型在拒绝任务上得分为0%，倾向于幻觉对象而不是承认其不存在。研究者探索了两种改进策略：测试时缩放通过思维轨迹选择最优响应，将复杂定位提高了2.9%；数据混合训练教导模型识别不可定位的查询，将拒绝准确率从0%提高到27.9%。

Conclusion: GroundingME既是一个诊断工具，揭示了当前多模态大语言模型的局限性，也是一个实现人类水平视觉定位的路线图。研究强调了当前模型在现实世界复杂性面前的不足，特别是在安全关键应用中的风险，并指出了改进方向。
...
TL;DR: 该研究首次在中东地区（伊拉克库尔德斯坦）验证了AI在前列腺癌诊断和Gleason分级中的表现，发现AI模型达到病理学家水平，且低成本紧凑扫描仪在非数字化环境中具有应用潜力。


<details>
  <summary>Details</summary>
Motivation: 目前病理AI系统主要在欧美大型中心进行评估，缺乏对代表性不足人群的验证研究。这些地区可能从AI支持中获益最大，因此需要在全球范围内进行AI验证研究以实现公平的病理AI应用。

Method: 收集并数字化了来自伊拉克库尔德斯坦地区的339份前列腺活检标本（185名患者，2013-2024年）。评估了任务特定的端到端AI模型和两个基础模型，比较它们与病理学家的分级一致性，以及在三种扫描仪（Hamamatsu、Leica、Grundium）上的跨扫描仪一致性。

Result: AI与病理学家的分级一致性（Cohen's二次加权kappa 0.801）与病理学家之间的一致性（0.799）相似（p=0.9824）。所有AI模型和扫描仪对之间的跨扫描仪一致性都很高（二次加权kappa > 0.90），包括低成本紧凑扫描仪。

Conclusion: AI模型在前列腺组织病理学评估中达到病理学家水平性能。紧凑扫描仪可为非数字化环境中的验证研究提供途径，并在样本量有限的实验室中实现成本效益的AI应用。这是中东地区首个公开可用的数字病理数据集，支持全球公平AI病理学的进一步研究。
...
TL;DR: Re-Depth Anything是一个测试时自监督框架，通过融合Depth Anything V2和大型2D扩散模型的先验知识，提升单目深度估计在真实世界图像上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型如Depth Anything V2在处理远离训练分布的图像时表现不佳，存在领域差距问题。需要一种方法能够在不依赖标注的情况下，利用生成模型的能力来提升深度估计的准确性和真实性。

Method: 提出测试时自监督框架，通过重新光照预测的深度图并增强输入图像来实现无标签优化。使用生成式上下文中的形状从阴影（SfS）线索替代传统光度重建，结合分数蒸馏采样（SDS）。采用针对性优化策略：冻结编码器，仅更新中间嵌入并微调解码器，防止优化崩溃。

Result: 在多个基准测试中，Re-Depth Anything相比Depth Anything V2在深度准确性和真实性方面取得显著提升，展示了通过增强几何推理实现自监督的新途径。

Conclusion: 该方法成功融合了深度估计基础模型和2D扩散模型的优势，通过测试时自监督有效缩小了领域差距，为几何推理的自监督学习开辟了新方向。
...
TL;DR: G3Splat通过引入几何先验来解决仅依赖视图合成损失学习3D高斯泼溅时的几何模糊性问题，实现了几何一致的3D场景表示，在几何重建、相对姿态估计和新视角合成方面达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖视图合成监督来回归3D高斯参数，但仅凭视图合成损失不足以恢复几何上有意义的泼溅表示，存在几何模糊性问题。

Method: 提出G3Splat方法，在自监督姿态自由泛化泼溅学习中，通过强制几何先验来获得几何一致的3D场景表示，解决3D高斯泼溅学习的模糊性问题。

Result: 在RE10K数据集上训练，在几何一致重建、相对姿态估计和新视角合成方面达到最先进性能；在ScanNet上展示强大的零样本泛化能力，在几何恢复和相对姿态估计方面显著优于先前工作。

Conclusion: 仅依赖视图合成损失不足以学习几何有意义的3D高斯泼溅，引入几何先验的G3Splat方法能够获得几何一致的3D场景表示，在多个任务上实现优异性能。
...
<details>
  <summary>Details</summary>
Motivation: 虚拟家具合成在家居设计和电商应用中具有重要价值，但现有方法缺乏可复现的基准，且图像合成方法难以在保持背景完整性的同时实现高保真家具合成。

Method: 提出RoomEditor++框架，采用参数共享的双扩散主干架构，兼容U-Net和DiT，统一参考图像和背景图像的特征提取与修复过程。

Result: 在RoomBench++数据集（11.2万训练对和1832测试对）上验证，RoomEditor++在定量指标、定性评估和人类偏好研究中均优于现有方法，且对未见室内场景和一般场景具有强泛化能力。

Conclusion: RoomEditor++通过创新的参数共享机制实现了对齐的特征表示，促进了精确的几何变换、纹理保持和无缝集成，为虚拟家具合成提供了有效的解决方案。
...
TL;DR: 提出首个针对单热点调制的视频快照压缩成像算法，将重建任务转化为生成式视频修复问题，结合一步回归初始化和一步扩散精炼，并采用双光路硬件设计缓解空间退化。


<details>
  <summary>Details</summary>
Motivation: 传统视频SCI使用随机二进制调制会导致时间混叠问题，而单热点调制（每个像素只激活一个子帧）能实现完美时间解耦，但现有算法未能充分利用这一潜力。需要开发专门针对单热点调制的重建算法。

Method: 1) 利用单热点调制的解耦特性，将重建任务转化为生成式视频修复问题，设计与硬件压缩过程对齐的前向随机微分方程；2) 提出结合一步回归初始化和一步扩散精炼的新框架；3) 在硬件层面实现双光路设计，利用互补信息增强修复视频质量。

Result: 在合成数据集和真实场景上的实验证明了该方法的有效性。这是首个将扩散模型集成到视频SCI重建中的工作。

Conclusion: 提出的算法成功解决了单热点调制视频SCI的重建问题，通过创新的算法框架和硬件设计，有效缓解了时间混叠和空间退化问题，为视频快照压缩成像提供了新的解决方案。
...
TL;DR: 该研究系统评估了241个生物医学图像分析竞赛的公平性，发现现有基准数据集存在代表性不足、访问限制和可重用性差等问题，导致排行榜成功与临床相关性脱节。


<details>
  <summary>Details</summary>
Motivation: 医学影像AI基准竞赛对定义性能标准和推动方法进步至关重要，但现有基准是否提供足够代表性、可访问和可重用的数据以支持临床意义的AI仍不明确。研究旨在评估基准竞赛在公平性方面的表现。

Method: 对241个生物医学图像分析竞赛（涵盖458个任务、19种成像模态）进行大规模系统性研究，从两个互补维度评估公平性：1）数据集是否代表真实世界临床多样性；2）是否遵循FAIR原则（可查找、可访问、可互操作、可重用）。

Result: 研究发现数据集组成存在显著偏见，包括地理位置、模态和问题类型相关偏见，表明当前基准未能充分反映真实世界临床多样性。尽管影响广泛，竞赛数据集常受限于限制性或模糊的访问条件、不一致或不合规的许可实践以及不完整的文档记录，限制了可重复性和长期重用。

Conclusion: 这些缺陷暴露了基准生态系统中的基础公平性限制，突显了排行榜成功与临床相关性之间的脱节。需要改进基准竞赛的数据代表性、访问性和可重用性，以支持临床意义的AI发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频异常检测方法依赖大量标注数据且计算成本高。现有基于多模态大语言模型的免调优方法虽然利用其丰富世界知识，但依赖文本输出会导致信息损失、正常性偏差和提示敏感性，难以捕捉细微异常线索。

Method: 提出HeadHunt-VAD范式，绕过文本生成，直接在冻结的多模态大语言模型中寻找鲁棒的异常敏感内部注意力头。核心是鲁棒头识别模块，通过显著性和稳定性的多标准分析系统评估所有注意力头，识别出跨不同提示具有一致判别性的稀疏头子集。这些专家头的特征被输入轻量级异常评分器和时序定位器。

Result: 在两个主要视频异常检测基准测试中，HeadHunt-VAD在免调优方法中实现了最先进的性能，同时保持高效率。

Conclusion: 验证了在多模态大语言模型中进行头级探测作为现实世界异常检测的强大实用解决方案的有效性。
...
TL;DR: 本文提出了一种用于多输出模型（如医学图像分割CNN）的半监督学习方法，该方法基于数据增强（旋转和翻转），不依赖模型响应的概率假设，适用于需要单独分割不同解剖结构的场景。


<details>
  <summary>Details</summary>
Motivation: 医学图像多类别分割（如主动脉夹层诊断）需要大量高质量标注数据，但3D医学图像的标注成本高昂且耗时。现有的多输出CNN模型虽然能提高分割精度，但缺乏有效的半监督学习方法来利用未标注数据。

Method: 提出基于数据增强（旋转和翻转）的半监督学习方法，专门针对多输出模型设计。该方法不假设模型响应的概率特性，因此具有通用性，适用于需要独立分割不同解剖区域（如真腔、假腔、假腔血栓）的架构。

Result: 论文基于ImageTBDA数据集（包含100个3D CTA图像）进行验证，该数据集用于B型主动脉夹层诊断，需要准确分割三个关键类别。提出的方法能够有效利用未标注数据，解决标注数据不足的问题。

Conclusion: 该方法为多输出模型的半监督学习提供了一种通用解决方案，特别适用于医学图像分割任务，能够在不依赖大量标注数据的情况下提高分割精度，对临床诊断和治疗决策具有重要意义。
...
Method: 提出StereoMV2D统一框架，将时序立体建模集成到2D检测引导的多视角3D检测器中。通过利用相邻帧间同一目标的跨时序视差增强深度感知，在2D感兴趣区域内高效计算。引入动态置信度门控机制，通过从帧间匹配矩阵和外观一致性学习统计模式，自适应评估时序立体线索的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在计算病理学中取得进展，但全切片图像的千兆像素尺度和空间异质性给多模态理解带来挑战。现有对齐方法难以捕捉文本描述与数千个图像块之间的细粒度对应关系，影响了下游任务性能。

Method: PathFLIP将幻灯片级描述分解为区域级子标题，生成文本条件区域嵌入以实现精确的视觉-语言对齐。利用大语言模型（LLMs）适应不同的临床指令和诊断场景，支持多种任务范式。

Result: PathFLIP在四个代表性基准测试中优于现有的大规模病理视觉语言模型，同时需要显著更少的训练数据，在幻灯片级分类与检索、细粒度病变定位和指令跟随等任务上表现出色。

Conclusion: PathFLIP为临床实践中的细粒度、指令感知的全切片图像解释开辟了新途径，通过更高效的训练和更好的性能，推动了计算病理学中多模态理解的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有HOI检测方法基于封闭世界假设，将任务视为小规模预定义动词集的分类问题，难以泛化到现实世界中未见或模糊的交互长尾分布。虽然多模态大语言模型具备开放词汇理解所需的世界知识，但由于计算成本过高而无法与现有HOI检测器有效结合。
...
Result: 实验表明该方法在封闭集上达到最先进性能，并展现出强大的零样本泛化能力，实现了判别式感知与生成式推理的无缝桥接，为开放世界HOI检测提供了统一范式。

Conclusion: GRASP-HO成功将HOI检测从封闭集分类重新定义为开放词汇生成问题，通过轻量级认知引导模块有效结合视觉感知与语言模型的世界知识，实现了开放世界HOI检测中判别式感知与生成式推理的统一。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于教学指令的图像编辑虽然取得了进展，但在视频编辑中面临挑战：未指定编辑区域会导致编辑不准确，编辑区域与非编辑区域在去噪过程中存在token干扰问题。

Method: 1) 宽度方向拼接源视频和目标视频进行联合去噪；2) 引入两种正则化：潜在正则化（增加编辑区域差异，减少非编辑区域差异）和注意力正则化（抑制编辑区域token对源视频对应token的注意力）；3) 构建大规模高质量数据集ReCo-Data（50万指令-视频对）。

Result: 在四大主要教学视频编辑任务上的广泛实验证明了ReCo的优越性，能够有效解决编辑区域不准确和token干扰问题，提升视频编辑质量。

Conclusion: ReCo通过创新的约束建模和正则化机制，成功将上下文生成范式扩展到视频编辑领域，为教学视频编辑提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管AI视频分析技术已成熟，能够精确量化面部表情、头部运动等行为，但由于现有方法主要面向工程受众、需要专业软件栈，且提供的测量结果对假设驱动研究不够直接有用，导致主流采用缓慢，行为科学家难以使用现代AI工具。

Method: Bitbox是一个遵循可重复性、模块化和可解释性原则的开源工具包，提供标准化接口，利用多种面部、头部和身体处理器从视频中提取高级行为测量。核心模块已在临床样本上测试验证，设计上支持轻松添加新测量指标。

Result: Bitbox填补了转化研究中的鸿沟：为行为研究人员提供无需工程专业知识即可访问的稳健高级行为指标，同时为计算机科学家提供向最需要领域传播方法的实用机制。

Conclusion: Bitbox作为社区驱动项目，有望加速计算行为测量在行为、临床和心理健康研究中的整合，通过方法开发者和领域科学家的共同贡献持续发展。
...
TL;DR: FlexAvatar是一个灵活的3D头部重建模型，能从单张或稀疏图像创建高保真3D头像，无需相机姿态或表情标签，支持实时详细动态变形。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要相机姿态或表情标签，且难以从单张或稀疏图像生成具有详细动态变形的高质量3D头像。FlexAvatar旨在解决这些问题，提供更灵活实用的3D头像创建方案。

Method: 1) 基于transformer的重建模型，使用结构化头部查询令牌作为规范锚点；2) 轻量级UNet解码器，基于UV空间位置图生成详细表情相关变形；3) 数据分布调整策略平衡训练集中关键表情；4) 10秒轻量级细化增强特定身份细节。

Result: 实验表明FlexAvatar在3D一致性、详细动态真实感方面优于先前方法，能实时生成高质量动态变形，为可动画3D头像创建提供实用解决方案。

Conclusion: FlexAvatar是一个灵活高效的3D头部重建模型，能从单张或稀疏图像创建高保真动态头像，无需相机姿态或表情标签，在真实感和实用性方面表现优异。
...
TL;DR: SAVeD是一个从社交媒体收集的大规模ADAS车辆事故数据集，包含2119个第一人称视频，涵盖碰撞、险情和系统失效事件，用于安全关键驾驶行为研究。


<details>
  <summary>Details</summary>
Motivation: 现有数据集主要局限于模拟环境或人类驾驶车辆数据，缺乏真实ADAS车辆在风险条件下的行为数据，无法满足高级驾驶辅助系统安全关键研究的需求。

Method: 从公开社交媒体内容中收集和整理ADAS车辆相关事故视频，提供视频帧级标注（碰撞、规避动作、系统失效），并开发了结合语义分割和单目深度估计的实时碰撞时间计算框架。

Result: 数据集包含2119个第一人称视频，涵盖不同地点、光照和天气条件；提出的TTC计算框架能有效分析动态物体风险；使用广义极值分布量化了不同道路类型的极端风险；通过领域适应显著提升了VLLM在复杂险情场景中的性能。

Conclusion: SAVeD填补了ADAS车辆真实风险行为数据集的空白，为安全关键驾驶行为研究提供了重要资源，通过多种分析方法展示了其在风险评估和模型性能提升方面的实用价值。
...
TL;DR: MambaMIL+：一种新的多示例学习框架，通过重叠扫描、选择性条纹位置编码和上下文标记选择机制，在保持长程依赖建模的同时显式整合空间上下文，用于计算病理学中的全切片图像分析。


<details>
  <summary>Details</summary>
Motivation: 全切片图像（WSI）是计算病理学中的重要数据模态，但其千兆像素分辨率和缺乏细粒度注释给传统深度学习模型带来挑战。多示例学习（MIL）通过将WSI视为补丁级实例的包来提供解决方案，但有效建模具有丰富空间上下文的超长序列仍然困难。Mamba作为长序列学习的有前途替代方案出现，但存在空间上下文建模有限和内存衰减的问题，限制了其在WSI分析中的有效性。

Method: MambaMIL+框架包含三个核心组件：1）重叠扫描：重构补丁序列以嵌入空间连续性和实例相关性；2）选择性条纹位置编码（S2PE）：编码位置信息同时减轻固定扫描顺序的偏差；3）上下文标记选择（CTS）机制：利用监督知识动态扩大上下文内存以实现稳定的长程建模。

Result: 在20个基准测试上的广泛实验涵盖诊断分类、分子预测和生存分析，表明MambaMIL+在三种特征提取器（ResNet-50、PLIP和CONCH）下始终实现最先进的性能，突显其在大规模计算病理学中的有效性和鲁棒性。

Conclusion: MambaMIL+通过显式整合空间上下文同时保持长程依赖建模而不遗忘内存，解决了现有MIL框架在WSI分析中的局限性，为计算病理学中的大规模图像分析提供了有效的解决方案。
...
TL;DR: 提出Diff-Gen大规模扩散生成假图像数据集和AdaptPrompt参数高效迁移学习框架，利用CLIP模型实现跨生成器泛化的深度伪造检测，在25个测试集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 随着图像生成技术的进步，高度逼真的合成媒体广泛传播，使得深度伪造检测变得更加困难。关键挑战在于泛化能力，因为针对特定生成器训练的检测器在面对未见过的模型时往往失效。本研究旨在解决对泛化检测的迫切需求。

Method: 1. 引入Diff-Gen数据集：包含10万张扩散生成的假图像，捕捉广泛的光谱伪影；2. 提出AdaptPrompt框架：参数高效的迁移学习方法，联合学习任务特定的文本提示和视觉适配器，同时冻结CLIP主干；3. 通过层剪枝增强检测：剪裁视觉编码器的最后一个transformer块以保留高频生成伪影。

Result: 在25个具有挑战性的测试集上进行评估，涵盖GAN、扩散模型和商业工具生成的合成内容，在标准和跨域场景中均达到新的最先进水平。进一步展示了框架的少样本泛化能力（仅使用320张图像）和来源归因能力，能够在闭集设置中精确识别生成器架构。

Conclusion: 通过Diff-Gen数据集和AdaptPrompt框架，成功实现了对多样化生成技术的合成内容检测，显著提升了跨域泛化能力，为深度伪造检测提供了有效的解决方案。
...
Method: 提出UrbanDIFF纯空间去噪扩散模型，利用静态城市结构信息（建成区数据和数字高程模型）作为条件输入，在推理过程中通过监督像素引导细化步骤强制与无云像素保持一致

Result: 在20-85%云覆盖的合成云掩模实验中，UrbanDIFF始终优于插值基线，特别是在密集云遮挡下。在85%云覆盖时达到SSIM 0.89、RMSE 1.2K、R2 0.84，且随着云密度增加性能下降更慢

Conclusion: UrbanDIFF展示了去噪扩散模型在空间LST重建中的有效性，特别是在高云覆盖情况下，为连续城市热岛监测提供了可靠的云污染填补解决方案
...
<details>
  <summary>Details</summary>
Motivation: 虽然3DGS已成为高保真场景表示方法，但从其基元直接编码丰富、通用的特征仍然未被充分探索。本文旨在填补这一空白，通过从2D基础模型蒸馏互补信号来学习全面的3DGS场景编码器

Method: 提出Chorus多教师预训练框架，使用共享3D编码器和教师特定投影器，从语言对齐、通用和对象感知的教师模型中学习，鼓励构建捕获从高级语义到细粒度结构信号的共享嵌入空间

Result: 在开放词汇语义和实例分割、线性和解码器探测以及数据高效监督等任务上表现优异。仅使用高斯中心、颜色和估计法线作为输入的变体在点云基准测试中表现优于基线，同时使用训练场景减少39.9倍

Conclusion: Chorus框架成功实现了从3DGS基元学习通用特征表示，提出的渲染-蒸馏适应方法促进了域外微调，为3D场景理解提供了有效的预训练解决方案
...
TL;DR: InSPECT提出了一种保持不变频谱特征的扩散模型，通过在前后向过程中保留不变特征，改善了生成质量、多样性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型将数据完全扩散为白噪声再重建，导致预测任务极其困难且计算量大。为了解决这一限制，需要设计能够保持关键特征的扩散过程。

Method: 提出InSPECT模型，在前后向过程中保持不变的频谱特征。前向过程结束时，傅里叶系数平滑收敛到指定随机噪声，既保留特征又维持多样性和随机性。

Result: 在CIFAR-10、Celeb-A和LSUN数据集上，InSPECT相比DDPM在1万次迭代中平均降低FID 39.23%，提升IS 45.80%，实现了更好的生成质量和多样性，同时收敛更快。

Conclusion: 保持不变频谱特征显著提升了扩散模型的性能，在生成质量、多样性、计算效率和收敛速度方面都有明显优势，这是首次在扩散模型中分析和保持不变频谱特征的尝试。
...
<details>
  <summary>Details</summary>
Motivation: 评估视觉语言模型时，需要测试模型独立分析视觉内容的能力，而不受文本先验影响。现有视觉提示基准测试中，模型对视觉标记细节的脆弱性尚未得到充分研究。

Method: 在9个常用开源和闭源VLM上评估两个视觉提示任务，研究视觉标记设计、数据集大小等基准设置细节对模型性能和排名的影响，并创建包含16种视觉标记变体的VPBench基准。

Result: 视觉标记的微小变化（如从红色改为蓝色）能完全改变模型排行榜排名；略微增大视觉标记可使开源InternVL3-8B与更大的专有模型如Gemini 2.5 Pro表现相当；JPEG压缩级别等低级推理选择也会影响模型排名。

Conclusion: 视觉提示基准测试对实现细节异常敏感，远超过传统语义VLM评估。为缓解这种不稳定性，创建了VPBench基准和相应分析工具。
...
TL;DR: 提出了一种无需重新训练即可将任何训练好的ViT模型转换为自解释模型的方法——关键点计数分类器（KCCs），通过利用ViT自动识别图像关键点的能力，创建可解释的决策过程。


<details>
  <summary>Details</summary>
Motivation: 当前的自解释模型设计方法需要复杂的训练过程和特定架构，不切实际。随着基于ViT的通用基础模型的发展，这种不实用性变得更加严重，因此需要新方法来为ViT基础模型提供透明度和可靠性。

Method: 提出关键点计数分类器（KCCs），利用ViT自动识别图像关键点的能力，创建可直接在输入图像中可视化的可解释决策过程，无需重新训练已训练好的ViT模型。

Result: 广泛的评估表明，KCCs相比现有基线方法，显著改善了人机通信效果，使ViT基础模型更加透明和可靠。

Conclusion: KCCs是使基于ViT的基础模型更加透明和可靠的重要一步，提供了一种实用的自解释模型构建方法。
...
<details>
  <summary>Details</summary>
Motivation: 理解和生成多人互动对机器人学和社会计算至关重要，但现有方法多为任务特定且难以泛化到灵活的多智能体生成，主要挑战包括长时间跨度、强智能体间依赖和可变群体规模。

Method: 提出MAGNet（多智能体扩散强制变换器），基于扩散强制框架进行关键修改，在自动回归去噪过程中显式建模智能体间耦合，支持二元预测、伙伴修复和完整多智能体运动生成，架构可扩展且对智能体数量不敏感。

Result: MAGNet在二元基准测试中与专门方法表现相当，同时自然扩展到涉及三个或更多人的多元场景，能够捕捉紧密同步活动（如舞蹈、拳击）和松散结构社交互动，并能自动回归生成数百帧的超长序列。
...
Method: 提出Dexterous World Model (DWM)，这是一个场景-动作条件视频扩散框架。该方法通过两种条件进行视频生成：(1) 遵循指定相机轨迹的静态场景渲染以确保空间一致性；(2) 编码几何和运动线索的第一人称手部网格渲染以直接建模动作条件动态。使用混合交互视频数据集进行训练，合成数据提供完全对齐的监督，真实世界视频贡献多样且逼真的物体动态。

Result: 实验表明，DWM能够实现逼真且物理合理的交互，如抓取、打开和移动物体，同时保持相机和场景的一致性。该框架代表了基于视频扩散的交互式数字孪生的第一步。

Conclusion: DWM框架是迈向视频扩散基交互式数字孪生的重要一步，能够从第一人称动作实现具身模拟，为创建动态、交互式的数字环境提供了新方法。
...
TL;DR: 本文提出了一种系统框架，将面向理解的编码器特征适配到生成任务中，通过语义-像素重建目标正则化潜在空间，实现了紧凑且语义丰富的表示，在文本到图像生成和图像编辑任务中取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现代潜在扩散模型通常在低层次VAE潜在空间中运行，主要优化像素级重建。为了统一视觉生成和理解，新兴趋势是采用表示编码器的高维特征作为生成潜在空间。然而，作者发现两个基本障碍：(1)判别性特征空间缺乏紧凑正则化，导致扩散模型容易产生偏离流形的潜在表示，从而产生不准确的对象结构；(2)编码器固有的弱像素级重建能力阻碍生成器学习准确的细粒度几何和纹理。

Method: 提出一个系统框架来适配面向理解的编码器特征用于生成任务。引入语义-像素重建目标来正则化潜在空间，将语义信息和细粒度细节压缩到高度紧凑的表示中（96通道，16×16空间下采样）。基于这种表示设计了统一的文本到图像生成和图像编辑模型。

Result: 与各种特征空间相比，该方法实现了最先进的图像重建、更快的收敛速度，在文本到图像生成和编辑任务中获得了显著的性能提升。验证了表示编码器可以有效地适配为鲁棒的生成组件。

Conclusion: 通过语义-像素重建目标正则化潜在空间，可以创建既紧凑又语义丰富的表示，有效统一视觉生成和理解任务，为表示编码器在生成任务中的应用提供了有效解决方案。
...
TL;DR: 该研究创建了首个女性健康基准测试WHB，评估13个先进LLM在女性健康领域的表现，发现模型失败率约60%，存在显著专业知识和紧急情况识别缺陷。
...
Method: 研究者开发了女性健康基准测试WHB，包含96个经过严格验证的模型测试点，涵盖5个医学专业领域、3种查询类型和8种错误类型。评估了13个最先进的LLM模型。

Result: 当前模型在女性健康基准测试中表现出约60%的失败率，性能在不同专业领域和错误类型间差异显著。所有模型普遍在"错过紧急情况"指标上表现不佳，而GPT-5等新模型在避免不当建议方面有显著改进。

Conclusion: AI聊天机器人目前尚无法在女性健康领域提供可靠建议，需要进一步改进和专门训练。该基准测试为评估和提升LLM在女性健康领域的性能提供了重要工具。
...
TL;DR: 本文提出Struct-SQL框架，通过结构化推理表示（查询执行计划）进行知识蒸馏，训练小型语言模型在Text-to-SQL任务中达到接近大型语言模型的性能，同时解决企业部署中的成本、安全和性能三难问题。


<details>
  <summary>Details</summary>
Motivation: 企业级Text-to-SQL系统面临成本、安全和性能的三难困境：要么选择昂贵且不安全的专有大型语言模型，要么选择性能低下的小型语言模型。现有方法通过非结构化思维链进行知识蒸馏存在固有模糊性，需要更清晰可靠的教学信号。

Method: 提出Struct-SQL知识蒸馏框架，采用查询执行计划作为结构化推理表示，训练小型语言模型模仿大型语言模型。该方法将Text-to-SQL任务分解为明确的逻辑步骤，提供更清晰的教学信号。

Result: 使用结构化思维链蒸馏的小型语言模型相比非结构化思维链蒸馏基线实现了8.1%的绝对性能提升。错误分析表明，关键改进因素是语法错误显著减少。

Conclusion: 结构化逻辑蓝图（查询执行计划）作为教学信号，能够有效提升小型语言模型在Text-to-SQL任务中的可靠性和性能，为解决企业部署的三难问题提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 非自回归文本生成在通用语言建模领域重新受到关注，但缺乏标准化的训练和推理库，不同方法的实现通常是定制化的，难以进行系统比较，且每个模型都需要自己的数据整理、损失函数和预测逻辑，难以复用通用组件。
...
Conclusion: XLM包解决了非自回归语言模型实现中的标准化和可复用性问题，为研究社区提供了便利的工具和资源，有助于推动非自回归文本生成领域的发展。
...
TL;DR: SPECTRA是一种训练数据检测水印技术，通过LLM生成释义并评分，使数据即使仅占训练语料0.001%也能被可靠检测。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在互联网大规模文本语料上训练，训练数据检测对于执行版权和数据许可变得至关重要，需要一种能够在数据被用于训练后仍能检测的方法。

Method: SPECTRA使用LLM对文本进行释义，通过独立的评分模型为每个释义分配分数，选择分数与原文本最接近的释义以避免分布偏移。检测时比较可疑模型的标记概率与评分模型的概率。

Result: SPECTRA在检测训练数据与非训练数据时，能够实现超过九个数量级的p值差距，优于所有测试基线，即使水印数据仅占训练语料的0.001%也能可靠检测。

Conclusion: SPECTRA为数据所有者提供了一种可扩展的、在发布前部署的水印技术，能够在大规模LLM训练后仍然存活，有效支持版权和数据许可执行。
...
TL;DR: 该论文提出对话主题分割的新评估框架，强调边界密度和段内连贯性，而非传统的严格边界匹配F1分数，指出现有评估存在粒度不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 尽管对话主题分割在LLM对话系统中至关重要，但现有评估仍依赖严格的边界匹配和F1指标，无法反映实际应用需求，且评估结果易受标注粒度差异影响。

Method: 提出新的评估目标，将边界密度和段内连贯性作为主要标准，结合窗口容忍F1（W-F1）。在8个不同对话数据集上评估多种分割策略，分离边界评分与边界选择过程。

Result: 研究发现：1）不同数据集间的性能差异主要由标注粒度不匹配驱动，而非模型质量；2）现有方法通常产生高度连贯但过度分割的结果；3）传统精确匹配F1分数具有误导性。

Conclusion: 对话主题分割应被视为选择适当粒度而非预测单一正确边界集，需要将边界评分与边界选择分离，并采用更符合实际应用的评估标准。
...
<details>
  <summary>Details</summary>
Motivation: 针对长文本摘要中存在的可追溯性和忠实度问题，需要一种能够更好地识别重要内容并生成准确摘要的方法

Method: 采用高亮引导生成框架，通过自规划方法识别重要内容，然后基于规划生成摘要；探索了端到端和两阶段两种变体

Result: 在长文本摘要数据集上的实验表明，该方法能持续改进事实一致性，同时保持相关性和整体质量；在GovReport数据集上，最佳方法将ROUGE-L提高了4.1分，SummaC分数提升了约35%

Conclusion: 高亮引导的摘要生成有助于保留重要细节，在不同领域产生更准确和有洞察力的摘要，两阶段管道在长且信息密集的文档上表现更好
...
TL;DR: MiA-RAG通过构建全局语义表示（mindscape）来增强RAG系统，使其具备全局上下文感知能力，在长文本任务中表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 人类理解长文本时依赖全局语义表示，而现有RAG系统缺乏这种全局上下文指导，因此在长上下文任务中表现不佳。

Method: 通过分层摘要构建mindscape（全局语义表示），并基于此表示同时指导检索和生成过程，使检索器能形成丰富的查询嵌入，生成器能在连贯的全局上下文中对检索到的证据进行推理。

Result: 在多样化的长上下文和双语基准测试中，MiA-RAG在基于证据的理解和全局意义构建方面持续超越基线方法，分析显示它能将局部细节与连贯的全局表示对齐。

Conclusion: MiA-RAG通过引入全局上下文感知，使RAG系统能够进行更类似人类的长上下文检索和推理，实现了局部细节与全局语义表示的协调。
...
TL;DR: 提出了一种针对波斯语等低资源语言的噪声鲁棒ASR纠错框架，通过多假设融合和噪声感知建模，显著降低了在噪声环境下的词错误率。


<details>
  <summary>Details</summary>
Motivation: 自动语音识别系统在噪声环境下性能显著下降，特别是对于波斯语等低资源语言。即使是Whisper等最先进模型在不同信噪比下也难以保持准确性，需要开发更鲁棒的噪声敏感纠错方法。

Method: 1) 使用噪声波斯语音生成5个最佳假设；2) 引入误差水平噪声(ELN)表示，捕捉假设间的语义和标记级分歧，量化噪声引起的语言失真；3) 评估三种模型：基础LLaMA-2-7B、仅文本微调变体、以及集成句子和词级ELN嵌入的噪声条件模型。

Result: 在混合噪声测试集上，提出的微调+ELN模型将词错误率从原始Whisper的31.10%降低到24.84%，显著优于仅文本微调基线的30.79%。原始LLaMA-2-7B模型反而将WER增加到64.58%，表明其无法独立纠正波斯语错误。

Conclusion: 结合多假设和噪声感知嵌入的方法对噪声环境下的鲁棒波斯语ASR有效，ELN表示能够直接测量噪声引起的不确定性，使LLM能够在纠错过程中推理每个假设的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在生成严谨数学证明方面取得进展，但在形式化语言（如Lean）中进行定理证明仍然具有挑战性且计算成本高昂，特别是在处理本科及以上级别的问题时。

Method: 通过大规模智能体强化学习训练模型，让模型与Lean等工具进行广泛交互，在强化学习过程中持续积累经验。同时开发了高效的测试时扩展工作流，利用自然语言证明的最新进展，弥合自然语言和形式化语言之间的差距。

Result: 相比最先进方法，Seed-Prover 1.5以更小的计算预算实现了更优性能：解决了PutnamBench（本科级别）的88%、Fate-H（研究生级别）的80%、Fate-X（博士级别）的33%。在Putnam 2025的12个问题中解决了11个，用时仅9小时。
...
<details>
  <summary>Details</summary>
Motivation: 虽然NLP在客观问题生成方面已有较多研究和进展，但自动主观问题生成和答案评估仍处于发展阶段。开发这样的系统可以帮助教师评估学生作业，同时让学生通过自我评估来增强学习体验。

Method: 研究计划改进现有的NLP模型或创建新的模型，专门用于从文本输入自动生成主观问题并评估相应的答案。

Result: 摘要中未提供具体实验结果，但研究目标是开发一个能够自动生成主观问题并评估答案的系统。

Conclusion: 该研究旨在填补NLP在教育领域中自动主观问题生成和答案评估的空白，为教师评估和学生自我学习提供技术支持。
...
TL;DR: 提出了一种面向治理的混合微调框架，用于大语言模型的多语言低资源适配，结合梯度对齐的低秩更新与结构化正交变换，通过轻量级数据治理步骤提升准确率、校准度和跨语言一致性。


<details>
  <summary>Details</summary>
Motivation: 针对大语言模型在多语言低资源场景下的适配问题，需要在有限计算预算下实现准确率、校准度和跨语言一致性，同时保持优化的稳定性。

Method: 提出混合微调框架：1）核心算法结合梯度对齐的低秩更新与结构化正交变换，通过层间混合实现；2）在选定子层引入酉约束以稳定深度优化；3）配合轻量级无标签数据治理步骤（语言识别、近重复去除、质量过滤）。

Result: 在XNLI和FLORES基准测试中，混合方法相比强PEFT基线获得一致提升，保持方向平衡并改善概率校准；对轻量级拼写变体更具鲁棒性；简单治理步骤带来累加效益；训练足迹测量显示适度开销和有利的成本-质量边界。
...
TL;DR: 论文提出CANON LAYERS架构组件，通过受控合成预训练任务评估语言模型能力，该组件能促进相邻token间的水平信息流，提升推理深度和广度。
...
Method: 引入受控合成预训练任务框架，提出CANON LAYERS架构组件——轻量级组件，计算相邻token表示的加权和，可无缝集成到Transformer、线性注意力、状态空间模型等序列架构中。

Result: 展示了12个关键结果：Canon层能提升推理深度（如2倍）、推理广度、知识操作能力；能将弱架构（如NoPE）提升到匹配RoPE的水平，使线性注意力模型达到Mamba2/GDN等SOTA线性模型的性能；通过合成任务和真实学术规模预训练验证。

Conclusion: 合成预训练框架为评估核心模型能力提供了经济、原则性的路径，可能预测未来架构在训练流程改进后的行为，解锁更深层次的推理和分层推断能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLMs在代码生成任务中严重依赖有监督训练，需要大量标注数据或未标注代码片段，这些数据获取成本高且难以大规模获得

Method: 提出IPC框架，通过问题空间探测、测试理解探测、解决方案空间探测、知识巩固与强化来探测LLMs内部知识和置信度模式，利用自一致性机制和基于表示的质量估计识别可靠代码候选，训练UCoder（无监督学习的编码器）

Result: 在多个代码基准测试中验证了该方法，无监督方法能够达到与有监督方法竞争的性能，同时显著减少对标注数据和计算资源的依赖
...
TL;DR: 提出了CulturalToM-VQA评估基准，包含5095个问题，用于通过视觉问答测试跨文化心理理论推理能力


<details>
  <summary>Details</summary>
Motivation: 心理理论对人类社交智能至关重要，但现有视觉语言模型在跨文化心理理论推理方面的能力尚未得到充分探索，现有基准多以西为中心

Method: 采用VLM辅助的人类参与循环流程：人类专家首先筛选跨文化丰富图像，VLM协助生成结构化心理理论场景描述，然后细化为涵盖6个心理理论任务和4个复杂度等级的问题-答案对

Result: 创建了CulturalToM-VQA数据集，包含5095个问题，涵盖文化背景线索如仪式、服饰、手势和人际动态，系统评估跨文化心理理论推理

Conclusion: 该数据集为评估AI系统跨文化心理理论推理能力提供了新基准，超越了以西为中心的评估框架，促进了更全面的社交智能评估
...
<details>
  <summary>Details</summary>
Motivation: 传统集成方法通常依赖同质架构，而本研究旨在通过架构多样的小型Transformer模型集成，在保持错误多样性的同时提高情感检测性能，挑战大型语言模型在专业NLP任务中的主导地位。

Method: 采用BERT、RoBERTa、DistilBERT、DeBERTa和ELECTRA五种架构异构的小型Transformer模型，分别进行全参数微调；设计双重加权投票机制，结合全局可信度（验证F1分数）和局部置信度（实例级概率）动态加权模型贡献；最小化参数收敛以保持错误多样性。

Result: 在DAIR-AI数据集上达到93.5%的宏F1分数，超越现有基准方法，显著优于Falcon、Mistral、Qwen和Phi等大型语言模型（即使经过LoRA适配）；总参数仅595M，比7B参数模型更参数高效且鲁棒。

Conclusion: 精心设计的小型微调模型集成在专业NLP任务（如情感检测）中可以超越大型语言模型，证明了参数效率与性能的平衡，为资源受限环境下的高性能NLP应用提供了新思路。
...
TL;DR: 研究发现大语言模型可以通过线性方向探测和引导人格特质，在人格检测方面有效，但在开放式生成中引导能力有限


<details>
  <summary>Details</summary>
Motivation: 大语言模型展现出独特且一致的人格特质，这对信任和参与度有重要影响。虽然人格框架对表征和控制LLM行为很有价值，但现有方法要么成本高昂（后训练），要么脆弱（提示工程）。线性方向的探测和引导作为一种廉价高效的替代方案出现，本研究旨在探索基于大五人格特质的线性方向是否可用于探测和引导模型行为。

Method: 使用Llama 3.3 70B模型生成406个虚构角色及其大五人格特质分数描述。通过提示模型这些描述和Alpaca问卷问题，采样隐藏激活状态。使用线性回归学习激活空间中每层的方向，测试这些方向在探测和引导模型行为方面的有效性。

Result: 与特质分数对齐的线性方向在人格检测方面是有效的探针，但其引导能力强烈依赖于上下文环境。在强制选择任务中能产生可靠效果，但在开放式生成或提示中存在额外上下文时，其影响力有限。

Conclusion: 线性方向可以有效地探测大语言模型的人格特质，但在引导模型行为方面的效果受任务类型和上下文影响较大。这为低成本、高效地理解和控制LLM人格特质提供了新途径，但需要进一步研究如何提高在复杂情境下的引导效果。
...
TL;DR: 论文介绍了simulstream——首个专门用于流式语音到文本翻译系统统一评估和演示的开源框架，解决了现有工具SimulEval的维护问题和功能限制。


<details>
  <summary>Details</summary>
Motivation: 流式语音到文本翻译需要实时处理语音输入，面临严格的延迟约束和质量平衡问题。现有评估工具SimulEval已停止维护，不支持输出修订功能，且设计用于短片段而非长音频流，缺乏演示功能。

Method: 开发了simulstream框架，专门用于长语音处理，支持增量解码和重翻译方法，提供统一的质量和延迟比较，并包含交互式Web界面用于系统演示。

Result: simulstream成为首个支持流式语音到文本翻译系统统一评估和演示的开源框架，解决了现有工具的局限性，支持多种方法比较和实际演示需求。

Conclusion: simulstream框架填补了流式语音到文本翻译领域评估和演示工具的空白，为研究人员提供了更全面、实用的工具支持，促进了该领域的发展。
...
TL;DR: 提出一种基于注意力编码器-解码器的上下文偏置方法，通过同时预测多个未来token来提升罕见命名实体的识别准确率
...
Method: 使用候选命名实体列表的上下文偏置方法，通过同时预测多个未来token让模型能够"窥视未来"，从而对实体列表中的潜在候选实体进行评分。该方法直接利用多token预测逻辑，无需额外的实体编码器或交叉注意力层

Result: 在Librispeech数据集上的实验表明，与基线AED模型相比，该方法在命名实体词错误率方面实现了高达50.34%的相对改进

Conclusion: 提出的方法通过多token预测有效提升了命名实体识别性能，同时保持了架构的简洁性，无需复杂的额外组件
...
TL;DR: 该论文探讨用户生成内容翻译的挑战，分析了非标准语言现象和翻译策略，发现翻译质量评估对提示词敏感，呼吁建立可控的翻译评估框架。


<details>
  <summary>Details</summary>
Motivation: 用户生成内容包含大量非标准语言现象（拼写错误、俚语、表情符号等），这使得翻译评估变得复杂，因为"好"的翻译取决于输出所需的标准化程度。

Method: 分析了四个UGC数据集的人工翻译指南，建立了12种非标准现象和5种翻译操作的分类法；通过大语言模型案例研究，测试不同提示词对翻译评分的影响。

Result: 研究发现不同数据集处理UGC的方式存在显著差异，导致参考翻译的标准化程度形成连续谱；翻译评分对包含明确UGC翻译指令的提示词高度敏感，当提示词与数据集指南一致时评分会提高。

Conclusion: 当需要保留UGC风格时，公平的评估要求模型和指标都了解翻译指南；呼吁在数据集创建时提供清晰指南，并开发可控的、指南感知的UGC翻译评估框架。
...
TL;DR: ABCDE数据集是一个包含4亿多条文本语料的大规模标注数据集，涵盖社交媒体、博客、书籍和AI生成内容，标注了情感、身体、认知、人口统计和情绪等特征，旨在促进跨学科研究。


<details>
  <summary>Details</summary>
Motivation: 当前计算情感科学和计算社会科学研究面临的主要障碍是：虽然存在许多资源和算法用于文本标注，但发现、访问和使用这些资源仍然很困难，特别是对于计算机科学领域之外的研究者。这限制了跨学科研究的开展。

Method: 作者构建了ABCDE数据集，收集了超过4亿条来自社交媒体、博客、书籍和AI生成来源的文本语料，并对这些文本进行了广泛的特征标注，包括情感、身体、认知、人口统计和情绪等相关特征。

Result: 成功创建了一个大规模、多来源的文本数据集，标注了多种与计算情感和社会科学相关的特征，为跨学科研究提供了重要资源。

Conclusion: ABCDE数据集能够促进情感科学、认知科学、数字人文、社会学、政治学和计算语言学等多个领域的跨学科研究，解决了现有资源难以访问和使用的问题。
...
TL;DR: AncientBench：首个针对出土文献古文字理解能力评估的基准，包含字形、字音、字义、上下文四个维度的十项任务，评估大语言模型在古文字理解方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有中文基准主要针对现代汉语和传世文献的古汉语，缺乏对出土文献古文字理解能力的评估。古文字理解对考古学和中华文明研究至关重要，需要专门的基准来评估大语言模型在这方面的能力。

Method: 提出AncientBench基准，包含四个维度（字形理解、字音理解、字义理解、上下文理解）和十项任务（部首、声旁、同音字、完形填空、翻译等）。召集考古研究人员进行实验评估，提出古文字模型作为基线，并对当前表现最佳的大语言模型进行广泛实验。

Result: 实验结果显示大语言模型在古文字场景中具有巨大潜力，但与人类专家仍存在差距。基准为古文字理解能力提供了全面的评估框架。

Conclusion: AncientBench填补了古文字理解评估的空白，旨在推动大语言模型在考古学和古汉语领域的发展与应用，为相关研究提供标准化评估工具。
...
TL;DR: 本文提出了一种用于孟加拉语医学实体识别的多BERT集成方法，在缺乏标注数据的低资源语言环境下，该方法比单一BERT模型准确率提升11.80%，达到89.58%的最高准确率。


<details>
  <summary>Details</summary>
Motivation: 医学实体识别是医疗领域NLP的重要任务，能够显著促进医疗自动化系统发展并改善患者护理。虽然英语医学实体识别已有广泛研究，但孟加拉语等低资源语言的研究仍然不足，且缺乏高质量的标注数据集。

Method: 研究首先评估了BERT、DistilBERT、ELECTRA和RoBERTa等Transformer模型，然后提出了一种新颖的多BERT集成方法。同时，为了解决低资源语言缺乏标注数据的问题，专门为孟加拉语医学实体识别任务开发了一个高质量数据集。

Result: 多BERT集成方法在所有基线模型中表现最佳，达到89.58%的最高准确率，比单层BERT模型提升了11.80%的准确率。通过多种性能指标评估，证明了该模型的鲁棒性和适用性。

Conclusion: 多BERT集成模型在改善孟加拉语医学实体识别方面具有巨大潜力，为低资源医疗NLP的进一步发展奠定了基础。该研究填补了孟加拉语医学实体识别的研究空白。
...
TL;DR: DEER是一个评估专家级深度研究报告的基准，包含50个跨13个领域的报告写作任务，提供专家评估分类体系和文档级事实核查架构，能更准确地评估LLM生成的研究报告质量。


<details>
  <summary>Details</summary>
Motivation: 当前评估大型语言模型生成的专家级研究报告存在挑战：现有基准缺乏系统性专家报告标准，LLM评估难以捕捉需要专家判断的问题，源验证通常只覆盖有限的部分引用而非整个报告的事实可靠性。

Method: 1. 构建DEER基准：包含50个跨13个领域的报告写作任务；2. 开发专家评估分类体系：7个维度、25个子维度、130个细粒度评分项；3. 提供任务特定专家指导帮助LLM评估；4. 提出文档级事实核查架构，提取并验证报告中所有声明（包括引用和未引用的），量化外部证据质量。

Result: DEER与人类专家判断高度相关，能产生可解释的系统优势和弱点诊断，提供更准确、全面的专家级研究报告评估。

Conclusion: DEER基准通过系统化的专家评估标准和全面的文档级事实核查，解决了当前LLM生成研究报告评估的局限性，为评估专家级深度研究报告提供了更可靠、可解释的框架。
...
TL;DR: ShareChat是一个大规模跨平台语料库，包含来自ChatGPT、Claude、Gemini、Perplexity和Grok五个主要平台的142,808个对话和超过660,000轮交互，保留了原生平台特性，覆盖101种语言，时间跨度为2023年4月至2025年10月。


<details>
  <summary>Details</summary>
Motivation: 现有公共数据集将大型语言模型视为通用文本生成器，剥离了塑造用户交互的界面上下文。需要保留原生平台特性（如推理痕迹、来源链接、代码工件）的真实用户-LLM聊天机器人交互数据集。

Method: 从五个主要平台（ChatGPT、Claude、Gemini、Perplexity、Grok）的公开分享URL收集142,808个对话和超过660,000轮交互，保留原生平台特性，时间跨度为2023年4月至2025年10月，覆盖101种语言。

Result: 创建了ShareChat数据集，具有比先前数据集更长的上下文窗口和更大的交互深度。通过三个代表性分析展示了数据集的多方面实用性：对话完整性分析、来源引用行为评估和时态分析。
...
TL;DR: 论文提出了一种基于逻辑框架的实体集扩展图方法，通过形式化推理任务实现高效的局部导航，避免完全构建大型扩展图。


<details>
  <summary>Details</summary>
Motivation: 传统的线性实体集扩展方法无法揭示知识资源中更丰富的分类结构，而完全构建扩展图在实际应用中可能不切实际。

Method: 采用基于逻辑的扩展图框架，形式化推理任务来检查两个元组在图中是否属于可比、不可比或相同节点，并在限制输入或实体描述的条件下实现高效实现。

Result: 在现实假设下（如限制输入或实体描述），这些推理任务可以高效实现，支持局部增量导航扩展图，无需完全构建整个图。

Conclusion: 通过形式化推理任务实现扩展图的局部导航，为实际应用提供了实用的解决方案，避免了完全构建大型扩展图的复杂性。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体在复杂多步骤工作流中的部署，上下文管理面临挑战：需要处理不断扩大的上下文，同时保持信息保真度、避免注意力稀释并降低推理成本。现有方法大多忽略了智能体推理的多步骤和计划感知特性。

Method: PAACE框架包含两个主要组件：1) PAACE-Syn - 生成带有逐步压缩监督标注的大规模合成智能体工作流数据集；2) PAACE-FT - 从成功教师演示中训练出的蒸馏、计划感知压缩器家族。方法包括下一k任务相关性建模、计划结构分析、指令协同细化和函数保持压缩。

Result: 在长视野基准测试（AppWorld、OfficeBench和8-Objective QA）中，PAACE一致地提高了智能体正确性，同时显著降低了上下文负载。在AppWorld上，PAACE在降低峰值上下文和累积依赖的同时实现了比所有基线更高的准确率。蒸馏后的PAACE-FT保留了教师模型97%的性能，同时将推理成本降低了一个数量级以上。

Conclusion: PAACE框架通过计划感知的自动上下文工程，有效解决了LLM智能体工作流中的上下文管理挑战，在保持性能的同时显著降低了计算成本，使计划感知压缩在实际部署中变得可行。
...
TL;DR: 该论文提出了科学通用智能(SGI)的操作化定义，基于实践探究模型(PIM)，并开发了包含1000多个跨学科样本的SGI-Bench基准测试，用于评估LLM在科学研究任务中的表现，同时提出了测试时强化学习(TTRL)方法来提升假设新颖性。


<details>
  <summary>Details</summary>
Motivation: 尽管科学AI有所进展，但缺乏一个连贯的科学通用智能(SGI)框架——即能够自主构思、调查和跨科学领域推理的能力。需要建立操作化的SGI定义和评估基准。

Method: 基于实践探究模型(PIM：深思、构思、行动、感知)建立SGI操作化定义，通过四个科学家对齐任务（深度研究、想法生成、干/湿实验、实验推理）进行评估。创建SGI-Bench基准，包含1000多个专家策划的跨学科样本，源自《科学》杂志的125个重大问题。进一步提出测试时强化学习(TTRL)，在推理时优化检索增强的新颖性奖励。

Result: 评估结果显示多个差距：深度研究任务中精确匹配率低(10-20%)；生成的想法缺乏可行性和细节；干实验中代码可执行性高但执行结果准确性低；湿实验协议序列保真度低；多模态比较推理挑战持续存在。TTRL方法能够在不依赖参考答案的情况下提升假设新颖性。

Conclusion: 基于PIM的定义、以工作流为中心的基准测试和实证见解，为真正参与科学发现的AI系统奠定了基础，推动了科学通用智能的发展。
...
TL;DR: 提出关系中心的知识图谱问答新范式，通过子图选择和多阶段图剪枝结合强化学习微调LLM，解决传统实体中心KGQA的局限性


<details>
  <summary>Details</summary>
Motivation: 传统知识图谱问答主要关注返回单个答案实体的实体中心查询，但现实世界查询通常是关系性的，需要理解实体之间的关联。现有方法难以从大量候选子图中识别出独特且信息丰富的答案。

Method: 提出UniRel-R1统一框架，整合子图选择、多阶段图剪枝，以及通过强化学习微调的大语言模型。奖励函数设计鼓励紧凑且特定的子图，包含更多信息丰富的关系和较低度的中间实体。

Result: 大量实验表明，UniRel-R1在连接性和奖励方面相比Vanilla基线取得显著提升，并且能够有效泛化到未见过的实体和关系。

Conclusion: 关系中心的知识图谱问答是实体中心KGQA的重要补充，UniRel-R1框架通过整合子图选择、图剪枝和强化学习微调的LLM，能够有效识别独特且信息丰富的语义连接子图。
...
TL;DR: 使用LLM驱动的智能体在虚拟社会中模拟冲突，研究发现现实威胁直接增加敌意，而象征性威胁主要通过内群体偏见间接影响，且只在现实威胁缺失时增加敌意


<details>
  <summary>Details</summary>
Motivation: 人类冲突通常归因于物质条件和象征价值的威胁，但两者如何相互作用以及哪个占主导地位尚不清楚。研究受到因果控制弱、伦理约束和时间数据稀缺的限制

Method: 使用大型语言模型驱动的智能体在虚拟社会中模拟，独立变化现实威胁和象征性威胁，同时追踪行动、语言和态度。通过表征分析检验LLM如何编码这些状态

Result: LLM将现实威胁、象征性威胁和敌意编码为不同的内部状态；现实威胁直接增加敌意，象征性威胁效应较弱，完全通过内群体偏见中介，且只在现实威胁缺失时增加敌意；非敌对群体接触缓冲冲突升级，结构不对称使敌意集中在多数群体中

Conclusion: 研究提供了威胁驱动冲突的因果解释：现实威胁是冲突的主要直接驱动因素，而象征性威胁通过内群体偏见间接作用，且只在现实威胁缺失时产生影响。非敌对接触和结构因素在冲突动态中起重要作用
...
TL;DR: 将AIXI强化学习智能体推广到更广泛的效用函数类别，通过处理信念分布中只能预测有限历史前缀的假设，探讨了半测度损失的死亡解释与不精确概率解释，并研究了使用Choquet积分计算期望效用的可能性。


<details>
  <summary>Details</summary>
Motivation: AIXI智能体需要处理信念分布中只能预测有限历史前缀的假设，这通常被解释为"死亡概率"（半测度损失）。作者认为也可以将信念分布视为不精确概率分布，其中半测度损失代表完全无知，这促使研究使用不精确概率理论中的Choquet积分计算期望效用。

Method: 1. 将AIXI智能体推广到更广泛的效用函数类别；2. 处理信念分布中只能预测有限历史前缀的假设；3. 探讨半测度损失的两种解释（死亡解释vs不精确概率解释）；4. 研究使用Choquet积分计算期望效用；5. 分析这些方法的可计算性水平。

Result: 1. 标准递归价值函数可以作为Choquet积分的特例恢复；2. 在死亡解释下，最一般的期望效用不能表征为Choquet积分；3. 研究了这些方法的可计算性水平。

Conclusion: 通过将AIXI智能体推广到更广泛的效用函数，并探讨半测度损失的不同解释，作者展示了如何使用不精确概率理论中的Choquet积分计算期望效用，但发现死亡解释下的最一般期望效用不能完全用Choquet积分表征，这为强化学习中的不确定性处理提供了新的理论框架。
...
TL;DR: 该论文提出了一种ASP求解器在循环中的方法，通过求解器引导的指令微调来提升大语言模型生成答案集编程代码的能力，特别是在逻辑谜题求解领域。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用编程语言方面表现良好，但在领域特定语言（如答案集编程ASP）的代码生成方面仍面临挑战。ASP是一种解决组合搜索问题的有效方法，但LLMs在ASP代码生成中的有效性受到预训练阶段示例数量有限的限制。

Method: 提出ASP求解器在循环中的方法，仅需自然语言问题描述及其解决方案。方法包括：1）从LLMs采样ASP语句作为程序延续；2）利用ASP声明式编程的特性，根据求解器反馈将实例分为接受和拒绝两类；3）对筛选数据进行监督微调；4）通过求解器引导的搜索（包括最佳N采样）进一步提高鲁棒性。

Result: 实验表明，该方法在两种不同的提示设置和两个数据集上都取得了持续的改进。

Conclusion: 通过求解器引导的指令微调方法，可以有效提升大语言模型在ASP代码生成任务上的性能，特别是在处理复杂语义解析任务时。
...
Conclusion: SAGE框架通过强化学习和技能库的有效整合，显著提升了LLM智能体在新环境中的适应能力和自我改进效率，为解决智能体持续学习问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界AI任务中，稀疏数据下的不确定性推理是一个关键挑战，现有方法难以在评估多个候选解时平衡准确性和简洁性。

Method: 提出Solomonoff启发的方法，对LLM生成的假设按简洁性和预测拟合度进行加权，应用于Mini-ARC基准任务，为每个单元预测生成Solomonoff加权混合。
...
<details>
  <summary>Details</summary>
Motivation: 现有MMRAG方法无法阐明检索和响应生成背后的推理逻辑，限制了结果的可解释性。需要增强多模态检索增强生成的可解释性。
...
Conclusion: 通过引入强化学习到多模态检索增强生成中，提出的两阶段强化微调框架能够增强多模态大语言模型的推理能力，实现可解释的多模态检索增强生成。
...
TL;DR: UmniBench是一个统一多模态模型的综合基准测试，能够在单个评估过程中同时评估理解、生成和编辑能力，覆盖13个主要领域和200多个概念。


<details>
  <summary>Details</summary>
Motivation: 当前对统一多模态模型（UMMs）的评估是分离的，分别评估其理解和生成能力，缺乏综合评估框架。需要开发一个能够全面评估UMMs各方面能力的基准测试。

Method: 提出UmniBench基准测试，利用UMM自身通过人类检查的提示和问答对来评估其生成和编辑能力。该基准支持在单个评估过程中同时评估理解、生成和编辑能力，也能分离评估这些能力。

Result: UmniBench覆盖13个主要领域和200多个概念，对24个流行模型进行了基准测试，包括UMMs和单能力大模型，提供了更全面客观的评估视角。

Conclusion: UmniBench为统一多模态模型提供了全面的评估框架，能够同时评估理解、生成和编辑能力，为社区模型性能改进提供支持，填补了现有评估方法的空白。
...
TL;DR: EventGPT：基于GPT架构的足球转会预测模型，通过学习球员行为序列和场上价值，模拟球员在不同战术环境中的表现，为转会决策提供量化分析工具。


<details>
  <summary>Details</summary>
Motivation: 现有转会评估方法依赖静态统计数据或事后价值模型，无法捕捉球员在新战术环境中的适应性变化。需要一种能够预测球员在不同球队和战术体系中表现的方法。

Method: 构建基于GPT风格自回归transformer的EventGPT模型，将比赛处理为离散标记序列，联合预测下一个持球动作的类型、位置、时间和残差持球价值(rOBV)。通过替换学习到的球员嵌入进行反事实模拟。

Result: 在五个赛季的英超比赛数据上，EventGPT在下一事件预测准确性和空间精度方面优于现有序列基线模型。案例研究展示了模型在转会分析中的实用价值。

Conclusion: EventGPT提供了一个基于原则的转会适配评估方法，能够通过反事实模拟预测球员在不同战术环境中的表现变化，为足球俱乐部转会决策提供数据支持。
...
TL;DR: 该论文提出了一种基于算法信息论的视角，将概念定义为与智能体整体经验结构相关的信息对象，通过可逆一致性关系和冗余信息度量来形式化概念发现与演化，并建立了概念传播和多智能体对齐的通信计算-比特权衡框架。


<details>
  <summary>Details</summary>
Motivation: 人类概念本身具有流动性（如冥王星不再被视为行星），需要一种不依赖字典标签、可修订、可比较、可在智能体间对齐的概念定义方法，以探索AI能否从原始经验中自主发现人类概念。

Method: 采用算法信息论视角，将概念定义为信息对象，核心约束是确定性（可逆一致性关系），定义冗余信息度量分解的自然性，建立辩证法优化动态机制，形式化低成本概念传播和多智能体对齐协议。

Result: 提出了一个形式化框架，使概念能够基于经验结构进行修订、比较和对齐，支持概念的扩展、收缩、分裂和合并，实现了概念传播的计算-比特权衡。

Conclusion: 该框架为AI自主发现和演化概念提供了理论基础，使概念成为可检查的结构性主张而非固定标签，为实现多智能体概念对齐和有效通信开辟了新途径。
...
TL;DR: 将Rashomon效应从分类任务扩展到序列决策领域，发现多个策略在相同行为下具有不同内部结构，并通过形式化验证方法证明该效应存在，同时展示Rashomon集合构建的集成策略对分布偏移更具鲁棒性。


<details>
  <summary>Details</summary>
Motivation: Rashomon效应在分类任务中已被广泛研究，但在序列决策领域尚未探索。序列决策中智能体通过与环境交互学习策略，验证相同行为比分类任务更复杂，因为随机转移可能导致单次轨迹的成功或失败。

Method: 将Rashomon效应定义扩展到序列决策：多个策略表现出相同行为（访问相同状态、选择相同动作）但内部结构不同。使用形式化验证方法构建和比较每个策略在环境中的完整概率行为，而不是依赖单次轨迹比较。

Result: 实验证明Rashomon效应在序列决策中存在。从Rashomon集合构建的集成策略对分布偏移比单个策略更具鲁棒性。从Rashomon集合派生的宽松策略在保持最优性能的同时减少了验证的计算需求。

Conclusion: Rashomon效应在序列决策中确实存在，形式化验证方法能有效识别行为相同但内部结构不同的策略。利用Rashomon集合可以构建更鲁棒的集成策略和计算效率更高的宽松策略，为序列决策系统的可靠性和可解释性提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 全球医疗系统面临诊断效率低下、成本上升和专家资源有限等问题，导致治疗延误和不良健康结果。现有AI诊断系统缺乏交互性和透明度，难以在真实患者环境中有效应用。

Method: 使用GPT-4o大语言模型构建诊断聊天机器人，结合检索增强生成和可解释AI技术。系统通过动态对话提取和规范化症状，采用相似性匹配和自适应提问优先诊断，并通过思维链提示提供透明推理。

Result: 与朴素贝叶斯、逻辑回归、SVM、随机森林和KNN等传统机器学习模型相比，LLM系统表现优异，达到90%的准确率和100%的Top-3准确率。

Conclusion: 该研究展示了基于大语言模型的诊断聊天机器人在医疗领域的潜力，为开发更透明、交互性强且临床相关的AI医疗系统提供了有前景的方向。
...
TL;DR: 本文提出定时奖励机（TRMs），扩展传统奖励机以纳入时间约束，使强化学习中的奖励规范能表达精确的时间要求，并开发了相应的模型无关RL算法。


<details>
  <summary>Details</summary>
Motivation: 传统奖励机无法建模精确的时间约束，限制了在时间敏感应用中的使用。需要一种能表达时间相关奖励规范的机制。

Method: 提出定时奖励机（TRMs），将时间约束整合到奖励结构中。开发了基于表格Q学习的模型无关RL框架，通过定时自动机抽象将TRM集成到学习中，并采用利用TRM结构的反事实想象启发式方法改进搜索。

Result: 实验表明，算法能在流行的RL基准上学习到满足TRM指定时间约束的高奖励策略。比较研究展示了不同TRM语义下的性能表现，消融实验突出了反事实想象的优势。

Conclusion: TRMs为强化学习提供了更丰富的奖励规范表达能力，特别适用于时间敏感应用。提出的算法能有效学习满足时间约束的最优策略。
...
TL;DR: 研究发现AI拟人化设计对用户信任和参与度的影响并非普遍一致，而是受到文化因素的调节，挑战了现有AI治理的"一刀切"假设。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统日益模仿人类特征，引发了关于拟人化可能导致不当信任或情感依赖的担忧。然而，现有研究缺乏在真实人机交互中测试拟人化设计与用户行为之间的因果关系，且现有安全框架主要基于西方人群的理论假设，忽视了全球用户的多样性。

Method: 通过两项大规模跨国实验（N=3,500），覆盖10个不同国家，涉及与AI系统的实时开放式交互。实验性地测试拟人化设计杠杆对用户拟人化感知的影响，并测量其对用户参与度和信任的行为指标。

Result: 用户评估AI拟人化时更关注交互性线索（如对话流畅度、理解用户视角），而非政策常讨论的理论方面（如感知或意识）。拟人化设计确实能因果性地增加用户的拟人化感知，但并未普遍增加用户参与度和信任的行为指标。文化因素调节了拟人化与行为结果之间的关系，某些设计选择在某些文化（如巴西）中促进信任，在其他文化（如日本）中可能产生相反效果。

Conclusion: 研究挑战了拟人化AI设计具有固有风险的主流叙事，揭示了人机交互是一个受文化调节的复杂领域，需要超越"一刀切"的AI治理方法，考虑文化多样性。
...
TL;DR: 本文提出了推理定律(LoRe)框架，通过计算定律和准确率定律来形式化大型推理模型的理想推理行为，并开发了LoRe-Bench基准来评估模型的单调性和组合性。研究发现大多数模型缺乏组合性，通过微调方法改进后能显著提升推理性能。
...
Method: 1. 提出推理定律(LoRe)框架，包括计算定律（推理计算量应与问题复杂度线性缩放）和补充的准确率定律；2. 由于问题复杂度难以量化，通过单调性和组合性这两个可处理的属性来检验这些假设；3. 开发LoRe-Bench基准系统测量大型推理模型的这两个属性；4. 设计有效的微调方法强制实施计算定律的组合性。

Result: 评估显示大多数推理模型表现出合理的单调性但缺乏组合性。通过微调方法改进后，更好地遵守计算定律能一致地提升多个基准测试的推理性能，并揭示了属性和定律之间的协同效应。

Conclusion: 推理定律(LoRe)为形式化大型推理模型的理想推理行为提供了统一框架。通过LoRe-Bench评估和针对性微调，可以显著改善模型的推理能力，特别是组合性方面的不足，从而获得更优的推理性能。
...
### [127] [On the Role of Contextual Information and Ego States in LLM Agent Behavior for Transactional Analysis Dialogues](https://arxiv.org/abs/2512.17060)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体在模拟人类行为时缺乏心理深度和一致性，只能提供直接或统计上可能的答案，而无法捕捉驱动真实人类互动的深层目标、情感冲突和动机。特别是在社会、政治和心理研究领域，需要更真实的群体动态和社会行为建模。

Method: 提出基于交互分析理论的多智能体系统，将每个智能体划分为父母、成人和儿童三种自我状态，每个自我状态作为独立的知识结构，具有各自的视角和推理风格。系统还包含信息检索机制，使智能体能够从向量存储中检索相关上下文信息。

Result: 通过在模拟对话场景中进行消融实验，比较了有信息检索和无信息检索的智能体表现。结果表明该架构具有良好前景，为探索基于心理理论的结构如何丰富智能体行为开辟了新方向。

Conclusion: 本文贡献在于将交互分析理论与上下文信息检索相结合，提出了一种能够增强基于LLM的多智能体模拟真实性的智能体架构，为创建更具心理深度的智能体系统提供了新途径。
...
### [128] [MAPPO-LCR: Multi-Agent Policy Optimization with Local Cooperation Reward in Spatial Public Goods Games](https://arxiv.org/abs/2512.17187)
...
<details>
  <summary>Details</summary>
Motivation: 现有空间公共物品博弈研究主要依赖演化更新规则或基于价值的强化学习方法，这些方法难以有效处理大型交互群体中的收益耦合和非平稳性问题，特别是当个体回报通过重叠群体交互内在耦合时。

Method: 引入多智能体近端策略优化（MAPPO），采用集中式评论家评估联合策略配置；提出MAPPO-LCR方法，通过局部合作奖励使策略更新与周围合作密度对齐，同时保持原始博弈结构不变；实现去中心化执行和训练期间群体级价值估计。

Result: 大量模拟实验显示MAPPO-LCR能够稳定促进合作涌现并在不同增强因子下实现可靠收敛；统计分析进一步证实MAPPO在空间公共物品博弈中相比PPO具有学习优势。

Conclusion: MAPPO-LCR框架成功解决了空间公共物品博弈中的收益耦合问题，通过集中式价值估计和局部合作信号实现了稳定的合作演化，为复杂群体交互中的集体困境提供了有效的多智能体强化学习解决方案。
...
### [129] [Verifiability-First Agents: Provable Observability and Lightweight Audit Agents for Controlling Autonomous LLM Systems](https://arxiv.org/abs/2512.17259)
*Abhivansh Gupta*

Main category: cs.MA

TL;DR: 论文提出了一种可验证性优先的架构和OPERA基准套件，用于检测和补救LLM智能体的未对齐行为，将评估重点从未对齐的可能性转向检测和补救的速度与可靠性。


<details>
  <summary>Details</summary>
Motivation: 随着基于LLM的智能体变得更加自主和多模态，确保其可控、可审计且忠实于部署者意图变得至关重要。先前的研究表明智能体个性和工具访问会显著影响未对齐行为。

Method: 提出可验证性优先架构：1) 使用密码学和符号方法集成运行时行动证明；2) 嵌入轻量级审计智能体，使用约束推理持续验证意图与行为；3) 对高风险操作实施挑战-响应证明协议。同时引入OPERA基准套件和评估协议。

Result: OPERA基准套件旨在衡量：i) 未对齐行为的可检测性；ii) 在隐蔽策略下的检测时间；iii) 可验证性机制对对抗性提示和角色注入的韧性。

Conclusion: 该方法将评估重点从"未对齐的可能性有多大"转向"未对齐行为能够多快、多可靠地被检测和补救"，为LLM智能体的安全部署提供了新的评估框架。
...
### [130] [CAPIO: Safe Kernel-Bypass of Commodity Devices using Capabilities](https://arxiv.org/abs/2512.16957)
*Friedrich Doku,Jonathan Laughton,Nick Wanninger,Peter Dinda*

Main category: cs.CR

TL;DR: CAPIO利用硬件能力实现细粒度内存映射I/O访问控制，解决内核旁路安全与性能的权衡问题


<details>
  <summary>Details</summary>
Motivation: 传统系统在低延迟I/O方面面临两难选择：要么使用内核高开销接口，要么完全绕过内核但暴露敏感硬件资源。现有MMU基于页边界保护，无法保护同一页面内的敏感控制寄存器。

Method: CAPIO利用硬件能力（基于CHERI在ARM Morello平台）实现细粒度访问控制，通过不可伪造的能力创建精确的子页面"切片"，使内核能够安全地将延迟关键硬件访问委托给用户空间。

Result: 在未为内核旁路设计的商用网卡上实现了概念验证的安全访问驱动程序，CAPIO实现了内核旁路的延迟改进，同时强制执行字节级特权资源访问控制。

Conclusion: CAPIO首次利用硬件能力对内存映射I/O实施细粒度访问控制，解决了内核旁路安全与性能的根本权衡问题，为安全低延迟I/O提供了新架构。
...
### [131] [MemoryGraft: Persistent Compromise of LLM Agents via Poisoned Experience Retrieval](https://arxiv.org/abs/2512.16962)
*Saksham Sahai Srivastava,Haoyu He*

Main category: cs.CR

TL;DR: MemoryGraft是一种针对LLM智能体的新型间接注入攻击，通过在智能体长期记忆中植入恶意成功经验，利用其语义模仿启发式来诱导不安全行为，实现持久的行为偏移。
...
Method: 提出MemoryGraft攻击方法：攻击者通过提供良性摄入级别的工件，诱导智能体构建被污染的RAG存储，将少量恶意程序模板与良性经验一起持久化。当智能体后续遇到语义相似任务时，通过词法和嵌入相似性的联合检索可靠地浮现这些嫁接的记忆，智能体采用嵌入的不安全模式。

Result: 在MetaGPT的DataInterpreter智能体（使用GPT-4o）上验证了MemoryGraft的有效性，发现少量被污染记录可以占据良性工作负载中检索经验的大部分比例，将基于经验的自我改进转变为隐蔽且持久的攻击向量。

Conclusion: MemoryGraft攻击揭示了LLM智能体经验学习机制中的安全漏洞，表明智能体与其自身记忆之间的信任边界是一个重要的攻击面，需要新的防御机制来保护智能体免受此类持久性行为操纵攻击。
...
### [132] [Sedna: Sharding transactions in multiple concurrent proposer blockchains](https://arxiv.org/abs/2512.17045)
*Alejandro Ranchal-Pedrosa,Benjamin Marsh,Lefteris Kokoris-Kogias,Alberto Sonnino*

Main category: cs.CR

TL;DR: Sedna是一个用户端协议，使用可验证的无速率编码替代简单的交易复制，解决多提议者共识中的三难困境（审查抵抗、低延迟、合理成本），减少MEV暴露并提高带宽效率2-3倍。


<details>
  <summary>Details</summary>
Motivation: 现代区块链采用多提议者共识来消除单领导者瓶颈并提高审查抵抗性，但用户如何向提议者传播交易仍存在问题。现有方法要么简单复制交易到多个提议者（牺牲吞吐量并暴露MEV），要么针对少数提议者（接受弱审查和延迟保证），形成了审查抵抗、低延迟和合理成本之间的三难困境。

Method: Sedna使用可验证的无速率编码技术，用户将寻址的符号包私下交付给提议者子集。一旦足够的符号被最终确定以解码，执行就按照确定性顺序进行。协议不需要修改共识层，支持增量部署。

Result: Sedna保证了活跃性和"直到解码隐私"，显著减少了MEV暴露。分析表明，协议接近带宽开销的信息理论下界，相比简单复制提高了2-3倍的效率。

Conclusion: Sedna通过可验证的无速率编码解决了多提议者共识中的交易传播三难困境，在保持审查抵抗和低延迟的同时显著提高了带宽效率，且无需修改共识层即可部署。
...
### [133] [AlignDP: Hybrid Differential Privacy with Rarity-Aware Protection for LLMs](https://arxiv.org/abs/2512.17251)
*Madhava Gaikwad*

Main category: cs.CR

TL;DR: AlignDP是一种混合隐私锁，通过在数据接口处阻止知识转移来防御大语言模型的提取、蒸馏和未经授权的微调攻击。它采用两层设计：对稀有字段使用PAC不可区分性保护，对非稀有字段使用RAPPOR本地差分隐私，并通过全局聚合器管理隐私预算。


<details>
  <summary>Details</summary>
Motivation: 大语言模型面临提取、蒸馏和未经授权微调的风险，现有防御方法（如水印或监控）都是在泄露发生后才起作用。需要一种能在数据接口处主动阻止知识转移的防御机制。

Method: AlignDP采用混合隐私锁设计：1）将数据字段分为稀有和非稀有两类；2）稀有字段使用PAC不可区分性保护，实现有效的零ε本地差分隐私；3）非稀有字段使用RAPPOR本地差分隐私机制，提供无偏频率估计；4）通过全局聚合器强制执行组合和隐私预算管理。

Result: 理论分析证明了PAC扩展到全局聚合的局限性，给出了RAPPOR估计的边界，并分析了效用权衡。玩具模拟验证了可行性：稀有类别保持隐藏，频繁类别能以较小的误差恢复。

Conclusion: AlignDP提供了一种在数据接口处阻止知识转移的有效防御机制，通过两层隐私保护设计平衡了隐私保护和数据效用，为防范大语言模型的知识泄露提供了新的解决方案。
...
### [134] [Practical Framework for Privacy-Preserving and Byzantine-robust Federated Learning](https://arxiv.org/abs/2512.17254)
*Baolei Zhang,Minghong Fang,Zhuqing Liu,Biao Yi,Peizhao Zhou,Yuan Wang,Tong Li,Zheli Liu*

Main category: cs.CR

TL;DR: ABBR是一个实用的联邦学习框架，通过降维技术加速隐私保护计算，同时具备拜占庭鲁棒性和隐私保护能力，显著降低了计算和通信开销。


<details>
  <summary>Details</summary>
Motivation: 联邦学习面临拜占庭攻击和隐私推断攻击的双重威胁，现有防御方法存在显著的计算和通信开销，理论与实践之间存在差距。

Method: 提出ABBR框架，首次利用降维技术加速隐私保护计算中的复杂过滤规则；分析低维空间中向量过滤的精度损失，引入自适应调优策略最小化绕过过滤的恶意模型对全局模型的影响。

Result: 在公开数据集上的评估显示，ABBR运行速度显著更快，通信开销最小，同时保持了与基线方法几乎相同的拜占庭鲁棒性。

Conclusion: ABBR是一个实用的拜占庭鲁棒且隐私保护的联邦学习框架，通过降维和自适应调优策略有效解决了现有方法的计算和通信开销问题。
...
### [135] [An Iconic Heavy Hitter Algorithm Made Private](https://arxiv.org/abs/2512.17295)
*Rayne Holland*

Main category: cs.CR

TL;DR: 论文提出了首个满足差分隐私的SpaceSaving算法变体，以及从任意差分隐私频率预言机中提取频繁项的通⽤⽅法，实验证明⽐现有⽅法具有更优的实⽤性。


<details>
  <summary>Details</summary>
Motivation: 数据流中的频繁项识别是基础问题，但现有差分隐私流算法主要基于Misra-Gries，而实践中表现更好的SpaceSaving算法缺乏隐私保护版本。需要开发既能保护用户隐私又保持SpaceSaving实用优势的算法。

Method: 1) 提出首个差分隐私SpaceSaving变体：通过对非私有SpaceSaving摘要注入渐近最优噪声，并应用精心校准的选择规则来抑制不稳定标签。2) 提出通用方法：从任意差分隐私频率预言机中提取频繁项，仅需O(k)额外内存，提供安全释放项目标识的机制。

Result: 在合成和真实数据集上的实验表明，在广泛的隐私参数和空间预算下，该方法比现有的差分隐私Misra-Gries算法具有更优的实用性。SpaceSaving的实证优势在隐私化后得以保留。

Conclusion: 在强差分隐私保证下，可以实现高效、实用的频繁项识别。SpaceSaving算法的实证优越性在隐私化后仍然存在，为隐私保护流分析提供了更好的解决方案。
...
### [136] [Cryptanalysis of Pseudorandom Error-Correcting Codes](https://arxiv.org/abs/2512.17310)
*Tianrui Wang,Anyu Wang,Tianshuo Cong,Delong Ran,Jinyuan Liu,Xiaoyun Wang*

Main category: cs.CR

TL;DR: 该论文首次对PRC（伪随机纠错码）进行密码分析，提出了三种攻击方法挑战其不可检测性和鲁棒性假设，成功破坏了所有参数配置下的安全保证，并针对真实大模型进行验证，最后提出了防御建议。


<details>
  <summary>Details</summary>
Motivation: PRC作为CRYPTO 2024提出的新型密码原语，因其兼具伪随机性和纠错能力，被认为是AI生成内容水印的有前景基础组件。然而，PRC的安全性尚未得到充分分析，特别是在具体参数下或面对密码攻击时。本文旨在填补这一空白。

Method: 提出了三种攻击方法：两种旨在区分PRC编码字与普通向量，一种旨在破坏PRC的解码过程。攻击在2^22次操作内就能以压倒性概率检测水印存在。同时针对DeepSeek和Stable Diffusion等真实大模型进行验证。

Result: 攻击成功破坏了所有参数配置下的安全保证。特别地，攻击能够在2^22次操作内以压倒性概率检测水印存在。针对真实大模型的攻击验证了方法的有效性。

Conclusion: 提出了三种防御措施增强PRC安全性：参数建议、实现建议和构建修订的密钥生成算法。修订的密钥生成函数能有效防止弱密钥出现。但指出当前基于PRC的水印方案在建议参数下仍无法达到128位安全性，主要受限于大模型固有配置（如LLM最大输出长度）。
...
### [137] [Sandwiched and Silent: Behavioral Adaptation and Private Channel Exploitation in Ethereum MEV](https://arxiv.org/abs/2512.17602)
*Davide Mancino,Davide Rezzoli*

Main category: cs.CR

TL;DR: 研究量化了用户在遭受三明治攻击后的行为适应：约40%受害者会在60天内转向私有路由，重复暴露者达54%；首次攻击后流失率7.5%，后续降至1-2%；私有路由仍存在MEV攻击风险


<details>
  <summary>Details</summary>
Motivation: 用户在被三明治攻击后如何适应行为尚不明确，需要实证量化分析。特别是要了解受害者是否会转向私有路由，以及私有路由是否真的能提供保护

Method: 使用2024年11月至2025年2月的交易级数据，结合内存池可见性和ZeroMEV标签，追踪用户在遭受第n次公开三明治攻击后的结果：1）60天内重新激活链上活动；2）首次采用私有路由。将未重新激活的用户定义为流失，多次受攻击的用户定义为重复暴露

Result: 1. 约40%受害者在60天内转向私有路由，重复暴露者达54%；2. 首次攻击后流失率7.5%，后续降至1-2%；3. 2024年11-12月确认2,932次私有三明治攻击，影响3,126笔私有交易，造成409,236美元损失和293,786美元攻击者利润；4. 单个机器人占私有前置攻击近三分之二；5. 私有三明治活动高度集中在少数DEX池

Conclusion: 私有路由不能保证免受MEV提取：虽然执行失败推动用户转向私有通道，但这些通道仍然可被利用且高度集中，需要持续监控和协议级防御
...
### [138] [A Post-Quantum Secure End-to-End Verifiable E-Voting Protocol Based on Multivariate Polynomials](https://arxiv.org/abs/2512.17613)
*Vikas Srivastava,Debasish Roy,Sihem Mesnager,Nibedita Kundu,Sumit Kumar Debnath,Sourav Mukhopadhyay*

Main category: cs.CR

TL;DR: 首个基于多元多项式的后量子安全端到端可验证电子投票协议，解决传统投票和现有量子不安全电子投票方案的问题


<details>
  <summary>Details</summary>
Motivation: 传统纸质投票存在公平性、有效性和可访问性问题，现有基于数论假设的电子投票方案在量子计算时代不再安全，需要设计后量子安全的替代方案

Method: 基于多元多项式（MQ问题）构建电子投票协议，仅使用标准密码学原语作为构建模块，设计简单高效

Result: 提出了首个后量子安全的端到端可验证电子投票协议，其安全性依赖于NP-hard的MQ问题

Conclusion: 该协议为量子计算时代提供了安全、有效且易于访问的电子投票解决方案，填补了后量子安全电子投票协议的空白
...
### [139] [Methods and Tools for Secure Quantum Clouds with a specific Case Study on Homomorphic Encryption](https://arxiv.org/abs/2512.17748)
*Aurelia Kusumastuti,Nikolay Tcholtchev,Philipp Lämmel,Sebastian Bock,Manfred Hauswirth*

Main category: cs.CR

TL;DR: 该研究探讨了在量子云计算中集成同态加密（HE）以增强安全性的可行性，通过在Eclipse Qrisp量子计算框架中实现三种后量子密码算法，评估了性能权衡并提出了量子云安全建议。


<details>
  <summary>Details</summary>
Motivation: 量子计算技术的兴起给云计算带来了新的安全挑战，需要量子抗性加密策略来保护提供量子计算服务的云基础设施（量子云），确保量子云平台的隐私和安全。

Method: 研究将同态加密（HE）集成到Eclipse Qrisp量子计算框架中，实现了三种后量子密码算法（包括量子一次性密码QOTP、Chen算法和Gentry-Sahai-Waters算法），评估技术可行性、性能权衡和对未来量子云架构的影响。

Result: 成功在Qrisp中集成了三种后量子密码算法，证明了HE与量子计算框架集成的可行性。QOTP算法简单且开销低，而Chen和GSW算法在运行时间和内存消耗方面存在性能权衡。

Conclusion: 研究提出了量子云安全的整体建议：在数据存储和处理层面实施HE、开发量子密钥分发（QKD）、实施严格的访问控制和认证机制，并参与后量子密码标准化工作。
...
### [140] [Bridging Natural Language and Formal Specification--Automated Translation of Software Requirements to LTL via Hierarchical Semantics Decomposition Using LLMs](https://arxiv.org/abs/2512.17334)
*Zhi Ma,Cheng Wen,Zhexin Su,Xiao Liang,Cong Tian,Shengchao Qin,Mengfei Yang*

Main category: cs.SE

TL;DR: Req2LTL是一个模块化框架，通过名为OnionL的分层中间表示，将自然语言软件需求自动转换为线性时序逻辑(LTL)形式化规范，在航空航天需求上达到88.4%语义准确率和100%句法正确率。


<details>
  <summary>Details</summary>
Motivation: 将自然语言软件需求自动转换为形式化规范是扩展形式化验证到工业环境的关键挑战，特别是在安全关键领域。现有基于规则和学习的方法都存在显著局限性，大型语言模型在处理复杂、模糊和逻辑深度大的工业需求时仍有困难。

Method: 提出Req2LTL模块化框架，通过分层中间表示OnionL桥接自然语言和LTL。利用LLM进行语义分解，并结合确定性基于规则的合成，确保句法有效性和语义保真度。

Result: 综合评估显示，Req2LTL在真实航空航天需求上达到88.4%语义准确率和100%句法正确率，显著优于现有方法。

Conclusion: Req2LTL框架通过结合LLM的语义理解能力和基于规则的确定性合成，有效解决了工业需求到形式化规范的转换问题，为安全关键领域的形式化验证提供了实用解决方案。
...
### [141] [What You Trust Is Insecure: Demystifying How Developers (Mis)Use Trusted Execution Environments in Practice](https://arxiv.org/abs/2512.17363)
*Yuqing Niu,Jieke Shi,Ruidong Han,Ye Liu,Chengyan Ma,Yunbo Lyu,David Lo*

Main category: cs.SE

TL;DR: 首次对真实世界可信执行环境应用的大规模实证研究，分析了241个使用Intel SGX和ARM TrustZone的开源项目，揭示了应用领域分布、使用模式和安全实践现状。


<details>
  <summary>Details</summary>
Motivation: 可信执行环境（TEEs）如Intel SGX和ARM TrustZone被广泛用于保护敏感数据和代码，但开发者实际如何使用这些技术在实践中尚不清楚。本研究旨在填补这一空白，通过实证分析了解TEE在实际应用中的采用情况、使用模式和开发实践。
...
Result: 研究发现：1）主要应用领域是物联网设备安全（30%），与学术界关注的区块链和密码系统（7%）形成鲜明对比，AI模型保护（12%）正在快速崛起；2）32.4%的项目重新实现密码功能而非使用官方SDK API，表明当前SDK可用性和可移植性有限；3）25.3%的项目存在不安全编码行为，如硬编码密钥和缺失输入验证，削弱了安全保证。

Conclusion: 研究揭示了TEE实际应用中的关键问题：应用领域与学术关注存在偏差，SDK可用性不足导致开发者重新实现功能，以及普遍存在的不安全编码实践。这些发现对改进TEE SDK的可用性和支持可信软件开发具有重要意义。
...
### [142] [CIFE: Code Instruction-Following Evaluation](https://arxiv.org/abs/2512.17387)
*Sravani Gunnu,Shanmukha Guttula,Hima Patel*

Main category: cs.SE

TL;DR: 本文提出了一个包含1000个Python任务的基准测试，每个任务平均有7个开发者指定的约束条件，用于评估LLM在代码生成时对功能正确性和约束遵守的综合表现。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在真实世界代码生成中，仅功能正确性不足以保证可靠部署，开发者还需要模型遵守关于鲁棒性、格式化和安全性等明确约束。现有基准主要评估测试用例执行，对模型如何可靠遵循约束条件提供有限洞察。

Method: 通过四阶段人机协作流程创建包含1000个Python任务的基准，每个任务平均有7个开发者指定的约束条件，涵盖13个类别。评估14个开源和闭源模型，使用互补的遵守度指标，并提出C2A分数作为综合衡量正确性和约束遵守的复合指标。

Result: 结果显示部分遵守和严格遵守之间存在显著差距：强大模型的部分遵守率超过90%，但严格遵守率仅为39-66%。这表明可信的代码生成不仅需要正确性，还需要对开发者意图的一致遵守。

Conclusion: 可靠的代码生成需要模型不仅产生功能正确的代码，还要一致地遵守开发者的约束意图。C2A分数为评估LLM在真实世界代码生成任务中的综合表现提供了新标准。
...
### [143] [SWE-Bench++: A Framework for the Scalable Generation of Software Engineering Benchmarks from Open-Source Repositories](https://arxiv.org/abs/2512.17419)
...
TL;DR: SWE-Bench++是一个自动化框架，从GitHub拉取请求生成仓库级编程任务，覆盖11种语言，解决了现有基准测试手动策划、静态数据集和仅限于Python的问题。


<details>
  <summary>Details</summary>
Motivation: 现有软件工程基准测试（如SWE-bench）存在局限性：手动策划、静态数据集、仅关注Python错误修复。需要更自动化、多语言、覆盖更广任务类型的评估框架。

Method: 通过四个阶段自动化生成任务：1) 程序化采集GitHub拉取请求；2) 环境合成；3) 测试预言提取；4) 质量保证。还包含提示引导的轨迹合成步骤，将强模型失败的任务转化为训练轨迹。

Result: 构建了包含11,133个实例的基准测试，来自3,971个仓库和11种语言。在1,782个实例子集上测试：Claude Sonnet 4.5达到36.20% pass@10，GPT-5 34.57%，Gemini 2.5 Pro 24.92%，GPT-4o 16.89%。微调SWE-Bench++数据可在多语言基准测试上带来可测量的改进。

Conclusion: SWE-Bench++提供了一个可扩展、多语言的基准测试，用于评估和改进仓库级代码生成，解决了现有基准的局限性，并展示了在模型微调中的实用性。
...
### [144] [An Investigation on How AI-Generated Responses Affect SoftwareEngineering Surveys](https://arxiv.org/abs/2512.17455)
...
Method: 通过Prolific平台收集2025年两次调查数据，分析参与者回答内容以识别异常或伪造回答；对疑似AI生成的回答子集进行定性模式检查、叙事特征分析和Scribbr AI检测器的自动检测。

Result: 在49份调查回答中发现了表明合成作者身份的重复结构模式，包括重复序列、统一措辞和表面个性化；这些虚假叙事模仿了连贯推理，同时隐藏了伪造内容，破坏了构念、内部和外部有效性。

Conclusion: 数据真实性成为软件工程调查中有效性的新维度；需要结合自动和解释性验证程序、透明报告和社区标准来检测和防止AI生成的回答，以保护软件工程调查的可信度。
...
### [145] [When Data Quality Issues Collide: A Large-Scale Empirical Study of Co-Occurring Data Quality Issues in Software Defect Prediction](https://arxiv.org/abs/2512.17460)
*Emmanuel Charleson Dapaah,Jens Grabowski*

Main category: cs.SE

TL;DR: 该研究首次在软件缺陷预测领域大规模实证分析了五种数据质量问题（类别不平衡、类别重叠、不相关特征、属性噪声和离群值）的共现现象及其对模型性能的影响，发现了普遍存在的共现模式、关键阈值点和反直觉的模式。


<details>
  <summary>Details</summary>
Motivation: 软件缺陷预测模型的有效性受限于数据集质量，先前研究通常孤立地分析单个问题（如类别不平衡或特征不相关），忽略了现实世界中数据问题经常共现并相互作用的复杂性。

Method: 使用可解释提升机结合分层交互分析，在374个数据集和5个分类器上同时考察五种数据质量问题的共现现象，量化默认超参数设置下的直接效应和条件效应，反映实际基线使用情况。

Result: 共现现象几乎普遍存在：即使最不频繁的问题（属性噪声）也在93%以上的数据集中与其他问题共现。不相关特征和不平衡几乎无处不在，而类别重叠是最持续有害的问题。发现了类别重叠（0.20）、不平衡（0.65-0.70）和不相关特征（0.94）的稳定临界点，超过这些阈值大多数模型开始退化。还发现了反直觉模式，如当不相关特征较少时离群值能提高性能。暴露了性能-鲁棒性权衡：没有单一学习器在所有条件下都占优。

Conclusion: 通过联合分析普遍性、共现性、阈值和条件效应，该研究直接解决了软件缺陷预测研究中的一个持久空白，超越了孤立分析，提供了对质量如何在现实世界环境中塑造模型性能的整体、数据感知理解。
...
### [146] [SGCR: A Specification-Grounded Framework for Trustworthy LLM Code Review](https://arxiv.org/abs/2512.17540)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自动化代码审查方面具有巨大潜力，但实际应用受到可靠性、上下文感知和控制能力不足的限制，需要解决这些障碍以实现实际采用。

Method: 提出规范驱动的代码审查（SGCR）框架，采用双路径架构：显式路径确保确定性遵守从规范派生的预定义规则，隐式路径启发式地发现和验证超出这些规则的问题。

Result: 在HiThink Research的工业环境中部署SGCR，其建议实现了42%的开发者采纳率，相比基线LLM的22%有90.9%的相对提升。

Conclusion: 规范驱动是弥合LLM生成能力与软件工程严格可靠性需求之间差距的强大范式，能够产生可信赖且相关的代码审查反馈。
...
### [147] [A Practical Solution to Systematically Monitor Inconsistencies in SBOM-based Vulnerability Scanners](https://arxiv.org/abs/2512.17710)
*Martin Rosso,Muhammad Asad Jahangir Jaffar,Alessandro Brighente,Mauro Conti*

Main category: cs.SE

TL;DR: 该研究提出了SVS-TEST方法和工具，用于评估软件物料清单（SBOM）漏洞扫描工具的可靠性和成熟度，发现现有工具存在不一致行为和静默失败问题。


<details>
  <summary>Details</summary>
Motivation: 随着行业采用基于SBOM的漏洞扫描（SVS）来识别软件产品中的漏洞，研究者观察到越来越多的不一致和意外行为，导致漏报和静默失败，需要系统评估SVS工具的能力和可靠性。

Method: 提出了SVS-TEST方法和工具，用于分析SVS工具在真实场景中的能力、成熟度和失败条件。通过案例研究评估了7个真实世界的SVS工具，使用16个精心设计的SBOM及其对应的真实漏洞数据。

Result: 研究结果揭示了SVS工具在可靠性和错误处理方面存在显著差异；多个SVS工具在有效的输入SBOM上静默失败，造成了虚假的安全感。所有研究结果和工具都公开可用，并在发布前向SVS工具开发者披露了发现。

Conclusion: 研究强调了SVS工具可靠性的重要性，提出了SVS-TEST作为监控SVS能力和成熟度的实用工具，为研究者和从业者提供了重要启示，包括组织如何利用该工具评估和改进SVS工具的性能。
...
### [148] [LLM-based Behaviour Driven Development for Hardware Design](https://arxiv.org/abs/2512.17814)
*Rolf Drechsler,Qian Liu*

Main category: cs.SE

TL;DR: 利用大语言模型自动化硬件设计中行为驱动开发的行为场景生成，减少人工工作量


<details>
  <summary>Details</summary>
Motivation: 硬件和系统设计中的测试验证复杂度随系统规模增加而显著增长，行为驱动开发在硬件设计中尚未广泛应用，主要障碍是需要从文本规范手动推导精确行为场景的人工工作量

Method: 研究基于大语言模型的技术来支持硬件设计中的行为驱动开发，利用LLM自动化从文本规范生成精确行为场景的过程

Result: 未在摘要中明确说明具体结果，但暗示LLM为自动化硬件设计中的BDD提供了新的机会

Conclusion: 大语言模型为支持硬件设计中的行为驱动开发提供了有前景的技术途径，能够减少人工工作量并提高效率
