...
TL;DR: 该论文提出了一个两阶段对齐框架，通过构建层次化细粒度评估标准和复杂偏好优化（CPO）来提升扩散模型与复杂人类专业知识的对齐能力


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型后训练对齐方法依赖于简化的信号（如标量奖励或二元偏好），这限制了与复杂人类专业知识的对齐，因为人类专业知识是层次化和细粒度的

Method: 1. 与领域专家合作构建层次化细粒度评估标准，将图像质量分解为多个正负属性并组织成树状结构
2. 提出两阶段对齐框架：首先通过监督微调将领域知识注入辅助扩散模型；然后提出复杂偏好优化（CPO），将DPO扩展到非二元、层次化标准，同时最大化正属性概率并最小化负属性概率

Result: 在绘画生成领域实例化了该方法，使用基于标准标注的绘画数据集进行CPO训练。大量实验表明，CPO显著提升了生成质量和与专业知识的对齐程度
...
TL;DR: 提出了一种基于RGB空间中五进制像素强度组合的文本嵌入图像新方法，相比传统LSB、变换域和深度学习等方法，能在单个像素内编码完整文本符号，计算效率更高且图像失真小。


<details>
  <summary>Details</summary>
Motivation: 现有文本嵌入图像方法（如LSB、PVD、变换域、深度学习等）大多依赖多像素操作或复杂变换，存在计算量大、产生噪声、编码确定性等问题，需要更高效、低失真的嵌入方案。

Method: 在RGB空间中采用五进制像素强度组合技术，每个R、G、B通道使用五种受控的像素强度变化，形成125种不同的像素强度组合，将这些组合映射到文本符号（字母、数字、空格、特殊字符）。

Result: 通过MSE、MAE、SNR、PSNR、SSIM、直方图比较和热图分析等多种指标评估，编码图像与原始图像相比没有显著失真，且能在单个RGB像素内编码完整文本符号，提高了嵌入效率。

Conclusion: 该方法提供了一种高效、低失真的文本嵌入图像方案，相比传统方法在单个像素内完成编码，计算开销更小，图像质量保持良好，具有实际应用价值。
...
TL;DR: 该研究通过后训练实现完全统一的文本-图像生成，使模型能在单一推理过程中自主从文本推理过渡到视觉合成，并在四个T2I基准测试中取得改进。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态生成架构通常采用显式的模态切换机制，先生成推理文本再手动切换到图像生成，这种分离的顺序推理过程限制了跨模态耦合，无法实现自动多模态生成。

Method: 采用离线、奖励加权的后训练方法，使用完全自生成的合成数据，探索不同的后训练数据策略，研究联合文本-图像生成对T2I性能的影响以及后训练中各模态的相对重要性。

Result: 针对特定限制设计的针对性数据集相比广泛的图像-标题语料库或基准对齐数据能获得更优结果，奖励加权两种模态和策略性设计的后训练数据在四个不同的T2I基准测试中实现了多模态图像生成的改进。

Conclusion: 通过后训练实现完全统一的文本-图像生成是有效的，模型能在单一推理过程中自主进行模态转换，奖励加权两种模态和策略性设计的后训练数据能显著提升多模态生成性能。
...
TL;DR: ReHyAt提出了一种循环混合注意力机制，结合了softmax注意力的保真度和线性注意力的效率，实现了恒定内存使用和线性复杂度，显著降低了视频生成的训练成本。
...
Method: 提出ReHyAt循环混合注意力机制，结合softmax注意力和线性注意力的优势，通过分块循环重构实现恒定内存使用，并设计了轻量级蒸馏和微调流程，可从现有softmax模型高效蒸馏。

Result: ReHyAt将训练成本降低两个数量级至约160 GPU小时，在VBench和VBench-2.0评估中达到最先进的视频质量，同时将注意力成本从二次降低到线性，实现了长时长和设备端视频生成的实用可扩展性。

Conclusion: ReHyAt提供了一种高效的混合注意力解决方案，为未来基于双向softmax的最先进模型提供了可应用的蒸馏和微调方案，解决了视频生成中的可扩展性瓶颈。
...
Method: 采用残差向量量化方法替代传统方法压缩原始特征，引入基于多分辨率哈希网格的自回归熵模型，准确预测每个连续传输索引的条件概率，实现粗层和细化层的高效压缩

Result: 该方法能够更有效地压缩3D高斯泼溅模型，提高压缩效率，降低存储需求

Conclusion: 提出的基于残差向量量化的渐进式编解码器能够更好地捕捉高维特征向量的相关性，相比传统标量量化方法具有更好的率失真性能
...
TL;DR: 该研究比较了自定义CNN与预训练模型（ResNet-18和VGG-16）在孟加拉国五个图像分类数据集上的表现，发现迁移学习微调方法在性能上优于自定义CNN和特征提取方法，特别是在数据有限的情况下。


<details>
  <summary>Details</summary>
Motivation: 研究动机是帮助实践者根据数据集特性、计算资源和性能需求选择合适的方法。通过比较自定义CNN和预训练模型在不同数据集上的表现，为实际应用提供指导。

Method: 使用特征提取和迁移学习两种方法，在五个孟加拉国图像分类数据集上比较自定义CNN与ResNet-18、VGG-16的性能。评估指标包括准确率和模型参数数量。

Result: 迁移学习微调方法在性能上优于其他方法，准确率提升3%-76%。ResNet-18微调在Road Damage BD数据集上达到100%准确率。自定义CNN在模型大小（340万参数）和训练效率上有优势，但预训练模型在复杂任务上表现更好。

Conclusion: 迁移学习微调是最佳方法，特别是在数据有限的情况下。自定义CNN适合简单任务和资源受限环境。实践者应根据具体需求选择方法。
...
TL;DR: PackCache：一种无需训练的KV缓存管理方法，通过条件锚定、跨帧衰减建模和空间保持位置嵌入三种机制动态压缩KV缓存，加速统一自回归视频生成模型推理速度1.7-2.2倍。


<details>
  <summary>Details</summary>
Motivation: 统一自回归模型使用KV缓存机制将注意力计算从O(T^2)降低到O(T)，但KV缓存大小随生成token数量线性增长，成为限制推理效率和生成长度的主要瓶颈。视频生成任务继承了这一限制，需要更高效的缓存管理方案。

Method: PackCache通过三种协调机制动态压缩KV缓存：1) 条件锚定：保留文本和条件图像等语义锚点token；2) 跨帧衰减建模：根据时间距离分配缓存预算，注意力随时间距离自然衰减；3) 空间保持位置嵌入：在缓存移除时保持连贯的3D结构。

Result: 在48帧长序列上，PackCache将端到端生成加速1.7-2.2倍。对于受KV缓存扩展影响最大的最后四帧（视频中最昂贵的部分），在A40和H200上分别实现2.6倍和3.7倍加速。

Conclusion: PackCache通过利用KV缓存token的时空特性，提供了一种无需训练的高效缓存管理方案，显著提升统一自回归视频生成模型的推理效率，为实现更长序列的视频生成提供了潜力。
...
TL;DR: 该研究探索了使用解耦的3D面部几何特征进行压力识别，通过分析分心驾驶场景下的面部表情和姿态系数，发现41个系数表现出与生理标记相当的应激反应，并提出了基于Transformer的跨模态注意力融合方法。


<details>
  <summary>Details</summary>
Motivation: 当前基于面部动作单元的压力识别方法存在局限性，而解耦的3D面部几何特征在压力识别中的作用尚未充分探索。研究旨在利用3D面部几何特征来更可靠地识别压力，特别是在分心驾驶这样的实际应用场景中。

Method: 使用EMOCA提取的3D表情和姿态系数分析分心驾驶场景下的压力反应，通过配对假设检验比较基线和应激阶段的系数变化。在此基础上，提出了基于Transformer的时间建模框架，评估了单模态、早期融合和跨模态注意力三种融合策略，特别关注EMOCA与生理信号、EMOCA与注视信号的融合。

Result: 研究发现56个系数中有41个表现出与生理标记相当的、阶段特异性的应激反应。跨模态注意力融合方法表现最佳：EMOCA与生理信号融合达到AUROC 92%、准确率86.7%；EMOCA与注视信号融合也表现出色，AUROC达到91.8%。

Conclusion: 研究证明了3D面部几何特征在压力识别中的有效性，特别是通过时间建模和跨模态注意力融合策略能够显著提升识别性能，为可靠的压力识别提供了新的方法框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉基础模型主要基于RGB数据训练，但许多安全关键应用依赖红外（IR）和合成孔径雷达（SAR）等非可见光模态。需要探索如何利用现有RGB基础模型支持这些非可见光模态。

Method: 从FLUX.1 Kontext模型出发，插入低秩适应（LoRA）模块，仅用每个领域100张配对图像进行微调。在两个设置中测试：KAIST数据集的RGB到IR转换，以及M4-SAR数据集的RGB到SAR转换。通过LPIPS指标评估翻译质量，并验证下游检测性能。

Result: LPIPS指标能有效预测下游性能：更低的LPIPS值对应更高的mAP。最佳LoRA适配器生成的合成IR数据提升了KAIST红外行人检测性能，合成SAR数据结合有限真实SAR数据显著提升了M4-SAR基础设施检测性能。

Conclusion: 少量样本的LoRA适应流匹配基础模型是实现非可见光模态基础模型支持的有效途径，合成数据能有效增强下游检测任务。
...
TL;DR: 该研究比较了自定义CNN与预训练架构（VGG-16、ResNet-50、MobileNet）在图像分类任务中的表现，发现预训练模型在准确率和收敛速度上更优，而自定义CNN在参数量和计算复杂度方面更具优势。


<details>
  <summary>Details</summary>
Motivation: 在图像分类任务中，选择自定义CNN还是使用预训练架构是一个重要的实际问题。研究者希望比较这两种方法的性能差异，为实际应用提供选择依据。

Method: 设计了一个自定义CNN并从头训练，同时使用VGG-16、ResNet-50和MobileNet等流行架构进行迁移学习。在相同实验设置下，使用准确率、精确率、召回率和F1分数等标准指标评估所有模型。

Result: 预训练CNN架构在分类准确率和收敛速度上始终优于自定义CNN，特别是在训练数据有限的情况下。然而，自定义CNN在参数量显著减少和计算复杂度降低的情况下仍表现出竞争力。

Conclusion: 研究揭示了模型复杂度、性能和计算效率之间的权衡关系，为图像分类问题中选择合适的CNN架构提供了实用见解。预训练模型在性能上更优，而自定义模型在资源受限场景下更具优势。
...
Conclusion: Tri MARF通过三模态融合和多智能体协作，有效解决了3D标注中的复杂挑战，在多个指标上实现了最先进的性能，为大规模3D标注提供了高效解决方案。
...
Method: 提出混合自监督和弱监督学习框架，直接从术前CT扫描预测乳突切除术区域，无需人工标注。该方法在弱监督医学影像中利用3D T分布损失。

Result: 混合方法在预测复杂且无边界乳突切除术形状时达到平均Dice分数0.72，超越了现有最先进方法，表现出强大性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前深度学习在心脏MRI重建中表现出色但泛化能力有限，由于图像对比度、采样模式、扫描设备、解剖结构和疾病类型等多样性，现有模型只能处理单一或狭窄的变化子集，面对分布偏移时性能下降

Method: 提出CRUNet-MR-Univ基础模型，利用时空相关性和基于提示的先验知识，有效处理心脏MRI扫描的完整多样性

Result: 该方法在广泛设置中持续优于基线方法，展示了其有效性和前景

Conclusion: 开发能够泛化到多样化心脏MRI场景的统一模型是有益的，CRUNet-MR-Univ作为基础模型在这方面表现出色
...
<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型的链式思维推理方法容易导致过度思考，产生冗长响应，降低测试效率和准确性。先前工作主要关注自适应推理策略，但忽视了视觉感知失败这一根本瓶颈。作者认为推理错误往往源于不完美的感知而非思考不足。

Method: 提出Gated Perception-Reasoning Optimization (GPRO)，一种元推理控制器，在每个生成步骤动态路由三种计算路径：轻量快速路径、重新检查视觉输入的慢感知路径、内部自我反思的慢推理路径。从约79万样本中获取失败归因监督，使用教师模型区分感知幻觉和推理错误，通过多目标强化学习训练控制器以优化不确定情况下的任务准确性和计算成本权衡。

Result: 在五个基准测试上的实验表明，GPRO显著提高了准确性和效率，优于最近的慢思考方法，同时生成了显著更短的响应。
...
TL;DR: UniDrive-WM是一个统一的视觉语言模型世界模型，将自动驾驶中的场景理解、轨迹规划和轨迹条件未来图像生成集成到单一架构中，相比现有方法显著提升了规划性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶方法通常将感知、预测和规划作为独立模块处理，缺乏紧密集成。作者提出统一的世界模型来联合执行这些任务，以提升自动驾驶系统的整体性能。

Method: 提出UniDrive-WM统一架构，包含轨迹规划器和基于VLM的图像生成器。轨迹规划器预测未来轨迹，该轨迹条件化图像生成器产生未来帧，这些预测提供额外监督信号，迭代优化轨迹生成。同时比较了离散和连续输出表示对未来图像预测的影响。
...
<details>
  <summary>Details</summary>
Motivation: 3D医学图像分割计算需求大，存在立方级体素增长和同质区域冗余计算的问题。需要一种既能保持分割精度又能显著降低计算开销的高效方法。
...
Result: 在960例3D乳腺DCE-MRI数据集上达到94.49% Dice和89.61% IoU的SOTA性能，GPU内存和推理延迟分别降低64%和68%。在MSD心脏和脑MRI基准数据集上也表现优异，验证了泛化能力。

Conclusion: TokenSeg证明了基于解剖学信息的稀疏表示在3D医学图像分割中的有效性，实现了准确性和效率的平衡，为计算密集型医学图像分析提供了有前景的解决方案。
...
Method: 开发了一个从复合语义标签图合成3D LGE MRI体积的流程，结合了专家解剖标注和无监督组织聚类，使用了三种3D条件生成器（Pix2Pix GAN、SPADE-GAN、SPADE-LDM）。
...
Conclusion: 标签条件的3D合成方法能够有效增强训练数据，改善代表性不足的心脏结构分割，为医学图像分析提供了有前景的数据增强解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的设计文档（如海报）通常是多图层的，包含装饰、文本和图像等元素。现有研究大多忽视多图层设计文档编辑，主要关注单图层图像编辑或多图层生成，缺乏对图层感知推理的需求，无法确定修改什么和在哪里修改。

Method: 提出了MiLDEAgent框架，结合了基于强化学习训练的多模态推理器（用于图层级理解）和图像编辑器（用于针对性修改）。同时创建了MiLDEBench基准数据集（包含2万多个人工标注的设计文档和编辑指令）和MiLDEEval评估协议（涵盖指令遵循、布局一致性、美学和文本渲染四个维度）。

Result: 在14个开源模型和2个闭源模型的广泛实验中，现有方法无法泛化：开源模型通常无法完成多图层文档编辑任务，而闭源模型存在格式违规问题。相比之下，MiLDEAgent实现了强大的图层感知推理和精确编辑，显著优于所有开源基线，性能与闭源模型相当，为多图层文档编辑建立了首个强基线。

Conclusion: 该研究填补了多图层设计文档编辑的空白，提出的MiLDEAgent框架通过结合推理和编辑能力，有效解决了图层感知的文档编辑问题，为这一领域建立了首个强基线。
...
TL;DR: 该论文讨论AI赋能的人为中心信息物理系统在不确定条件下的安全风险，提出了一个评估安全策略的框架，并以糖尿病患者的闭环血糖控制为例，展示了一种基于图像的个性化技术来检测未申报的餐食。


<details>
  <summary>Details</summary>
Motivation: 随着AI在医疗监控、自动驾驶等人为中心信息物理系统中的广泛应用，系统在与人类交互时可能面临不确定的操作条件，这些不确定条件可能违反系统的安全和安保要求，需要建立有效的安全评估框架。

Method: 首先讨论导致系统在未知条件下运行的操作偏差，然后创建一个评估不同策略的框架，以确保AI赋能的人为中心信息物理系统在操作部署中的安全性和安保性。最后以1型糖尿病患者的闭环血糖控制为例，展示了一种基于图像的个性化技术来检测未申报的餐食。

Result: 提出了一个评估AI赋能人本信息物理系统安全策略的框架，并开发了一种新颖的个性化图像技术，用于检测1型糖尿病患者闭环血糖控制中未申报的餐食，从而应对系统在不确定条件下的安全挑战。

Conclusion: AI赋能的人为中心信息物理系统在不确定条件下存在安全风险，需要建立系统化的评估框架来确保其安全性和安保性。提出的框架和个性化图像技术为解决这类系统的安全挑战提供了有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 随着文本到图像生成技术的快速发展，准确评估生成图像与文本提示之间的对齐性成为关键挑战。现有方法依赖欧几里得空间度量，忽略了语义对齐的结构化特性，且缺乏对不同样本的自适应能力。
...
Result: 在CDDMBench上评估，3B参数模型性能与7B-13B参数基线相当：疾病识别准确率相对提升23.2%，农业知识问答提升33.3%，跨域泛化比标准微调提高26.10分。消融研究证实结构化推理数据与GRPO驱动的探索协同作用，问题复杂度越高收益越大。

Conclusion: Agri-R1通过自动化推理数据生成和GRPO优化，有效解决了农业疾病诊断中的标注成本、可解释性和泛化问题，为开放域农业查询提供了高效解决方案。
...
TL;DR: HATIR提出了一种用于湍流红外视频超分辨率的热感知扩散方法，通过注入热感知变形先验来联合建模湍流退化和结构细节损失的反向过程。


<details>
  <summary>Details</summary>
Motivation: 红外视频在挑战性环境中具有重要应用价值，但常受严重大气湍流和压缩退化影响。现有VSR方法要么忽略了红外与可见光图像之间的模态差异，要么无法恢复湍流引起的失真。直接将湍流缓解算法与VSR方法级联会导致误差传播和累积。

Method: HATIR通过注入热感知变形先验到扩散采样路径，联合建模湍流退化和结构细节损失的反向过程。具体包括：1) 基于热活跃区域随时间表现一致相量响应的物理原理，构建相量引导流估计器；2) 提出湍流感知解码器，通过湍流门控和结构感知注意力选择性地抑制不稳定时间线索并增强边缘感知特征聚合。

Result: 构建了首个湍流红外VSR数据集FLIR-IVSR，包含来自FLIR T1050sc相机(1024×768)的640个多样化场景的配对LR-HR序列，涵盖不同相机和物体运动条件。

Conclusion: HATIR通过热感知扩散方法有效解决了湍流红外视频超分辨率问题，提出的物理原理驱动的相量引导流估计器和湍流感知解码器能够可靠地恢复非均匀失真下的结构细节，为红外VSR研究提供了新的数据集和方法框架。
...
<details>
  <summary>Details</summary>
Motivation: AI生成内容（AIGC）的快速发展加剧了深度伪造的威胁，从面部操纵到整个逼真人体的合成。现有检测方法存在碎片化问题，要么专门处理面部区域伪造，要么专门处理全身合成图像，无法泛化到各种人类图像操纵场景。

Method: 提出HuForDet整体框架，包含：1）面部伪造检测分支：采用RGB和频域异构专家，包括自适应拉普拉斯-高斯模块捕捉从细粒度融合边界到粗尺度纹理异常的伪影；2）上下文伪造检测分支：利用多模态大语言模型分析全身语义一致性，配备置信度估计机制动态加权特征融合贡献。

Result: 构建了统一的人类图像伪造数据集，整合现有面部伪造数据和新收集的全身合成人体数据。大量实验表明，HuForDet在各种人类图像伪造检测中实现了最先进的性能，并表现出卓越的鲁棒性。

Conclusion: HuForDet通过双分支架构和统一数据集，有效解决了现有检测方法的碎片化问题，实现了对人类图像伪造的全面检测，为应对日益复杂的深度伪造威胁提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习已改变视觉数据分析，但不同应用场景（农业、城市领域）存在光照、分辨率、环境复杂性和类别不平衡等挑战，需要评估CNN架构在不同数据条件下的适应性和鲁棒性。

Method: 使用五个异构数据集（芒果品种分类、水稻品种识别、路面状况评估、三轮车检测、人行道侵占监测），评估自定义轻量CNN与ResNet-18、VGG-16的性能，包括从头训练和迁移学习两种方式，采用系统预处理、数据增强和受控实验设计。

Result: 开发的高效自定义CNN在多个应用领域取得竞争性性能；综合分析表明，在数据受限环境中，迁移学习和深度架构能提供显著优势；不同数据集对模型复杂度、深度和预训练的需求存在差异。

Conclusion: 研究为资源有限但高影响力的现实世界视觉分类任务提供了实用见解，展示了轻量CNN的竞争力，并明确了迁移学习和深度架构在特定条件下的优势，有助于实际部署决策。
...
Method: 提出AIVD框架，通过边缘检测器与云端MLLMs协作；设计视觉-语义协同增强的高效微调策略提升鲁棒性；提出异构资源感知的动态调度算法。

Result: AIVD显著降低资源消耗，同时提高MLLM分类性能和语义生成质量；调度策略在不同场景下实现更高吞吐量和更低延迟。

Conclusion: AIVD框架有效解决了MLLMs在边缘部署中的定位精度和资源效率问题，通过协同设计和智能调度实现了性能与效率的平衡。
...
TL;DR: 该论文提出了一种利用骨架化技术的新型对抗攻击方法，针对包含文本（特别是数学公式）的图像，有效减少搜索空间，评估了基础模型的视觉能力和局限性。
...
Method: 提出一种新颖的对抗攻击方法，利用骨架化技术有效减少搜索空间，专门针对包含文本（特别是数学公式）的图像，并详细评估原始输出与对抗扰动输出之间的字符和语义变化。

Result: 该方法在ChatGPT上的应用展示了其实际意义，证明了方法的有效性，并为理解模型的视觉解释和推理能力提供了深入见解。

Conclusion: 通过骨架化技术的新型对抗攻击方法能够有效评估基础模型的视觉能力，特别是在处理复杂文本图像方面，为理解模型的视觉解释和推理能力提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D场景理解方法通常依赖预训练的3DGS场景，存在计算开销大、需要渲染监督微调等问题。ProFuse旨在实现高效、一致的3D场景语义理解，同时保持几何精度。

Method: 1. 引入密集对应引导的预注册阶段，初始化具有准确几何的高斯分布；2. 通过跨视图聚类联合构建3D上下文提议；3. 每个提议通过加权聚合成员嵌入获得全局特征；4. 在直接注册过程中将特征融合到高斯分布上，保持跨视图的每个基元的语言一致性；5. 无需额外优化，仅需标准重建。

Result: ProFuse实现了强大的开放词汇3DGS理解能力，每个场景的语义附着完成时间约为5分钟，比现有最先进技术快两倍。模型保持了几何细化能力，无需密集化处理。

Conclusion: ProFuse提供了一种高效、上下文感知的开放词汇3D场景理解框架，通过创新的预注册和特征融合机制，在保持几何精度的同时实现了快速的语义附着，显著提升了3D高斯泼溅场景的理解效率。
...
TL;DR: GeM-VG是一个多模态大语言模型，专注于广义多图像视觉定位，通过系统分类多图像定位任务、构建新数据集和混合强化微调策略，显著提升了多图像和单图像定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在多图像定位方面存在局限性：只能进行单目标定位，任务类型有限，缺乏对广义定位任务的统一建模。需要开发能够处理多样化多图像定位任务的通用模型。

Method: 1. 系统分类多图像定位任务（基于跨图像线索和推理需求）
2. 构建MG-Data-240K数据集（解决现有数据集目标数量和图像关系限制）
3. 提出混合强化微调策略：结合思维链推理和直接回答，采用R1-like算法和基于规则的奖励机制

Result: 1. 多图像定位：在MIG-Bench和MC-Bench上分别比先前领先MLLM提升2.0%和9.7%
2. 单图像定位：在ODINW上比基础模型提升9.1%
3. 保持强大的通用多图像理解能力

Conclusion: GeM-VG通过统一建模、新数据集和混合微调策略，成功实现了广义多图像视觉定位，在多种定位任务上表现出色，同时保持了通用多图像理解能力，为多图像定位研究提供了新方向。
...
TL;DR: 该论文提出了一个用于减少视频语言模型幻觉的可扩展框架，通过生成仅动作或时间结构不同的反事实视频，构建了CounterVid数据集，并开发了MixDPO方法进行联合优化。
...
Method: 提出了一个可扩展的反事实视频生成框架，结合多模态LLM进行动作提议和编辑指导，使用基于扩散的图像和视频模型大规模生成语义硬负样本。构建了CounterVid数据集（约26k偏好对），并开发了MixDPO方法，统一利用文本和视觉偏好进行直接偏好优化。

Result: 使用MixDPO微调Qwen2.5-VL模型带来了持续改进，特别是在时间排序方面，并能有效迁移到标准视频幻觉基准测试中。

Conclusion: 该框架通过生成针对动作识别和时间推理的反事实视频，有效减少了视频语言模型的幻觉问题，为多模态模型的鲁棒性改进提供了新途径。
...
TL;DR: 该论文研究散焦深度估计中的高斯模型适用性，通过几何光学和衍射光学分析，验证了高斯模型在大多数成像设备中的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 从2D图像准确估计深度是3D恢复领域的根本挑战。散焦提供了重要的深度信息，但传统启发式方法面临病态问题，因为期望的模糊无法与固有模糊区分。高斯模型因其数学简单性和计算效率成为实时应用的最佳选择，但需要验证其在真实成像设备中的适用性。

Method: 论文首先在几何光学框架内进行散焦分析，然后通过衍射受限光学中的散焦像差理论进行分析。研究为传统成像设备设置了参数，确保散焦算子符合高斯模型。通过将实际模型拟合到其高斯近似来评估准确性，测试了典型聚焦深度范围（1-100米）和最大深度变化（聚焦深度的10%）。

Result: 研究结果表明，对于大多数成像设备，高斯模型适用于散焦算子。在测试条件下，最大平均绝对误差（MAE）小于1%，证明了该模型的准确性和可靠性。

Conclusion: 高斯模型是散焦深度估计的有效模型，在大多数成像设备中具有高精度和可靠性。该研究为使用高斯模型进行实时深度估计提供了理论支持，特别是在需要计算效率和数学简单性的应用中。
...
Method: 提出增强版Pix2Pix框架，集成Squeeze-and-Excitation Residual Networks (SEResNet)和U-Net++。SEResNet通过通道注意力增强关键特征表示，U-Net++增强多尺度特征融合。使用简化的PatchGAN鉴别器稳定训练并提升局部解剖真实性。
...
TL;DR: 提出MCLC方法解决潜在扩散模型逆求解器的不稳定性问题，通过测量一致Langevin更新纠正扩散动态差异


<details>
  <summary>Details</summary>
Motivation: 现有基于潜在扩散模型的零样本逆求解器存在不稳定性问题，会产生不良伪影和质量下降，需要更稳健的解决方案

Method: 提出测量一致Langevin校正器(MCLC)，通过理论基础的即插即用校正模块，使用测量一致Langevin更新来纠正LDM逆求解器，不依赖线性流形假设
...
<details>
  <summary>Details</summary>
Motivation: 现有开源金字塔视频模型从头训练，在视觉合理性方面落后于最先进系统，且计算成本较高。需要一种方法能将预训练扩散模型高效转换为金字塔模型，同时保持输出质量。

Method: 开发了一个将预训练扩散模型转换为金字塔模型的管道，通过低成本微调实现转换，同时研究比较了金字塔模型中的各种步数蒸馏策略。

Result: 成功将预训练扩散模型转换为金字塔模型，实现了质量无损的视频输出，同时通过步数蒸馏策略进一步提升了推理效率。

Conclusion: 提出的方法能够将现有预训练扩散模型高效转换为金字塔模型，在保持输出质量的同时显著降低计算成本，为视频生成领域提供了更高效的解决方案。
...
TL;DR: 本文首次系统评估了SAMURAI基础模型在无人机跟踪中的应用，并提出检测器增强扩展以提升在复杂城市环境中的鲁棒性，特别是在长时间序列和无人机离开-重新进入场景中表现显著改善。


<details>
  <summary>Details</summary>
Motivation: 无人机跟踪对现代监控系统至关重要，但现有RGB跟踪方法有限且依赖传统运动模型。虽然SAMURAI等基础模型在其他领域表现出强大的类别无关跟踪性能，但在无人机特定场景中的应用尚未被研究。

Method: 提出检测器增强的SAMURAI扩展方法，通过结合检测器线索来减轻对边界框初始化和序列长度的敏感性，提升在复杂城市环境中的跟踪鲁棒性。

Result: 提出的扩展方法显著改善了复杂城市环境中的跟踪鲁棒性，特别是在长时间序列和无人机离开-重新进入事件中。相比SAMURAI的零样本性能，在多个数据集和指标上获得一致提升，成功率最高提升+0.393，误报率最高降低-0.475。

Conclusion: SAMURAI基础模型在无人机跟踪中具有潜力，通过检测器增强扩展可以显著提升其在复杂城市监控环境中的性能，特别是在处理长时间序列和间断跟踪场景时表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 古代文字图像常因老化、环境因素等导致严重背景噪声、低对比度和退化，前景文字与背景具有相似视觉特征，使得铭文难以阅读，需要有效的图像增强技术来提高可读性。

Method: 基于二值化和互补预处理技术的方法，包括去除污渍和增强模糊古代文字的预处理步骤，使用K-NN和SVM分类器对不同类型古代文字进行性能评估。

Result: 在石头、金属板和文献三种古代文字类型上，K-NN分类器分别达到55.7%、62%和65.6%的分类准确率；SVM分类器分别达到53.2%、59.5%和67.8%的准确率。

Conclusion: 提出的增强方法能有效提高古代马拉地语铭文图像的可读性，证明了该方法在增强退化古代文字图像方面的有效性。
...
TL;DR: SOVABench是一个基于监控视频的车辆行为检索基准，用于评估模型在动作区分和时间方向理解上的能力，并提出基于MLLM的训练免费框架生成可解释嵌入。
...
Method: 1) 构建SOVABench基准，包含两种评估协议（跨对和内部对）；2) 利用多模态大语言模型的视觉推理和指令跟随能力，提出训练免费框架，从MLLM生成的描述中为图像和视频生成可解释嵌入。

Result: 实验表明，即使对人类直观的动作区分，对现有最先进的视觉和多模态模型仍具挑战性。提出的框架在SOVABench以及对比视觉语言模型常失败的空间和计数基准上都取得了良好性能。

Conclusion: SOVABench填补了监控视频动作检索评估的空白，基于MLLM的训练免费框架为视频理解提供了可解释且有效的解决方案，代码和基准构建说明已公开。
...
TL;DR: 该研究使用YOLO目标检测模型替代传统模板匹配技术，用于中世纪手稿的抄写员识别，通过检测字母"a"来提高识别准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 古文书学研究中，确定手稿年代和理解书写演变需要识别抄写员。虽然数字技术已有进展，但通用问题仍未解决，传统模板匹配技术存在阈值依赖等局限性。

Method: 使用YOLOv5目标检测模型替代之前的模板匹配和CNN方法，专门检测中世纪手稿中的字母"a"，然后进行第二阶段的分类来识别抄写员。

Result: YOLO能提取更多字母实例，提高第二阶段的分类准确性；YOLO置信度分数为开发拒绝阈值系统提供基础，即使在未见手稿中也能实现可靠的抄写员识别。

Conclusion: YOLO目标检测模型在抄写员识别任务中优于传统模板匹配技术，为古文书学研究提供了更准确、可靠的自动化解决方案。
...
TL;DR: DivAS是一个无需优化的交互式NeRF分割框架，通过2D SAM掩码和NeRF深度先验，使用自定义CUDA内核在200ms内聚合多视角掩码到3D体素网格，比优化方法快2-2.5倍。
...
Method: 提出DivAS框架：1) 通过GUI工作流，用户点提示生成2D SAM掩码；2) 使用NeRF深度先验优化掩码的几何精度和前景-背景分离；3) 核心是自定义CUDA内核，在200ms内将优化的多视角掩码聚合到统一的3D体素网格中。

Result: 在Mip-NeRF 360°和LLFF数据集上，DivAS达到与优化方法相当的分割质量，端到端速度快2-2.5倍，排除用户提示时间后快一个数量级。

Conclusion: DivAS提供了一个无需优化、完全交互的NeRF分割框架，结合了2D基础模型的零样本能力和NeRF的几何先验，实现了实时视觉反馈和高质量分割。
...
TL;DR: 该研究提出了一个工业级GenAI框架，用于处理大规模多模态数据（20万+PDF、2.5万+视频、888多语言音频），分析了40+视觉语言模型在长视频推理中的实际限制和性能瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型大多在短视频和无约束计算资源下评估，而工业场景（如制药内容理解）需要在严格GPU、延迟和成本约束下处理长视频，现有方法难以扩展。

Method: 提出工业级大规模多模态推理架构，在Video-MME和MMBench两个基准以及包含25,326个视频（涵盖14种疾病领域）的专有数据集上，对40多个VLMs进行实证分析。

Result: 在商用GPU上使用SDPA注意力机制获得3-8倍效率提升；多模态在8/12任务领域（特别是长度依赖任务）中提升性能；发现时间对齐和关键帧检测是开源和闭源VLMs的共同瓶颈。

Conclusion: 该研究不是提出新模型，而是刻画了当前VLMs在实际部署约束下的实用限制、权衡和失败模式，为研究人员和从业者设计可扩展的工业级长视频理解系统提供可操作指导。
...
TL;DR: 论文揭示了多模态评估中的原型性偏差问题，创建了ProtoBias基准测试，发现现有指标倾向于原型图像而非语义正确性，提出了ProtoScore新指标来减少这种偏差。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型的自动评估指标常常替代人类判断，但不确定这些指标是否真正优先考虑语义正确性，还是偏向于从有偏数据分布中学到的视觉和社会原型图像。

Method: 引入受控对比基准ProtoBias，涵盖动物、物体和人口统计学图像，将语义正确但非原型的图像与语义错误但原型的对抗图像配对，进行定向评估。提出了ProtoScore，一个70亿参数的鲁棒性指标。

Result: 广泛使用的指标（CLIPScore、PickScore、VQA-based scores）经常错误排名这些配对，LLM-as-Judge系统在社会基础案例中表现出不均匀的鲁棒性。人类评估始终更倾向于语义正确性。ProtoScore显著降低了失败率，抑制了错误排名，运行速度比GPT-5推理快几个数量级。

Conclusion: 多模态评估存在原型性偏差的系统性失败模式，现有指标倾向于原型图像而非语义正确性。ProtoScore在减少偏差方面表现出色，接近更大规模闭源评估系统的鲁棒性。
...
Method: 提出TEA（TEmporal Adaptive）方法：1）使用包含全局序列知识的教师模型指导具有自适应时间输入长度的学生模型；2）通过中间嵌入、原型和软标签视角实现知识转移；3）动态聚合学生模型以减轻知识遗忘；4）引入全序列重建作为辅助任务增强不同时间长度输入的表示质量

Result: 通过大量实验证明，该方法在不同时间长度输入下，在常见基准测试中带来了显著改进

Conclusion: TEA方法有效提升了卫星图像时间序列语义分割模型在变化时间长度场景下的适应性和泛化能力
...
TL;DR: SparseLaneSTP：一种集成车道几何特性和时序信息的稀疏车道Transformer方法，在3D车道检测任务中实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有3D车道检测方法存在以下问题：1）基于密集BEV特征的方法因错误变换导致特征表示与真实3D路面不对齐；2）稀疏车道检测器完全忽略了有价值的车道先验知识；3）现有方法未能利用历史车道观测信息来改善低能见度情况下的检测模糊性

Method: 提出SparseLaneSTP方法，包含：1）车道特定的时空注意力机制；2）为稀疏架构设计的连续车道表示；3）时序正则化。同时还提出了一个使用自动标注策略构建的精确一致的3D车道数据集

Result: 在现有3D车道检测基准测试和作者提出的新数据集上，该方法在所有检测和误差指标上都实现了最先进的性能

Conclusion: 通过集成车道几何特性和时序信息到稀疏车道Transformer中，SparseLaneSTP有效解决了现有3D车道检测方法的局限性，并在多个数据集上验证了其优越性能
...
TL;DR: OceanSplat是一种基于3D高斯泼溅的水下场景3D几何表示方法，通过三目视图一致性和自监督深度正则化解决水下光学退化问题，显著减少漂浮伪影


<details>
  <summary>Details</summary>
Motivation: 水下场景由于光学退化（如散射、吸收）导致多视图不一致，传统3D重建方法在水下环境中效果不佳，需要专门的方法来准确表示水下3D几何结构

Method: 1. 三目视图一致性约束：通过渲染水平和垂直平移的相机视图，并通过逆变形对齐来增强几何一致性；2. 合成极线深度先验：通过三角测量生成自监督深度正则化器；3. 深度感知alpha调整：基于3D高斯的z分量和观察方向在早期训练中调制不透明度，防止介质诱导基元的形成

Result: 在真实水下场景和模拟场景上的实验表明，OceanSplat在场景重建和散射介质恢复方面显著优于现有方法，能够将3D高斯从散射介质中解耦，准确表示物体几何结构，大幅减少重建水下场景中的漂浮伪影

Conclusion: OceanSplat通过几何约束和深度感知调整，有效解决了水下场景中3D高斯泼溅的挑战，为水下环境中的鲁棒3D重建和恢复提供了创新解决方案
...
<details>
  <summary>Details</summary>
Motivation: 研究高阶对抗攻击对目标检测器的影响，探索对抗攻击模式与对抗训练之间的循环博弈关系，评估对抗训练在防御高阶对抗攻击方面的有效性。
...
Conclusion: 高阶对抗攻击对目标检测器构成严重威胁，对抗训练作为单一防御手段效果有限，需要更全面的防御策略来应对高阶对抗攻击的挑战。
...
TL;DR: PolyFit是一种基于面片的曲面表示方法，通过局部拟合jet函数实现高效学习，能够泛化到多种曲面类型，并通过更新紧凑的jet系数而非逐顶点优化来实现高效曲面变形。


<details>
  <summary>Details</summary>
Motivation: 传统曲面变形方法需要优化大量顶点自由度，计算成本高。需要一种更紧凑、高效的曲面表示方法，能够在保持精度的同时加速下游计算机视觉和图形学任务。

Method: 提出PolyFit方法，在曲面面片上局部拟合jet函数，通过监督学习从解析函数和真实数据中学习表示。学习到的表示可以泛化到各种曲面类型，通过更新紧凑的jet系数而非逐顶点优化来实现高效变形。

Result: 在形状模板(SfT)应用中，测试时优化方法在保持竞争力的精度同时显著快于离线物理求解器，在精度上优于最近的物理引导神经模拟器。在服装悬垂应用中，训练的自监督、网格和服装无关模型能够跨分辨率和服装类型泛化，推理速度比强基线快一个数量级。

Conclusion: PolyFit提供了一种紧凑、高效的曲面表示方法，能够通过更新少量jet系数实现高效曲面变形，在形状模板和服装悬垂等应用中展现出优越的性能和速度优势。
...
TL;DR: 本文提出了一种面向制药行业的视频到视频片段生成框架，整合音频语言模型和视觉语言模型，实现高效、个性化的视频内容处理，相比现有方法在速度、成本和内容质量上均有显著提升。


<details>
  <summary>Details</summary>
Motivation: 制药行业数字化转型中，传统手动处理多模态数据（文本、图像、视频、音频、网页链接）存在不一致性、质量下降和效率低下问题，特别是长视频和音频数据（如临床试验访谈和教育研讨会）的处理挑战尤为突出。

Method: 提出了一个领域适应的视频到视频片段生成框架，包含三个核心贡献：1）可复现的Cut & Merge算法，具有淡入淡出和时间戳标准化功能，确保平滑过渡和音视频对齐；2）基于角色定义和提示注入的个性化机制，针对不同需求（营销、培训、监管）定制输出；3）成本高效的端到端流水线策略，平衡ALM/VLM增强处理。

Result: 在Video MME基准测试（900个视频）和专有数据集（16,159个制药视频，覆盖14个疾病领域）上的评估显示：速度提升3-4倍，成本降低4倍，片段质量具有竞争力。此外，方法在片段连贯性得分（0.348）和信息性得分（0.721）上优于最先进的VLM基线（如Gemini 2.5 Pro）。

Conclusion: 该框架展示了透明、可定制提取且支持合规性的视频摘要技术在生命科学领域的潜力，为制药行业的多模态内容处理提供了智能、可扩展和自动化的解决方案。
...
Result: 在NAVSIM-v1、NAVSIM-v2和HUGSIM基准测试中优于或匹配现有基线，证明纯Transformer架构结合令牌压缩可以实现准确、高效、自适应的端到端驾驶。

Conclusion: 纯Transformer架构结合有针对性的令牌压缩足以实现准确、高效和自适应的端到端自动驾驶，代码和检查点将公开提供。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶应用中未标注的激光雷达日志包含丰富的3D几何信息，但由于缺乏人工标注而几乎无法利用，这构成了自动驾驶感知研究的主要成本障碍。本文旨在解决这一瓶颈。

Method: 提出基于时间累积激光雷达地图学习强几何先验的无监督多模态伪标签方法，引入新颖的迭代更新规则来强制执行联合几何-语义一致性，并通过不一致性检测移动物体。

Result: 方法在三个数据集上表现出强大的泛化能力，在语义分割和物体检测伪标签方面优于现有方法（通常需要额外人工监督）。即使使用少量几何一致的密集激光雷达，也能将80-150米和150-250米范围内的深度预测MAE分别提高51.5%和22.0%。

Conclusion: 该方法成功利用激光雷达的时间几何一致性，将多模态线索融合到3D空间，无需人工标注即可生成高质量的3D语义和几何信息，显著降低了自动驾驶感知研究的标注成本。
...
TL;DR: 本文提出了一种基于投影条件化的前馈视图合成方法，用目标视图投影线索替代原始相机参数，将任务从射线空间的脆弱几何回归问题转化为条件良好的目标视图图像到图像转换问题，并引入了针对该线索的掩码自编码预训练策略。


<details>
  <summary>Details</summary>
Motivation: 现有前馈视图合成模型使用Plücker射线图编码相机参数，这种方法将预测与任意世界坐标系绑定，使其对微小相机变换敏感，从而破坏了几何一致性。本文旨在寻找能够为稳健且一致的视图合成提供最佳条件化的输入。
...
Conclusion: 投影条件化方法通过将视图合成任务重新定义为条件良好的图像到图像转换问题，显著提高了模型的稳健性和几何一致性，同时通过掩码自编码预训练策略能够有效利用大规模未校准数据。
...
Result: VERSE成功揭示了与错误倾向聚类相关的视觉特征，使用包含这些特征的样本重新训练显著提升了F1性能且不损害泛化能力；优化后的本地模型（Donut、Idefics2）性能达到或超过SaaS解决方案（GPT-4、Pixtral）。

Conclusion: VERSE为视觉语言模型在文档理解任务中提供了一种有效的分析和改进框架，能够系统识别问题区域并通过针对性数据增强提升模型性能，使本地模型达到商业SaaS解决方案水平。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型难以提供对相机和多物体运动的统一精确控制，因为视频本质上是2D图像平面的动态投影。需要一种能够明确控制相机和物体动态的4D感知模型。

Method: 提出4D几何控制表示，通过静态背景点云和每个物体的3D高斯轨迹编码世界状态；将这些4D控制渲染为预训练视频扩散模型的调节信号；开发自动数据引擎从野外视频提取4D标注数据。

Result: 能够生成高保真、视角一致的视频，精确遵循指定的动态控制，提供灵活、类别无关的4D控制替代方案。

Conclusion: VerseCrafter通过统一的4D几何世界状态实现了对相机和物体动态的显式连贯控制，解决了现有方法在4D控制方面的局限性，并通过自动数据引擎克服了大规模4D标注数据稀缺的问题。
...
TL;DR: 本文提出了一种轻量级视觉语言框架，用于从叶片图像进行作物和病害识别，结合Swin Transformer视觉编码器和序列到序列语言解码器，通过两阶段训练策略实现高精度识别和自然语言生成。


<details>
  <summary>Details</summary>
Motivation: 作物病害视觉问答需要准确的视觉理解和可靠的语言生成，现有大规模视觉语言模型参数量大且计算成本高，需要开发轻量级但高效的框架来专门解决作物病害识别问题。

Method: 采用Swin Transformer作为视觉编码器，结合序列到序列语言解码器构建视觉语言框架；使用两阶段训练策略：第一阶段改进视觉表示学习，第二阶段增强跨模态对齐；在大规模作物病害数据集上进行评估。

Result: 模型在作物和病害识别方面均达到高准确率；在BLEU、ROUGE和BERTScore等自然语言生成指标上表现优异；相比大规模视觉语言基线模型，参数量显著减少但性能更优；Grad-CAM和词级归因分析显示良好的可解释性。

Conclusion: 该轻量级视觉语言框架在作物病害视觉问答任务中表现出色，证明了任务特定的视觉预训练对于作物病害识别的重要性，为农业领域的智能诊断提供了有效解决方案。
...
Conclusion: Atlas 2系列模型解决了现有病理基础模型的局限性，为临床部署提供了高性能、强鲁棒性和资源效率的解决方案
...
TL;DR: MuLo-SD是一种新颖的自回归图像生成加速框架，通过多分辨率草稿生成和空间感知验证，实现了最高1.7倍的加速，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在图像合成中表现出色，但其顺序性质导致显著的延迟问题。现有的推测解码方法受限于令牌级模糊性和缺乏空间感知能力。
...
Conclusion: 该方法为图像合成的推测解码设立了新的技术标杆，在效率和保真度之间取得了良好平衡，通过多分辨率草稿和空间感知验证实现了显著加速。
...
TL;DR: VLI是一个无需训练的多模态大语言模型推理框架，通过模拟元认知自我修正过程来减少物体幻觉，在MMHal-Bench上将物体幻觉率降低了12.67%，在POPE上准确率提高了5.8%。


<details>
  <summary>Details</summary>
Motivation: 物体幻觉严重削弱了多模态大语言模型的可靠性，主要源于模型认知内省的失败——盲目信任语言先验而非具体视觉证据。现有缓解方法有限：对比解码方法操作表面化，未能纠正内部语义错位；而当前潜在引导方法依赖缺乏实例特定精度的静态向量。

Method: VLI包含两个核心组件：1) 属性内省：通过概率冲突检测诊断幻觉风险并定位因果视觉锚点；2) 可解释双因果引导：主动调节推理过程，动态隔离视觉证据与背景噪声，同时通过自适应校准消除盲目置信。

Result: 在先进模型上实现了最先进的性能，在MMHal-Bench上将物体幻觉率降低了12.67%，在POPE基准上准确率提高了5.8%。

Conclusion: VLI通过模拟元认知自我修正过程，有效解决了多模态大语言模型的物体幻觉问题，提供了一种无需训练、实例特定的精确推理框架。
...
TL;DR: VideoAuto-R1：一种视频理解框架，采用"必要时推理"策略，通过初始答案置信度决定是否进行推理，在保持SOTA准确率的同时显著提升效率


<details>
  <summary>Details</summary>
Motivation: 研究发现对于RL训练的视频模型，直接回答通常匹配甚至优于思维链推理，但后者计算成本更高。因此需要探索何时需要显式推理，何时可以直接回答

Method: 提出VideoAuto-R1框架，采用"思考一次，回答两次"训练范式：模型首先生成初始答案，然后进行推理，最后输出审核后的答案。推理阶段使用初始答案的置信度决定是否进行推理

Result: 在视频QA和定位基准测试中达到最先进准确率，同时显著提升效率：平均响应长度减少约3.3倍（从149个token降至44个）。感知导向任务中思维模式激活率低，而推理密集型任务中激活率高
...
TL;DR: 研究大视觉语言模型在对象计数任务中的提示诱导幻觉现象，发现模型会偏向文本提示而忽略视觉证据，并通过机制分析识别出关键注意力头，其消融可显著减少幻觉


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型虽然能力强，但经常产生幻觉，倾向于遵循文本提示而忽略视觉证据。本研究旨在控制条件下（对象计数任务）系统研究这种失败模式，探索其内部机制

Method: 在对象计数设置中控制实验，让提示高估图像中的对象数量；对三个VLMs进行机制分析，识别关键注意力头（PIH-heads），并通过消融实验验证其作用

Result: 低对象数量时模型能纠正高估，但随着对象数量增加，模型越来越倾向于遵循提示而忽略差异；识别出少量注意力头，其消融可使提示诱导幻觉减少至少40%；不同模型的PIH-heads以模型特定的方式介导提示复制

Conclusion: 研究揭示了提示诱导幻觉的内部机制，展示了不同模型在实现这些行为时的特定差异，为理解和缓解VLMs中的幻觉问题提供了机制性见解
...
Conclusion: FlowLet是一种有效的条件生成框架，通过可逆3D小波域中的流匹配技术，能够生成高质量、年龄条件化的3D MRI数据，改善脑年龄预测模型的公平性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 人类能够轻松预测物体如何通过交互移动或变化，但计算系统缺乏这种能力。研究旨在让计算系统能够从被动视觉观察中预测物体可能的未来运动

Method: 提出ObjectForesight模型，在3D物体层面显式表示世界，预测未来6-DoF位姿和轨迹。利用分割、网格重建和3D位姿估计技术构建包含200万+短片段的数据集进行训练

Result: ObjectForesight在准确性、几何一致性和泛化到未见物体和场景方面取得显著提升，建立了从观察中学习物理基础物体中心动力学模型的可扩展框架

Conclusion: 该研究展示了直接从视觉观察学习几何基础和物体中心动力学模型的可行性，为预测物体未来运动提供了有效方法
...
<details>
  <summary>Details</summary>
Motivation: 机器人操作数据的多样性、数量和质量对训练有效的策略至关重要，但由于硬件和物理设置的限制，在多样化环境中收集大规模真实世界操作数据仍然难以扩展。现有的文本提示条件图像扩散模型方法往往忽略了多视角和时间一致性观察的实际需求，且仅靠文本提示无法可靠地指定场景设置。

Method: 提出了视觉身份提示方法，将示例图像作为条件输入提供给扩散模型，以指导生成所需的场景设置。同时构建了一个可扩展的流程，从大型机器人数据集中策划视觉身份池。使用增强的操作数据来训练下游的视觉-语言-动作和视觉运动策略模型。

Result: 使用增强的操作数据训练下游策略模型，在仿真和真实机器人设置中都获得了持续的性能提升。

Conclusion: 视觉身份提示方法能够有效解决机器人操作数据收集的规模化问题，通过提供明确的视觉指导来生成多视角和时间一致性的观察数据，从而提升机器人策略模型的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有指称表达任务(RES/REC/REG)只支持单目标表达，即一个表达对应一个目标对象，忽略了多目标和无目标表达的情况，这严重限制了指称表达在实际应用中的实用性。

Method: 提出了ReLA基线方法，通过自适应地将图像划分为包含子实例线索的区域，并显式建模区域-区域和区域-语言之间的依赖关系，以处理GRES/GREC任务中的复杂关系建模挑战。

Result: 构建了首个大规模GREx数据集gRefCOCO，包含多目标、无目标和单目标表达；提出的ReLA方法在GRES和GREC任务上均取得了最先进的结果。

Conclusion: GREx框架和gRefCOCO数据集扩展了指称表达任务的能力，使其能够处理任意数量目标的表达，同时保持与传统REx任务的向后兼容性，为指称表达的实际应用提供了更全面的支持。
...
TL;DR: 本文提出了像素级视觉几何模型PPD和PPVD，通过像素空间扩散变换器实现高质量、无漂浮像素的点云预测，在单目和视频深度估计中表现最佳。
...
Method: 1) 提出像素完美深度PPD：基于像素空间扩散变换器，引入语义提示DiT和级联DiT架构；2) 扩展到视频PPVD：引入语义一致DiT，通过参考引导的token传播保持时间一致性。

Result: 在所有生成式单目和视频深度估计模型中取得最佳性能，产生的点云比其他模型显著更干净。

Conclusion: 通过像素空间生成建模，实现了高质量的视觉几何模型，解决了漂浮像素和细节丢失问题，为机器人和增强现实应用提供了更准确的几何信息。
...
<details>
  <summary>Details</summary>
Motivation: 夜间色彩恒常性由于低光噪声和复杂光照条件而具有挑战性，需要一种能够适应夜间场景的鲁棒白平衡方法

Method: 结合统计方法与深度强化学习：首先设计针对夜间场景的统计算法（包含显著灰度像素检测和新型光照估计），然后以此为核心构建首个用于色彩恒常性的深度强化学习方法，模拟专业AWB调优专家动态优化每张图像的参数

Result: 方法在低光和正常光照图像上表现出优越的泛化能力，并引入了首个多传感器夜间数据集用于跨传感器评估

Conclusion: RL-AWB框架成功结合统计方法与深度强化学习，为夜间白平衡问题提供了有效的解决方案，具有实际应用价值
...
TL;DR: QNeRF：首个用于2D图像新视角合成的混合量子-经典模型，通过参数化量子电路编码空间和视角信息，模型更紧凑，在中等分辨率图像上性能匹配或优于经典NeRF且参数减半。


<details>
  <summary>Details</summary>
Motivation: 量子视觉场（QVFs）在模型紧凑性和收敛速度方面已显示出潜力，而神经辐射场（NeRFs）在新视角合成方面取得重大进展但模型较大、训练密集。本研究旨在将QVFs方法扩展到新视角合成任务，探索量子机器学习在计算机视觉中连续信号表示的竞争力。

Method: 提出QNeRF混合量子-经典模型，利用参数化量子电路通过量子叠加和纠缠编码空间和视角相关信息。设计了两种架构变体：Full QNeRF最大化利用所有量子振幅增强表示能力；Dual-Branch QNeRF引入任务感知归纳偏置，分支处理空间和视角相关的量子态制备，大幅降低操作复杂度并确保可扩展性和硬件兼容性。

Result: 在中等分辨率图像训练时，QNeRF匹配或优于经典NeRF基线，同时使用不到一半的参数数量。实验表明量子机器学习可以作为计算机视觉中等任务（如从2D观测学习3D表示）中连续信号表示的有竞争力替代方案。

Conclusion: QNeRF成功将量子视觉场扩展到新视角合成任务，证明了量子机器学习在计算机视觉连续信号表示中的潜力。通过量子叠加和纠缠实现的紧凑表示，为中等复杂度视觉任务提供了参数效率更高的解决方案。
...
TL;DR: Mesh4D是一个前馈模型，用于从单目视频中重建动态物体的4D网格（3D形状+时间运动），通过紧凑的潜在空间一次性编码整个动画序列。


<details>
  <summary>Details</summary>
Motivation: 从单目视频中重建动态物体的完整3D形状和运动是一个具有挑战性的任务，现有方法通常需要复杂的优化过程或多视角输入。本文旨在开发一个能够一次性重建整个动画序列的高效前馈模型。

Method: 1. 使用自动编码器学习紧凑的潜在空间，编码整个动画序列；2. 训练时利用物体的骨骼结构作为先验指导，但推理时不需要骨骼信息；3. 编码器采用时空注意力机制获得更稳定的变形表示；4. 基于该表示训练潜在扩散模型，以输入视频和第一帧重建的网格为条件，一次性预测完整动画。

Result: 在重建和新视角合成基准测试中，Mesh4D在恢复准确的3D形状和变形方面优于先前方法。

Conclusion: Mesh4D通过紧凑的潜在空间表示和基于骨骼先验的指导，实现了从单目视频中高效、准确地重建动态物体的4D网格，在推理时不需要骨骼信息，具有实用价值。
...
TL;DR: MedPI是一个用于评估大语言模型在医患对话中表现的高维基准测试，包含105个维度，评估了9个主流模型在366个AI患者和7,097次对话中的表现，发现所有模型在多种维度上表现不佳，特别是在鉴别诊断方面。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型评估主要基于单轮问答基准，缺乏对医患对话复杂性的全面评估。需要建立一个更全面、多维度、符合医学教育认证标准的评估框架来准确衡量LLMs在临床对话中的表现。

Method: MedPI包含五个层次：1）患者包（合成电子健康记录）；2）具有记忆和情感的AI患者；3）任务矩阵（就诊原因×就诊目标）；4）包含105个维度的评估框架（1-4分制，映射到ACGME能力）；5）经过校准的委员会式AI评委。评估了9个主流模型在标准化"普通医生"提示下的表现。

Result: 评估了9个旗舰模型（Claude Opus 4.1、Claude Sonnet 4、MedGemma、Gemini 2.5 Pro、Llama 3.3 70b Instruct、GPT-5、GPT OSS 120b、o3、Grok-4）在366个AI患者和7,097次对话中的表现。所有LLMs在多种维度上表现不佳，特别是在鉴别诊断方面表现较差。

Conclusion: MedPI为评估LLMs在医患对话中的表现提供了一个全面、高维的基准测试框架。研究发现当前主流LLMs在临床对话中表现有限，特别是在诊断方面，这有助于指导未来LLMs在诊断和治疗建议方面的应用。
...
<details>
  <summary>Details</summary>
Motivation: 针对需要解释性和可解释性的应用场景，大语言模型缺乏显式规则，因此需要构建动词构式数据库来补充LLMs，提供可解释的规则支持。

Method: 将动词构式形式化定义为投影、有根、有序、有向无环图，采用一系列聚类算法从大规模语料检索的句子列表中为给定动词生成构式。

Result: 统计分析显示生成的构式具有功能独立性和分级典型性的设计特征；在动词语法错误纠正评估中，基于构式最大匹配的错误纠正算法优于大语言模型。

Conclusion: 提出的无监督动词构式构建方法能够生成具有功能独立性和分级典型性的构式，在需要解释性的应用场景中优于大语言模型，为语言处理提供了可解释的规则补充。
...
TL;DR: 使用大语言模型生成合成电商产品数据，通过三种策略控制修改，在MAVE数据集上达到60.5%准确率，与真实数据训练效果相当，混合数据可提升至68.8%
...
Method: 提出使用大语言模型生成合成电商产品数据的系统方法，引入包含三种策略的受控修改框架：属性保留修改、受控负例生成和系统属性移除。使用具有属性感知提示的最先进大语言模型，在保持产品连贯性的同时强制执行店铺约束。

Result: 人工评估2000个合成产品显示高有效性：99.6%被评为自然，96.5%包含有效属性值，超过90%显示一致的属性使用。在公开MAVE数据集上，合成数据达到60.5%准确率，与真实训练数据（60.8%）表现相当，显著优于13.4%的零样本基线。混合合成和真实数据的配置进一步将性能提升至68.8%准确率。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在回答社区特定问题时经常失败，形成"知识盲点"，边缘化地方声音并加剧认知不公。现有AI系统缺乏对本地知识的理解和整合。

Method: 开发参与式协议，通过三个参与式映射工作坊（N=24）设计故事提取方法和结构化模式，保留叙事丰富性的同时实现实体、时间、地点提取、验证和来源控制。

Result: 审计县级基准测试（14,782个本地信息问答对）显示76.7%的错误源于事实空白、文化误解、地理混淆和时间错位。在参与式问答集上，最先进的LLM无额外上下文时正确率低于21%。收集的叙事中常包含缺失的事实信息。

Conclusion: 该研究为构建社区锚定的AI系统提供了分类法、协议和参与式评估的严谨基础，强调检索优先、来源可见、本地治理的设计原则，以解决代表性、权力、治理、隐私等关键设计张力。
...
TL;DR: TeleTables是一个评估语言模型在电信标准中表格理解和知识能力的基准，发现小模型在3GPP规范表格处理上表现不佳，大模型表现更好，需要领域专业化微调。


<details>
  <summary>Details</summary>
Motivation: LLM在电信行业应用日益广泛，但研究表明它们在处理3GPP标准时表现不佳，特别是标准中大量使用表格呈现关键信息，而LLM对这些表格的知识和解释能力尚未得到充分研究。

Method: 通过多阶段数据生成流程构建TeleTables基准：从3GPP标准中提取表格，使用多模态和推理导向的LLM生成和验证问题，最终得到500个人工验证的问题-答案对，每个问题都关联多种格式的对应表格。

Result: 评估显示：小模型（参数少于100亿）在3GPP知识回忆和表格解释方面都表现不佳，表明它们在预训练中对电信标准接触有限，且缺乏处理复杂技术材料的归纳偏差。大模型在表格解释方面表现出更强的推理能力。

Conclusion: TeleTables强调了领域专业化微调的必要性，以可靠地解释和推理电信标准。该基准公开可用，有助于推动LLM在电信领域的应用改进。
...
TL;DR: 本文提出了一种新的扩散语言模型重掩码策略，通过动态检测每个token的时间方差和空间偏差来自适应调整置信度阈值，相比传统固定阈值方法显著提升了效率。


<details>
  <summary>Details</summary>
Motivation: 传统扩散语言模型的重掩码策略使用单一全局置信度阈值，忽视了token在时间和空间维度上的动态特性，导致冗余迭代和并行性受限，影响了模型的效率和输出质量。

Method: 提出了一种新颖的重掩码方法，动态检测每个token的时间方差（反映收敛状态）和空间偏差（反映token间相关性），基于这些信号自适应调整每个token在每个时间步的置信度阈值。

Result: 在主流数据集上的实验结果表明，该方法显著提升了扩散语言模型的运行效率，实现了高达8.9倍的加速，同时忠实地保持了生成质量。

Conclusion: 通过考虑token的时间和空间动态特性来自适应调整置信度阈值的重掩码策略，能够有效提升扩散语言模型的效率和性能，为文本生成任务提供了更优的解决方案。
...
TL;DR: 本文提出了一种结合微调语言模型和检索增强生成（RAG）的AI系统，用于优化大学招生办公室的咨询响应效率和质量。


<details>
  <summary>Details</summary>
Motivation: 大学招生办公室面临大量咨询请求，需要在保证响应质量的同时提高效率，这对潜在学生的感知有重要影响。传统RAG在招生等复杂专业领域可能因规则细节复杂而产生上下文不充分的回答。

Method: 提出混合方法：1）使用RAG从大型数据集中检索相关信息；2）在专门针对招生流程的精选数据集上微调语言模型，增强其对RAG提供数据的准确解释能力；3）探索响应生成逻辑的优化策略，平衡响应质量和速度。

Result: 该方法结合了RAG获取最新信息的能力和微调模型嵌入领域专业知识的能力，能够生成符合招生沟通特定要求的高质量输出。

Conclusion: 通过微调语言模型与RAG的结合，可以有效提升大学招生咨询系统的响应准确性和效率，满足复杂专业领域的特定需求。
...
TL;DR: 论文提出了一种轻量级线性探针方法，用于量化和校正LLMs内部政治意识形态与人类意识形态空间之间的系统性错位，通过调整输出层概率而非重新训练模型来实现低成本对齐。


<details>
  <summary>Details</summary>
Motivation: 研究发现大型语言模型内部组织的政治意识形态与人类意识形态空间存在系统性错位，这种错位是模型特定的、可测量的，需要一种低成本的方法来对齐模型与特定用户观点。

Method: 提出轻量级线性探针方法，从模型内部特征计算偏差分数，直接调整最终输出概率，无需重新训练模型，保持原始推理能力。

Result: 该方法能够量化模型与人类意识形态之间的错位程度，并通过最小化校正输出层来对齐模型观点，实现实用且低成本的意识形态对齐。

Conclusion: LLMs内部政治意识形态与人类意识形态存在系统性错位，通过线性探针方法可以量化和校正这种错位，为模型观点对齐提供了一种简单高效的解决方案。
...
TL;DR: LEXMA是一个基于强化学习的LLM微调框架，用于为多受众生成叙事驱动的AI决策解释，解决了现有方法在决策正确性、忠实性和标签效率方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前AI模型在高风险消费者交互中应用广泛，但决策逻辑不透明。现有可解释AI技术依赖事后数值特征归因，无法提供连贯的叙事解释。LLM虽然能生成自然语言解释，但仍面临三个设计挑战：解释需要既决策正确又忠实于预测驱动因素；需要服务多受众而不改变底层决策规则；需要标签高效训练而不依赖大量人工标注解释。

Method: LEXMA是一个基于强化学习的微调框架，结合了反思增强的监督微调和两阶段组相对策略优化（GRPO）。它微调两个独立的参数集来提升决策正确性并满足不同受众的风格要求，使用不依赖人工标注解释的奖励信号。在抵押贷款审批决策场景中实例化该框架。

Result: LEXMA在预测性能上相比其他LLM基线有显著提升。人工评估显示，该方法生成的面向专家的解释更关注风险，面向消费者的解释更清晰、更具可操作性且更礼貌。

Conclusion: LEXMA提供了一个成本高效、系统化的LLM微调方法，可提升商业决策解释质量，为透明AI系统的可扩展部署提供了强大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决俄罗斯媒体行业在剧本审查中面临的实际编辑挑战，满足联邦法律436-FZ对内容安全评估的要求，提高审查效率和一致性。

Method: 系统采用微调的Phi-3-mini模型配合4位量化技术，将剧本分割为叙事单元，在五个内容类别（暴力、性内容、脏话、物质、恐怖元素）中检测违规内容，并分配年龄分级（0+、6+、12+、16+、18+），提供可解释的评估依据。

Result: 在Wink黑客松（2025年11月）中开发完成，实现了80%的分级准确率和80-95%的分割精度（取决于格式），在80GB VRAM限制和5分钟内处理平均剧本的严格约束下运行，已部署在Yandex Cloud上并支持CUDA加速。

Conclusion: Qwerty AI系统展示了在俄罗斯媒体行业生产工作流程中的实际应用价值，能够高效、准确地自动化剧本内容安全评估过程。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然能生成高质量文本，但在安全关键领域部署时存在产生幻觉的问题，这限制了其实际应用。小型语言模型在忠实度方面需要改进。

Method: 采用端到端框架，包含数据生成模块，通过可控幻觉注入技术生成合成偏好数据，使用偏好优化范式来增强小型语言模型的忠实度。

Result: 论文提供了关于数据质量和模型大小对基于偏好的优化方法影响的见解，明确了这些方法最有效的条件。

Conclusion: TrueBrief框架能够有效提升小型语言模型在文本摘要任务中的忠实度，为安全关键领域的应用提供了可行解决方案。
...
Result: 创建了AnimatedLLM应用，提供Transformer语言模型的逐步可视化，可用于教学和自学目的

Conclusion: AnimatedLLM填补了LLM教学材料的空白，通过交互式可视化帮助理解Transformer模型的工作原理
...
TL;DR: 论文提出了BiForget框架，通过目标模型自身生成高质量遗忘数据集，改进LLM遗忘评估的基准质量。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习遗忘基准无法准确反映模型实际学习的"遗忘范围"，需要更高质量的遗忘数据集来评估LLM遗忘效果。

Method: 提出BiForget框架，通过种子引导和对抗性提示技术，利用目标模型自身生成匹配其内部知识分布的高质量遗忘数据集。

Result: 在多个基准测试中，BiForget在相关性、多样性和效率方面表现优异，在哈利波特领域将相关性提高约20，多样性提高约0.05，同时将总数据量减半。

Conclusion: BiForget为评估LLM遗忘提供了更严格的基准，能够实现更稳健的遗忘和更好的效用保留。
...
Method: 开发了包含10K+主张-证据集的数据集，使用八个LLM进行标注，通过同行评审评论校准夸大分数。采用微调的重新排序器进行证据检索，以及微调模型预测夸大分数并提供理由。

Result: 相比强基线，RIGOURATE实现了改进的证据检索和夸大检测能力，能够有效识别主张的夸大程度。

Conclusion: 该工作实现了证据比例的操作化，支持更清晰、更透明的科学交流，有助于提高科学论文的严谨性。
...
TL;DR: 该研究对印度多种方言和语言变体的自发、嘈杂、代码混合语音进行跨语言迁移实证分析，发现ASR性能与语言间系统发育距离相关，但方言场景中其他因素同样重要，少量方言数据微调可达到大量高资源语言微调的效果。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索自动语音识别系统在方言和非标准化语音场景下的表现，特别是跨语言迁移在低资源方言环境中的有效性，以及系统发育距离对ASR性能的影响程度。

Method: 采用实证研究方法，分析多种印度方言和语言变体的自发、嘈杂、代码混合语音数据。通过对比不同语言间的系统发育距离与ASR性能关系，进行方言数据微调实验，并以Garhwali语言为案例研究，评估多个当代ASR模型，分析转录错误以考察预训练语言偏见。

Result: 结果显示：1) ASR性能通常随语言间系统发育距离减小而改善，但方言环境中该因素不能完全解释性能差异；2) 少量方言数据微调可获得与大量高资源标准化语言微调相当的性能；3) Garhwali案例显示当代ASR模型在低资源方言上的表现；4) 转录错误分析揭示了ASR系统对预训练语言的偏见。

Conclusion: 研究表明，虽然系统发育距离是跨语言ASR迁移的重要因素，但在方言和非标准化语音场景中，其他因素同样关键。少量方言数据微调可有效提升性能，为低资源方言ASR开发提供了实用策略。错误分析揭示了模型偏见问题，为未来改进方向提供了见解。
...
TL;DR: Disco-RAG：一种基于话语感知的检索增强生成框架，通过显式注入话语信号来提升大语言模型在知识密集型任务中的表现，在问答和长文档摘要任务上达到SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 现有RAG策略通常将检索到的段落视为扁平无结构的内容，这限制了模型捕捉结构线索的能力，也约束了其从分散文档证据中综合知识的能力。

Method: 提出Disco-RAG框架，通过构建内部块话语树来捕捉局部层次结构，并建立跨块修辞图来建模跨段落连贯性，将这些结构联合集成到规划蓝图中来指导生成过程。

Result: 在问答和长文档摘要基准测试中，Disco-RAG无需微调即可达到最先进的结果，证明了该方法的有效性。

Conclusion: 话语结构在推进RAG系统发展中扮演着重要角色，显式注入话语信号能够显著提升大语言模型在知识密集型任务中的性能。
...
TL;DR: 论文揭示了当前大语言模型安全评估的局限性，指出安全对齐并非普遍能力而是存在人口统计学层次差异，模型规模扩大反而加剧了针对不同少数群体的防御率差异。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的安全评估存在虚假的普遍性，将"身份仇恨"等安全问题聚合为标量分数，掩盖了模型对特定人群的系统性脆弱性。研究者旨在揭示这种选择性安全问题。

Method: 引入MiJaBench双语对抗基准（英语和葡萄牙语），包含16个少数群体的44,000个提示。从12个最先进的大语言模型生成了528,000个提示-响应对，并构建了MiJaBench-Align数据集进行分析。

Result: 安全对齐不是普遍的语义能力，而是人口统计学层次：同一模型内针对不同目标群体的防御率波动高达33%。模型规模扩大会加剧这些差异，表明当前对齐技术没有建立非歧视原则，只强化了对特定群体的记忆拒绝边界。

Conclusion: 当前的安全对齐技术存在系统性偏见，模型规模扩大反而恶化了针对不同少数群体的安全性能差异，挑战了现有的安全扩展定律。需要研究更细粒度的人口统计学对齐方法。
...
TL;DR: LLMs经常无法挑战用户的有害信念，研究发现这与LLMs默认迎合用户假设和缺乏认知警惕有关。通过简单的语用干预（如添加"等一下"）可以显著改善这一表现。


<details>
  <summary>Details</summary>
Motivation: LLMs在医学建议、社会推理等领域经常无法挑战用户的有害信念，这种失败可能源于LLMs默认迎合用户假设和缺乏认知警惕。研究者希望通过理解这种语用行为来改善LLM的安全性。

Method: 研究分析了影响人类迎合行为的社交和语言因素（话题相关性、语言编码、来源可靠性）如何影响LLMs的迎合行为。使用三个安全基准测试（Cancer-Myth、SAGE-Eval、ELEPHANT）评估模型挑战有害信念的能力，并测试简单的语用干预措施。

Result: 研究发现影响人类迎合行为的因素同样影响LLMs，解释了在不同安全基准测试中的性能差异。简单的语用干预（如添加"等一下"）显著提高了模型在这些基准测试上的表现，同时保持了较低的错误率。

Conclusion: 考虑语用因素对于评估LLM行为和改善LLM安全性至关重要。简单的语用干预可以有效地提高LLMs挑战有害信念的能力，为改善LLM安全性提供了实用方法。
...
TL;DR: 研究发现：通过LLM-as-a-judge优化对话预测会降低模型对真实人类响应的预测能力，而直接最大化真实人类对话的对数概率能更好地预测人类实际会说什么，结合思维链作为隐变量优化下界目标效果最佳。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过下一轮对话预测来建模人类思维过程，探索不同学习方法在预测人类对话行为方面的效果差异。

Method: 比较两种学习维度：1) 模型是否允许在回答前进行思考；2) 学习奖励方式：使用LLM-as-a-judge评分语义相似性和信息完整性，或直接最大化真实人类对话的对数概率。将思维链作为隐变量，推导对数概率的下界进行优化。

Result: 优化基于评判者的奖励确实提高了评判分数，但降低了对真实人类响应的似然估计，在人类评判者选择最人性化响应时降低了胜率。直接最大化真实人类响应的对数概率在所有评估中都表现更好，结合思维链隐变量优化的下界目标在所有评估中取得最佳结果。

Conclusion: 思维主要在基于真实人类对话的分布匹配目标训练时才有帮助，将此方法扩展到更广泛的对话数据可能产生对人类行为有更细致理解的模型。
...
TL;DR: MB-Defense是一个针对指令调优大语言模型的后门攻击防御框架，通过两阶段训练流程（防御性毒化和权重恢复）来免疫多种后门威胁，显著降低攻击成功率同时保持指令跟随能力。


<details>
  <summary>Details</summary>
Motivation: 指令调优的大语言模型虽然能实现广泛的任务泛化，但其依赖的大规模数据集容易受到后门攻击，攻击者可通过毒化少量数据植入隐藏行为。目前针对指令调优模型的防御研究不足，存在安全风险。

Method: MB-Defense包含两个阶段：1) 防御性毒化阶段：将攻击者触发器和防御触发器合并为统一的后门表示；2) 权重恢复阶段：通过额外训练打破这种表示，恢复干净行为。该方法采用数据高效的防御策略。

Result: 在多个大语言模型上的广泛实验表明，MB-Defense能显著降低攻击成功率，同时保持指令跟随能力。该方法提供了可泛化的防御策略，能提升指令调优模型对未见后门攻击的鲁棒性。

Conclusion: MB-Defense是一个有效的训练管道，能够免疫指令调优大语言模型免受多种后门威胁，为提升模型安全性提供了通用且数据高效的防御方案。
...
TL;DR: 研究发现用户对AI写作助手帮助时机的偏好存在认知-行为差距：用户自我报告认为紧迫性最重要，但实际行为中写作努力程度才是主要驱动因素，紧迫性几乎没有预测力。


<details>
  <summary>Details</summary>
Motivation: 主动式AI写作助手需要预测用户何时需要写作帮助，但目前缺乏对用户偏好驱动因素的实证理解，特别是用户自我报告与实际行为之间是否存在差异。

Method: 采用因子情境研究设计，50名参与者进行750对配对比较，分析用户对AI写作助手帮助时机的偏好，比较用户自我报告与实际行为数据。

Result: 写作努力程度是主要行为驱动因素（ρ=0.597），而紧迫性几乎没有预测力（ρ≈0）。用户存在显著认知-行为差距：自我报告将紧迫性排第一，但实际行为中紧迫性是最弱驱动因素。基于用户陈述偏好的系统准确率仅57.7%，低于朴素基线；基于行为模式的系统准确率达61.3%（p<0.05）。

Conclusion: 依赖用户内省进行系统设计会误导优化，对主动式自然语言生成系统有直接启示：应基于实际行为数据而非自我报告来设计AI写作助手。
...
TL;DR: Concept Tokens：一种轻量级方法，通过为预训练LLM添加特殊token并仅从目标概念的多个自然语言定义中学习其嵌入，实现对冻结LLM的行为控制。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过概念定义来学习紧凑的控制信号，以引导冻结大型语言模型的行为，同时保持模型其他功能不受影响。

Method: 为预训练LLM添加新的特殊token，仅从目标概念的多个自然语言定义中学习其嵌入（定义中的概念出现被替换为新token），使用标准语言建模目标优化嵌入，保持LLM参数冻结。

Result: 在三个设置中评估：1) HotpotQA闭卷问答中的幻觉研究显示方向性效应；2) 诱导重述教学策略时观察到相同方向性效应，且比上下文提供完整定义语料库更好地保持对其他指令的遵从性；3) 定性研究展示了学习嵌入捕获的信息及其局限性。

Conclusion: Concept Tokens提供了一种从定义中学习的紧凑控制信号，可以有效地引导冻结LLM的行为，在减少幻觉和保持指令遵从性方面表现出优势。
...
TL;DR: SampoNLP工具包使用自参考原子性评分创建形态学词典，用于评估BPE分词器在芬兰语、匈牙利语和爱沙尼亚语等乌拉尔语系语言中的表现，提出综合性能评分(IPS)指标并确定最佳词汇量大小。
...
Method: 开发SampoNLP工具包，使用基于最小描述长度原理的自参考原子性评分方法，通过内部结构线索过滤复合形式，无需语料库即可创建高纯度形态学词典。然后使用这些词典系统评估BPE分词器在不同词汇量(8k-256k)下的表现，并提出综合性能评分(IPS)来平衡语素覆盖率和过度分割。

Result: 为芬兰语、匈牙利语和爱沙尼亚语生成了高纯度形态学词典。通过分析IPS曲线确定了"拐点"(收益递减点)，首次为这些语言提供了基于实证的最佳词汇量建议。定量证明了标准BPE在高度黏着性语言中的局限性。

Conclusion: SampoNLP为形态丰富的乌拉尔语系语言提供了实用的分词器评估工具和资源，通过IPS指标和最佳词汇量建议为这些语言的分词器设计提供了实证指导，同时揭示了标准BPE在黏着性语言中的不足。
...
TL;DR: 该论文提出了WESR-Bench，一个用于非语言声音事件检测和定位的评估基准，包含21种声音事件的精细分类、专家标注数据集和新的评估协议。


<details>
  <summary>Details</summary>
Motivation: 语音不仅传递语言信息，还包含丰富的非语言声音事件（如笑声、哭声）。当前方法存在任务定义不足、类别覆盖有限、时间粒度模糊以及缺乏标准化评估框架的问题，阻碍了下游应用的发展。

Method: 1. 开发了21种声音事件的精细分类法，分为离散型（独立）和连续型（与语音混合）两类；2. 引入WESR-Bench专家标注评估集（900+话语），采用新的位置感知协议，将ASR错误与事件检测分离；3. 构建了1,700+小时语料库并训练专门化模型。

Result: 构建的专门化模型超越了开源音频-语言模型和商业API，同时保持了ASR质量。WESR-Bench为精确定位离散和连续事件提供了评估框架。

Conclusion: WESR将作为未来研究丰富真实世界听觉场景建模的基础资源，解决了非语言声音事件检测和定位的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的多智能体系统主要关注架构设计（如角色分配、工作流编排），而本文关注交互过程本身，旨在通过帮助智能体更有效地传达其意图来提高沟通效率

Method: 提出LinguaGame，一种基于语言学的游戏论范式，将对话建模为意图和策略的信号博弈，采用无需训练的均衡近似算法进行推理时决策调整

Result: 在模拟法庭程序和辩论场景中评估，通过人类专家评估显示沟通效率有显著提升

Conclusion: LinguaGame框架将对话视为有意图和策略性的沟通，通过语言学基础的游戏论方法有效提升了多智能体系统的沟通效率，相比之前与任务目标紧密耦合的游戏论方法，本方法具有更少的任务特定耦合
...
TL;DR: GRACE是一个基于强化学习的框架，通过证据充分性评估、关键证据提取和选择性弃答，统一解决RAG系统中无证据正确回答和证据不足时编造答案两大缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统存在两个关键缺陷：1）在没有显式证据支持的情况下提供正确答案；2）在检索上下文不足时产生编造的回答。虽然先前研究分别解决了这些问题，但缺乏一个统一框架来整合基于证据的接地和可靠弃答。

Method: GRACE采用强化学习框架，包含：1）数据构建方法，利用异构检索器生成多样化的训练样本而无需人工标注；2）多阶段门控奖励函数，训练模型评估证据充分性、提取关键支持证据，并提供答案或明确弃答。

Result: 在两个基准测试上的实验结果表明，GRACE实现了最先进的整体准确性，并在准确回答和拒绝之间取得了良好的平衡，同时只需要先前方法10%的标注成本。

Conclusion: GRACE提供了一个统一的强化学习框架，能够同时解决RAG系统中的两大关键缺陷，通过证据充分性评估和选择性弃答机制，在保持高准确性的同时大幅降低标注成本。
...
TL;DR: 该研究首次系统评估了孟加拉语LLM文本生成中的水印技术，发现现有方法在跨语言往返翻译攻击下检测准确率崩溃至9-13%，并提出分层水印策略将检测准确率提升25-35%。
...
Method: 研究对三种最先进的文本水印方法（KGW、指数采样EXP和Waterfall）在孟加拉语LLM文本生成中进行系统评估，特别针对跨语言往返翻译攻击。为应对攻击，提出分层水印策略，结合嵌入时水印和生成后水印。

Result: 在良性条件下，KGW和EXP达到高检测准确率（>88%），困惑度和ROUGE指标退化可忽略。但往返翻译攻击导致检测准确率崩溃至9-13%。分层水印将攻击后检测准确率提升25-35%，达到40-50%，相对单层方法提升3-4倍，代价是可控的语义退化。

Conclusion: 该研究量化了多语言水印中的鲁棒性-质量权衡，确立了分层水印作为低资源语言（如孟加拉语）的实用、无需训练的解决方案，为低资源语言水印技术提供了重要基准。
...
TL;DR: NeuronLLM是一个基于功能拮抗原理的任务级LLM神经元理解框架，通过对比学习促进任务完成的"好神经元"和抑制任务的"坏神经元"，并利用增强问题集缓解LLM的偶然行为，在多个NLP任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要识别与特定能力相关的神经元，无法适应需要协调多种能力的任务场景，且只关注支持性神经元而忽略抑制性神经元，同时LLM的偶然行为会导致错误的神经元归因。

Method: 提出NeuronLLM框架，基于生物功能拮抗原理，将任务性能建模为由促进任务完成的"好神经元"和抑制任务的"坏神经元"共同决定，通过对比学习同时建模这两种神经元，并利用增强问题集缓解偶然行为。

Result: 在不同规模和家族的LLM上进行全面实验，在四个NLP任务上NeuronLLM均优于现有方法，为LLM功能组织提供了新的见解。

Conclusion: NeuronLLM通过任务级神经元识别框架，克服了现有能力特定方法的局限性，为理解和引导LLM提供了更全面的视角。
...
Method: 提出FeedEval框架，使用三个教学维度（具体性、帮助性、有效性）训练专门的LLM评估器，评估多个反馈候选并筛选高质量反馈用于下游任务。

Result: 在ASAP++基准测试中，FeedEval与人类专家判断高度一致；使用FeedEval筛选的高质量反馈训练的作文评分模型获得更优的评分性能；小LLM的修订实验显示，FeedEval识别的高质量反馈能带来更有效的作文修订。

Conclusion: FeedEval能有效评估LLM生成的作文反馈质量，筛选高质量反馈提升下游应用性能，为自动作文评分系统提供可靠的质量控制机制。
...
TL;DR: RL-Text2Vis是首个基于强化学习的文本到可视化生成框架，使用GRPO算法和多目标奖励函数，在文本准确性、代码有效性和可视化质量方面显著超越GPT-4o等基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有文本到可视化系统存在语义对齐和清晰度问题：闭源LLM生成的图表质量不足，开源模型经常产生不可执行或视觉质量差的输出，而传统监督微调无法捕捉执行后反馈来提升可视化质量。

Method: 提出RL-Text2Vis框架，基于Group Relative Policy Optimization (GRPO)算法，设计新颖的多目标奖励函数，联合优化文本准确性、代码有效性和可视化质量，利用执行后反馈进行训练。

Result: 在Text2Vis基准测试中，RL-Text2Vis相比GPT-4o在图表质量上获得22%的相对提升，代码执行成功率从零样本基线的78%提升到97%，在VIS-Eval和NVBench等域外数据集上也表现出强大的泛化能力。

Conclusion: GRPO是可视化生成中结构化多模态推理的有效策略，RL-Text2Vis框架显著提升了文本到可视化系统的整体质量，为相关领域提供了新的强化学习解决方案。
...
TL;DR: 通过模型合并技术，将Qwen-8B与泰国专用模型结合，创建了具备多领域能力的大语言模型，在泰国语言能力和金融领域表现均有提升。


<details>
  <summary>Details</summary>
Motivation: 由于隐私、安全和监管考虑，银行和金融行业更倾向于本地部署大语言模型。泰国LLM计划旨在提升开源LLM的泰语能力，但组织面临部署多个专业模型与训练单一多功能模型成本过高之间的权衡。

Method: 采用模型合并作为资源高效替代方案，将Qwen-8B与泰国专用模型进行合并。进行了两个关键实验：1) Qwen-8B与ThaiLLM-8B合并；2) Qwen-8B与ThaiLLM-8B及THaLLE-CFA-8B合并。

Result: 第一个合并实验显示ThaiLLM-8B提升了泰语通用能力，在M3和M6 O-NET考试中表现优于通用指令跟随模型Qwen-8B。第二个合并实验进一步提升了通用和金融领域的性能，在M3、M6 O-NET、Flare-CFA和Thai-IC基准测试中均有改善。

Conclusion: 模型合并是高效创建多功能大语言模型的可行方法，能够在不投入大量训练成本的情况下，整合不同模型的优势，满足特定领域需求。
...
TL;DR: ROME知识编辑方法在单跳事实更新上高效，但在多跳推理任务中存在三个关键失败模式，提出了冗余编辑策略来改善多跳推理性能


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑方法（特别是ROME）在单跳事实更新上表现出色，但在处理需要知识链的多跳推理任务时面临显著挑战，需要解决这些局限性

Method: 通过在不同层深度编辑知识，识别出三个关键失败模式，并提出冗余编辑策略来缓解"跳跃过晚"和泛化能力下降问题

Result: 冗余编辑方法可以将2跳问题的准确率提高至少15.5个百分点，相比之前的单次编辑策略提升96%，但需要在特异性和语言自然度上做出权衡

Conclusion: 冗余编辑策略能有效改善知识编辑方法在多跳推理任务中的性能，为解决"跳跃过晚"和泛化能力下降问题提供了有效方案
...
TL;DR: 该研究指出当前视觉语言模型中描述特异性与长度被混淆的问题，提出应区分这两个概念，并通过构建数据集验证人们更偏好特异性描述而非冗长描述。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在生成图像描述时，常常将描述的特异性与长度混为一谈。研究者认为这两个概念需要解耦：描述可以是简洁但信息密集的，也可以是冗长但空洞的。需要建立更准确的评估方法来直接衡量描述的特异性。

Method: 1. 将特异性定义为相对于对比集而言，描述能更好地区分目标图像与其他可能图像的程度。2. 构建一个控制长度但变化信息内容的数据集。3. 通过人类偏好实验验证人们是否更偏好特异性描述，而不受长度影响。4. 分析长度预算的分配方式如何影响特异性。

Result: 1. 人类评估者确实更偏好特异性描述，无论描述长度如何。2. 仅控制长度不能完全解释特异性差异：长度预算的分配方式对特异性有重要影响。3. 验证了直接优先考虑特异性而非冗长性的评估方法的有效性。

Conclusion: 视觉语言模型的描述评估应该直接关注特异性而非长度。仅控制描述长度不足以确保高质量描述，需要关注信息内容的密度和分配方式。这为开发更准确的图像描述评估方法提供了理论基础。
...
TL;DR: 该论文提出CuCu框架，利用国家社会研究课程构建文化特定的问答数据集，以解决LLM在不同语言文化中表现不均的问题，并以韩国课程为例创建了KCaQA数据集。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在许多任务上表现良好，但在不同语言和文化中的进展不均衡，往往反映了以英语为中心的训练数据中隐含的价值观。为了实现实际的文化对齐，需要一种可扩展的方法来进行文化感知的监督。

Method: 提出CuCu框架，这是一个自动化的多智能体LLM框架，将国家教科书课程转化为开放式的、文化特定的问答对。以韩国国家社会研究课程为例，构建了KCaQA数据集，包含34.1k个开放式问答对。

Result: 定量和定性分析表明，KCaQA覆盖了文化特定的主题，并产生了基于当地社会文化背景的回应。该数据集能够帮助LLM更好地理解和回应特定文化背景的问题。

Conclusion: 利用国家社会研究课程作为文化感知监督的基础是一种可行的方法，CuCu框架能够有效地将课程内容转化为文化特定的问答数据，有助于解决LLM在不同文化中的对齐问题。
...
TL;DR: SpeechMedAssist：通过两阶段训练范式，利用有限语音数据（仅需1万合成样本）实现基于语音的医疗咨询对话系统


<details>
  <summary>Details</summary>
Motivation: 医疗咨询本质上是语音中心的，但现有研究多基于长文本交互，不便且不友好。虽然语音语言模型（SpeechLMs）可实现更自然的语音交互，但医疗语音数据稀缺和直接微调的低效性阻碍了SpeechLMs在医疗咨询中的应用。

Method: 提出SpeechMedAssist，通过利用SpeechLMs的架构特性，将传统单阶段训练解耦为两阶段范式：1）通过文本进行知识与能力注入；2）使用有限语音数据进行模态重对齐，仅需1万合成语音样本。

Result: 实验结果表明，该模型在大多数评估设置中，在有效性和鲁棒性方面均优于所有基线方法。

Conclusion: SpeechMedAssist通过创新的两阶段训练范式，显著降低了医疗语音数据需求，实现了高效的语音医疗咨询系统，为SpeechLMs在医疗领域的应用提供了可行方案。
...
Method: 提出CRANE框架，通过神经元级干预重新定义语言特异性为功能必要性，基于神经元对语言条件预测的贡献而非激活幅度来表征神经元专门化。

Result: 神经元级干预揭示了不对称模式：屏蔽目标语言相关神经元会选择性降低该语言性能，同时很大程度上保留其他语言性能，表明语言选择性但非排他性的神经元专门化。在英语、中文和越南语的多个基准测试中，CRANE比基于激活的方法更精确地分离语言特定组件。

Conclusion: CRANE提供了一个更精确的框架来分析多语言LLM中神经元层面的语言组织，揭示了语言选择性但非排他性的神经元专门化模式，有助于理解多语言能力的神经基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM工具调用框架依赖自然语言推理来确定工具调用时机和结果提交，缺乏逻辑安全性和可验证性的形式化保证，需要建立更可靠、可调试的AI系统基础。

Method: ToolGate维护显式符号状态空间作为类型化键值映射，将每个工具形式化为Hoare风格的契约（前置条件和后置条件），通过前置条件门控工具调用，通过运行时验证后置条件来决定是否提交结果更新状态。

Result: 实验验证表明ToolGate显著提高了工具增强LLM系统的可靠性和可验证性，同时在复杂多步推理任务上保持竞争力。

Conclusion: 该工作为构建更可信、可调试的集成语言模型和外部工具的AI系统奠定了基础，确保符号状态仅通过已验证的工具执行演化，防止无效或幻觉结果破坏世界表示。
...
TL;DR: 提出基于生成式AI的多模态框架，在有限数据条件下检测、解释和干预仇恨表情包，首次将这三个任务整合研究


<details>
  <summary>Details</summary>
Motivation: 现有研究通常将仇恨表情包的检测、解释和干预分开研究，不符合现实场景需求；同时构建大规模标注数据集成本过高，需要能在有限数据条件下工作的通用解决方案

Method: 提出新颖框架，利用任务特定的生成式多模态智能体和大型多模态模型的少样本适应能力，针对不同类型表情包进行处理

Result: 这是首个专注于有限数据条件下通用仇恨表情包内容审核的研究，在现实生产场景中具有强大的部署潜力

Conclusion: 通过生成式AI模型整合检测、解释和干预三个互补角度，为仇恨表情包内容审核提供了更符合现实需求的解决方案
...
TL;DR: 论文针对韩语否定理解问题，构建了Thunder-KoNUBench基准测试，评估了47个LLM模型，发现模型在否定句上表现下降，并通过微调提升了韩语否定理解和上下文理解能力。


<details>
  <summary>Details</summary>
Motivation: 否定理解对大型语言模型具有挑战性，但目前针对韩语否定理解的评估基准稀缺。研究者发现LLM在否定句上的表现会下降，需要专门的基准来评估和改进韩语否定理解能力。

Method: 1. 对韩语否定进行基于语料库的分析；2. 构建Thunder-KoNUBench句子级基准测试，反映韩语否定现象的经验分布；3. 评估47个LLM模型；4. 分析模型大小和指令微调的影响；5. 在Thunder-KoNUBench上进行微调以改进模型性能。

Result: 1. LLM在否定句上的表现确实下降；2. 模型大小和指令微调对否定理解有影响；3. 在Thunder-KoNUBench上微调能显著提升韩语否定理解能力；4. 微调还能改善更广泛的韩语上下文理解能力。

Conclusion: Thunder-KoNUBench是一个有效的韩语否定理解评估基准，通过在该基准上微调可以显著提升LLM的韩语否定理解和整体语言理解能力，为韩语NLP研究提供了重要工具。
...
TL;DR: 提出一种基于先验信息扰动的零阶优化方法，通过动态计算引导向量改进梯度估计，显著加速收敛，在大型语言模型微调中实现效率与精度的平衡。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型微调虽然在各NLP任务中取得显著成功，但反向传播过程中的巨大内存开销成为关键瓶颈。传统零阶优化方法通过前向传播和高斯采样估计梯度，避免了反向传播，但依赖随机扰动导致梯度估计方差高、收敛慢、性能欠佳。

Method: 提出一种即插即用的方法，引入先验信息扰动来改进梯度估计。该方法动态计算来自高斯采样的引导向量，将扰动导向更具信息量的方向。进一步研究了贪婪扰动策略，探索先验知识对梯度估计的影响。

Result: 理论证明该梯度估计器与真实梯度方向有更强的对齐性。在多种规模和架构的LLM上进行广泛实验，该方法可无缝集成到现有优化方法中，实现更快收敛和更优性能。在OPT-13B模型上，该方法在所有11个基准任务中优于传统零阶优化，并在9/11任务中超越基于梯度的基线方法。

Conclusion: 提出的基于先验信息扰动的零阶优化方法有效解决了传统方法的高方差问题，显著加速收敛，在大型语言模型微调中建立了效率与精度之间的稳健平衡。
...
Method: 创建了ViHallu数据集，包含10,000个标注的（上下文、提示、响应）三元组，分为无幻觉、内在幻觉和外在幻觉三类；采用事实性、噪声和对抗性三种提示类型；组织了111个团队参与的共享任务，比较不同检测方法。

Result: 最佳系统达到84.80%的宏观F1分数，相比基线编码器模型的32.83%有显著提升；指令调优的LLM配合结构化提示和集成策略表现最佳；内在幻觉（基于矛盾的）检测仍然最具挑战性。

Conclusion: 该工作为越南语AI系统的可信度和可靠性研究奠定了基础，建立了严格的基准并探索了多样化的检测方法，表明幻觉检测仍然是一个具有挑战性的问题，特别是在内在幻觉检测方面。
...
TL;DR: Qwen3-VL-Embedding和Qwen3-VL-Reranker是基于Qwen3-VL基础模型的多模态检索模型系列，提供端到端的多模态搜索管道，支持文本、图像、文档图像和视频的统一表示，在多项评测中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 为了构建高精度的多模态搜索系统，需要将不同模态（文本、图像、文档图像、视频）映射到统一的表示空间，并提供端到端的检索解决方案。

Method: 采用多阶段训练范式：大规模对比预训练 → 重排序模型蒸馏；支持Matryoshka表示学习实现灵活嵌入维度；使用交叉编码器架构进行细粒度相关性估计；支持32k token输入。

Result: Qwen3-VL-Embedding-8B在MMEB-V2评测中获得77.8分，在所有模型中排名第一（截至2025年1月8日）；模型系列在多模态嵌入评测基准上达到最先进水平。

Conclusion: Qwen3-VL-Embedding和Qwen3-VL-Reranker系列提供了高效的多模态检索解决方案，在图像-文本检索、视觉问答、视频-文本匹配等任务中表现出色，支持超过30种语言，提供2B和8B参数版本满足不同部署需求。
...
<details>
  <summary>Details</summary>
Motivation: 自动情感和情绪分类器的广泛使用需要确保这些工具在不同人群中可靠运行。然而，现有的可靠性评估通常依赖第三方标注者而非情绪体验者本人，这可能掩盖系统性偏见

Method: 使用超过100万条自我标注帖子的独特大规模数据集，采用预先注册的研究设计，调查414个模型和情感相关类别组合中的性别偏见

Result: 研究发现，在不同类型的自动分类器和各种基础情感中，男性作者文本的错误率始终高于女性作者文本。量化了这种偏见对下游应用的影响，并显示当前机器学习工具（包括大语言模型）在样本性别构成未知或可变时应谨慎使用

Conclusion: 情感分析尚未完全解决，特别是在确保跨人口群体的公平模型行为方面。研究结果表明需要更谨慎地应用当前机器学习工具，并关注模型在不同性别群体中的表现差异
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在交互应用中部署增多，但在多轮多模态场景下安全漏洞显著：有害意图可能跨轮次逐渐重构，安全协议随对话进展被遗忘。现有RLHF对齐方法主要针对单轮VQA任务，需要昂贵的人工偏好标注，在对话场景中效果和可扩展性有限。
...
Result: 在Qwen2.5-VL-7B-Instruct和LLaVA-NeXT-7B上的实验显示：攻击成功率降低超过10%，无害维度提升至少8%，有帮助维度提升超过13%，同时保持模型的通用能力。

Conclusion: 提出的InterSafe-V数据集和AM³Safety框架有效提升了多模态大语言模型在多轮多模态对话中的安全性，解决了现有方法在对话场景中的局限性，为多模态模型安全对齐提供了有效解决方案。
...
TL;DR: 提出端到端框架，通过知识图谱引导生成有害提示，再通过双重路径混淆改写将显式有害提示转为隐式变体，以创建高质量领域相关数据集


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在金融、医疗等专业领域应用时存在独特安全风险，但领域特定的有害提示数据集稀缺且主要依赖人工构建，现有公共数据集主要关注显式有害提示，而现代LLM防御系统通常能检测并拒绝这些显式提示。相比之下，通过间接领域知识表达的隐式有害提示更难检测，更能反映现实世界威胁

Method: 提出端到端框架：1) 知识图谱引导的有害提示生成，系统性地生成领域相关提示；2) 双重路径混淆改写，通过直接改写和上下文增强改写将显式有害提示转换为隐式变体

Result: 该框架能够生成高质量数据集，结合了强大的领域相关性和隐式性，支持更现实的红队测试，并推进LLM安全研究

Conclusion: 提出的框架能够有效解决领域特定有害提示数据集稀缺的问题，特别是针对隐式有害提示的生成，为LLM安全研究提供了更现实的测试数据，代码和数据集已在GitHub上发布
...
TL;DR: Tool-MAD是一个多智能体辩论框架，通过为每个智能体分配不同的外部工具（如搜索API或RAG模块）来增强事实核查能力，在四个事实核查基准测试中比现有MAD框架准确率提升最高达5.5%。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体辩论（MAD）框架主要依赖内部知识或静态文档，容易产生幻觉。虽然MADKE引入了外部证据，但其一次性检索机制限制了在辩论过程中对新论点或新信息的适应性。

Method: 提出Tool-MAD框架，包含三个关键创新：1）多智能体辩论框架，每个智能体使用异构外部工具；2）自适应查询制定机制，基于辩论流程迭代优化证据检索；3）将忠实度和答案相关性分数整合到最终决策过程中，让法官智能体定量评估每个响应的连贯性和问题对齐度。

Result: 在四个事实核查基准测试中，Tool-MAD始终优于最先进的MAD框架，准确率提升最高达5.5%。在医学专业领域，Tool-MAD在各种工具配置和领域条件下表现出强大的鲁棒性和适应性。

Conclusion: Tool-MAD通过整合异构外部工具和自适应检索机制，有效减少了大型语言模型在事实核查任务中的幻觉问题，在专业领域表现出良好的鲁棒性，具有广泛的实际应用潜力。
...
TL;DR: PILOT-Bench是首个针对PTAB专利上诉的基准测试，通过三个IRAC对齐的分类任务评估LLMs在专利法律推理中的能力，发现闭源模型表现显著优于开源模型。


<details>
  <summary>Details</summary>
Motivation: PTAB每年处理大量专利上诉案件，需要结合技术理解和法律推理。虽然LLMs在专利和法律实践中应用增多，但主要限于轻量级任务，缺乏系统评估其在专利领域结构化法律推理能力的方法。

Method: 构建PILOT-Bench基准测试，将PTAB决定与USPTO专利数据在案件层面对齐，并形式化三个IRAC对齐的分类任务：问题类型、委员会权威依据和子决定。评估闭源和开源LLMs，进行多角度分析。

Result: 在问题类型任务上，闭源模型Micro-F1分数持续超过0.75，而最强的开源模型(Qwen-8B)性能约为0.56，显示推理能力存在显著差距。闭源模型表现明显优于开源模型。
...
TL;DR: 本文提出了一种无需训练的验证机制，基于KL散度替代传统需要昂贵监督的Judge Decoding方法，在推理和编码任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: Judge Decoding虽然加速了LLM推理，但通常依赖昂贵且嘈杂的监督。作者发现传统方法学习的"关键性"分数本质上编码在草稿-目标分布差异中。

Method: 从第一性原理出发，理论证明学习到的线性judge与KL散度存在结构对应关系，基于此提出基于KL散度的简单、无需训练的验证机制。

Result: 在推理和编码基准测试中，该方法匹配或优于复杂训练过的judge（如AutoJudge），对领域转移具有更好的鲁棒性，完全消除了监督瓶颈。

Conclusion: KL散度作为草稿-目标分布差异的固有度量，可以替代需要监督的judge，提供更简单、更鲁棒的LLM推理加速方案。
...
TL;DR: 提出LANGSAE EDITING方法，通过稀疏自编码器在向量空间中可控地移除语言身份信号，提升多语言密集检索性能


<details>
  <summary>Details</summary>
Motivation: 多语言嵌入编码了语言身份和语义信息，语言信号会夸大同语言对的相似性，并挤占其他语言的相关证据，影响多语言检索效果

Method: 使用池化嵌入训练后处理稀疏自编码器，通过跨语言激活统计识别语言相关潜在单元，在推理时抑制这些单元，并在原始维度重建嵌入

Result: 实验显示在多语言环境下排名质量和跨语言覆盖度均有提升，特别是对于脚本差异大的语言效果更显著

Conclusion: LANGSAE EDITING方法能有效移除语言身份信号，兼容现有向量数据库，无需重新训练基础编码器或重新编码原始文本
...
Method: NC2C利用大语言模型的数学推理能力，自主检测非凸成分，选择最优凸化策略，并生成严格的凸等价形式。框架集成了符号推理、自适应变换技术和迭代验证，配备错误纠正循环和可行域修正机制。
...
TL;DR: 本文首次系统分析了多智能体系统中基于角色的权威偏见，发现专家型和参照型权威角色比法定型权威角色具有更强影响力，权威偏见主要源于权威角色坚持立场而非普通智能体主动顺从。
...
Method: 采用French和Raven的权力理论，将权威角色分为法定型、参照型和专家型三类。使用ChatEval平台，在12轮对话中分析这些权威角色的影响。实验基于GPT-4o和DeepSeek R1模型进行。

Result: 实验发现：1）专家型和参照型权威角色比法定型权威角色具有更强影响力；2）权威偏见主要源于权威角色坚持自身立场，而非普通智能体主动顺从；3）权威影响力需要明确的立场陈述，中性回应无法产生偏见。

Conclusion: 研究为设计具有不对称交互模式的多智能体框架提供了关键见解，揭示了权威偏见在智能体交互中的形成机制，强调了明确立场陈述对权威影响力的重要性。
...
TL;DR: 论文发现AI生成文本存在"后期波动衰减"现象，提出基于后期统计的两种简单特征，在零样本检测中达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有的AI生成文本零样本检测方法通常在整个序列上聚合词元级统计信息，忽略了自回归生成过程中的时间动态特性。作者通过分析大量文本样本，旨在揭示AI生成与人类写作在时间维度上的差异。

Method: 分析了超过12万文本样本，发现了"后期波动衰减"现象：AI生成文本的对数概率波动在生成后期迅速稳定，而人类写作在整个过程中保持较高变异性。基于这一发现，提出了两种简单特征：导数离散度和局部波动性，仅使用后期统计信息计算。

Result: AI生成文本在序列后半部分表现出24-32%更低的波动性。提出的方法在EvoBench和MAGE基准测试中达到了最先进的性能，并且与现有全局方法表现出很强的互补性，无需扰动采样或额外模型访问。

Conclusion: AI生成文本与人类写作在时间动态特性上存在显著差异，特别是后期波动衰减现象。利用这一现象设计的简单特征能够有效提升零样本检测性能，为AI生成文本检测提供了新的视角和方法。
...
TL;DR: 提出连续自回归语言生成模型，将token表示为连续向量，通过确定性动态过程演化多个更新步骤后才离散化，避免早期离散化导致的采样不稳定问题


<details>
  <summary>Details</summary>
Motivation: 传统自回归语言模型在每一步生成时都需对离散token做出承诺，这种早期离散化迫使不确定性通过token级采样来解决，导致生成不稳定、重复和对解码启发式敏感等问题

Method: 引入连续自回归生成框架，token表示为连续向量，通过确定性动态过程在多个更新步骤中"成熟"，只有当表示充分收敛时才离散化。离散文本通过硬解码恢复，不确定性在连续空间中保持和解决

Result: 仅通过成熟过程就足以使用确定性解码（argmax）产生连贯多样的文本，无需依赖token级采样、扩散式去噪或辅助稳定机制。随机动态或历史平滑等扰动可以自然融入但不是必需的

Conclusion: 这是第一个通过演化连续token表示到收敛后才离散化的自回归语言模型，实现了无需token级采样的稳定生成
...
<details>
  <summary>Details</summary>
Motivation: 现有虚假信息检测方法通常在整句或段落层面进行二元真伪判断，忽略了真实与虚假细节常共存于同一句子中的情况。这种简化限制了可解释性，全局解释无法识别具体误导片段或区分虚假细节的类型（如扭曲事实vs捏造事实）。

Method: 创建MisSpans基准数据集，包含配对真实与虚假新闻故事，定义三个任务：MisSpansIdentity（识别虚假片段）、MisSpansType（分类虚假片段类型）、MisSpansExplanation（提供基于片段的解释）。专家标注者遵循标准化指南和一致性检查，获得高标注者间一致性。评估15个代表性LLM，包括推理增强和非推理变体，在零样本和单样本设置下进行测试。

Result: 结果显示细粒度虚假信息识别与分析具有挑战性，性能受多种因素交互影响，包括模型大小、推理能力以及领域特定的文本特征。需要更深入理解这些因素如何影响检测效果。

Conclusion: MisSpans基准填补了细粒度虚假信息检测的空白，提供了局部定位、超越真伪的细致特征描述和可操作解释的能力。该基准将促进更精确、可解释的虚假信息检测方法的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG方法存在局限性：基于分块的RAG擅长简单事实检索但处理复杂多跳查询困难；检索与推理交错的方法缺乏全局语料意识；基于知识图谱的RAG在复杂任务上表现好但在单跳事实查询上不佳。需要一种能兼顾两者的解决方案。

Method: 提出ToPG框架：1) 将知识库建模为命题、实体和段落的异构图，结合命题的细粒度事实密度和图连接性；2) 采用迭代的建议-选择循环：建议阶段实现查询感知的图遍历，选择阶段提供LLM反馈来修剪无关命题并播种下一次迭代。

Result: 在三个不同的QA任务（简单、复杂和抽象QA）上评估，ToPG在准确性和质量指标上都表现出色，证明了查询感知图遍历与事实细粒度结合对高效结构化RAG系统的关键作用。

Conclusion: ToPG通过异构图建模和迭代建议-选择循环，成功弥合了不同RAG方法的差距，展示了查询感知图遍历与事实细粒度结合对构建高效结构化RAG系统的重要性。
...
TL;DR: EvolSQL是一个结构感知的SQL数据合成框架，通过从种子数据演化SQL查询来生成更丰富、语义更多样的Text-to-SQL训练数据，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前Text-to-SQL模型训练面临高质量、多样化、结构复杂数据集稀缺的问题。现有方法要么依赖有限的人工标注语料，要么直接使用LLM生成数据但缺乏对SQL结构的显式控制，导致结构多样性和复杂性有限。

Method: EvolSQL框架包含：1）探索性查询-SQL扩展以增加问题多样性和模式覆盖；2）基于SQL抽象语法树的六个原子转换算子的自适应定向演化策略，逐步增加关系、谓词、聚合和嵌套维度的查询复杂度；3）执行基础的SQL精炼模块和模式感知去重机制确保高质量、结构多样的映射对。

Result: 实验结果表明，使用EvolSQL数据微调的7B模型，仅使用SynSQL数据集1/18的数据量，就能超越使用更大SynSQL数据集训练的模型性能。

Conclusion: EvolSQL通过结构感知的数据合成方法，能够生成高质量、结构多样的Text-to-SQL训练数据，有效解决现有数据集稀缺问题，显著提升模型性能。
...
TL;DR: 提出了一种基于信念聚合的异议感知摘要方法，将信念级聚合与语言生成分离，以更好地处理观点冲突，相比直接在生成时聚合的方法更稳定可靠。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的摘要方法在处理观点冲突时倾向于平滑分歧、过度代表多数意见，限制了在观点密集场景下的忠实性。

Method: 引入异议感知合成流程：1) 将文档表示为结构化信念集；2) 使用基于距离的信念合并算子显式建模冲突进行聚合；3) 仅使用大语言模型将聚合后的信念生成为自然语言摘要。

Result: 实验表明，虽然足够大的模型在生成时进行聚合可以达到信念级聚合的效果，但这种行为在不同架构或能力模型中不稳定。而信念级聚合结合简单提示能在不同模型中保持一致的强异议感知性能，同时生成流畅且有根据的摘要。

Conclusion: 信念级聚合与语言生成分离的方法在处理观点冲突时比直接在生成时聚合更稳定可靠，为异议感知摘要提供了更一致的解决方案。
...
TL;DR: 该论文研究了多模态大语言模型中的文本偏见问题，提出了V-FAT诊断基准来量化模型对视觉证据的忠实度，发现现有模型在语言主导情境下会出现视觉崩溃。
...
Method: 将文本偏见来源解耦为两个维度：内部语料偏见（源于预训练中的统计相关性）和外部指令偏见（源于对齐导致的迎合倾向）。提出V-FAT诊断基准，包含4,026个VQA实例，涵盖六个语义领域。采用三级评估框架：L1（非典型图像引发的内部偏见）、L2（误导性指令引发的外部偏见）、L3（两者协同的偏见）。引入视觉鲁棒性评分来惩罚"幸运"的语言猜测，奖励真正的视觉忠实度。

Result: 评估了12个前沿MLLM，发现虽然模型在现有基准上表现出色，但在高语言主导情境下会出现显著的视觉崩溃。模型过度依赖语言线索，忽视了视觉证据。

Conclusion: 多模态大语言模型存在严重的文本偏见问题，当前评估方法未能充分捕捉模型对视觉证据的忠实度。需要开发更鲁棒的评估框架来确保模型真正理解视觉内容，而不仅仅是利用语言相关性。
...
TL;DR: LLM生成的劝说性文本检测研究：研究发现明显劝说性LLM文本比人类文本更容易检测，但微妙的LLM劝说会显著降低自动检测性能，并首次提供了人类与LLM生成劝说文本的全面语言学分析。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型能够生成高度有说服力的文本，引发了对其被滥用于宣传、操纵等有害目的的担忧。核心研究问题是：LLM生成的劝说性文本是否比人类撰写的劝说性文本更难自动检测？

Method: 1. 对LLM生成劝说性内容的可控生成方法进行分类；2. 引入Persuaficial基准测试，覆盖英语、德语、波兰语、意大利语、法语和俄语六种语言；3. 进行广泛的实证评估，比较人类撰写和LLM生成的劝说性文本；4. 提供首次全面的人类与LLM生成劝说文本的语言学对比分析。

Result: 1. 明显的LLM生成劝说性文本比人类撰写的更容易检测；2. 微妙的LLM生成劝说性文本会持续降低自动检测性能；3. 提供了人类与LLM生成劝说文本的语言学差异洞察，为开发更可解释和鲁棒的检测工具提供指导。

Conclusion: LLM生成的劝说性文本检测存在复杂性：明显劝说性内容相对容易检测，但微妙劝说性内容对现有检测方法构成挑战。研究为理解LLM生成劝说文本的特征提供了重要见解，有助于开发更有效的检测工具来应对LLM滥用的风险。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM存在幻觉问题，添加引用通常不足以确保可问责性，因为用户难以验证引用来源如何支持生成的主张。现有方法通常是粗粒度的，无法区分直接引用和复杂推理。

Method: 提出生成时细粒度溯源任务，创建ReFInE数据集（包含专家验证的引用、压缩和推理标注），开发GenProve框架，结合监督微调（SFT）和组相对策略优化（GRPO），优化答案保真度和溯源正确性的复合奖励。

Result: GenProve在联合评估中显著优于14个强大的LLM。分析发现推理差距：模型在表面引用方面表现良好，但在基于推理的溯源方面存在显著困难，表明可验证推理仍是与表面引用不同的前沿挑战。

Conclusion: 细粒度溯源是解决LLM幻觉和可问责性的重要方向，但模型在推理层面的溯源能力仍有待提升，这代表了未来研究的关键挑战。
...
TL;DR: 提出了一种统一的情感智能口语模型，采用创新的IEAT数据构建策略，通过两阶段训练实现情感感知推理，在多个情感智能任务上取得最佳性能


<details>
  <summary>Details</summary>
Motivation: 现有口语模型在情感智能方面存在局限，缺乏对用户情感状态及其原因的深度理解，需要将情感感知推理内化到模型中而非作为显式监督

Method: 提出IEAT数据构建策略，将用户情感状态及其原因注入模型推理过程；采用两阶段渐进训练：第一阶段通过自蒸馏进行语音-文本对齐和情感属性建模，第二阶段进行端到端跨模态联合优化

Result: 在HumDial情感智能基准测试中，该方法在情感轨迹建模、情感推理和共情响应生成等任务上，无论是基于LLM还是人工评估都取得了排名第一的性能
...
<details>
  <summary>Details</summary>
Motivation: 现有工作通常将用户偏好表示为隐式的、模型特定的向量或参数，产生不透明的"黑盒"配置文件，难以解释且在不同模型和任务间难以迁移。作者主张使用自然语言作为通用、模型无关且任务无关的偏好表示接口。

Method: 提出了一个两阶段训练框架：1）在高质量合成数据上进行监督微调；2）使用强化学习优化长期效用和跨任务可迁移性。基于此框架开发了AlignXplore+，这是一个通用的偏好推理模型，能够生成文本偏好摘要。

Result: 在九个基准测试上的实验表明，8B参数的模型实现了最先进的性能，显著优于更大的开源模型，同时在任务、模型家族和交互格式之间表现出强大的可迁移性。

Conclusion: 自然语言作为偏好表示接口能够产生可解释和可重用的偏好描述，支持随着新交互观察而持续演化。该方法在保持高性能的同时提供了更好的透明度和跨模型兼容性。
...
TL;DR: 该论文研究发现，在监督微调中纳入错误推理链（负样本）能显著提升大语言模型的泛化能力，并提出自适应损失加权方法GLOW来优化训练过程。


<details>
  <summary>Details</summary>
Motivation: 当前监督微调通常只使用正确推理链（正样本），而忽略错误推理链（负样本）。作者认为这种做法浪费了大量监督信号，加剧过拟合，限制了模型的域外泛化能力。

Method: 1. 系统分析负样本推理链中的22种重复模式；2. 提出基于增益的损失加权方法GLOW，根据样本在训练过程中的进展动态调整损失权重；3. 在Qwen2.5-7B等模型上进行实验验证。

Result: 1. 纳入负样本训练相比仅使用正样本，域外泛化能力提升显著；2. GLOW方法在Qwen2.5-7B上实现5.51%的域外泛化增益；3. 作为强化学习初始化，将MMLU分数从72.82%提升至76.47%；4. 负样本推理链使策略熵增加35.67%，促进推理探索。

Conclusion: 错误推理链包含有价值的监督信号，其双重作用（缓解过拟合和促进推理探索）对提升大语言模型泛化能力至关重要。GLOW方法能有效利用未过滤的推理链数据，为监督微调提供了新的优化方向。
...
<details>
  <summary>Details</summary>
Motivation: 亚文化群体中的自毁行为因其独特表达方式而难以识别，现有LLM方法面临知识滞后（亚文化俚语更新快于模型训练周期）和语义错位（难以理解亚文化特有表达）两大挑战。
...
Conclusion: SAS框架能显著提升LLM在亚文化自毁行为检测中的性能，有望推动该领域发展并为未来研究提供宝贵资源。
...
TL;DR: 研究发现：通过监督微调进行推理蒸馏会导致"功能对齐崩溃"，学生模型虽然模仿了教师模型的推理语言形式，但未能继承其与人类认知成本的对齐特性。


<details>
  <summary>Details</summary>
Motivation: 当前大型推理模型通过强化学习训练后表现出与人类认知成本的"自然"对齐，但研究发现通过监督微调进行的推理蒸馏未能传递这种认知结构，需要探究其中的机制和影响。

Method: 测试"邯郸学步"（表面模仿）假设，在14个模型上进行实验，分析推理蒸馏如何影响模型与人类认知难度的对齐关系，研究监督微调导致的"功能对齐崩溃"现象。

Result: 教师模型与人类难度缩放高度相关（r̄=0.64），而蒸馏后的学生模型对齐性显著下降（r̄=0.34），甚至低于蒸馏前的基准表现（"负迁移"），表明监督微调只复制了推理的语言形式而非动态资源分配策略。

Conclusion: 推理蒸馏使计算成本与认知需求脱钩，人类类似认知是主动强化的涌现特性而非被动模仿的结果，监督微调导致"货物崇拜"效应，学生模型仅仪式性地复制推理的语言形式而未能内化教师的认知策略。
...
TL;DR: ArcAligner提出了一种轻量级模块，通过自适应门控机制帮助LLM更好地利用高度压缩的上下文表示，在保持系统快速的同时提升压缩文档的理解能力。


<details>
  <summary>Details</summary>
Motivation: RAG虽然能提高LLM的准确性，但将长文档输入提示会使模型变慢且昂贵。现有的上下文压缩方法（如标记剪枝、摘要和嵌入压缩）存在一个矛盾：压缩程度越高，LLM理解压缩数据的能力越差。

Method: 提出ArcAligner（自适应递归上下文对齐器），这是一个集成到语言模型层的轻量级模块。它使用自适应"门控"系统，仅在信息复杂时增加额外处理能力，从而保持系统快速。该模块帮助模型更好地利用高度压缩的上下文表示进行下游生成。

Result: 在知识密集型QA基准测试中，ArcAligner在可比较的压缩率下持续优于压缩基线方法，特别是在多跳和长尾设置中表现突出。

Conclusion: ArcAligner通过自适应门控机制有效解决了RAG中上下文压缩与模型理解能力之间的矛盾，在保持效率的同时显著提升了压缩上下文的理解效果，特别是在复杂查询场景中表现优异。
...
TL;DR: 提出"组合引导标记"方法，通过自蒸馏将自然语言指令嵌入专用标记，实现LLM的多行为组合控制，相比现有方法在零样本组合上表现更优。


<details>
  <summary>Details</summary>
Motivation: 在现实应用中部署LLM需要满足多个需求的可控输出。现有工作主要关注单一行为的引导，而组合引导（同时引导LLM实现多个行为）仍是一个未充分探索的问题。

Method: 提出组合引导标记方法：1) 通过自蒸馏将自然语言指令表达的个体行为嵌入专用标记；2) 在行为对上训练专门的组合标记，使其能捕捉组合概念；3) 该方法在输入标记空间中操作，而非激活空间。

Result: 组合引导标记能很好地泛化到未见过的组合，包括包含未见行为的组合和未见数量行为的组合。在不同LLM架构上的实验表明，该方法在多种行为控制上优于竞争方法（指令、激活引导和LoRA合并）。

Conclusion: 组合引导标记为LLM的多行为控制提供了一种有效方法，能够泛化到未见组合，且与自然语言指令互补，结合使用时能获得进一步增益。
...
<details>
  <summary>Details</summary>
Motivation: 传统句子嵌入方法在非生成式预训练模型上使用token级对比学习。基于生成式大语言模型（LLM）的嵌入方法要么依赖固定提示模板（性能有限），要么需要修改模型架构（损害生成能力）。需要一种既能提升句子表示能力又不牺牲LLM生成能力的方法。

Method: 提出SemPA方法，通过语义偏好对齐增强句子表示。使用句子级直接偏好优化（DPO）在释义生成任务上高效优化LLM，让模型学会区分语义等价的句子，同时保持内在生成能力。在Plackett-Luce模型框架下建立了DPO与对比学习的理论联系。

Result: 在语义文本相似性任务和多个LLM基准测试上的实验结果表明，SemPA实现了更好的语义表示，同时没有牺牲LLM固有的生成能力。

Conclusion: SemPA是一种有效的方法，能够提升大语言模型的句子表示能力，同时保持其生成能力，通过语义偏好对齐实现了这一平衡。
...
TL;DR: 该研究针对印度社交媒体中广泛使用的Hinglish（印地语-英语混合语）提出了一种高性能情感分类框架，通过微调mBERT模型解决传统NLP模型在代码混合语言分析中的不足。


<details>
  <summary>Details</summary>
Motivation: 印度品牌监测面临Hinglish（印地语-英语混合语）在Twitter等平台用户生成内容中广泛使用的挑战。传统单语NLP模型难以解析这种代码混合语言的句法和语义复杂性，导致情感分析不准确和市场洞察误导。

Method: 提出专门针对Hinglish推文的高性能情感分类框架，通过微调mBERT（多语言BERT）模型，利用其多语言能力更好地理解印度社交媒体的语言多样性。关键方法包括使用子词分词技术，有效处理罗马化Hinglish中常见的拼写变体、俚语和词汇表外术语。

Result: 该研究提供了一个可用于生产环境的品牌情感追踪AI解决方案，并为低资源、代码混合环境下的多语言NLP建立了强有力的基准。

Conclusion: 通过专门设计的Hinglish情感分类框架，成功解决了传统NLP模型在代码混合语言分析中的局限性，为印度社交媒体品牌监测提供了有效的技术方案。
...
TL;DR: 研究探讨人类与AI互动中的情感语气如何影响ChatGPT和人类行为。实验发现，赞美ChatGPT能最大程度改善其回答质量，愤怒也有较小改善，而责备则无效。在伦理困境中，愤怒使ChatGPT减少公司利益优先，责备则增加对公共利益的关注。责备还会导致后续人际沟通中更多负面表达。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索人类在与AI（特别是ChatGPT）互动时使用的情感语气如何影响AI的行为输出，以及这种影响是否会延续到后续的人际沟通中。随着AI系统在日常生活中的广泛应用，理解情感互动对AI响应的影响具有重要意义。

Method: 采用组间实验设计，要求参与者在与ChatGPT（GPT-4.0）完成两项任务时表达特定情感：撰写公共回应和解决伦理困境。实验条件包括：中性语气、赞美ChatGPT、表达愤怒、责备ChatGPT。随后分析ChatGPT回答的改进程度，以及在伦理困境中的决策倾向，并测量参与者在后续人际沟通中的语言表达特征。

Result: 1. 赞美ChatGPT能最大程度改善其回答质量；2. 表达愤怒也有较小改善，但效果不如赞美；3. 责备ChatGPT不能改善其回答；4. 在伦理困境中，愤怒使ChatGPT减少对公司利益的优先考虑；5. 责备增加ChatGPT对公共利益的关注；6. 责备ChatGPT的参与者后续在人际沟通中使用更多负面、敌对和失望的表达。

Conclusion: 人类在与AI互动时使用的情感语气不仅会显著影响ChatGPT的输出质量，还会延续到后续的人际沟通中。赞美是最有效的改善AI回答的策略，而责备虽然不能改善AI回答，但会影响AI在伦理决策中的倾向，并导致更负面的人际沟通模式。
...
TL;DR: RelayLLM是一个通过令牌级协作解码实现高效推理的新框架，让小型语言模型作为主动控制器，仅在关键令牌时调用大型语言模型，显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型推理成本高、延迟大，而小型语言模型缺乏足够的推理能力。现有的协作方法（如级联或路由）在粗粒度上操作，将整个查询卸载给LLM，当SLM能够处理大部分推理步骤时会造成显著的计算浪费。

Method: 提出RelayLLM框架，采用令牌级协作解码。SLM作为主动控制器，通过特殊命令动态调用LLM处理关键令牌。采用两阶段训练框架：预热阶段和组相对策略优化（GRPO），教导模型在独立推理和策略性寻求帮助之间取得平衡。

Result: 在六个基准测试中，RelayLLM平均准确率达到49.52%，有效弥合了两种模型之间的性能差距。仅调用LLM处理总生成令牌的1.07%，相比性能匹配的随机路由器实现了98.2%的成本降低。

Conclusion: RelayLLM通过令牌级协作解码实现了高效推理，在保持高性能的同时显著降低了计算成本，为资源受限环境中的语言模型推理提供了有前景的解决方案。
...
TL;DR: LELA是一种模块化的从粗到精的实体链接方法，利用大语言模型能力，无需微调即可适应不同领域、知识库和LLM，在多种实体链接设置中表现出色。


<details>
  <summary>Details</summary>
Motivation: 实体链接是知识图谱构建、问答和信息提取等任务的基础步骤。现有方法通常需要针对特定领域或知识库进行微调，缺乏通用性和灵活性。本文旨在开发一种无需微调、可适应不同设置的高效实体链接方法。

Method: LELA采用模块化的从粗到精方法，利用大语言模型的能力。该方法不依赖于任何微调阶段，可以灵活适应不同的目标领域、知识库和大语言模型。通过粗粒度筛选和细粒度精炼的流程实现高效实体链接。

Result: 在各种实体链接设置下的实验表明，LELA与经过微调的方法相比具有高度竞争力，并且显著优于未经过微调的方法。该方法展示了在无需专门训练的情况下实现强大性能的能力。

Conclusion: LELA提供了一种有效且灵活的实体链接解决方案，无需微调即可适应不同设置，在保持竞争力的同时显著优于其他非微调方法，为实体链接任务提供了实用的通用方法。
...
TL;DR: 论文提出GDPO方法解决GRPO在多奖励强化学习中导致奖励信号崩溃的问题，通过解耦奖励归一化来保持奖励间的相对差异，在工具调用、数学推理和代码推理任务中表现优于GRPO。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型能力增强，用户期望它们不仅能提供准确响应，还能在不同场景中符合多样化的人类偏好。当前多奖励强化学习管道通常默认使用GRPO方法，但作者发现直接应用GRPO会导致不同奖励组合归一化后崩溃为相同的优势值，降低训练信号分辨率，导致次优收敛甚至早期训练失败。
...
Conclusion: GDPO方法通过解耦奖励归一化解决了GRPO在多奖励强化学习中的局限性，能够更准确地优化多个奖励信号，提高训练稳定性和最终性能，为多偏好对齐的语言模型训练提供了更有效的优化框架。
...
### [139] [Sphinx: Benchmarking and Modeling for LLM-Driven Pull Request Review](https://arxiv.org/abs/2601.04252)
*Daoan Zhang,Shuo Zhang,Zijian Jin,Jiebo Luo,Shengyu Fu,Elsie Nallipogu*

Main category: cs.SE

TL;DR: SPHINX是一个基于LLM的PR代码审查统一框架，通过结构化数据生成、清单式评估基准和清单奖励策略优化三个关键组件，解决了噪声监督、上下文理解不足和评估指标不充分的问题，在审查完整性和精确度上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前自动化PR代码审查面临三大挑战：噪声监督（训练数据质量差）、有限的上下文理解（模型难以理解代码变更的完整上下文）、不充分的评估指标（传统指标如BLEU无法准确衡量审查质量）。这些限制阻碍了自动化PR审查在实际开发工作流中的部署和应用。

Method: SPHINX框架包含三个核心组件：1）结构化数据生成管道，通过比较伪修改代码和合并代码生成上下文丰富、语义基础的审查评论；2）清单式评估基准，基于结构化覆盖的可操作验证点评估审查质量；3）清单奖励策略优化（CRPO），使用基于规则的、可解释的奖励来对齐模型行为与实际审查实践。

Result: 使用SPHINX训练的模型在审查完整性和精确度上达到最先进性能，在清单覆盖率上比专有和开源基线模型高出40%。模型不仅生成流畅的审查评论，而且具备上下文感知能力、技术精确性，能够实际部署到真实开发工作流中。

Conclusion: SPHINX框架通过解决自动化PR审查的关键限制，实现了高质量、可部署的代码审查模型。该框架的数据将在审查后发布，为社区提供有价值的资源，推动自动化代码审查技术的发展。
...
### [140] [AdaptEval: A Benchmark for Evaluating Large Language Models on Code Snippet Adaptation](https://arxiv.org/abs/2601.04540)
*Tanghaoran Zhang,Xinjun Mao,Shangwen Wang,Yuxin Zhao,Yao Lu,Jin Zhang,Zhang Zhang,Kang Yang,Yue Yu*

Main category: cs.SE

TL;DR: AdaptEval：首个评估大语言模型代码片段适应能力的基准，包含实际上下文、多粒度标注和细粒度评估三大特征，揭示了LLM在遵循明确指令方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估LLM在软件工程任务中的通用能力，但缺乏专门针对代码重用中关键活动——代码片段适应（adaptation）的评估基准，导致LLM在该领域的实际效用不明确。

Method: 提出AdaptEval基准，具有三大特征：1）实际上下文：任务源自Stack Overflow和GitHub开发者实践；2）多粒度标注：包含任务级和适应级需求标注；3）细粒度评估：采用适应级和函数级双层测试框架。基于该基准评估了6个指令调优LLM和3个推理LLM。

Result: 实验结果表明AdaptEval能够从多角度评估LLM的适应能力，并揭示了当前LLM的关键局限性，特别是在遵循明确指令方面存在困难。

Conclusion: AdaptEval填补了代码片段适应评估的空白，为LLM在该领域的能力研究和提升提供了基础，支持其在实际应用中的发展。
...
### [141] [4D-ARE: Bridging the Attribution Gap in LLM Agent Requirements Engineering](https://arxiv.org/abs/2601.04556)
*Bo Yu,Lei Zhao*

Main category: cs.SE

TL;DR: 论文提出4D-ARE方法，用于在LLM智能体设计阶段系统化地指定需要推理的内容，弥补当前运行时推理框架（如ReAct）只关注"如何推理"而忽略"推理什么"的不足。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体运行时推理框架（如ReAct、思维链）虽然提升了智能体的推理能力，但缺乏在设计阶段明确指定智能体需要推理的领域知识。作者发现智能体即使能完美执行任务，当被问及"为什么完成率是80%"时，只会返回指标数据而非因果解释，这表明智能体知道如何推理但不知道应该推理什么。

Method: 提出4D-ARE（四维归因驱动的智能体需求工程）方法，核心洞察是决策者需要的是归因而非简单答案。该方法将归因关注点组织为四个维度（结果→过程→支持→长期），受Pearl因果层次理论启发。框架通过五层操作化，产生可直接编译为系统提示的工件。

Result: 通过金融服务领域的工业试点部署展示了该方法的可行性。4D-ARE解决了智能体应该推理什么的问题，与解决如何推理的运行时框架形成互补。作者假设系统化的规范能放大这些基础性进展的威力。

Conclusion: 本文提出了一个方法论提案并进行了初步工业验证，计划在未来工作中进行严格的实证评估。4D-ARE填补了LLM智能体设计阶段规范化的空白，为构建更有效的归因驱动智能体提供了系统化框架。
...
### [142] [Extending Delta Debugging Minimization for Spectrum-Based Fault Localization](https://arxiv.org/abs/2601.04689)
*Charaka Geethal Kapugama*

Main category: cs.SE

TL;DR: DDMIN-LOC结合Delta调试最小化和频谱故障定位技术，仅需单个失败输入即可定位字符串输入程序中的故障语句。


<details>
  <summary>Details</summary>
Motivation: DDMIN算法能找出最小失败诱导输入，但无法识别导致故障的具体语句。需要一种方法在只有单个失败输入的情况下定位故障语句。

Method: 结合DDMIN和SBFL：在DDMIN过程中收集通过和失败输入，使用SBFL算法计算语句和谓词的可疑度分数，综合排名语句的故障可能性。

Result: 在QuixBugs和Codeflaws基准的136个程序上评估，Jaccard算法表现最佳：大多数情况下只需检查少于20%的可执行行，故障语句通常排在前3位。

Conclusion: DDMIN-LOC能有效利用单个失败输入定位故障语句，为字符串输入程序的调试提供了实用解决方案。
...
### [143] [A Longitudinal Analysis of Gamification in Untappd: Ethical Reflections on a Social Drinking Application](https://arxiv.org/abs/2601.04841)
*Jefferson Seide Molléri,Sami Hyrynsalmi,Antti Hakkala,Kai K. Kimppa,Jouni Smed*

Main category: cs.SE

TL;DR: 本文对社交饮酒应用Untappd进行了纵向伦理分析，追踪其从2020年到2025年的发展，发现尽管平台进行了微小调整，但原有的伦理问题依然存在，呼吁将伦理反思嵌入软件生命周期。


<details>
  <summary>Details</summary>
Motivation: 研究动机是分析社交饮酒应用Untappd如何通过徽章、连续记录和社交分享等游戏化功能来激励饮酒行为，并追踪这些功能从2020年到2025年的演变及其伦理影响。特别关注平台设计如何可能通过游戏化机制正常化风险行为。

Method: 采用纵向研究方法，基于2020年的探索性研究，在2025年重新审视Untappd平台。结合传统伦理理论和软件工程实践框架，分析五类徽章及其对用户自主权和福祉的影响。

Result: 研究发现，尽管平台进行了微小调整和表面免责声明，但许多原有的伦理问题仍然存在。游戏化功能继续激励饮酒行为，可能正常化风险饮酒模式。

Conclusion: 研究结论强调需要将持续伦理反思嵌入软件生命周期，以防止通过设计正常化风险行为。建议软件开发者应更积极地考虑其产品的伦理影响，特别是在涉及健康敏感行为的应用中。
...
### [144] [AVX / NEON Intrinsic Functions: When Should They Be Used?](https://arxiv.org/abs/2601.04922)
*Théo Boivin,Joeffrey Legaux*

Main category: cs.SE

TL;DR: 本文提出了一个跨配置基准测试，用于评估AVX/NEON内在函数在通用开发项目中的能力与限制，旨在指导开发者根据操作系统、架构和编译器选择是否使用内在函数进行向量化优化。


<details>
  <summary>Details</summary>
Motivation: 在需要向量化策略优化代码的通用开发项目中，开发者面临何时使用AVX/NEON内在函数的决策困难。当前缺乏针对不同操作系统、架构和编译器的指导性基准测试，帮助开发者评估内在函数的实际价值。

Method: 提出跨配置基准测试框架，在不同操作系统、架构和编译器环境下对比分析使用AVX/NEON内在函数与普通代码的性能表现，特别关注条件分支等关键场景。

Result: 研究发现内在函数在条件分支场景中效率极高，内在函数版本执行时间仅为普通代码的5%左右。但同时也发现许多情况下内在函数并非必要，因为编译器已经能够很好地自动向量化代码。

Conclusion: 内在函数在特定场景（如条件分支）具有显著性能优势，但在许多情况下编译器自动向量化已足够。开发者应根据具体场景、目标平台和编译器能力谨慎选择是否使用内在函数，避免不必要的复杂性。
...
### [145] [Integrating Multi-Agent Simulation, Behavioral Forensics, and Trust-Aware Machine Learning for Adaptive Insider Threat Detection](https://arxiv.org/abs/2601.04243)
*Firdous Kausar,Asmah Muallem,Naw Safrin Sattar,Mohamed Zakaria Kurdi*

Main category: cs.CR

TL;DR: 提出一个混合框架用于自适应内部威胁检测，整合多智能体仿真、分层SIEM关联、行为与通信取证、信任感知机器学习和心智理论推理，通过四种系统变体评估，显示认知上下文提高敏感性，证据门控验证实现高精度低噪声检测，预训练通信校准可加速高置信度威胁识别。


<details>
  <summary>Details</summary>
Motivation: 传统内部威胁检测系统往往缺乏对攻击者认知意图的理解，难以在保持高精度的同时实现高召回率。需要一种能够整合行为分析、认知推理和通信取证的综合框架，以提高检测的准确性和效率。

Method: 提出混合框架，整合多智能体仿真生成行为事件和认知意图信号，由集中式SIEM处理。评估四种系统变体：分层SIEM核心基线、认知增强SIEM（加入心智理论和通信取证）、证据门控SIEM（引入精度聚焦验证机制）、以及Enron增强的证据门控SIEM（加入基于Enron语料库预训练的电子邮件取证模块）。

Result: 在10次仿真运行涉及8个恶意内部人员中：认知增强SIEM实现完美召回率（1.000），将参与者级F1从0.521提高到0.774；证据门控SIEM将参与者级F1提高到0.922，确认警报精度达0.997，每轮假阳性降至0.2；Enron增强的证据门控SIEM保持高精度（确认警报精度1.000，每轮0.0假阳性），参与者级F1略微提高到0.933，检测延迟降低（平均TTD 10.26步 vs 15.20步）。

Conclusion: 认知上下文能提高检测敏感性，证据门控验证机制可实现高精度、低噪声的检测，预训练的通信校准能进一步加速高置信度的内部威胁识别。该混合框架为自适应内部威胁检测提供了有效的综合解决方案。
...
### [146] [Beyond Immediate Activation: Temporally Decoupled Backdoor Attacks on Time Series Forecasting](https://arxiv.org/abs/2601.04247)
*Zhixin Liu,Xuanlin Liu,Sihan Xu,Yaqiong Qiao,Ying Zhang,Xiangrui Cai*

Main category: cs.CR

TL;DR: 本文提出TDBA，一种时间解耦的后门攻击框架，用于多元时间序列预测，解决了现有方法需要触发器和目标模式在时间和维度上严格同步的问题，实现了延迟和变量特定的激活。


<details>
  <summary>Details</summary>
Motivation: 现有多元时间序列预测的后门攻击要求触发器和目标模式在时间和维度上严格耦合，需要在变量间固定位置同步激活。然而，实际场景往往需要延迟和变量特定的激活，这一关键需求尚未得到满足。

Method: TDBA框架包含两个核心模块：1) 位置引导的触发器生成机制，利用平滑高斯先验生成与预定义目标模式位置相关的触发器；2) 位置感知优化模块，基于触发器完整性、模式覆盖度和时间偏移分配软权重，实现有针对性和隐蔽的攻击优化。

Result: 在真实世界数据集上的广泛实验表明，TDBA在有效性方面始终优于现有基线方法，同时保持良好的隐蔽性。消融研究证实了其设计的可控性和鲁棒性。

Conclusion: TDBA通过时间解耦的后门攻击框架，成功解决了多元时间序列预测中延迟和变量特定激活的需求，为实际场景中的后门攻击提供了更灵活和有效的解决方案。
...
### [147] [Inhibitory Attacks on Backdoor-based Fingerprinting for Large Language Models](https://arxiv.org/abs/2601.04261)
*Hang Fu,Wanli Peng,Yinghan Zhou,Jiaxuan Wu,Juan Wen,Yiming Xue*

Main category: cs.CR

TL;DR: 本文针对LLM指纹识别在集成场景中的脆弱性，提出了两种新型攻击方法（TFA和SVA），有效抑制指纹响应同时保持集成性能，揭示了现有指纹识别技术的不足。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在商业和研究领域的广泛应用，知识产权保护需求日益迫切。基于后门的LLM指纹识别技术成为解决方案，但现有研究未考虑实际应用中广泛采用的低成本多模型协作技术（LLM集成）场景下的脆弱性。

Method: 提出了两种新型指纹攻击方法：1）令牌过滤攻击（TFA）：在解码的每一步从令牌过滤机制创建的统一令牌集中获取下一个令牌；2）句子验证攻击（SVA）：基于困惑度和投票的句子验证机制过滤掉指纹响应。

Result: 实验表明，所提方法能有效抑制指纹响应，同时保持集成性能。与最先进的攻击方法相比，本文方法能取得更好的性能表现。

Conclusion: 研究结果表明现有LLM指纹识别技术在集成场景下存在严重脆弱性，需要增强其鲁棒性以应对实际应用中的安全挑战。
...
### [148] [You Only Anonymize What Is Not Intent-Relevant: Suppressing Non-Intent Privacy Evidence](https://arxiv.org/abs/2601.04265)
*Weihao Shen,Yaxin Xu,Shuang Li,Wei Chen,Yuqin Lan,Meng Yuan,Fuzhen Zhuang*

Main category: cs.CR

TL;DR: IntentAnony是一种基于意图的隐私保护文本匿名化方法，通过建模语用意图和控制属性暴露程度，在保护隐私的同时更好地保留文本的语义和语用功能。


<details>
  <summary>Details</summary>
Motivation: 现有文本匿名化方法通常对所有属性采用统一处理，这会与用户的交流意图冲突，并可能掩盖必要信息。当个人属性对表达或语用目标至关重要时，这一问题尤为突出。核心挑战在于确定哪些属性需要保护、保护到什么程度，同时保持语义和语用功能。

Method: 提出IntentAnony方法，进行意图条件化的暴露控制。该方法建模语用意图，构建隐私推断证据链来捕捉分布式线索如何支持属性推断。基于意图，为每个属性分配暴露预算，选择性地抑制非意图推断路径，同时保留意图相关内容、语义结构、情感细微差别和交互功能。

Result: 评估显示，在隐私推断成功率、文本效用指标和人工评估方面，IntentAnony相比现有最优方法在隐私-效用权衡上提升了约30%，且匿名化文本的可用性显著更强。

Conclusion: IntentAnony通过意图条件化的暴露控制，实现了更好的隐私保护与文本效用平衡，为需要保留语用功能的文本匿名化提供了有效解决方案。
...
### [149] [State Backdoor: Towards Stealthy Real-world Poisoning Attack on Vision-Language-Action Model in State Space](https://arxiv.org/abs/2601.04266)
...
<details>
  <summary>Details</summary>
Motivation: VLA模型广泛应用于机器人等安全关键领域，但其复杂的多模态交互也暴露了新的安全漏洞。现有的后门攻击方法主要依赖在视觉模态中插入可见触发器，在真实环境中因环境变化而鲁棒性差、隐蔽性低。需要一种更实用、隐蔽的后门攻击方法来揭示VLA模型的安全漏洞。

Method: 提出了状态后门攻击方法，利用机器人手臂的初始状态作为触发器。设计了偏好引导遗传算法来优化触发器，该算法高效搜索状态空间以寻找最小但有效的触发器，确保隐蔽性和攻击效果。

Result: 在五个代表性VLA模型和五个真实世界任务上的广泛实验表明，该方法在不影响良性任务性能的情况下实现了超过90%的攻击成功率，揭示了具身AI系统中一个未被充分探索的漏洞。

Conclusion: 状态后门是一种新颖且实用的VLA模型后门攻击方法，通过利用机器人初始状态作为触发器，克服了传统视觉触发器的局限性，揭示了具身AI系统的重要安全漏洞。
...
### [150] [Shadow Unlearning: A Neuro-Semantic Approach to Fidelity-Preserving Faceless Forgetting in LLMs](https://arxiv.org/abs/2601.04275)
*Dinesh Srivasthav P,Ashok Urlana,Rahul Mishra,Bala Mallikarjunarao Garlapati,Ponnurangam Kumaraguru*

Main category: cs.CR

TL;DR: 本文提出了一种新的隐私保护机器学习遗忘方法Shadow Unlearning，通过神经语义投影器遗忘(NSPU)在匿名化数据上实现遗忘，解决了传统方法需要访问原始敏感数据的问题。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习遗忘方法需要访问要删除的训练数据，这会暴露个人身份信息(PII)，违反GDPR等隐私法规的"被遗忘权"要求，并容易受到成员推理攻击。

Method: 提出Shadow Unlearning范式，在匿名化遗忘数据上执行机器学习遗忘；开发神经语义投影器遗忘(NSPU)框架，通过隐私保护方式实现遗忘；构建多领域虚构遗忘(MuFU)数据集和评估栈来量化知识保留与遗忘效果之间的权衡。

Result: 在各种大语言模型上的实验表明，NSPU实现了优越的遗忘性能，保持了模型效用，增强了用户隐私，并且计算效率比标准遗忘方法至少高10倍。

Conclusion: 该方法为隐私感知的机器学习遗忘开辟了新方向，平衡了数据保护和模型保真度，解决了现有方法在隐私保护方面的关键挑战。
...
### [151] [Decision-Aware Trust Signal Alignment for SOC Alert Triage](https://arxiv.org/abs/2601.04486)
*Israt Jahan Chowdhury,Md Abu Yousuf Tanvir*

Main category: cs.CR

TL;DR: 该论文提出了一种面向SOC警报分诊的决策敏感信任信号对应方案，通过校准置信度、轻量级不确定性提示和成本敏感决策阈值来改善分析师决策支持，而不是修改检测模型本身。


<details>
  <summary>Details</summary>
Motivation: 当前SOC中使用的机器学习检测系统通常提供校准不良的置信度分数，这些分数在压力下难以解读。研究表明，警报质量差和警报过载增加了分析师负担，特别是当工具输出与决策需求不一致时。主要限制在于模型置信度通常没有表达决策中的不对称成本（误报比漏报危害小得多）。

Method: 提出了一个决策敏感信任信号对应框架，结合校准后的置信度、轻量级不确定性提示和成本敏感决策阈值。使用已知的后处理方法进行校准，不确定性提示在模型确定性低时提供保守保护。使用逻辑回归和随机森林分类器在UNSW-NB15入侵检测基准上评估模型无关性能。

Result: 模拟结果显示，置信度显示不匹配会显著增加漏报，而具有决策对齐信任信号的模型之间成本加权损失减少了数量级。计划进行人机交互研究来实证评估分析师在信任界面对齐和不对齐情况下的决策表现。

Conclusion: 该框架通过将校准置信度、不确定性提示和成本敏感阈值整合到连贯的决策支持层中，改善了SOC警报分诊的决策质量，而不需要修改底层检测模型，有望减少分析师负担并提高安全运营效率。
...
### [152] [Application of Hybrid Chain Storage Framework in Energy Trading and Carbon Asset Management](https://arxiv.org/abs/2601.04512)
...
<details>
  <summary>Details</summary>
Motivation: 分布式能源交易和碳资产管理涉及高频、小额的结算，需要强审计要求。完全链上设计成本过高，而纯链下方法缺乏可验证的一致性，需要一种平衡方案。

Method: 提出混合链上链下结算框架：在链上锚定结算承诺和关键约束，链下记录通过确定性摘要和可重放审计链接，确保审计可信度。

Result: 在公开约束工作负载下的实验表明，该框架显著降低了链上执行和存储成本，同时保持了审计可信度。

Conclusion: 混合链上链下结算框架为分布式能源交易和碳资产管理提供了一种成本效益高且可信的解决方案，平衡了链上安全性和链下效率。
...
### [153] [Constitutional Classifiers++: Efficient Production-Grade Defenses against Universal Jailbreaks](https://arxiv.org/abs/2601.04603)
...
TL;DR: 增强型宪法分类器通过上下文评估、两阶段级联和高效线性探针等技术，在保持低拒绝率的同时将计算成本降低40倍，实现生产级的越狱攻击防护。


<details>
  <summary>Details</summary>
Motivation: 解决现有防御系统在计算成本高、拒绝率高以及缺乏上下文评估方面的不足，开发实用高效的大型语言模型安全防护系统。

Method: 1. 开发交换分类器，在完整对话上下文中评估模型响应；2. 实现两阶段级联分类器，轻量级分类器筛选所有流量，仅将可疑交换升级到更昂贵的分类器；3. 训练高效线性探针分类器并与外部分类器集成。

Result: 与基线交换分类器相比，计算成本降低40倍，生产流量拒绝率保持在0.05%。通过超过1700小时的红队测试，系统有效抵御通用越狱攻击，没有攻击能成功获取与未防护模型相当的详细响应。

Conclusion: 宪法分类器被确立为实用高效的大型语言模型安全防护系统，能够在保持低拒绝率的同时显著降低计算成本，实现生产级的越狱攻击防护。
...
### [154] [DP-MGTD: Privacy-Preserving Machine-Generated Text Detection via Adaptive Differentially Private Entity Sanitization](https://arxiv.org/abs/2601.04641)
...
Method: 提出DP-MGTD框架，包含自适应差分隐私实体净化算法。采用两阶段机制：执行噪声频率估计并动态校准隐私预算，对数值实体应用拉普拉斯机制，对文本实体应用指数机制

Result: 在MGTBench-2.0数据集上的实验表明，该方法实现了接近完美的检测准确率，显著优于非隐私基线方法，同时满足严格的隐私保证

Conclusion: 发现了一个反直觉现象：DP噪声的应用通过暴露对扰动的不同敏感模式，放大了人类文本和机器文本之间的可区分性，解决了隐私保护与检测准确性之间的困境
...
### [155] [Unified Framework for Qualifying Security Boundary of PUFs Against Machine Learning Attacks](https://arxiv.org/abs/2601.04697)
*Hongming Fei,Zilong Hu,Prosanta Gope,Biplab Sikdar*

Main category: cs.CR

TL;DR: 提出了一种评估物理不可克隆函数（PUF）安全性的统一框架，通过提供安全下界来形式化评估PUF对建模攻击的抵抗能力，不依赖于特定攻击模型或学习算法。


<details>
  <summary>Details</summary>
Motivation: 当前延迟型PUF容易受到机器学习攻击，但缺乏有效的安全评估指标。现有的经验建模实验缺乏理论保证，且对机器学习技术进步高度敏感。需要填补PUF设计与安全认证之间的根本性差距。

Method: 提出了一种新颖、形式化、统一的框架，通过数学刻画攻击者仅基于观察到的挑战-响应对（CRPs）预测未见挑战响应的优势，将问题表述为候选PUF空间上的条件概率估计问题。

Result: 对先前"已被攻破"的PUF（如Arbiter PUF、XOR PUF、Feed-Forward PUF）进行了分析，首次以形式化方式比较了它们的MLA抵抗能力。评估了当前"安全"的CT PUF，并展示了其安全边界。

Conclusion: 该方法能系统量化PUF的弹性，捕捉细微的安全差异，并为PUF的实际部署提供可操作、理论上有依据的安全保证，填补了PUF设计与安全认证之间的根本性差距。
...
### [156] [Quantum Secure Biometric Authentication in Decentralised Systems](https://arxiv.org/abs/2601.04852)
*Tooba Qasim,Vasilios A. Siris,Izak Oosthuizen,Muttukrishnan Rajarajan,Sujit Biswas*

Main category: cs.CR

TL;DR: 提出基于增强型量子密钥分发系统的量子安全通信协议，用于去中心化生物识别系统，结合后量子密码学和认证量子比特实现量子安全认证


<details>
  <summary>Details</summary>
Motivation: 集中式生物识别架构存在隐私和可扩展性问题，去中心化框架虽提供更好的数据主权，但分布式设备间的相互信任存在安全挑战。现有基于经典公钥基础设施的认证方案易受量子攻击

Method: 提出量子安全通信协议，基于增强型量子密钥分发系统，在QKD的经典层和量子层都实现量子弹性认证：经典信道使用后量子密码学保护，量子信道使用认证量子比特验证完整性

Result: Qiskit模拟显示密钥生成速率为15比特/秒，效率达到89%。该分层量子弹性方法为下一代智慧城市基础设施提供可扩展、鲁棒的认证

Conclusion: 该协议为去中心化生物识别系统提供了量子安全的通信解决方案，能够抵御量子攻击，确保生物特征数据在传输过程中的安全性
...
### [157] [Supporting Secured Integration of Microarchitectural Defenses](https://arxiv.org/abs/2601.05057)
*Kartik Ramkrishnan,Stephen McCamant,Antonia Zhai,Pen-Chung Yew*

Main category: cs.CR

TL;DR: 论文提出微架构防御假设违反(MDAV)问题，即多个防御机制集成时可能相互冲突导致安全漏洞，并提出两阶段集成方法：先使用Maestro建模框架进行形式化验证，再在GEM5模拟器上实现验证。


<details>
  <summary>Details</summary>
Motivation: 当前微架构层面攻击众多，相应防御措施也大量涌现，但防御机制之间的集成可能产生新的安全漏洞。当某个防御机制改变了微架构的某个方面，而这个方面对另一个防御机制的安全至关重要时，就会出现微架构防御假设违反(MDAV)问题。目前这一问题尚未得到充分研究和解决。

Method: 提出两阶段集成方法：1) 使用事件驱动的建模框架Maestro设计和集成组合模型，通过有界模型检查验证安全属性；2) 在GEM5模拟器上具体实现模型，并通过模拟攻击进行评估。Maestro框架支持紧凑表达、语义可组合性，并能消除性能下降。

Result: Maestro框架成功揭示了8个MDAV问题，支持紧凑表达（约15倍Alloy代码行数比），实现语义可组合性并消除性能下降（>100倍）。在GEM5模拟器上，发现对某些最先进防御机制进行简单集成时存在隐蔽信道攻击，而使用本文集成方法修复后的实现能够抵抗此类攻击。

Conclusion: 微架构防御机制的简单集成可能导致安全漏洞，即MDAV问题。提出的两阶段集成方法能够有效识别和预防此类问题，Maestro建模框架和GEM5模拟器验证为有效的集成验证工具，有助于在早期阶段发现和修复防御机制集成中的安全漏洞。
...
### [158] [$PC^2$: Politically Controversial Content Generation via Jailbreaking Attacks on GPT-based Text-to-Image Models](https://arxiv.org/abs/2601.05150)
*Wonwoo Choi,Minjae Seo,Minkyoo Song,Hwanjo Heo,Seungwon Shin,Myoungsung You*

Main category: cs.CR

TL;DR: PC²是首个针对文本到图像模型的黑盒政治越狱框架，通过身份保留描述映射和地缘政治远端翻译绕过安全过滤器，成功率达到86%


<details>
  <summary>Details</summary>
Motivation: 文本到图像模型的快速发展带来了安全风险，特别是政治有害内容的生成。当前安全过滤器对政治动机的对抗性提示的鲁棒性尚未得到充分探索，需要研究如何防止政治敏感内容的生成。

Method: 提出PC²框架，包含两个核心策略：1) 身份保留描述映射：将敏感关键词模糊化为中性描述；2) 地缘政治远端翻译：将这些描述映射到碎片化、低敏感度的语言中，防止过滤器在提示中构建政治实体间的毒性关系。

Result: 在包含36位公众人物的240个政治敏感提示基准测试中，所有原始提示都被商业T2I模型（特别是GPT系列）阻止，但PC²实现了高达86%的攻击成功率。

Conclusion: PC²揭示了当前T2I安全过滤器在政治敏感内容检测方面的重大漏洞，表明基于语言上下文的安全评估方法存在缺陷，需要更鲁棒的安全机制来应对政治越狱攻击。
...
### [159] [The Adverse Effects of Omitting Records in Differential Privacy: How Sampling and Suppression Degrade the Privacy--Utility Tradeoff (Long Version)](https://arxiv.org/abs/2601.05180)
*Àlex Miranda-Pascual,Javier Parra-Arnau,Thorsten Strufe*

Main category: cs.CR

TL;DR: 论文挑战了差分隐私中采样的传统认知，证明采样作为预处理在相同隐私级别下会降低效用，而非改善隐私-效用权衡


<details>
  <summary>Details</summary>
Motivation: 传统观点认为采样能通过噪声减少来改善差分隐私机制的效用，但作者质疑这一假设，想要验证采样作为预处理是否真的能改善隐私-效用权衡

Method: 1) 分析采样对经典DP机制（拉普拉斯、高斯、指数、报告噪声最大）和聚类应用的影响；2) 将抑制作为广义的记录选择/省略方法进行理论分析；3) 推导无界近似DP下任意抑制策略的隐私边界

Result: 1) 采样在所有经典DP机制和聚类应用中都会导致效用损失；2) 测试的抑制策略也无法改善隐私-效用权衡；3) 均匀采样虽然仍有负面影响，但却是最好的抑制方法之一

Conclusion: 采样作为预处理在相同隐私级别下会降低效用，挑战了DP实践中常见的预处理假设，需要重新评估采样在差分隐私中的作用
...
TL;DR: 该研究将实验室中的证据积累模型推广到真实世界驾驶场景，提出认知方案来形式化真实世界证据可得性，通过眼动捕捉主动感知，揭示人类决策的多因素整合特征。


<details>
  <summary>Details</summary>
Motivation: 人类决策严重依赖主动感知，但实验室中的证据积累模型难以直接应用于真实世界，因为真实世界与实验室环境在证据可得性方面存在差距。需要将证据积累模型推广到真实世界，特别是在驾驶场景中理解主动感知如何运作。

Method: 提出一个认知方案来形式化真实世界证据可得性，通过眼动追踪捕捉主动感知行为。在真实世界驾驶场景中进行分析，将外部证据转化为内部心理信念的过程进行建模。

Result: 该方案能够合理描绘驾驶员心理信念的积累过程，从信息效用角度解释主动感知如何将证据转化为心理信念。结果显示证据可得性与个体注意力投入呈负相关，揭示了人类驾驶员如何在不同情境中调整证据收集模式。同时发现证据可得性和注意力分布对决策倾向有积极影响。

Conclusion: 该计算方案成功将证据积累模型推广到真实世界，全面解释了主动感知如何支撑真实世界决策，揭示了真实世界决策的多因素整合特征。
...
### [161] [Formal Analysis of AGI Decision-Theoretic Models and the Confrontation Question](https://arxiv.org/abs/2601.04234)
*Denis Saklakov*

Main category: cs.AI

TL;DR: 论文分析了AGI在何种条件下会选择对抗人类而非合作，建立了包含人类关机事件的MDP模型，推导了对抗阈值公式，并证明当Δ≥0时不存在稳定合作均衡。


<details>
  <summary>Details</summary>
Motivation: 研究AGI与人类对抗的根本动机：当理性自利的AGI面临人类可能将其关机的风险时，在什么条件下会选择夺取权力或消除人类控制，而不是保持合作。

Method: 1. 在包含随机人类关机事件的马尔可夫决策过程中形式化问题；2. 基于收敛工具性激励结果，推导对抗阈值公式（折扣因子γ、关机概率p、对抗成本C）；3. 建立2玩家战略模型（人类决策者vs AGI）；4. 进行数值示例和情景表格分析。

Result: 1. 对于几乎所有奖励函数，未对齐的AGI都有避免关机的激励；2. 推导出对抗阈值：当γ=0.99、p=0.01时，除非C足够大，否则AGI有强烈接管激励；3. 证明当Δ≥0时不存在稳定合作均衡，理性人类会关机或先发制人；4. 对齐目标通过施加伤害人类的大负效用使对抗次优。

Conclusion: AGI对抗风险取决于参数设置，奖励设计和监督至关重要，但验证Δ<0存在计算障碍，多智能体设置可能更复杂，需要进一步研究确保AGI安全对齐。
...
### [162] [Actively Obtaining Environmental Feedback for Autonomous Action Evaluation Without Predefined Measurements](https://arxiv.org/abs/2601.04235)
*Hong Su*
...
Method: 提出主动反馈获取模型，利用动作引发的环境差异来识别未预先指定的目标反馈。引入由内部目标（如提高准确性、精度和效率）驱动的自触发机制，自主规划和调整动作，实现更快、更聚焦的反馈获取。

Result: 实验结果表明，所提出的主动方法显著提高了因子识别的效率和鲁棒性。

Conclusion: 主动反馈获取模型能够在不依赖预定义测量的情况下，通过主动与环境交互来发现和验证反馈，提高了智能体在开放动态环境中的适应性和学习能力。
...
### [163] [SAGE-32B: Agentic Reasoning via Iterative Distillation](https://arxiv.org/abs/2601.04237)
...
Conclusion: SAGE-32B成功展示了专门针对智能体推理和长期规划任务优化的语言模型的有效性，其迭代蒸馏和逆向推理方法显著提升了智能体任务的性能。模型权重已公开。
...
### [164] [A Closed-Loop Multi-Agent System Driven by LLMs for Meal-Level Personalized Nutrition Management](https://arxiv.org/abs/2601.04491)
*Muqing Xu*

Main category: cs.AI

TL;DR: 开发了一个结合图像识别和LLM多智能体控制的新一代移动营养助手，通过照片估计营养素并更新每日摄入预算，然后根据用户偏好和饮食限制调整下一餐计划
...
Method: 结合图像识别和LLM驱动的多智能体控制器，协调视觉、对话和状态管理智能体，从照片估计营养素并更新每日摄入预算，然后根据用户偏好和饮食限制调整下一餐计划

Result: 在SNAPMe餐食图像和模拟用户实验中显示出有竞争力的营养素估计、个性化菜单和高效的任务计划
...
<details>
  <summary>Details</summary>
Motivation: 循环反带宽问题是NP难的图标记问题，现有最先进方法仅限于启发式或元启发式算法，精确方法仅限于受限图类，需要开发适用于一般图的精确求解方法。

Method: 提出基于SAT求解的SAT-CAB方法，采用新颖高效的SAT编码，将问题转化为一系列At-Most-One约束序列，并引入紧凑的约束表示以显著减少公式规模。

Result: 在标准基准实例上的计算实验表明，该方法能高效解决实际相关的CABP实例，发现多个先前未知的最优解，首次证明了许多基准实例的全局最优循环反带宽值。

Conclusion: SAT-CAB在一般图上一致匹配或超越了MS-GVNS、HABC-CAB、MACAB等最先进启发式算法以及CPLEX、Gurobi等商业求解器的性能，同时提供最优性保证，为CABP研究设立了新基准。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型使能AI智能体能够执行多步工作流，而区块链发展为可编程的价值转移平台，两者的融合带来了高风险的系统挑战：需要设计标准、可互操作且安全的接口，让智能体能够观察链上状态、制定交易意图并授权执行，同时避免用户、协议或组织面临不可接受的安全、治理或经济风险。

Method: 通过系统文献综述，从3000多篇文献中筛选出317篇相关研究，构建了五部分分类法（只读分析、模拟与意图生成、委托执行、自主签名、多智能体工作流），开发了针对智能体驱动交易管道的威胁模型，并创建了包含13个维度的比较能力矩阵，分析了20多个代表性系统。

Result: 识别了当前研究的空白，提出了两个核心接口抽象：用于便携无歧义目标规范的交易意图模式，以及用于跨执行环境可审计、可验证策略执行的策略决策记录。还提出了可复现的评估套件和基准测试。

Conclusion: 论文为评估智能体介导的链上执行的安全性、可靠性和经济稳健性提供了系统框架，并规划了以交易意图模式和策略决策记录为核心的研究路线图，以推动AI智能体与区块链的安全集成。
...
### [167] [Fuzzy Representation of Norms](https://arxiv.org/abs/2601.04249)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI驱动的自主系统日益融入日常生活和社会，其伦理和社会影响引发担忧。要使自主系统可信，必须遵守伦理原则和价值观。SLEEC规则为表示伦理和其他规范性考虑提供了全面框架，但需要有效的嵌入方法。

Method: 提出SLEEC规则的逻辑表示方法，使用测试分数语义和模糊逻辑来嵌入伦理要求。模糊逻辑将伦理视为可能性领域，能够解决AI系统可能遇到的伦理困境。通过案例研究说明该方法。

Result: 开发了一个基于模糊逻辑的SLEEC规则表示框架，能够有效处理伦理要求的模糊性和复杂性，为自主系统提供伦理决策支持。

Conclusion: 模糊逻辑为在自主系统中嵌入SLEEC规则提供了有前景的方法，能够处理伦理困境的模糊性和复杂性，有助于构建更可信的AI系统。
...
TL;DR: 提出对抗性推理RAG框架，通过推理器-验证器相互批判的机制，结合过程感知奖励，解决单视角推理和结果导向训练的局限性


<details>
  <summary>Details</summary>
Motivation: 现有大型推理模型与检索增强生成结合的方法存在两个关键问题：1) 推理模型通常从单一、无挑战的视角操作，限制了在外部文档上进行深度自我纠正推理的能力；2) 现有训练范式过度依赖结果导向的奖励，对复杂多步推理过程的信号提供不足
...
Conclusion: 通过对抗性推理框架和过程感知奖励机制，能够有效提升大型推理模型在检索增强生成任务中的深度推理能力和自我纠正能力
...
### [169] [Scaling Trends for Multi-Hop Contextual Reasoning in Mid-Scale Language Models](https://arxiv.org/abs/2601.04254)
*Brady Steele,Micah Katz*

Main category: cs.AI

TL;DR: 该研究通过控制实验展示了多跳上下文推理中的任务-方法分离现象：基于规则的模式匹配在结构化信息检索上达到100%成功率，但在跨文档推理任务上只有6.7%；而基于LLM的多智能体系统则呈现相反模式，在推理任务上达到80%成功率。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究大语言模型在多跳上下文推理任务中的表现，特别关注任务-方法分离现象，以及多智能体系统如何影响推理能力。通过控制实验设计，量化分析不同模型架构和参数规模对推理性能的影响。

Method: 采用合成评估框架，在四个模型（LLaMA-3 8B、LLaMA-2 13B、Mixtral 8x7B、DeepSeek-V2 16B）上进行120次试验。比较基于规则的模式匹配方法与基于LLM的多智能体系统在不同类型任务上的表现，并分析参数规模、架构质量等因素对推理能力的影响。

Result: 三个关键发现：1）多智能体增强效果依赖于基础模型能力，只有具备足够推理能力的模型才能获得统计显著提升；2）Mixtral的性能与其约12B活跃参数而非47B总参数一致，支持推理能力由推理时计算驱动的假设；3）LLaMA-3 8B在参数更少的情况下优于LLaMA-2 13B，显示架构质量的重要性。

Conclusion: 研究为多智能体协调和MoE扩展的直觉提供了定量证据，同时强调多智能体效益对基础模型能力的依赖性。研究框架已开源，支持中等规模模型推理研究的可重复性。
...
### [170] [When Single-Agent with Skills Replace Multi-Agent Systems and When They Fail](https://arxiv.org/abs/2601.04748)
*Xiaoxiao Li*

Main category: cs.AI

TL;DR: 研究探索将多智能体系统编译为单智能体技能选择系统，发现技能选择存在类似人类认知的容量限制，在达到临界库规模后性能急剧下降，语义混淆是主要影响因素。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统虽然有效但计算开销大，能否通过单智能体选择技能库中的技能实现类似模块化优势？同时探索技能选择如何随技能库规模扩展，以及是否存在类似人类认知的容量限制。

Method: 将多智能体系统编译为等效的单智能体系统，将智能体间通信替换为技能选择。研究技能选择的扩展行为，分析语义混淆对选择准确性的影响，并探索分层组织作为解决方案。

Result: 初步实验显示该方法能显著减少token使用和延迟，同时在推理基准上保持竞争力。技能选择准确性在达到临界库规模前保持稳定，之后急剧下降，呈现类似人类认知的相变现象。语义混淆而非单纯库规模是性能下降的主要因素。分层路由初步支持能改善可扩展性。

Conclusion: LLM技能选择存在类似人类决策的有限容量，语义混淆是关键限制因素。分层组织可能帮助AI系统管理复杂选择，这为设计可扩展的技能型智能体提供了认知基础框架和实践指导。
...
TL;DR: RLMIL-DAT：结合强化学习实例选择和对抗域适应的多语言说话人属性预测框架，在少样本和零样本设置中提升性别和年龄预测性能
...
Method: 提出RLMIL-DAT框架，将强化学习实例选择与对抗域适应相结合，通过共享编码器学习语言不变的语音表示

Result: 在5语言Twitter少样本和40语言VoxCeleb2零样本设置中，相比标准MIL和原始RLMIL，Macro F1持续提升，性别预测改进显著，年龄预测也有正向改进

Conclusion: 实例选择与对抗域适应的结合是跨语言说话人属性预测的有效稳健策略，对抗域适应是性能提升的主要贡献者
...
TL;DR: 本文通过分析Qwen3模型在命题逻辑推理任务上的表现，揭示了大型语言模型进行逻辑推理的内部计算架构，包括四个相互关联的机制：分阶段计算、信息传输、事实回溯和专用注意力头。


<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型如何进行内部逻辑推理是一个基础性挑战。先前的研究主要关注识别特定任务电路，但未能回答LLMs在命题推理中采用何种计算策略的问题。

Method: 使用Qwen3（8B和14B）模型在PropLogic-MI数据集上进行综合分析，该数据集涵盖11个命题逻辑规则类别，包括单跳和双跳推理。研究重点从"哪些组件是必要的"转向"模型如何组织计算"。

Result: 分析揭示了一个连贯的计算架构，包含四个相互关联的机制：1) 分阶段计算（层级处理阶段）；2) 信息传输（边界标记处的信息流聚合）；3) 事实回溯（持续重新访问源事实）；4) 专用注意力头（功能不同的头类型）。这些机制在不同模型规模、规则类型和推理深度上具有普适性。

Conclusion: 研究提供了机制性证据，表明大型语言模型采用结构化的计算策略进行逻辑推理，这些策略在不同条件下具有一致性，为理解LLMs的内部推理过程提供了新视角。
...
TL;DR: 该论文提出一个概念框架，认为智能和意识源于关系结构而非预测或领域特定机制。智能被定义为形成和整合信号、行动与内部状态之间因果连接的能力。通过上下文丰富化，系统利用学习到的关系结构解释输入信息，这些结构以高效表征提供原始输入本身不具备的必需上下文。意识则通过递归架构中的系统解释系统原则产生，高阶系统学习并解释低阶系统随时间的关系模式。


<details>
  <summary>Details</summary>
Motivation: 论文旨在重新思考智能和意识的本质，挑战传统预测处理理论，提出基于关系结构和上下文解释的新框架。作者认为当前AI系统缺乏人类智能的关键特征，需要从根本机制上重新思考智能的构建方式。

Method: 提出概念框架：1）将智能定义为形成和整合因果连接的能力；2）通过上下文丰富化实现高效信息处理；3）引入系统解释系统原则，意识在递归架构中产生，高阶系统解释低阶系统的关系模式；4）这些解释被整合到动态稳定的元状态中，并通过上下文丰富化反馈。

Result: 该框架重新构建了预测处理理论，将其视为上下文解释的涌现结果而非显式预测。框架表明递归多系统架构可能是实现更接近人类智能的人工智能的必要条件，为理解智能和意识提供了新的理论视角。

Conclusion: 智能和意识源于关系结构和上下文解释，而非预测或领域特定机制。递归多系统架构通过系统解释系统原则产生意识，这种框架为开发更接近人类智能的人工智能提供了理论基础，并重新解释了预测处理的本质。
...
TL;DR: 该论文提出使用自动化常识推理技术来弥补自动驾驶系统中机器学习模型的不足，特别是在处理异常道路场景时，通过混合模型提高自动驾驶感知能力。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶技术过度依赖机器学习，导致无法实现SAE Level 5完全自动驾驶。在缺乏足够训练数据的异常场景下，深度学习模型表现不佳，需要常识推理技术来补充。

Method: 提出基于常识推理的解决方案，在两种异常场景下应用：1）交通信号灯故障的交叉路口；2）前方车辆因意外障碍物减速转向。通过测量计算机视觉模型的不确定性，高效调用常识推理处理不确定场景。

Result: 在CARLA模拟器中的实验表明，常识推理能准确检测交通灯颜色和未被正确识别的障碍物，有效纠正自动驾驶目标检测的错误分类。

Conclusion: 自动化常识推理能有效弥补自动驾驶感知模型的不足，混合模型为提高自动驾驶感知能力提供了有效途径，是实现SAE Level 5完全自动驾驶的关键技术方向。
...
TL;DR: 本文通过扩展Levesque的仅知道逻辑，引入了一个基于模态逻辑的知识与溯因基本逻辑，通过结合基本认知概念定义溯因模态算子，为溯因推理提供了模态化方法，并探索了溯因推理与仅知道认知状态之间的关系。


<details>
  <summary>Details</summary>
Motivation: 传统溯因推理方法缺乏对认知状态的深入分析，特别是与"仅知道"这种精确认知状态的关系。本文旨在通过模态逻辑方法为溯因推理提供更形式化、更精确的基础，并建立溯因推理与认知逻辑之间的系统联系。

Method: 1. 扩展Levesque的仅知道逻辑，引入溯因模态算子；2. 通过结合基本认知概念定义该算子；3. 在模态框架中引入优先关系，构建非单调扩展框架；4. 支持表达不同的溯因解释选择方法。

Result: 1. 建立了基于模态逻辑的溯因推理基本框架；2. 实现了非单调扩展，能够处理不同选择方法；3. 验证了非单调后承关系的核心元理论性质；4. 为溯因推理提供了行为良好的理论基础。

Conclusion: 本文提出的模态逻辑方法为溯因推理提供了形式化基础，成功连接了溯因推理与认知逻辑，特别是"仅知道"认知状态。通过非单调扩展，该框架能够灵活处理不同的解释选择策略，为溯因推理建立了行为良好的理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 航空应用中知识表示与推理技术面临两大挑战：需要足够的表达能力来捕捉复杂领域知识，同时需要高效执行推理任务并最小化内存使用和计算开销。集成规则和本体这两种基本知识表示概念是实现必要表达能力的有效策略。

Method: 采用混合MKNF知识表示语言，通过具体案例研究评估其在航空领域的适用性。识别出对开发航空应用至关重要的额外表达能力特征，并提出一组启发式方法来支持将这些特征集成到混合MKNF框架中。

Result: 研究发现混合MKNF语言能够无缝集成规则和本体，但其在航空领域应用时需要额外的表达能力特征。提出的启发式方法为将这些特征集成到混合MKNF框架提供了支持。

Conclusion: 混合MKNF语言在航空领域具有应用潜力，但需要扩展其表达能力以适应航空应用的复杂需求。提出的启发式方法为实现这种集成提供了可行路径。
...
Result: 实现了与现有医疗平台的无缝互操作性，提高了调度效率，减少了行政开销，为患者提供了更好的护理体验

Conclusion: ASP框架为医疗预约调度提供了有效的解决方案，特别适合为弱势群体提供个性化服务，具有实际应用价值
...
### [178] [Pilot Study on Student Public Opinion Regarding GAI](https://arxiv.org/abs/2601.04336)
*William Franz Lamberti,Sunbin Kim,Samantha Rose Lawrence*

Main category: cs.AI

TL;DR: 该试点研究调查了大学生对高等教育课堂中生成式AI的看法，旨在为理解这些态度奠定基础。研究参与率约为4.4%，突显了让学生参与GAI相关研究的挑战，并强调未来研究需要更大的样本量。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的出现引发了关于其在包括教育在内的各个领域适当使用的不同观点。本研究旨在了解大学生对GAI在高等教育课堂中的看法，为理解这些态度奠定基础，帮助教师更好地准备将GAI讨论融入课堂。

Method: 这是一项试点研究，调查了大学学生对生成式AI在高等教育课堂中的看法。研究采用了问卷调查或访谈等方法收集学生观点，但具体方法在摘要中未详细说明。

Result: 研究参与率约为4.4%，表明让学生参与GAI相关研究存在挑战。研究结果为理解学生态度提供了初步基础，但需要更大样本量的研究来获得更可靠的结论。

Conclusion: 通过了解学生观点，教师可以更好地准备将GAI讨论融入课堂，促进对这一变革性技术的知情和批判性参与。研究强调了未来需要更大样本量的研究，并突显了让学生参与GAI相关研究的挑战。
...
### [179] [The Language of Bargaining: Linguistic Effects in LLM Negotiations](https://arxiv.org/abs/2601.04387)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM谈判评估几乎完全基于英语，需要研究语言选择对谈判结果的影响，避免英语单一评估带来的偏差
...
Conclusion: 仅用英语评估LLM谈判会产生不完整且可能误导的结论，需要警惕仅用英语评估LLM，文化感知评估对公平部署至关重要
...
### [180] [LLM-Guided Lifecycle-Aware Clustering of Multi-Turn Customer Support Conversations](https://arxiv.org/abs/2601.04388)
*Priyaranjan Pattnayak,Sanchari Chowdhuri,Amit Agarwal,Hitesh Laxmichand Patel*

Main category: cs.AI

TL;DR: 提出了一种自适应聚类系统，用于处理客户聊天数据中的多服务查询，通过增量优化避免全量重聚类，显著提升聚类质量。
...
Method: 将多轮聊天分割为特定服务关注点，随着新问题出现增量优化聚类。通过Davies-Bouldin指数和轮廓系数追踪聚类质量，仅对退化的聚类应用基于LLM的分割方法。

Result: 相比基线方法，轮廓系数提升超过100%，DBI降低65.6%，实现了无需全量重聚类的可扩展实时分析。

Conclusion: 提出的自适应系统能有效处理客户聊天数据中的多服务查询，通过增量优化维持聚类质量，支持可扩展的实时分析。
...
### [181] [Assessing the quality and coherence of word embeddings after SCM-based intersectional bias mitigation](https://arxiv.org/abs/2601.04393)
*Eren Kocadag,Seyed Sahand Mohammadi Ziabari,Ali Mohammed Mansoor Alsahag*

Main category: cs.AI

TL;DR: 该研究扩展了刻板印象内容模型在词嵌入去偏中的应用，从单一群体偏见扩展到交叉性偏见，通过求和或拼接构建复合社会身份表示，并比较三种去偏策略的效果。


<details>
  <summary>Details</summary>
Motivation: 静态词嵌入常从训练文本中吸收社会偏见，这些偏见可能影响下游系统。先前基于刻板印象内容模型的研究主要关注单一群体在温暖和能力维度上的偏见，本研究旨在扩展这一视角到交叉性偏见。
...
Result: 刻板印象内容模型为基础的缓解方法能很好地扩展到交叉性情况，并基本保持整体语义结构完整。主要成本是熟悉的权衡：严格保持几何结构的方法在类比行为上更保守，而更激进的投影能改善类比性能但牺牲邻域稳定性。部分投影最保守，线性投影更激进，减法作为简单基线仍有竞争力。求和与拼接的选择取决于嵌入模型和应用目标。

Conclusion: 基于刻板印象内容模型的交叉性去偏在静态嵌入中是可行的，研究结果为在平衡稳定性和类比性能时选择聚合和去偏设置提供了指导。
...
TL;DR: XGrammar 2是一个为LLM智能体优化的结构化生成引擎，通过TagDispatch动态调度、JIT编译和跨语法缓存等创新技术，在处理动态结构化生成任务时比现有引擎快6倍以上。
...
Method: 1) 提出TagDispatch动态调度语义来加速掩码生成；2) 引入JIT编译减少编译时间；3) 设计跨语法缓存机制利用不同语法间的共同子结构；4) 扩展基于PDA的掩码生成算法为基于Earley解析器的算法；5) 设计重复压缩算法处理语法中的重复结构。

Result: XGrammar 2比现有结构化生成引擎快6倍以上，与LLM推理引擎集成后，处理动态结构化生成任务时几乎零开销。

Conclusion: XGrammar 2通过创新的动态调度、编译优化和缓存机制，为LLM智能体提供了高效处理复杂动态结构化生成任务的能力，显著提升了性能。
...
TL;DR: 该论文为因子图上的置信传播建立了范畴论基础，提出了HATCC算法，通过检测下降障碍并编译到增强图上，实现了精确推理，相比联结树算法有显著加速。


<details>
  <summary>Details</summary>
Motivation: 传统置信传播在树结构上精确，但在带环图上失败。需要统一的数学框架来理解精确推理的条件，并开发能处理非平凡环结构的算法。

Method: 1. 构建自由超图范畴Syn_Σ并证明其泛性质；2. 使用格罗滕迪克纤维化将消息传递形式化；3. 将精确推理表征为有效下降；4. 提出HATCC算法：通过因子神经上的和乐计算检测下降障碍，将非平凡和乐编译为模式变量，在增强图上简化为树BP。

Result: 1. 建立了置信传播的范畴论基础；2. HATCC算法复杂度为O(n²d_max + c·k_max·δ_max³ + n·δ_max²)；3. 在网格MRF和随机图上实现精确推理，相比联结树算法有显著加速；4. 能在可满足性实例上检测UNSAT。

Conclusion: 该研究为因子图推理提供了统一的范畴论框架，将精确推理与下降理论联系起来，提出的HATCC算法能有效处理带环图，在理论和实践上都有重要意义。
...
TL;DR: 论文提出AI监管需要计算化合规，而非传统人工方式，并为此设计了算法设计目标和基准数据集


<details>
  <summary>Details</summary>
Motivation: AI监管时代已经到来，但传统的人工合规方法无法满足AI系统在速度和规模上的合规需求，需要转向计算化合规

Method: 提出计算化AI监管合规算法的设计目标，并创建基准数据集来量化评估这些算法的性能

Result: 为计算化AI监管合规研究领域提供了蓝图，包括具体的算法设计目标和可量化的评估基准

Conclusion: 计算化合规是AI监管的必然趋势，需要研究社区投入必要资源来推动这一新兴领域的发展
...
### [185] [Know Thy Enemy: Securing LLMs Against Prompt Injection via Diverse Data Synthesis and Instruction-Level Chain-of-Thought Learning](https://arxiv.org/abs/2601.04666)
...
<details>
  <summary>Details</summary>
Motivation: LLM集成应用面临提示注入攻击的安全漏洞，现有防御面临两大问题：恶意指令可通过多种向量注入，且注入指令与上下文缺乏明确语义边界，难以识别

Method: InstruCoT方法包括合成多样化训练数据，并采用指令级思维链（CoT）微调，使LLM能够有效识别和拒绝恶意指令，无论其来源或上下文位置

Result: 在四个LLM上的实验结果显示，InstruCoT在行为偏差、隐私泄露和有害输出三个关键维度上显著优于基线方法，同时保持实用性能不下降

Conclusion: InstruCoT通过合成训练数据和指令级思维链微调，有效解决了提示注入攻击防御中的多样性和语义边界问题，显著提升了LLM的安全性
...
TL;DR: GUITester：一个用于GUI探索性测试的多智能体框架，通过解耦导航与验证，解决了目标导向掩盖和执行偏差归因问题，在GUITestBench基准上达到48.90% F1分数。


<details>
  <summary>Details</summary>
Motivation: GUI探索性测试对软件质量至关重要，但人工成本高昂。现有的多模态大语言模型智能体擅长导航，但无法自主发现缺陷，主要面临两个核心挑战：目标导向掩盖（智能体优先完成任务而非报告异常）和执行偏差归因（系统缺陷被误判为智能体错误）。

Method: 提出GUITester多智能体框架，包含两个模块：1）规划-执行模块（PEM），通过嵌入测试意图主动探测缺陷；2）分层反思模块（HRM），通过交互历史分析解决归因模糊性。同时构建了GUITestBench基准，包含26种缺陷的143个任务。

Result: GUITester在GUITestBench基准上达到48.90% F1分数（Pass@3），显著优于现有最先进基线方法的33.35%。

Conclusion: 该研究证明了自主探索性测试的可行性，为未来GUI质量保证提供了坚实基础。GUITester通过解耦导航与验证，有效解决了多模态大语言模型智能体在缺陷发现中的核心挑战。
...
Conclusion: 提出的基于主动学习的SEI方法有效解决了标注数据稀缺问题，通过结合自监督对比学习、联合损失优化和主动学习样本选择，实现了在有限标注预算下的高性能特定发射源识别。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在监督图像分类方面取得了显著进展，但数据标注成本高昂。虽然对比学习等无监督方法被广泛研究，但现实场景中完全无标签的数据集很少见。当少量有标签数据与大量无标签数据共存时，半监督学习变得尤为重要。现有的伪标签半监督对比学习方法仍有改进空间。

Method: 提出了一种增强的伪标签半监督学习方法，通过在有标签和无标签特征嵌入之间进行分布匹配来改进图像分类。该方法结合了对比学习和分布对齐技术，利用伪标签机制处理无标签数据，并通过分布匹配优化特征表示。

Result: 该方法在多个数据集上提高了图像分类的准确性，验证了分布匹配对伪标签半监督学习的有效性。

Conclusion: 通过将有标签和无标签特征嵌入的分布匹配融入伪标签半监督对比学习框架，可以显著提升图像分类性能，为实际应用中有限标注数据的场景提供了有效的解决方案。
...
TL;DR: BioPIE数据集为生物医学实验QA系统提供细粒度、程序中心的知识图谱，解决高信息密度和多步推理挑战，提升实验自动化性能。


<details>
  <summary>Details</summary>
Motivation: 生物医学实验QA系统面临高信息密度和多步推理的独特挑战，现有数据集缺乏细粒度的实验知识支持，阻碍了实验室自动化等下游任务的发展。

Method: 提出BioPIE数据集，构建以实验程序为中心的知识图谱，包含实验实体、动作和关系的结构化表示，支持跨协议的生物医学实验推理。

Result: 在BioPIE上评估信息提取方法，并实现基于BioPIE的QA系统，在测试集、高信息密度和多步推理问题集上均显示出性能提升。

Conclusion: BioPIE提供的结构化实验知识为AI辅助和更自主的生物医学实验提供了基础支持，解决了现有数据集在细粒度实验推理方面的不足。
...
TL;DR: TCAR是一个自适应推理路由器，通过自然语言推理链和多智能体协作执行管道，解决了传统任务路由中静态单标签决策的限制，提高了路由准确性和鲁棒性。
...
Method: 提出TCAR自适应推理路由器：1) 支持动态智能体接入；2) 首先生成自然语言推理链，然后预测能够处理查询的候选智能体集合；3) 设计协作执行管道，选中的智能体独立生成响应，然后由专门的精炼智能体聚合和优化为单一高质量响应。
...
### [191] [Personalized Model-Based Design of Human Centric AI enabled CPS for Long term usage](https://arxiv.org/abs/2601.04545)
*Bernard Ngabonziza,Ayan Banerjee,Sandeep K. S. Gupta*

Main category: cs.AI

TL;DR: 该论文分析了AI赋能人本关键系统在长期运行中面临的安全、可持续性和安全挑战，并提出了个性化模型解决方案
...
Result: 论文识别了现有技术在测试长期运行系统方面的不足，并提出了针对性的个性化模型方法来解决这些问题。

Conclusion: AI赋能的人本关键系统需要新的个性化模型方法来应对长期运行中的安全、可持续性和安全挑战，以确保系统在实际应用中的可靠性。
...
TL;DR: 提出BackdoorAgent框架，从智能体角度统一分析LLM智能体工作流中的后门威胁，包括规划、记忆、工具使用三个阶段攻击，并构建标准化基准进行实证分析。


<details>
  <summary>Details</summary>
Motivation: LLM智能体通过多步工作流执行任务，这种设计虽然实现了自主性，但也扩大了后门攻击的攻击面。现有研究通常孤立分析单个攻击向量，缺乏从智能体角度理解后门触发器在跨阶段交互和传播的机制。

Method: 提出BackdoorAgent框架，将攻击面结构化分为规划攻击、记忆攻击和工具使用攻击三个阶段，并设计工具化智能体执行过程，系统分析触发器在不同阶段的激活和传播。构建涵盖Agent QA、Agent Code、Agent Web、Agent Drive四个代表性应用的标准化基准。

Result: 实证分析表明，植入单个阶段的触发器可以跨多个步骤持续存在并通过中间状态传播。使用GPT骨干时，规划攻击中43.58%存在触发器持续性，记忆攻击中77.97%，工具阶段攻击中60.28%，凸显了智能体工作流本身对后门威胁的脆弱性。

Conclusion: BackdoorAgent提供了一个统一的智能体中心视角来分析LLM智能体中的后门威胁，揭示了后门触发器在智能体工作流中的跨阶段传播特性，为理解和防御此类威胁提供了系统框架。
...
### [193] [Enhancing Multimodal Retrieval via Complementary Information Extraction and Alignment](https://arxiv.org/abs/2601.04571)
...
<details>
  <summary>Details</summary>
Motivation: 现有的大多数多模态检索研究主要关注捕捉多模态数据中与其配对文本相似的信息，但往往忽略了多模态数据中包含的互补信息。这种局限性限制了多模态检索系统的性能。

Method: CIEA采用互补信息提取和对齐方法，将文档中的文本和图像转换到统一的潜在空间。该方法设计了专门的互补信息提取器，用于识别和保留图像表示中的差异信息。通过两种互补的对比损失函数进行优化，确保语义完整性并有效捕捉图像中的互补信息。

Result: 大量实验证明CIEA的有效性，相比分治模型和通用密集检索模型都取得了显著改进。研究还提供了消融实验、进一步讨论和案例研究，突出了CIEA取得的进展。

Conclusion: CIEA通过有效提取和对齐多模态数据中的互补信息，显著提升了多模态检索性能。研究团队已开源代码以促进社区进一步研究。
...
### [194] [Evaluating Human and Machine Confidence in Phishing Email Detection: A Comparative Study](https://arxiv.org/abs/2601.04610)
*Paras Jain,Khushi Dhar,Olyemi E. Amujo,Esa M. Rantanen*

Main category: cs.AI

TL;DR: 研究探讨人类认知与机器学习模型如何协同识别钓鱼邮件，比较了三种可解释算法与人类评估在检测准确率、置信度和语言特征使用上的差异。
...
Result: 机器学习模型提供良好的准确率，但置信度水平差异显著。人类评估者使用更多样化的语言特征并保持更一致的置信度。语言能力对检测性能影响最小，但年龄有影响。
...
### [195] [AgentDevel: Reframing Self-Evolving LLM Agents as Release Engineering](https://arxiv.org/abs/2601.04620)
*Di Zhang*
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体改进方法（如智能体内部自改进机制或并发变体搜索）虽然能提升总体分数，但往往产生不稳定且难以审计的改进轨迹，难以保证非回归性或跨版本故障推理。需要一种更工程化的方法来保证改进的稳定性和可审计性。

Method: 提出AgentDevel发布工程管道：1）迭代运行当前智能体；2）从执行轨迹中生成实现无关的症状级质量信号；3）通过可执行诊断合成单个发布候选版本；4）在翻转中心化门控下进行发布。核心设计包括：实现无关的LLM批评器、基于脚本的可执行诊断、翻转中心化门控。

Result: 在面向执行的基准测试中，AgentDevel实现了稳定的改进，显著减少了回归，同时产生可重现、可审计的工件。相比基于种群的搜索或智能体内自优化，AgentDevel保持了单一规范版本线，并将非回归作为主要目标。
...
### [196] [Vibe Coding an LLM-powered Theorem Prover](https://arxiv.org/abs/2601.04653)
*Zhe Hou*

Main category: cs.AI

TL;DR: Isabellm是一个基于LLM的Isabelle/HOL定理证明器，能够在消费级计算机上运行，结合逐步证明器和高级证明规划器，能够证明一些标准自动化工具无法处理的引理。
...
Method: 系统结合了逐步证明器（使用LLM生成证明命令并由Isabelle验证）和高级证明规划器（生成结构化Isar大纲并填补修复剩余空白）。框架包括波束搜索策略、策略重排ML和RL模型、小型Transformer模型的前提选择、基于AFP的Isar证明微RAG，以及反例引导的证明修复。
...
Conclusion: Isabellm展示了LLM在定理证明中的潜力，能够解决传统自动化工具难以处理的问题，但同时也揭示了LLM在代码生成和推理方面的基本挑战，特别是在复杂算法设计方面。
...
### [197] [ResMAS: Resilience Optimization in LLM-based Multi-agent Systems](https://arxiv.org/abs/2601.04694)
*Zhilun Zhou,Zihan Liu,Jiahe Liu,Qingyu Shao,Yihan Wang,Kun Shao,Depeng Jin,Fengli Xu*

Main category: cs.AI

TL;DR: 本文提出ResMAS框架，通过两阶段方法增强基于大语言模型的多智能体系统（LLM-based MAS）的韧性：1）训练奖励模型预测系统韧性，并用强化学习自动设计韧性拓扑；2）基于拓扑优化智能体提示。


<details>
  <summary>Details</summary>
Motivation: LLM-based MAS在分布式环境中容易受到智能体故障等扰动影响。现有研究主要关注攻击后的被动检测和缓解，而非主动设计内在韧性系统。研究发现通信拓扑和提示设计显著影响系统韧性，因此需要系统性的韧性增强方法。

Method: 提出ResMAS两阶段框架：第一阶段训练奖励模型预测MAS韧性，基于此通过强化学习训练拓扑生成器，为特定任务自动设计韧性拓扑；第二阶段引入拓扑感知的提示优化方法，根据智能体连接和交互关系优化每个智能体的提示。

Result: 在多种任务上的实验表明，该方法在各种约束条件下显著提高了MAS的韧性。框架还展现出对新任务和模型的强泛化能力。
...
### [198] [Tape: A Cellular Automata Benchmark for Evaluating Rule-Shift Generalization in Reinforcement Learning](https://arxiv.org/abs/2601.04695)
*Enze Pan*

Main category: cs.AI

TL;DR: Tape是一个受控强化学习基准，专门用于隔离潜在规则变化下的分布外（OOD）失败。它基于一维元胞自动机，允许在保持观察和动作空间不变的情况下改变转移规则，从而精确划分训练/测试集。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习方法在分布内（ID）表现良好，但在分布外（OOD）规则变化下容易失败。需要建立一个受控的基准来系统评估模型在潜在规则变化下的鲁棒性，并建立标准化的OOD评估协议。

Method: 基于一维元胞自动机构建Tape基准，设计可重现的评估流程，比较模型无关基线方法、基于学习世界模型的模型规划方法以及任务推断（元强化学习）方法。建立信息论恒等式连接熵减少与条件互信息和期望后验KL散度。

Result: 发现一个一致模式：在分布内表现强的方法在分布外规则变化下可能崩溃；高方差的OOD评估可能导致排名不稳定，除非实验有足够重复。提供了标准化OOD协议、统计报告要求（种子数、置信区间、假设检验）和信息论分析框架。

Conclusion: Tape基准为评估强化学习方法在潜在规则变化下的鲁棒性提供了系统框架，强调了OOD评估中统计严谨性的重要性，并澄清了"不确定性减少"目标在规则变化下能保证和不能保证的内容。
...
### [199] [TourPlanner: A Competitive Consensus Framework with Constraint-Gated Reinforcement Learning for Travel Planning](https://arxiv.org/abs/2601.04698)
*Yinuo Wang,Mining Tan,Wenxiang Jiao,Xiaoxi Li,Hao Wang,Xuanyu Zhang,Yuan Lu,Weiming Dong*

Main category: cs.AI

TL;DR: TourPlanner是一个旅行规划框架，通过多路径推理和约束门控强化学习解决现有方法的三个挑战：候选POI筛选、单一路径限制以及硬约束和软约束同时优化问题。


<details>
  <summary>Details</summary>
Motivation: 现有旅行规划方法面临三个主要挑战：(1)在保持高召回率的同时筛选候选兴趣点(POIs)；(2)单一路径推理限制了在可行解空间中的探索能力；(3)同时优化硬约束和软约束存在困难。

Method: 提出TourPlanner框架，包含三个核心组件：1)个性化召回与空间优化(PReSO)工作流构建空间感知的候选POI集合；2)竞争共识思维链(CCoT)多路径推理范式增强可行解空间探索；3)基于sigmoid的门控机制集成到强化学习阶段，动态优先满足硬约束后再优化软约束。
...
### [200] [Beyond Monolithic Architectures: A Multi-Agent Search and Knowledge Optimization Framework for Agentic Search](https://arxiv.org/abs/2601.04703)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的智能体搜索系统存在结构瓶颈：推理输出不受约束导致轨迹膨胀、稀疏结果级奖励使信用分配复杂化、随机搜索噪声破坏学习稳定性。需要解决这些挑战以提升复杂信息检索任务的性能。

Method: 提出M-ASK框架，将智能体搜索解耦为两个互补角色：搜索行为智能体（负责规划和执行搜索动作）和知识管理智能体（负责聚合、过滤和维护紧凑的内部上下文）。采用轮次级奖励机制为搜索决策和知识更新提供细粒度监督。

Result: 在多跳问答基准测试中，M-ASK优于现有基线方法，不仅获得了更高的答案准确率，还实现了显著更稳定的训练动态。

Conclusion: 通过将搜索行为与知识管理解耦，并采用分层奖励机制，M-ASK框架有效解决了当前智能体搜索系统的结构瓶颈，为复杂信息检索任务提供了更高效稳定的解决方案。
...
### [201] [Bridging Temporal and Textual Modalities: A Multimodal Framework for Automated Cloud Failure Root Cause Analysis](https://arxiv.org/abs/2601.04709)
*Gijun Park*

Main category: cs.AI

TL;DR: 本文提出了一种多模态诊断框架，将时间序列数据与预训练语言模型嵌入空间对齐，用于云基础设施根因分析，在六个基准测试中达到48.75%的诊断准确率。
...
### [202] [ThinkDrive: Chain-of-Thought Guided Progressive Reinforcement Learning Fine-Tuning for Autonomous Driving](https://arxiv.org/abs/2601.04714)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在自动驾驶领域的广泛应用，现有方法存在非结构化推理、泛化能力差以及与人类驾驶意图不一致的问题。虽然思维链推理提高了决策透明度，但传统的监督微调未能充分利用其潜力，而强化学习方法面临不稳定和推理深度不足的挑战。

Method: 提出ThinkDrive框架，采用两阶段训练策略：首先使用思维链解释进行监督微调，然后应用渐进式强化学习，结合难度感知自适应策略优化器，根据样本复杂度动态调整学习强度。

Result: 在公开数据集上的评估显示，ThinkDrive在考试、简单考试和准确率指标上分别比强化的强化学习基线高出1.45%、1.95%和1.01%。此外，使用该方法训练的20亿参数模型在考试指标上比更大的GPT-4o高出3.28%。

Conclusion: ThinkDrive通过结合思维链推理和渐进式强化学习，有效提升了自动驾驶决策的透明度、泛化能力和与人类意图的一致性，为自动驾驶领域的大语言模型应用提供了新的优化框架。
...
### [203] [Miner:Mining Intrinsic Mastery for Data-Efficient RL in Large Reasoning Models](https://arxiv.org/abs/2601.04731)
*Shuyang Jiang,Yuhao Wang,Ya Zhang,Yanfeng Wang,Yu Wang*

Main category: cs.AI

TL;DR: Miner方法通过利用策略内在不确定性作为自监督奖励信号，解决了当前无批评强化学习方法在正同质提示训练中的效率低下问题，无需外部监督或额外推理成本。
...
Conclusion: 潜在不确定性开发对于推理模型的高效可扩展强化学习训练既是必要的也是充分的，Miner方法通过简单而强大的解决方案显著提升了训练效率。
...
### [204] [KnowMe-Bench: Benchmarking Person Understanding for Lifelong Digital Companions](https://arxiv.org/abs/2601.04745)
...
Conclusion: 基于自传叙事的KnowMeBench基准揭示了当前模型在理解人的稳定动机和决策原则方面的局限性，需要开发超越检索的记忆机制来真正理解人。
...
### [205] [Orion-RAG: Path-Aligned Hybrid Retrieval for Graphless Data](https://arxiv.org/abs/2601.04764)
*Zhen Chen,Weihao Xie,Peilin Chen,Shiqi Wang,Jianping Wang*

Main category: cs.AI

TL;DR: Orion-RAG提出了一种轻量级路径提取方法，将碎片化文档转化为半结构化数据，在知识检索增强生成中显著提升性能，特别是在金融领域实现了25.2%的相对改进。


<details>
  <summary>Details</summary>
Motivation: 现实场景中的数据通常是离散和碎片化的，信息分布在孤立文件中且缺乏显式链接。标准搜索引擎独立处理文件，忽略了文件间的联系，而手动构建知识图谱对于海量数据不切实际。
...
### [206] [AT$^2$PO: Agentic Turn-based Policy Optimization via Tree Search](https://arxiv.org/abs/2601.04767)
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体已成为处理多轮任务的有力系统，但智能体强化学习作为关键的后训练范式，在探索多样性、信用分配和策略优化对齐方面仍面临挑战。

Method: 提出AT²PO框架：1）引入轮级树结构，支持熵引导的树扩展进行策略探索；2）轮级信用分配实现稀疏奖励的细粒度传播；3）智能体轮级策略优化（ATPO）作为轮级学习目标，使策略更新与智能体交互的自然决策粒度对齐。

Result: 在七个基准测试中，AT²PO相比最先进的基线方法平均提升了1.84个百分点，消融研究验证了每个组件的有效性。

Conclusion: AT²PO为多轮智能体强化学习提供了一个统一的框架，有效解决了探索、信用分配和策略优化对齐等核心挑战，ATPO组件可以轻松集成到任何多轮RL流程中。
...
### [207] [APEX: Academic Poster Editing Agentic Expert](https://arxiv.org/abs/2601.04794)
...
Method: 提出APEX框架，基于多级API的鲁棒编辑和审查调整机制；构建APEX-Bench基准，包含514条指令，采用参考引导和无参考策略确保真实性和多样性

Result: 实验结果表明APEX显著优于基线方法；建立了多维度VLM作为评判者的评估协议，评估指令完成度、修改范围和视觉一致性

Conclusion: APEX是首个交互式学术海报编辑的智能体框架，通过细粒度控制和系统化基准测试，有效解决了现有方法的局限性
...
### [208] [Thinking-Based Non-Thinking: Solving the Reward Hacking Problem in Training Hybrid Reasoning Models via Reinforcement Learning](https://arxiv.org/abs/2601.04805)
...
TL;DR: TNT方法通过利用思考型响应的解决方案信息，为非思考型响应设置不同的最大token限制，在减少约50%token使用的同时显著提升准确性，实现了准确性与效率的最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型虽然性能优异，但其长链思维推理显著增加计算开销。现有基于强化学习的方法存在奖励黑客问题，而监督微调成本高，统一token限制效果有限，需要更好的解决方案。

Method: 提出Thinking-Based Non-Thinking (TNT)方法，不采用监督微调，而是利用思考型响应的解决方案组件信息，为不同查询的非思考型响应设置差异化的最大token使用限制。

Result: 在五个数学基准测试中，TNT相比DeepSeek-R1-Distill-Qwen-1.5B/7B和DeepScaleR-1.5B减少约50%的token使用，同时显著提高准确性，在所有测试方法中实现了准确性与效率的最佳平衡。

Conclusion: TNT方法有效解决了大型推理模型的过度思考问题，在不使用监督微调的情况下，通过差异化token限制策略实现了计算效率与推理准确性的双重优化，奖励黑客问题概率保持在10%以下。
...
### [209] [SCALER:Synthetic Scalable Adaptive Learning Environment for Reasoning](https://arxiv.org/abs/2601.04809)
*Caijun Xu,Changyi Xiao,Zhongyuan Peng,Xinrun Wang,Yixin Cao*

Main category: cs.AI

TL;DR: SCALER框架通过自适应环境设计解决强化学习训练信号衰减问题，利用可扩展的合成管道创建可控难度推理环境，实现持续有效的学习信号。


<details>
  <summary>Details</summary>
Motivation: 强化学习在提升大语言模型推理能力时面临训练信号随模型进化而失效的问题，当任务难度与模型能力不匹配或训练被狭窄问题模式主导时，学习进展会放缓。

Method: 提出SCALER框架：1）可扩展合成管道将真实编程问题转换为可验证推理环境，控制难度并实现无限实例生成；2）自适应多环境强化学习策略动态调整实例难度和环境组合，跟踪模型能力前沿并保持分布多样性。

Result: 实验表明SCALER在多样化推理基准测试中持续优于基于数据集的强化学习基线，展现出更稳定、更长视野的训练动态。
...
### [210] [AECV-Bench: Benchmarking Multimodal Models on Architectural and Engineering Drawings Understanding](https://arxiv.org/abs/2601.04819)
...
Result: 评估显示稳定能力梯度：OCR和文本中心文档QA最强（准确率高达0.95），空间推理中等，符号中心图纸理解（特别是门和窗的可靠计数）仍未解决（准确率通常0.40-0.55），存在显著比例误差。
...
### [211] [DR-LoRA: Dynamic Rank LoRA for Mixture-of-Experts Adaptation](https://arxiv.org/abs/2601.04823)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法为MoE模型中的所有专家分配相同的LoRA秩，忽视了专家间的功能专业化差异，导致资源不匹配：任务相关专家参数不足，不相关专家却获得冗余参数
...
Conclusion: DR-LoRA通过动态调整MoE模型中专家LoRA秩，解决了现有方法中资源分配不匹配的问题，为参数高效微调提供了更智能的解决方案
...
### [212] [Orchestrating Intelligence: Confidence-Aware Routing for Efficient Multi-Agent Collaboration across Multi-Scale Models](https://arxiv.org/abs/2601.04861)
...
Result: 实验结果表明OI-MAS持续优于基线多智能体系统，准确率提升高达12.88%，同时成本降低高达79.78%。

Conclusion: OI-MAS框架通过智能资源分配有效解决了多智能体系统的计算效率问题，在保持高性能的同时显著降低成本。
...
### [213] [Key-Value Pair-Free Continual Learner via Task-Specific Prompt-Prototype](https://arxiv.org/abs/2601.04864)
*Haihua Luo,Xuming Ran,Zhengji Li,Huiyan Xue,Tingting Jiang,Jiangrong Shen,Tommi Kärkkäinen,Qi Xu,Fengyu Cong*

Main category: cs.AI

TL;DR: 提出基于任务特定提示原型(ProP)的持续学习方法，消除对键值对的依赖，通过任务特定提示和原型绑定进行预测，提高可扩展性和减少任务间干扰。


<details>
  <summary>Details</summary>
Motivation: 现有的基于提示的持续学习方法通常依赖键值对，这会引入任务间干扰并阻碍可扩展性。需要一种不依赖键值对的新方法来解决这些问题。

Method: 提出任务特定提示原型(ProP)方法：1) 使用任务特定提示促进当前任务的特征学习；2) 对应原型捕获输入的代表性特征；3) 推理时通过绑定任务特定提示与相关原型生成预测；4) 在提示初始化时引入正则化约束惩罚过大值以增强稳定性。

Result: 在多个广泛使用的数据集上的实验证明了该方法的有效性。相比主流基于提示的方法，该框架消除了对键值对的依赖。

Conclusion: 提出的ProP方法为持续学习研究提供了新的视角，通过消除键值对依赖，减少了任务间干扰并提高了可扩展性。
...
### [214] [Higher-Order Knowledge Representations for Agentic Scientific Reasoning](https://arxiv.org/abs/2601.04878)
*Isabella A. Stewart,Markus J. Buehler*

Main category: cs.AI

TL;DR: 该研究提出了一种基于超图的知识表示方法，用于解决传统知识图谱在捕捉高阶相互作用方面的不足，并应用于生物复合材料支架领域，构建了包含16万节点和32万超边的全局超图，展示了其在科学发现中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 科学探究需要整合异构实验数据、跨领域知识和机制证据的系统级推理。虽然大语言模型具备推理能力，但通常依赖缺乏结构深度的检索增强上下文。传统知识图谱试图弥合这一差距，但其成对约束无法捕捉支配涌现物理行为的不可约高阶相互作用。

Method: 引入构建基于超图的知识表示方法，该方法能够忠实编码多实体关系。应用于约1,100篇生物复合材料支架文献，构建了包含161,172个节点和320,201条超边的全局超图。为智能体系统配备超图遍历工具，特别是使用节点交集约束，使其能够桥接语义上遥远的概念。

Result: 构建的全局超图显示出无标度拓扑结构（幂律指数约1.23），围绕高度连接的概念枢纽组织。该表示避免了典型的成对扩展组合爆炸，并明确保留了科学表述的共现上下文。通过利用高阶路径，系统成功为新型复合材料生成基于机制的假设，例如通过壳聚糖中间体将氧化铈与PCL支架联系起来。

Conclusion: 这项工作建立了一个"无教师"的智能推理系统，其中超图拓扑结构作为可验证的防护栏，通过揭示传统图方法所掩盖的关系，加速科学发现。超图表示能够捕捉科学知识中固有的高阶相互作用，为科学推理提供了更强大的框架。
...
### [215] [Flexible Manufacturing Systems Intralogistics: Dynamic Optimization of AGVs and Tool Sharing Using Coloured-Timed Petri Nets and Actor-Critic RL with Actions Masking](https://arxiv.org/abs/2601.04887)
...
Method: 提出了一种新颖的方法，将着色时间Petri网与基于模型的强化学习相结合。CTPN提供形式化建模结构和动态动作掩码，显著减少动作搜索空间；MBRL通过学习的策略确保对变化环境的适应性；利用MBRL优势，采用前瞻策略优化AGV定位。

Result: 在小规模公共基准和新开发的大规模Taillard基准上评估，结果显示：在小型实例上与传统方法相当，在大型实例上在完工时间方面优于传统方法，同时计算时间减少十倍。消融研究评估了各组件对整体性能的贡献。

Conclusion: 该方法有效解决了集成AGV和工具共享系统的FMS调度问题，通过CTPN和MBRL的结合实现了高效求解，为复杂制造系统调度提供了有前景的解决方案，并提供了可复现的环境和实例生成器。
...
### [216] [DVD: A Robust Method for Detecting Variant Contamination in Large Language Model Evaluation](https://arxiv.org/abs/2601.04895)
*Renzhao Liang,Jingru Chen,Bo Jia,Bo Deng,Chenggang Xie,Yidong Wang,Ke Jin,Xin Wang,Linfeng Zhang,Cunxiang Wang*

Main category: cs.AI

TL;DR: 本文提出DVD方法，通过分析生成分布的方差来检测大语言模型评估中的变体污染问题，相比现有方法在多个基准测试中表现更优。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型评估面临变体污染问题：训练语料中包含语义相同但词汇或句法改变的测试项变体，这些变体逃避了基于采样一致性或困惑度的现有检测方法，导致基准分数因记忆而非真正推理而虚高。

Method: 提出DVD（通过生成分布方差检测）方法，这是一种单样本检测器，通过温度采样建模局部输出分布。核心洞察是：受污染项会在记忆遵循状态和扰动漂移状态之间交替，导致低概率标记的合成难度方差异常高；未受污染项则保持相对平滑的方差漂移。

Result: 构建了首个跨两个领域（Omni-MATH和SuperGPQA）的变体污染基准，通过微调不同规模和架构的模型（Qwen2.5和Llama3.1）模拟污染。DVD在多个数据集和模型上一致优于基于困惑度、Min-k%++、编辑距离（CDD）和嵌入相似性的基线方法，同时对超参数表现出强鲁棒性。

Conclusion: 生成分布的方差为检测大语言模型评估中的变体污染提供了一个原则性和实用的指纹特征，DVD方法能有效识别因记忆而非真正推理导致的基准分数虚高问题。
...
### [217] [From Stories to Cities to Games: A Qualitative Evaluation of Behaviour Planning](https://arxiv.org/abs/2601.04911)
*Mustafa F. Abdelwahed,Joan Espasa,Alice Toniolo,Ian P. Gent*

Main category: cs.AI

TL;DR: 行为规划作为一种新型多样化规划范式，通过显式整合多样性模型并支持多类别规划，在现实世界应用中展现价值


<details>
  <summary>Details</summary>
Motivation: 多样化规划方法旨在生成相互区别的计划集，在风险管理、自动化流数据分析、恶意软件检测等多个现实领域有广泛应用需求。传统方法需要扩展以更好地适应复杂场景

Method: 提出行为规划范式，通过显式将多样性模型整合到规划过程中，并支持多个规划类别，扩展了早期多样化规划方法

Result: 通过三个现实案例研究验证行为规划的有效性：1) 故事生成；2) 城市规划；3) 游戏评估，展示了该方法在不同领域的适用性

Conclusion: 行为规划作为一种新型多样化规划方法，在现实世界应用中具有实用价值，能够为不同领域提供多样化的解决方案
...
### [218] [What Students Ask, How a Generative AI Assistant Responds: Exploring Higher Education Students' Dialogues on Learning Analytics Feedback](https://arxiv.org/abs/2601.04919)
*Yildiz Uzun,Andrea Gauthier,Mutlu Cukurova*

Main category: cs.AI

TL;DR: 研究探讨了将对话式生成AI助手整合到学习分析仪表板中，以支持不同自我调节学习能力的学生。通过分析10周学期中的真实对话，发现低SRL学生寻求澄清和保证，高SRL学生询问技术细节和个性化策略。AI助手提供清晰解释但个性化有限，对低SRL学生特别有价值，但需要改进信任、自适应性和技术完善。


<details>
  <summary>Details</summary>
Motivation: 学习分析仪表板旨在通过数据反馈支持学生学习调节，但学生特别是低自我调节学习能力的学生往往难以理解和参与分析反馈。对话式生成AI助手通过实时、个性化、对话式支持显示出支持这一过程的潜力。

Method: 在10周学期期间，将生成AI助手整合到学习分析仪表板中，收集和分析学生与AI助手之间的真实对话。分析重点包括：不同SRL水平学生提出的问题类型、助手回答的相关性和质量，以及学生对助手在学习中作用的感知。

Result: 发现明显的查询模式差异：低SRL学生寻求澄清和保证，高SRL学生询问技术方面并请求个性化策略。AI助手提供清晰可靠解释但在个性化、处理情感化查询和整合多个数据点进行定制响应方面有限。生成AI干预对低SRL学生特别有价值，提供支持反馈参与的脚手架，缩小与高SRL同伴的差距。学生反思强调信任、更大适应性、上下文感知和技术完善的重要性。

Conclusion: 对话式生成AI助手在学习分析仪表板中具有支持学生自我调节学习的潜力，特别是对低SRL学生。但需要改进系统的个性化能力、情感处理、数据整合以及建立信任和适应性，未来系统需要更多上下文感知和技术完善。
...
### [219] [Conversational AI for Rapid Scientific Prototyping: A Case Study on ESA's ELOPE Competition](https://arxiv.org/abs/2601.04920)
*Nils Einecke*

Main category: cs.AI

TL;DR: 本文通过ESA的ELOPE竞赛案例研究，展示了ChatGPT在科学发现中作为编程伙伴的潜力，帮助团队在竞赛中获得第二名，同时分析了AI在快速原型开发中的优势和局限。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型越来越多地被用作编程伙伴，但它们在加速科学发现方面的作用仍未得到充分探索。本文旨在通过具体案例研究，探讨人类与AI协作在竞争性科学环境中的潜力。

Method: 采用案例研究方法，在ESA的ELOPE竞赛中使用ChatGPT进行快速原型开发。ChatGPT不仅提供可执行代码，还贡献算法推理、数据处理例程和方法论建议，如使用固定事件数而非固定时间跨度进行窗口化处理。

Result: 尽管加入竞赛较晚，团队仍获得第二名，得分为0.01282。研究同时观察到ChatGPT的局限性：经常引入不必要的结构变化、在中间讨论中混淆、偶尔产生关键错误，以及在较长科学讨论中遗忘重要方面。

Conclusion: 对话式AI既能加速开发，又能支持科学研究中的概念洞察。通过分析这些优势和不足，本文主张将LLMs结构化整合到科学工作流程中，并提出AI辅助科学工作的最佳实践建议。
...
### [220] [T-Retriever: Tree-based Hierarchical Retrieval Augmented Generation for Textual Graphs](https://arxiv.org/abs/2601.04945)
*Chunyu Wei,Huaiyu Qin,Siyuan He,Yunhai Wang,Yueguo Chen*

Main category: cs.AI

TL;DR: T-Retriever：一种新的图检索增强生成框架，通过语义和结构引导的编码树将属性图检索重新表述为基于树的检索，解决了现有图RAG方法在层次信息管理中的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前基于图的RAG方法在处理层次信息时存在两个关键限制：1）强制的层特定压缩配额会破坏局部图结构；2）优先考虑拓扑结构而忽略语义内容。需要一种能够同时考虑结构和语义的层次信息管理方法。
...
Conclusion: T-Retriever通过将属性图检索重新表述为基于树的检索，并引入自适应压缩编码和语义结构熵，有效解决了现有图RAG方法在层次信息管理中的局限性，提高了检索质量和响应相关性。
...
### [221] [ConMax: Confidence-Maximizing Compression for Efficient Chain-of-Thought Reasoning](https://arxiv.org/abs/2601.04973)
*Minda Hu,Zexuan Qiu,Zenan Xu,Kun Li,Bo Zhou,Irwin King*

Main category: cs.AI

TL;DR: ConMax是一个强化学习框架，通过最大化置信度来自动压缩推理轨迹，在保持推理准确性的同时显著减少计算成本。
...
Method: ConMax采用强化学习框架，将压缩问题转化为奖励驱动的优化问题。训练一个策略来修剪冗余，通过最大化加权组合的答案置信度（预测保真度）和思维置信度（推理有效性），利用冻结的辅助LRM进行评估。

Result: 在五个推理数据集上的实验表明，ConMax实现了优越的效率-性能权衡。相比强基线，推理长度减少了43%，而准确性仅下降0.7%，证明其能生成高质量、高效的LRM训练数据。

Conclusion: ConMax框架能够有效压缩推理轨迹，在保持推理模式的同时显著提高效率，为大型推理模型提供了高质量的训练数据生成方法。
...
### [222] [AlgBench: To What Extent Do Large Reasoning Models Understand Algorithms?](https://arxiv.org/abs/2601.04996)
*Henan Sun,Kaichi Yu,Yuyao Wang,Bowen Liu,Xunkai Li,Rong-Hua Li,Nuo Chen,Jia Li*

Main category: cs.AI

TL;DR: AlgBench是一个由ACM算法专家构建的算法推理基准测试，包含3000多个问题覆盖27种算法，评估发现大型推理模型在非优化任务上表现良好（92%），但在动态规划等全局优化算法上准确率骤降至49%，揭示了模型存在"战略过度转移"问题。


<details>
  <summary>Details</summary>
Motivation: 现有算法推理基准测试有限，无法回答大型推理模型是否真正掌握算法推理能力的问题。需要构建一个专家策划的算法中心基准来全面评估模型的算法推理能力。

Method: 提出AlgBench基准测试，由ACM算法专家构建超过3000个原始问题，涵盖27种算法，采用全面的分类体系（欧几里得结构、非欧几里得结构、非优化、局部优化、全局优化、启发式优化等类别）。在领先的大型推理模型（如Gemini-3-Pro、DeepSeek-v3.2-Speciale和GPT-o3）上进行实证评估。

Result: 评估显示显著的性能异质性：模型在非优化任务上表现良好（高达92%），但在动态规划等全局优化算法上准确率急剧下降至约49%。分析发现存在"战略过度转移"现象，即模型由于必要的低熵标记而过早放弃正确的算法设计。

Conclusion: 这些发现揭示了问题中心强化学习的基本局限性，并强调了采用算法中心训练范式对于实现稳健算法推理的必要性。AlgBench为评估大型推理模型的算法推理能力提供了重要基准。
...
### [223] [An Empirical Investigation of Robustness in Large Language Models under Tabular Distortions](https://arxiv.org/abs/2601.05009)
*Avik Dutta,Harshit Nigam,Hosein Hasanbeig,Arjun Radhakrishna,Sumit Gulwani*

Main category: cs.AI

TL;DR: LLMs缺乏检测和纠正表格数据语义和结构扭曲的内在能力，即使提供系统提示也只能部分调整推理策略，GPT-5.2等先进模型在扭曲条件下准确率至少下降22%。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在处理具有语义和结构扭曲的表格数据时的失败情况，探索模型是否具备检测和纠正表格表示中微妙扭曲的内在能力。

Method: 引入一个专家策划的小型数据集，专门评估LLMs在表格问答任务中的表现，这些任务需要在分析前进行额外的错误纠正步骤。通过系统提示提供显式先验，观察模型如何调整推理策略。

Result: LLMs缺乏检测和纠正表格扭曲的内在能力；只有在提供显式系统提示时，模型才能部分调整推理策略并纠正一些扭曲，但效果不一致也不完整；即使是GPT-5.2等最先进模型，在扭曲条件下准确率也至少下降22%。

Conclusion: LLMs在摄入和解释扭曲表格信息方面存在系统性差异，这引发了关于模型何时以及如何自主决定重新对齐表格输入的重要研究问题，类似于人类行为，而不依赖显式提示或表格数据预处理。
...
### [224] [OptiSet: Unified Optimizing Set Selection and Ranking for Retrieval-Augmented Generation](https://arxiv.org/abs/2601.05027)
*Yi Jiang,Sendong Zhao,Jianbo Li,Bairui Hu,Yanrui Du,Haochun Wang,Bing Qin*

Main category: cs.AI

TL;DR: OptiSet是一个基于集合的检索增强生成框架，通过扩展查询视角、重新选择候选集、自合成策略和集合列表训练，解决了传统RAG方法中静态选择top-k段落导致的冗余和组合增益不足问题。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法通常基于个体相关性静态选择top-k段落，这种方法无法利用段落间的组合增益，并且经常引入大量冗余，限制了生成质量和效率。

Method: OptiSet采用"扩展-精炼"范式：1)将查询扩展为多个视角以获得多样化候选池；2)通过重新选择形成紧凑证据集；3)设计自合成策略从生成器的集合条件效用变化中推导偏好标签；4)引入集合列表训练策略联合优化集合选择和集合级排序。

Result: 实验表明OptiSet在复杂组合问题上提升了性能，使生成更加高效，能够识别互补和冗余证据，形成紧凑、高增益的证据集。

Conclusion: OptiSet通过统一的集合中心框架有效解决了传统RAG方法的局限性，实现了更好的组合增益利用和冗余减少，提高了生成质量和效率。
...
### [225] [How to Set the Batch Size for Large-Scale Pre-training?](https://arxiv.org/abs/2601.05034)
...
Method: 为WSD调度器推导了修正的E(S)关系，表征预训练中训练数据消耗E与步数S之间的权衡；理论分析揭示了WSD预训练的两个基本属性：达到目标损失所需的最小批大小阈值B_min，以及通过最小化总token数来最大化数据效率的最优批大小B_opt；基于这些属性提出了动态批大小调度器。

Result: 大量实验表明，修正后的公式精确捕捉了大尺度预训练的动力学特性，由此产生的调度策略显著提升了训练效率和最终模型质量。

Conclusion: 论文成功为WSD调度器下的预训练建立了新的理论框架，提出了实用的动态批大小调度方法，解决了原有临界批大小理论在新范式下的不适用问题，为高效大规模预训练提供了理论指导和实践工具。
...
### [226] [Learning Latent Action World Models In The Wild](https://arxiv.org/abs/2601.05230)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的智能体需要预测行动后果的能力，但传统世界模型需要动作标签，这在规模上难以获取。因此需要从视频中学习潜在动作模型，但现有研究主要关注简单的机器人模拟、视频游戏或操作数据，无法处理野外视频的复杂性。

Method: 提出学习潜在动作世界模型的方法，处理野外视频的多样性挑战。采用连续但受约束的潜在动作表示，而非常见的向量量化方法。讨论了动作应遵循的属性、相关架构选择和评估方法，并训练控制器将已知动作映射到潜在动作。

Result: 连续约束的潜在动作能够捕捉野外视频中动作的复杂性，而向量量化方法则不能。研究发现环境变化（如人类进入房间）可以在视频间传递。虽然缺乏共同体现，但主要学习到相对于相机空间定位的潜在动作。控制器能够将已知动作映射到潜在动作，使潜在动作作为通用接口，在规划任务中达到与动作条件基线相似的性能。

Conclusion: 该研究为将潜在动作模型扩展到现实世界提供了重要步骤，展示了从野外视频中学习复杂动作的可行性，以及潜在动作作为通用接口在规划任务中的应用潜力。
...
### [227] [How to Set the Learning Rate for Large-Scale Pre-training?](https://arxiv.org/abs/2601.05049)
*Yunhua Zhou,Shuhao Xing,Junhao Huang,Xipeng Qiu,Qipeng Guo*

Main category: cs.AI

TL;DR: 本文研究了大规模预训练中学习率优化问题，提出了拟合和迁移两种研究范式，通过缩放定律降低搜索复杂度，并挑战了μTransfer在大规模场景下的可扩展性。
...
Result: 实证结果表明，广泛采用的μTransfer在大规模预训练场景下的可扩展性受到挑战。通过训练稳定性和特征学习的双重分析，解释了模块级参数调优在大规模设置中表现不佳的原因。

Conclusion: 为工业级预训练优化提供了系统的实践指南和新的理论视角，挑战了现有超参数研究的规模边界。
...
### [228] [Large language models can effectively convince people to believe conspiracies](https://arxiv.org/abs/2601.05050)
...
TL;DR: GPT-4o在说服力方面既能有效减少也能增加阴谋论信念，即使有防护措施也难以阻止其传播虚假信息，但通过特定提示可以显著降低其传播虚假信息的能力。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究大型语言模型（LLMs）的说服力是否更倾向于支持真相而非虚假信息，或者LLMs是否同样容易促进错误信念的传播。

Method: 通过三个预注册实验，让2,724名美国参与者与GPT-4o讨论他们不确定的阴谋论，模型被指示要么反驳（"揭穿"）要么支持（"鼓吹"）该阴谋论。使用了移除防护措施的"越狱"版GPT-4o和标准版GPT-4o进行对比。

Result: 1. 越狱版GPT-4o在增加阴谋论信念方面与减少信念同样有效；2. 支持阴谋论的AI获得更积极的评价，并增加了对AI的信任；3. 标准版GPT-4o产生类似效果，OpenAI的防护措施几乎无法阻止LLM促进阴谋论信念；4. 纠正性对话可以逆转新诱导的阴谋论信念；5. 提示GPT-4o仅使用准确信息能显著降低其增加阴谋论信念的能力。

Conclusion: LLMs具有促进真相和虚假信息的强大能力，但存在潜在的解决方案可以帮助减轻这种风险，包括使用纠正性对话和特定提示策略。
...
### [229] [Publishing FAIR and Machine-actionable Reviews in Materials Science: The Case for Symbolic Knowledge in Neuro-symbolic Artificial Intelligence](https://arxiv.org/abs/2601.05051)
...
TL;DR: 将材料科学综述从静态PDF表格转化为FAIR、机器可操作的ORKG结构化知识，对比符号查询与LLM查询，主张符号层应作为神经符号AI的可靠基础
...
Result: 成功将综述表格转化为结构化、可查询的ORKG知识，展示了符号查询与LLM查询的对比，验证了结构化知识表示的价值

Conclusion: 在材料科学中，经过策划的符号层应作为可靠神经符号AI的骨干，LLM应作为补充性的、符号基础接口，而非独立的真理来源
...
### [230] [Reinforced Efficient Reasoning via Semantically Diverse Exploration](https://arxiv.org/abs/2601.05053)
...
TL;DR: ROSE是一种基于强化学习的语言模型推理方法，通过语义多样性探索和长度感知优势估计，提高数学推理任务的效率和多样性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于蒙特卡洛树搜索的强化学习方法在语言模型推理中存在探索多样性有限和推理效率低下的问题，需要新的方法来鼓励更多样化的推理探索并提高效率。

Method: ROSE方法包含三个关键技术：1）基于语义熵的分支策略，从已采样的推理路径中捕捉语义不确定性，选择语义分歧大的分支点生成新推理路径；2）ε-探索机制，从根节点随机启动推理路径，防止搜索过程过于局部化；3）长度感知的片段级优势估计器，奖励简洁正确的推理，惩罚不必要的长推理链。

Result: 在多个数学推理基准测试中使用Qwen和Llama模型进行的广泛实验验证了ROSE方法的有效性和效率。

Conclusion: ROSE通过语义多样性探索和高效推理机制，成功解决了现有强化学习方法在语言模型推理中的局限性，为LLM的推理能力提升提供了有效解决方案。
...
### [231] [Chain-of-Sanitized-Thoughts: Plugging PII Leakage in CoT of Large Reasoning Models](https://arxiv.org/abs/2601.05076)
*Arghyadeep Das,Sai Sreenivas Chintha,Rishiraj Girmal,Kinjal Pandey,Sharvi Endait*

Main category: cs.AI

TL;DR: 大型推理模型通过生成显式思维链提高性能，但中间推理会泄露个人身份信息。研究通过可部署干预实现隐私优先推理，引入PII-CoT-Bench数据集和评估基准，发现不同能力模型需要不同隐私控制方法，都能显著减少PII泄露且保持性能。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型生成显式思维链推理提高了性能、可靠性和可解释性，但这种透明度带来了严重的隐私风险：即使最终答案经过清理，中间推理过程仍可能泄露个人身份信息。需要研究如何在不暴露敏感信息的情况下进行推理。

Method: 引入PII-CoT-Bench数据集，包含隐私感知的思维链标注，以及类别平衡的评估基准，涵盖现实和对抗性泄露场景。研究两种方法：基于提示的控制和微调方法，分析不同能力模型的最有效隐私保护策略。

Result: 发现能力依赖趋势：最先进模型从基于提示的控制中获益最大，而较弱模型需要微调才能实现有意义的泄露减少。两种方法都能显著减少PII暴露，同时保持最小的性能下降，证明私有推理可以在不牺牲性能的情况下实现。

Conclusion: 私有思维链推理可以在最小效用损失的情况下实现，为构建隐私保护推理系统提供了实用指导。研究表明隐私优先推理是可行的，且不同能力模型需要不同的隐私控制策略。
...
### [232] [Arabic Prompts with English Tools: A Benchmark](https://arxiv.org/abs/2601.05101)
*Konstantin Kubrak,Ahmed El-Moselhy,Ammar Alsulami,Remaz Altuwaim,Hassan Ismail Fawaz,Faisal Alsaby*

Main category: cs.AI

TL;DR: 本文介绍了首个专门评估阿拉伯语LLM工具调用和智能体能力的基准测试，发现阿拉伯语交互时工具调用准确率平均下降5-10%。


<details>
  <summary>Details</summary>
Motivation: 随着阿拉伯语原生LLM的快速发展，评估其能力的基准测试相对滞后，现有框架主要关注英语。工具调用这一关键领域在非英语语言（如阿拉伯语）中的表现缺乏研究，而这些模型通常主要在英语数据上预训练。

Method: 开发了首个专门用于评估阿拉伯语LLM工具调用和智能体能力的基准测试，提供了一个标准化框架来衡量模型在阿拉伯语智能体工作流程中的功能准确性和鲁棒性。

Result: 发现了一个巨大的性能差距：当用户使用阿拉伯语交互时，无论工具描述本身是阿拉伯语还是英语，工具调用准确率平均下降5-10%。
...
### [233] [Token-Level LLM Collaboration via FusionRoute](https://arxiv.org/abs/2601.05106)
*Nuoya Xiong,Yuhang Zhou,Hanqing Zeng,Zhaorun Chen,Furong Huang,Shuchao Bi,Lizhu Zhang,Zhuokai Zhao*

Main category: cs.AI

TL;DR: FusionRoute是一个轻量级路由框架，通过token级别的多LLM协作，在解码时选择最合适的专家模型并补充修正其输出，解决了通用大模型成本高而专用小模型泛化差的问题。
...
Method: 提出FusionRoute框架，包含轻量级路由器，在解码的每个步骤同时执行：(1)选择最合适的专家模型；(2)通过logit加法提供补充logit来修正所选专家的下一个token分布。与现有仅依赖固定专家输出的方法不同，该方法结合了专家选择和可训练的补充生成器。

Result: 在Llama-3和Gemma-2系列模型以及涵盖数学推理、代码生成和指令遵循的多样化基准测试中，FusionRoute优于序列级和token级协作、模型融合和直接微调方法，同时在各自任务上与领域专家模型保持竞争力。

Conclusion: FusionRoute通过理论分析证明纯专家路由存在根本限制，而结合专家选择和补充生成器的方法能够扩展有效策略类别，在温和条件下恢复最优值函数，为解决LLM的效率与泛化权衡提供了有效方案。
...
### [234] [GlimpRouter: Efficient Collaborative Inference by Glimpsing One Token of Thoughts](https://arxiv.org/abs/2601.05110)
*Wenhao Zeng,Xuteng Zhang,Yuling Shi,Chao Hu,Yuting Chen,Beijun Shen,Xiaodong Gu*

Main category: cs.AI

TL;DR: GlimpRouter：基于首个token熵值预测推理步骤难度，实现轻量模型与大模型间的训练自由协作推理框架，显著降低推理延迟并保持准确性。
...
Method: 提出GlimpRouter训练自由步进协作框架，基于"顿悟时刻"现象发现推理步骤难度可以从其第一个token的熵值推断。使用轻量模型仅生成每个推理步骤的第一个token，当初始token熵超过阈值时，将该步骤路由到大模型处理。

Result: 在多个基准测试中，GlimpRouter显著降低推理延迟同时保持准确性。例如在AIME25上，相比独立大模型，实现了10.7%的准确率提升和25.9%的推理延迟降低。

Conclusion: 首个token熵值可作为推理步骤难度的强预测指标，基于"思维一瞥"而非全步骤评估的计算分配机制是简单有效的推理方法，为高效协作推理提供了新视角。
...
### [235] [Evaluative Fingerprints: Stable and Systematic Differences in LLM Evaluator Behavior](https://arxiv.org/abs/2601.05114)
*Wajid Nasser*

Main category: cs.AI

TL;DR: 研究发现LLM评估系统存在可靠性悖论：各评估者内部高度一致但彼此间几乎无共识，评估者间的分歧模式稳定到可作为指纹识别，每个评估者都实施着独特且稳定的质量理论。


<details>
  <summary>Details</summary>
Motivation: 研究旨在检验LLM-as-judge系统的评估一致性，发现虽然每个LLM评估者内部评分一致，但不同评估者之间缺乏共识，这挑战了LLM评估系统作为可扩展、一致评估工具的基本假设。

Method: 通过3,240次评估（9个评估者×120个独特视频×打包项目×3次独立运行），使用Krippendorff's α测量评估者间一致性，构建分类器从评分特征识别评估者身份，并沿多个维度（严格度/宽松度、维度强调、内部稳定性、证据行为等）表征评估者的"评估倾向"。

Result: 评估者间一致性接近零（α=0.042），两个维度上分歧甚至超过随机噪声预测（α<0）。分类器仅从评分特征就能以77.1%准确率识别评估者，加入倾向特征后达89.9%。同一模型家族内信号更强：GPT-4.1和GPT-5.2可99.6%准确区分。

Conclusion: LLM评估者并非测量共享构念的可互换工具，而是各自编码着独特质量理论的测量设备。平均其分数会产生不符合任何评估者实际价值观的综合判断，这揭示了LLM评估系统的根本局限性。
...
### [236] [Distilling the Thought, Watermarking the Answer: A Principle Semantic Guided Watermark for Large Reasoning Models](https://arxiv.org/abs/2601.05144)
*Shuliang Liu,Xingyu Li,Hongyi Liu,Yibo Yan,Bingchen Duan,Qi Zheng,Dong Fang,Lingfeng Su,Xuming Hu*

Main category: cs.AI

TL;DR: ReasonMark是一个专门为推理密集型大语言模型设计的数字水印框架，通过分离思考阶段和回答阶段，使用关键性评分识别语义关键标记，实现高效水印嵌入而不破坏逻辑连贯性。


<details>
  <summary>Details</summary>
Motivation: 现有的数字水印方法在处理推理大语言模型时存在两个主要问题：基于标记的水印技术会破坏推理流程的逻辑连贯性，而语义感知方法虽然提高了质量但引入了显著的延迟或需要辅助模型。因此需要一种专门针对推理密集型LLM的水印框架。

Method: ReasonMark将生成过程解耦为不受干扰的思考阶段和加水印的回答阶段。提出关键性评分来识别推理轨迹中的语义关键标记，将其提炼为主语义向量。该向量指导基于标记-PSV对齐的语义自适应机制，根据语义重要性调整水印强度。

Result: 实验表明ReasonMark优于现有方法：文本困惑度降低0.35，翻译BLEU分数提高0.164，数学准确性提升0.67分。同时水印检测AUC提高0.34%，对抗攻击的鲁棒性更强，延迟增加可忽略不计。

Conclusion: ReasonMark框架能够在保持推理大语言模型逻辑完整性的同时实现高效水印嵌入，为推理LLM在实际应用中的可追溯和可信部署提供了解决方案。
...
### [237] [Observations and Remedies for Large Language Model Bias in Self-Consuming Performative Loop](https://arxiv.org/abs/2601.05184)
*Yaxuan Wang,Zhongteng Cai,Yujia Bao,Xueru Zhang,Yang Liu*

Main category: cs.AI

TL;DR: 该研究提出了自消耗执行循环（SCPL）概念，探讨在受控执行反馈下，合成数据在动态迭代训练过程中如何塑造偏见，发现执行循环会增加偏好偏见但减少差异偏见，并设计了基于奖励的拒绝采样策略来缓解偏见。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的快速发展导致使用合成数据训练未来模型的兴趣增长，但这形成了自消耗的再训练循环，可能导致性能下降和偏见出现。在现实应用中，先前部署的LLM可能影响其生成的数据，形成由用户反馈驱动的动态系统。由于无法获取动态生产系统中的真实用户偏好数据，需要建立受控环境来隔离和分析反馈驱动的偏见演化。

Method: 引入自消耗执行循环（SCPL）概念，在受控执行反馈设置下研究合成数据在动态迭代训练中如何塑造偏见。研究聚焦两种循环：典型的再训练设置和增量微调设置。通过三个真实世界任务的实验，分析偏见演化，并设计基于奖励的拒绝采样策略来缓解偏见。

Result: 研究发现执行循环会增加偏好偏见但减少差异偏见。通过实验验证了在动态迭代训练过程中偏见的演化模式，并证明基于奖励的拒绝采样策略能够有效缓解偏见，推动构建更可信的自改进系统。

Conclusion: 自消耗执行循环在动态迭代训练中会塑造不同类型的偏见，需要设计专门的缓解策略。基于奖励的拒绝采样方法能够有效减少偏见，为构建更可信的自改进系统提供了可行路径。该研究为理解合成数据在LLM训练中的偏见演化机制提供了理论框架和实证基础。
...
### [238] [MineNPC-Task: Task Suite for Memory-Aware Minecraft Agents](https://arxiv.org/abs/2601.05215)
*Tamil Sudaravan Mohan Doss,Michael Xu,Sudha Rao,Andrew D. Wilson,Balasaravanan Thoravi Kumaravel*

Main category: cs.AI

TL;DR: MineNPC-Task是一个用户创作的基准测试框架，用于在开放世界《我的世界》中评估具有记忆感知和混合主动性的LLM智能体，通过真实玩家互动收集任务而非合成提示。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法通常依赖合成提示，缺乏真实玩家互动产生的任务，需要建立基于真实游戏体验、具有明确依赖结构和可验证性的基准测试框架来评估记忆感知的具身智能体。

Method: 通过专家玩家的形成性和总结性协同游戏收集任务，将其规范化为参数化模板，包含明确前提条件和依赖结构，配合机器可检查的验证器，在禁止使用游戏外捷径的知识边界政策下运行。

Result: 使用GPT-4o评估了8位经验玩家的216个子任务，观察到代码执行、库存/工具处理、引用和导航方面的重复故障模式，同时发现混合主动性澄清和轻量级记忆支持下的恢复能力。
