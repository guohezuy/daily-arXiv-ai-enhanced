...
TL;DR: 本文研究了多模态视觉语言模型中的成员推理攻击隐私风险，提出了一种神经科学启发的拓扑正则化框架来增强模型对隐私攻击的抵御能力，在保持模型效用的同时显著降低了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的广泛应用，其训练数据隐私泄露风险日益凸显。现有研究主要关注单模态系统的隐私攻击，而多模态模型对隐私攻击的脆弱性及其防御机制尚待探索。

Method: 提出神经科学启发的拓扑正则化框架(tau)，在三种视觉语言模型(BLIP、PaliGemma 2、ViT-GPT2)上应用该正则化，并在三个基准数据集(COCO、CC3M、NoCaps)上评估其对成员推理攻击的抵御能力。

Result: 实验结果显示，在BLIP模型上，神经增强版本将成员推理攻击成功率降低了24%(平均ROC-AUC)，同时保持了相似的模型效用(MPNet和ROUGE-2指标)。其他模型和数据集上的评估进一步验证了该方法的有效性。

Conclusion: 神经增强的多模态视觉语言模型在保持模型性能的同时，显著提高了对隐私攻击的抵御能力，为多模态模型的隐私保护提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 资源受限设备（如交通摄像头）上的视频对象识别面临挑战，需要平衡计算效率和识别精度。移动边缘计算提供了将计算密集型对象检测卸载到边缘服务器的可能性，但需要解决何时进行边缘检测与本地跟踪的决策问题。

Method: 提出了LTED-Ada算法，基于深度强化学习自适应选择本地跟踪和边缘检测；在多设备场景下使用联邦学习进行协作策略训练；通过硬件在环实验验证算法性能。

Result: 在多个树莓派4B设备和PC边缘服务器上的实验表明，LTED-Ada算法在帧率、识别精度和延迟要求方面表现出优越性能。

Conclusion: LTED-Ada算法通过智能的检测-跟踪决策机制，有效解决了资源受限设备上的视频对象识别问题，在多设备场景下通过联邦学习进一步提升了泛化能力。
...
TL;DR: DeeAD是一个无需训练的动作引导早期退出框架，通过评估中间轨迹的物理可行性来加速VLA规划模型的推理速度。它基于轻量级规划先验（如导航或低精度规划）来终止推理，并引入多跳控制器自适应跳过冗余层。


<details>
  <summary>Details</summary>
Motivation: 现有的Vision-Language Action (VLA)模型在自动驾驶中统一了感知、推理和轨迹生成，但由于深度transformer堆栈导致显著的推理延迟。

Method: 提出DeeAD框架，不依赖置信度分数，而是在预测轨迹与轻量级规划先验（导航或低精度规划）的偏差小于2米时终止推理。引入多跳控制器自适应跳过冗余层，无需重新训练即可集成到现有VLA模型中。

Result: 在Bench2Drive基准测试中，实现了高达28%的transformer层稀疏性和29%的延迟减少，同时保持了规划质量和安全性。

Conclusion: DeeAD框架有效加速了VLA规划模型的推理速度，显著减少延迟，同时不损害规划性能，为自动驾驶系统的实时性提供了解决方案。
...
TL;DR: DinoLizer是基于DINOv2的模型，用于定位生成式修复中的篡改区域。通过在DINOv2上添加线性分类头，在14×14补丁分辨率下预测篡改，使用滑动窗口策略处理大图像，在多个修复数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决生成式修复中篡改区域定位的挑战，利用DINOv2的强大表示能力来检测语义层面的篡改，同时忽略非语义编辑。

Method: 在预训练的DINOv2模型上添加线性分类头，使用滑动窗口策略处理任意尺寸图像，通过后处理细化二进制篡改掩码。

Result: 在多个生成模型修复数据集上超越最先进的局部篡改检测器，平均IoU比次优模型高12%，对常见后处理操作保持鲁棒性。

Conclusion: DinoLizer证明了Vision Transformers在篡改定位任务中的强大表示能力，消融研究确认了其相对于DINOv3的优越性。
...
TL;DR: CANVAS是一个用于评估视觉语言模型在工具调用式用户界面设计能力的基准，包含598个基于工具的设计任务，涵盖设计复制和设计修改两种任务类型。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估视觉语言模型在工具调用式UI设计性能的基准，无法了解这些模型在传统设计软件中与设计师协作的潜力。

Method: 构建CANVAS基准，包含从3.3K个移动UI设计中采样的598个工具设计任务，涵盖30个功能类别，通过上下文工具调用逐步更新设计。

Result: 领先模型展现出更策略性的工具调用，提高了设计质量，同时识别出模型常见的错误模式。

Conclusion: CANVAS基准为评估和提升基于工具的设计能力提供了指导，揭示了视觉语言模型在UI设计中的潜力。
...
TL;DR: 本文提出了文本引导的语义图像编码器（TIE），通过在图像编码过程中引入文本查询条件，使视觉语言模型能够根据具体任务生成更具针对性的图像表示，显著提升了多个基准测试的性能和推理效率。


<details>
  <summary>Details</summary>
Motivation: 传统的视觉语言模型中，图像编码器通常独立预训练后再与语言模型对齐，这种范式导致编码器处理图像时无法考虑具体的下游任务或文本查询，限制了模型的性能表现。

Method: 提出了文本引导的语义图像编码器（TIE），通过将输入文本查询作为条件信息，生成与查询相关的图像表示，使图像编码过程能够针对特定任务进行优化。

Result: 在1B和3B规模的九个图像到文本基准测试中，使用TIE的视觉语言模型分别平均提升了1.5和1.3个点，在DocVQA和InfoVQA等任务上提升高达6个点，同时仅使用一半的图像分块（token）就实现了更好的性能，显著提高了推理效率。

Conclusion: 文本引导的训练有效优化了编码器以捕捉关键视觉特征，TIE能够持续关注查询相关区域，增强了模型的可解释性和查询特定的基础能力，表明文本条件化训练是提升视觉语言模型性能的有效方法。
...
TL;DR: Δ-NeRF是一个模块化残差框架，用于增量式神经辐射场精炼，通过残差控制器、不确定性感知门控机制和视图选择策略，实现在不访问过去数据的情况下进行高效精炼，同时通过知识蒸馏压缩模型。
...
Method: 提出Δ-NeRF框架，包括：残差控制器注入逐层修正到冻结的基础NeRF；不确定性感知门控机制自适应结合基础和精炼预测；视图选择策略减少训练数据；知识蒸馏压缩模型。

Result: 在卫星图像上，Δ-NeRF性能与联合训练相当，训练时间减少30-42%，PSNR比朴素微调提升43.5%，在某些指标上超越联合训练。

Conclusion: Δ-NeRF提供了一种高效的增量NeRF精炼方法，解决了灾难性遗忘问题，在保持性能的同时显著减少训练时间和数据需求。
...
Method: 将未标注视频分割为动态前景和背景层，通过自组合学习主体与场景交互；采用变换感知训练流程，包含多层融合和增强实现可感知组合，以及身份保持损失确保前景保真度。
...
TL;DR: Sphinx是一个针对视觉感知和推理核心认知原语的合成环境，通过程序化生成包含图案、图块、图表、图标和几何原语的谜题，提供可验证的真实解，用于精确评估和大规模数据集构建。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够精确评估和提升多模态推理能力的基准测试环境，特别是针对视觉感知和推理的核心认知能力。

Method: 使用程序化生成方法创建包含25种任务类型的谜题，涵盖对称检测、几何变换、空间推理、图表解释和序列预测等，并采用强化学习与可验证奖励（RLVR）来提升模型性能。

Result: 评估显示最先进的GPT-5模型在Sphinx基准上仅达到51.1%的准确率，远低于人类表现；而RLVR方法显著提高了模型在这些任务上的准确性，并在外部视觉推理基准上获得收益。

Conclusion: Sphinx基准有效揭示了当前LVLMs在视觉推理方面的局限性，RLVR方法展示了提升多模态推理能力的潜力，为推进该领域发展提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 在临床诊断、治疗规划和手术导航中，准确检测血管树中心线并保持正确的树状拓扑结构至关重要。保持高召回率尤为重要，因为遗漏小分支可能导致致命的评估不完整或未检测到异常。

Method: 采用Producer-Refiner架构，基于Transformer解码器。Producer提出初始汇合轨迹，Refiner循环精炼这些轨迹生成最终轨迹形成中心线图。汇合轨迹表示法能够在精炼完整轨迹时明确强制执行有效的树状拓扑结构。

Result: 在多个公共中心线数据集上，RefTr实现了优于先前最先进方法的召回率和相当的精密度，同时提供更快的推理速度和显著减少的参数数量（解码器参数减少2.4倍）。

Conclusion: RefTr展示了作为3D医学成像中血管树分析的新最先进框架的潜力，在保持高召回率的同时实现了参数效率和推理速度的显著提升。
...
TL;DR: 提出了首个高分辨率（5472×3648px）立体DSLR数据集，包含18000张图像，系统性地变化焦距和光圈，捕捉专业相机系统的光学真实性和复杂性，用于深度估计、景深渲染等任务。


<details>
  <summary>Details</summary>
Motivation: 解决真实光学条件下可靠深度估计的挑战，填补大规模高保真实立体DSLR数据集的空白，缩小合成训练数据与真实相机光学之间的真实感差距。

Method: 使用两个相同的相机组件，在9个不同场景中捕获图像，涵盖10个焦距（28-70mm）和5个光圈（f/2.8-f/22），共50种光学配置，每个场景2000张图像。

Result: 创建了包含挑战性视觉元素（如多尺度光学幻觉、反射表面、透明玻璃墙等）的数据集，支持几何和光学效应的受控分析。

Conclusion: 该数据集有助于弥合合成训练数据与真实相机光学之间的真实感差距，展示了当前最先进方法在真实世界光学泛化方面的挑战，并发布了数据集、校准文件和评估代码以支持可重复研究。
...
### [13] [Privacy-Preserving Federated Vision Transformer Learning Leveraging Lightweight Homomorphic Encryption in Medical AI](https://arxiv.org/abs/2511.20983)
*Al Amin,Kamrul Hasan,Liang Hong,Sharif Ullah*

Main category: cs.CV

TL;DR: 提出结合Vision Transformers和同态加密的隐私保护联邦学习框架，用于多机构病理学分类，通过加密CLS令牌实现30倍通信减少，同时防止模型反演攻击。
...
Method: 使用Vision Transformers的CLS令牌作为768维特征表示，采用CKKS同态加密进行安全聚合，在传输到服务器前加密这些令牌。

Result: 梯度极易受模型反演攻击（PSNR: 52.26 dB, SSIM: 0.999, NMI: 0.741），而提出的CLS保护HE方法能防止此类攻击，每轮聚合仅需326 KB加密数据传输。在未加密域达到96.12%分类准确率，加密域达到90.02%。

Conclusion: 该方法在保持强隐私保证的同时显著减少通信开销，为医疗联邦学习提供安全有效的解决方案。
...
### [15] [GuardTrace-VL: Detecting Unsafe Multimodel Reasoning via Iterative Safety Supervision](https://arxiv.org/abs/2511.20994)
*Yuxiao Xiang,Junchi Chen,Zhenchao Jin,Changtao Miao,Haojie Yuan,Qi Chu,Tao Gong,Nenghai Yu*

Main category: cs.CV

TL;DR: GuardTrace-VL是一个视觉感知的安全审计器，通过联合图像-文本分析监控完整的问答-思考-回答流程，能够在推理阶段检测不安全内容。该方法在安全推理检测任务上达到93.1%的F1分数，比之前最好的多模态安全防御方法提升了13.5%。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态安全防护主要只评估输入问题和最终答案，忽略了中间推理过程。这种疏忽使得偏见推断或违反政策的视觉上下文使用等未被检测的危害在推理过程中出现。

Method: 提出了GuardTrace-VL，通过联合图像-文本分析监控完整的问答-思考-回答流程；构建了GuardTrace数据集，通过多样化提示策略生成，并经过MLRM和人工投票验证流程精炼；提出了三阶段渐进训练方案结合数据精炼过程。

Result: 在提出的测试集上，GuardTrace-VL模型在不安全推理检测任务上达到93.1%的F1分数，比之前最强的多模态安全防御方法提升了13.5%。

Conclusion: GuardTrace-VL能够有效检测推理过程中出现的不安全内容，填补了现有安全防护在中间推理过程监控方面的空白，显著提升了多模态安全防御能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法顺序估计雾模型参数容易导致误差传播，且无法有效处理真实世界中全局不均匀的雾况，需要一种能同时估计所有参数并能适应真实雾况的方法。

Method: 通过求解新的优化问题同时估计所有雾模型参数，假设雾仅在局部均匀，从而有效处理全局不均匀的真实雾。该方法可作为现有视觉SLAM或里程计系统的附加模块。

Result: 在合成雾天数据和真实雾天序列SDIRF数据集上的实验表明，该方法不仅在最准确估计上优于先前方法，而且能更好地适应真实雾况。

Conclusion: 提出的同时参数估计方法在雾天视觉感知中具有优越性能，发布的SDIRF数据集和代码将推动雾天视觉感知研究的发展。
...
<details>
  <summary>Details</summary>
Motivation: 跨视角物体对应任务（如ego-exo物体对应）由于视角和外观的剧烈变化而具有挑战性，现有分割模型如SAM2难以直接应用。
...
Conclusion: V^2-SAM有效解决了跨视角物体对应问题，通过几何感知和外观引导的提示生成器以及自适应选择机制，显著提升了跨视角对应性能。
...
TL;DR: 提出了Null-Text Test-Time Alignment (Null-TTA)方法，通过优化无分类器引导中的无条件嵌入来实现扩散模型的测试时对齐，避免奖励黑客问题并保持跨奖励泛化能力。
...
Method: 通过优化无分类器引导中的无条件嵌入，而不是操纵潜在变量或噪声变量，在语义连贯的流形上进行对齐，防止利用非语义噪声模式来提高奖励。
...
TL;DR: 本研究开发了一种深度学习模型，用于降低小脑桥脑角池对比增强T1加权MRI的造影剂剂量，通过从低剂量模拟图像中恢复标准剂量图像质量，在仅使用10%-30%标准剂量情况下实现了良好的图像质量和诊断性能。
...
Result: 在10%输入剂量下，DL恢复的T1ce将分割Dice系数从0.673提升至0.734，图像质量指标显著改善。10%和30%输入剂量的DL恢复图像均表现优秀，后者被认为更具信息性。

Conclusion: 深度学习模型显著改善了小脑桥脑角池低剂量MRI的图像质量，使得在仅使用10%-30%标准剂量的情况下即可实现病变检测和诊断特征分析。
...
Result: 在Kinetics-600上提升准确率3.8%-6.4%，MoViNets模型系列在各自FLOP约束下提升SOTA 3.8%-6.1%，MobileNetV3和MoViNets-Stream系列在可比内存占用下提升4.9%-6.4%。
...
TL;DR: 本文提出了一个开放词汇的视觉领域框架，用于探索神经元如何编码任意概念，通过语义分割生成掩码来计算组合解释，解决了传统方法依赖人工标注数据的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统组合解释方法依赖人工标注数据集，限制了其在特定领域和预定义概念中的应用。本文旨在克服这一限制，允许用户探索神经元对任意概念和数据集的信息编码方式。

Method: 提出三步框架：1) 指定任意概念；2) 使用开放词汇语义分割模型生成语义分割掩码；3) 从这些掩码中推导组合解释。

Result: 与先前方法相比，该框架在定量指标和人类可解释性方面表现良好，分析了从人工标注数据转向模型标注数据时解释的差异，并展示了框架在解释灵活性和任务适应性方面的额外能力。

Conclusion: 该开放词汇框架显著扩展了组合解释的适用范围，使其能够处理任意概念和数据集，为理解神经元信息编码提供了更灵活和通用的方法。
...
TL;DR: 本文介绍了UruDendro4数据集，包含102个火炬松横截面图像样本，用于树木年轮自动检测研究。该数据集支持树木年轮体积建模，并通过实验验证了DeepCS-TRD方法在年轮检测任务中的最佳性能。


<details>
  <summary>Details</summary>
Motivation: 解决木材横截面数据稀缺问题，提供包含不同高度样本的数据集以支持树木年轮体积建模研究，提高年轮自动检测的准确性和自动化程度。

Method: 构建UruDendro4数据集，包含102个火炬松横截面图像样本，每个样本都有人工标注的年轮。使用最先进方法进行自动年轮检测性能基准测试，并进行消融实验验证参数配置。

Result: DeepCS-TRD方法表现最佳，平均精度为0.838，平均召回率为0.782，适应兰德误差为0.084。实验证明包含该数据集的训练能提高模型在年轮检测任务中的泛化能力。

Conclusion: UruDendro4数据集填补了木材横截面数据空白，支持树木年轮体积建模研究，为自动年轮检测提供了有价值的基准和训练数据资源。
...
TL;DR: BUSTR是一个无需配对图像-报告监督的多任务视觉语言框架，通过结构化描述符和放射组学特征生成乳腺超声报告，解决了数据缺乏和模型幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 乳腺超声报告生成面临配对图像-报告数据集缺乏和大语言模型幻觉风险的限制，需要一种无需配对监督的解决方案。
...
Conclusion: 这种描述符感知的视觉模型结合标记级和对齐损失训练，无需配对图像-报告数据即可改善自动报告指标和临床效能。
...
TL;DR: 本文提出了表达性图像合成任务，通过StickerNet框架从真实用户编辑行为中学习非真实主义的创意贴纸放置，强调表达性和用户意图而非真实性。


<details>
  <summary>Details</summary>
Motivation: 传统图像合成研究关注视觉真实性和语义合理性，但在现代内容创作中，用户更倾向于创作艺术性、趣味性或社交参与性的非真实主义内容，需要新的任务定义来反映真实编辑平台上的用户行为。

Method: 提出StickerNet两阶段框架：首先确定合成类型，然后预测不透明度、遮罩、位置和缩放等放置参数。基于在线视觉创作平台的180万真实编辑行为构建数据集。

Result: 用户研究和定量评估表明，StickerNet优于常见基线方法，并接近人类放置行为，证明了从真实编辑模式中学习的有效性。

Conclusion: 这项工作引入了视觉理解的新方向，强调表达性和用户意图而非真实性，为创意图像编辑提供了新的研究范式。
...
<details>
  <summary>Details</summary>
Motivation: 多摄像头交通视频数据量大，传统使用VLM将视频转为文本再通过LLM分析的方法耗时严重，无法及时生成交通洞察和调查事件。

Method: 采用顺序处理策略，利用摄像头重叠区域，迭代应用不同token限制的VLM，将前一个摄像头的输出作为后一个摄像头的提示，并通过对象级相似性检测器智能跳过冗余的VLM调用。

Result: 在真实数据集上的实验结果表明，TrafficLens将视频到文本的转换时间减少了4倍，同时保持了信息准确性。
...
TL;DR: 提出了一种基于双整流流的免反演风格迁移框架，通过前向传递并行预测内容和风格轨迹，实现高效无失真的风格迁移。


<details>
  <summary>Details</summary>
Motivation: 主流基于扩散模型的免训练风格迁移方法依赖计算密集的反演过程，导致效率低下且在反演不准确时产生视觉失真。

Method: 使用双整流流并行预测内容和风格轨迹，通过动态中点插值融合两种路径的速度，结合注意力注入指导风格集成。

Result: 实验证明该方法在多样风格和内容上具有良好泛化能力，提供高效有效的风格迁移流程。

Conclusion: 该免反演框架通过联合建模内容、风格和风格化分布，实现了鲁棒的融合效果，避免了朴素叠加的缺点，在视觉保真度和计算效率方面表现优异。
...
TL;DR: 本文提出了一种无需测试时参考图像的伪装目标检测框架，通过将参考图像蒸馏为类别原型记忆，在推理时通过查询条件化的原型混合合成参考向量。
...
Method: 在训练期间将参考图像蒸馏到类别原型记忆中，通过查询条件化的原型混合在推理时合成参考向量，并使用双向注意力对齐模块弥合参考统计量与查询特征之间的表示差距。
...
<details>
  <summary>Details</summary>
Motivation: 图像可以看作前景对象在背景上的分层组合，这种分层表示支持元素的独立编辑，为内容创作提供更大灵活性。尽管生成模型取得了进展，但由于方法和数据有限，将单张图像分解为层仍然具有挑战性。

Method: 观察到分层分解与图像修复任务之间的强关联，提出通过轻量级微调将基于扩散的修复模型适配用于分层分解。引入具有线性注意力复杂度的多模态上下文融合模块来保留潜在空间中的细节。模型完全在开源资产构建的合成数据集上训练。

Result: 在对象移除和遮挡恢复方面实现了优越性能，为下游编辑和创意应用开辟了新的可能性。

Conclusion: 该方法成功地将修复模型适配用于图像分层分解任务，通过多模态上下文融合模块有效保留了图像细节，在合成数据集上训练取得了良好效果，为图像编辑应用提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有新闻图像描述方法存在三个关键问题：(1)信息覆盖不完整，(2)跨模态对齐弱，(3)视觉实体定位不理想。需要开发能够整合文本、视觉和结构化知识的新方法来提升描述质量。

Method: 提出MERGE框架，构建实体中心的多模态知识库(EMKB)，采用多阶段假设-描述策略改善跨模态对齐，通过图像内容引导的动态检索增强视觉实体匹配。
...
Conclusion: MERGE框架通过多模态知识检索和实体感知生成，有效提升了新闻图像描述的质量和实体识别能力，展现出强大的鲁棒性和领域适应性。
...
TL;DR: MetaRank是一个用于自动选择模型可迁移性评估(MTE)指标的元学习框架，通过将数据集和指标的文本描述编码到共享语义空间，学习数据集特征与指标机制之间的关系，从而为新的目标数据集推荐最合适的MTE指标。


<details>
  <summary>Details</summary>
Motivation: 现有的MTE指标选择方法通常是临时性的或基于历史平均性能，但研究发现不同MTE指标的有效性高度依赖于具体任务，没有一个指标在所有数据集上都是最优的。
...
Conclusion: MetaRank框架能够为新的目标数据集高效地推荐最合适的MTE指标，解决了MTE指标选择的任务依赖性问题。
...
Method: 提出了一种新的TMVC框架，引入原型来表示每个视图的邻居结构。该方法简化了视图内邻居关系的学习，并实现了视图内和视图间结构的动态对齐，从而促进跨视图共识的更高效和一致发现。

Result: 在多个公共多视图数据集上的广泛实验表明，与流行的TMVC方法相比，该方法在后续性能和鲁棒性方面具有竞争力。

Conclusion: 所提出的框架通过原型表示和动态结构对齐，有效解决了现有TMVC方法在计算效率和跨视图一致性方面的局限性，为可信多视图分类提供了更高效和可靠的解决方案。
...
TL;DR: CameraMaster是一个统一的相机感知框架，通过解耦相机指令和参数嵌入来实现精确的图像润饰控制，解决了现有方法在参数控制和多参数组合方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导扩散模型在图像编辑中存在两个主要问题：一是仅依赖模糊的文本提示难以实现精确的相机参数控制；二是训练独立头/权重进行参数调整会损害可扩展性、多参数组合能力和对细微变化的敏感性。

Method: CameraMaster通过显式解耦相机指令，协调整合两个关键信息流：捕获摄影师意图的指令表示和编码精确相机设置的参数嵌入。使用相机参数嵌入调制相机指令和内容语义，通过交叉注意力将调制后的指令注入内容特征，同时在去噪过程中将指令和相机嵌入作为条件和门控信号注入时间嵌入。

Result: 在包含78K图像-提示对的大规模数据集上训练和评估，实验表明CameraMaster对参数变化产生单调且接近线性的响应，支持无缝的多参数组合，并显著优于现有方法。

Conclusion: CameraMaster提供了一个统一的相机感知框架，能够实现精确的相机参数控制和多参数组合，在图像润饰任务中表现出色。
...
Result: 评估显示图像与其描述效用之间存在显著差距，在传统图像QA基准上表现相近的模型在描述效用方面下降高达32%。

Conclusion: CaptionQA提供了一个基于效用的描述质量评估框架，揭示了当前MLLM在生成具有实际应用价值的描述方面存在不足，并提供了扩展到新领域的开源工具链。
...
TL;DR: LungNoduleAgent是一个创新的协作多智能体系统，专门用于分析肺部CT扫描，通过模块化流程提高结节描述和恶性分级精度。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于学习的图像去模糊方法存在两难：基于模型的深度展开方法虽然能强制物理约束但会产生过度平滑和伪影，而生成模型虽然感知质量好但会因物理约束弱而产生幻觉细节。

Method: 提出了一种新颖框架，通过将高维压缩核建模为密集连续体来捕捉细微的退化变化，并利用这个丰富的描述符场来条件化ControlNet架构，强烈引导扩散采样过程。

Result: 在具有挑战性的严重模糊场景中，该方法在物理准确性和感知真实性之间取得了良好平衡，超越了最先进的基于模型方法和生成基线。

Conclusion: 该方法成功弥合了物理准确性和感知真实性之间的差距，为空间变化图像去模糊提供了一种有效的解决方案。
...
TL;DR: MUSE是首个统一的图像情感合成框架，能够同时进行情感生成和编辑，通过借鉴测试时间缩放策略，无需额外训练扩散模型或专用情感数据集。
...
Method: 采用测试时间缩放策略，利用现成情感分类器进行梯度优化，通过语义相似度确定最佳引导时机，使用多情感损失减少固有和相似情感的干扰。
...
TL;DR: 本文提出T-NIG模型，通过将时间参数融入正态逆伽马分布来生成阿尔茨海默病的长期预测图像，解决了不规则时间间隔序列数据中疾病特征保持困难的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在长期预测阿尔茨海默病时，面对不规则时间间隔的序列数据难以维持疾病相关特征。考虑到时间相关分布可以反映图像分布不均时的疾病特征变化，需要开发能够处理这种时间不确定性的模型。

Method: T-NIG模型使用两个不同时间点的脑图像，通过坐标邻域识别特征，将时间参数融入正态逆伽马分布来理解脑成像序列中特征的变化。模型利用不确定性估计来减少由于时间数据不足引起的认知和随机不确定性。

Result: 实验结果表明，T-NIG模型在数据集中的短期和长期预测任务中都表现出最先进的性能。该模型能够熟练预测疾病进展，同时即使在面对不规则时间数据分布时也能保持疾病相关特征。
...
TL;DR: 提出了一种新颖的虹膜认证匹配流程，通过3D-CNN学习虹膜特征的丰富时空表示，采用课程学习方式训练，提升了在旋转、尺度、反射和模糊等挑战下的鲁棒性。
...
Method: 将虹膜图像沿一个维度分割成子图像序列，输入3D-CNN捕获空间和时空线索；采用课程学习方式训练模型，结合三元组损失和ArcFace损失进行端到端训练。

Result: 该方法能够在深度度量域中嵌入时间依赖性，提高了特征的可区分性，为虹膜认证提供了鲁棒且可泛化的解决方案。

Conclusion: 提出的框架通过学习虹膜特征的时空表示，显著提升了虹膜认证系统在各种挑战条件下的性能和鲁棒性。
...
Result: 开发了能够学习统一场景级表示的雷达基础模型，并提出超越传统检测度量的定位感知评估指标。

Conclusion: RadarFM通过结构化空间语言监督成功构建了雷达基础模型，为雷达感知的统一表示学习提供了有效解决方案。
...
TL;DR: EM-KD是一种新颖的知识蒸馏范式，通过解决高效MLLMs中视觉令牌不平衡问题，使用匈牙利匹配算法对齐师生模型的视觉令牌，并引入VLAD和VSD两种蒸馏策略来提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有高效MLLMs通过压缩视觉令牌来减少资源消耗，但视觉信息丢失会降低理解能力。先前方法虽然引入知识蒸馏，但忽略了高效学生模型与原始教师模型之间由于视觉令牌不平衡导致的细粒度视觉理解差异。

Method: 首先计算师生模型视觉logits的曼哈顿距离，使用匈牙利匹配算法在空间维度对齐视觉令牌。然后引入两种蒸馏策略：1) 视觉-语言亲和力蒸馏(VLAD)，计算文本令牌与对齐视觉令牌的亲和矩阵，最小化师生亲和矩阵的平滑L1距离；2) 视觉语义蒸馏(VSD)，使用反向KL散度度量对齐视觉logits在词汇空间上的离散概率分布。

Result: 在多样化基准测试上的综合评估表明，EM-KD训练模型在准确性和效率上均大幅优于先前的高效MLLMs。与配备相同视觉令牌匹配策略的先前蒸馏方法相比，EM-KD也实现了更好的性能。
...
Method: 提出AnchorOPT框架，在锚点值和位置关系两个维度引入动态性：1）锚点值从任务特定数据中动态学习，而非手工设计的显式文本标记；2）锚点与软标记之间的位置关系通过可学习的位置矩阵根据训练阶段和任务上下文自适应优化。训练分为两个阶段：先学习锚点标记，然后冻结并转移到第二阶段优化软标记和位置矩阵。

Result: 大量实验表明，仅使用简单的可学习锚点和位置矩阵就能达到或超过一些包含额外可学习模块或正则化技术的方法的性能。作为即插即用模块，AnchorOPT能无缝集成到现有框架中，在各种数据集上带来一致的性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 在可控驾驶场景重建和3D场景生成中，需要在保持几何保真度的同时，在大视角变化下合成视觉合理的外观。然而，基于几何的3DGS和基于外观的扩散模型的有效融合面临挑战，缺乏像素级、3D一致的编辑标准往往导致过度恢复和几何漂移。

Method: 引入FaithFusion框架，使用像素级期望信息增益作为统一策略进行连贯的时空合成：EIG作为空间先验指导扩散模型细化高不确定性区域，同时通过像素级权重将编辑结果蒸馏回3DGS。这是一个即插即用的系统，无需额外的先验条件和结构修改。

Result: 在Waymo数据集上的广泛实验表明，该方法在NTA-IoU、NTL-IoU和FID指标上达到最先进性能，即使在6米车道偏移下仍保持107.47的FID。

Conclusion: FaithFusion通过像素级期望信息增益成功解决了3DGS和扩散模型融合中的几何保真度和视觉合理性平衡问题，为可控驾驶场景重建提供了有效的解决方案。
...
Method: 引入了Idis数据集，系统地在语义、数值和空间维度上变化干扰项，并通过跟踪属性计数来分析干扰项、推理长度和准确性的相互作用。

Result: 视觉干扰项会降低准确性而不增加推理长度，属性计数跟踪揭示了干扰项如何影响推理过程，这些趋势也适用于已建立的视觉偏见基准。

Conclusion: 提出了简单的提示策略来减轻推理模型中的偏见驱动预测，视觉干扰项与文本干扰项在影响机制上存在根本差异。
...
Method: 提出熵引导剪枝策略，使用数据依赖的条件熵偏差(CED)作为模块重要性评估指标；设计零样本自适应剪枝框架，自动确定剪枝时机和程度，避免一次性剪枝带来的模式崩溃问题。

Result: 在DiT和SiT模型上的实验表明，EntPruner实现了最高2.22倍的推理加速，同时在ImageNet和三个下游数据集上保持了竞争力的生成质量。

Conclusion: EntPruner框架有效解决了生成模型在下游任务中的参数冗余问题，通过熵引导的渐进式剪枝实现了显著的推理加速，同时保持了生成质量。
...
TL;DR: CtrlVDiff是一个统一的扩散框架，同时解决视频理解和可控视频生成的双重挑战。通过引入基于图形的模态（内在属性和语义）来补充几何线索，实现更精确的控制和编辑。


<details>
  <summary>Details</summary>
Motivation: 传统仅使用几何线索（如深度、边缘）的方法不足以约束外观、材质和光照，限制了物理上有意义的编辑（如重新照明或材质交换）并常导致时间漂移。需要更丰富的图形模态来提供互补约束。

Method: 提出CtrlVDiff统一扩散模型，采用混合模态控制策略（HMCS），融合深度、法线、分割、边缘和图形内在属性（反照率、粗糙度、金属度）等特征。构建MMVideo混合真实合成数据集进行训练。

Result: 在理解和生成基准测试中，CtrlVDiff提供了卓越的可控性和保真度，支持分层编辑（重新照明、材质调整、对象插入），超越现有最先进基线，并在某些模态不可用时保持鲁棒性。

Conclusion: 通过统一的扩散框架和丰富的图形模态，CtrlVDiff成功解决了视频理解和可控生成的双重挑战，实现了精确、可预测的控制和强时间一致性。
...
TL;DR: 本文提出了TEAR框架，一种针对文本到视频模型安全风险的自动化红队测试方法，专注于利用时间动态性来发现政策违规的视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有的安全评估方法主要关注静态图像和文本生成，无法捕捉视频生成中复杂的时间动态特性，因此需要专门针对T2V模型时间序列安全风险的评估框架。

Method: TEAR采用时间感知的测试生成器，通过两阶段优化方法：初始生成器训练和时间感知在线偏好学习，来生成看似无害但能利用时间动态性引发政策违规视频的文本提示，并使用精炼模型循环提升提示的隐蔽性和攻击效果。

Result: 在开源和商业T2V系统上的广泛实验评估显示，TEAR实现了超过80%的攻击成功率，相比之前最佳结果57%有显著提升。

Conclusion: TEAR框架有效揭示了T2V模型中与时间动态相关的安全风险，证明了针对视频生成时间特性的专门安全评估的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型倾向于使用全局原生分辨率视觉编码，虽然提升了能力但带来了更大的计算开销。为了解决这个问题，需要开发高效的视觉编码方法。

Method: 提出渐进式视觉压缩(PVC)方法，包含两个关键模块：改进的补丁嵌入支持灵活补丁大小缩放，窗口化令牌压缩在ViT层间分层部署以逐步聚合局部令牌表示。

Result: 转换后的ViT-UHD在相同MLLM架构下，性能与MoonViT相当但首令牌时间减少2.4倍。LLaVA-UHD v3性能与Qwen2-VL相当，首令牌时间进一步减少1.9倍。

Conclusion: PVC方法可以有效地将预训练的ViT重新配置为高效架构，同时保持通用性，为高效多模态大语言模型的研究提供了有力支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有的测试时扩展策略如Best-of-N在视觉自回归模型上效果不佳，因为它们在错误生成轨迹上消耗完整计算，且光栅扫描解码缺乏整个画布的蓝图。

Method: GridAR采用网格分区渐进生成方案，在同一位置生成多个部分候选，早期修剪不可行候选，并用可行候选作为锚点指导后续解码。结合布局指定提示重构策略，通过检查部分视图推断可行布局。

Result: 在N=4时，GridAR比Best-of-N（N=8）在T2I-CompBench++上性能提升14.4%，同时成本降低25.6%。在图像编辑任务中，PIE-Bench上语义保持度比更大N的基线提升13.9%。

Conclusion: GridAR框架有效解决了视觉自回归模型测试时扩展的挑战，在有限计算资源下实现了更高质量的图像生成和编辑。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D视觉语言模型在将3D场景有效标记为整体场景标记，并在多样化3D理解任务中利用这些标记方面存在挑战。

Method: 基于多尺度正态分布变换表示的三阶段场景标记化流程，包括构建多尺度NDT表示、交叉尺度特征融合，以及将MSDec重新用作通用接口支持人机交互提示和分割掩码解码。
...
TL;DR: 本文提出了UPA-RFAS框架，针对VLA模型开发通用且可迁移的对抗性补丁攻击，解决了现有方法在黑盒设置下过拟合单一模型的问题。


<details>
  <summary>Details</summary>
Motivation: VLA模型容易受到对抗攻击，但通用且可迁移的攻击方法研究不足，现有补丁方法大多过拟合单一模型，在黑盒设置中失效。

Method: 提出UPA-RFAS统一框架：1) 特征空间目标函数结合ℓ₁偏差先验和排斥性InfoNCE损失；2) 鲁棒增强的两阶段min-max过程；3) 两个VLA特定损失：补丁注意力主导和补丁语义错配。

Result: 实验表明UPA-RFAS在不同VLA模型、操作套件和物理执行中都能稳定迁移，暴露了基于补丁的攻击面。

Conclusion: UPA-RFAS建立了未来防御的强大基线，证明了VLA模型在跨模型、任务和视角下的通用对抗补丁攻击可行性。
...
<details>
  <summary>Details</summary>
Motivation: 基础模型能够学习跨模态的丰富可迁移表示，但在数据稀缺的下游任务中需要注入领域特定知识，而重新训练或微调基础模型计算成本高昂。

Method: 提出BotaCLIP框架，通过对比学习将高分辨率航空影像与植物样方数据对齐，采用正则化策略缓解灾难性遗忘，生成可迁移的表示用于下游预测器。

Result: 在植物存在预测、蝴蝶出现建模和土壤营养组丰度估计三个生态任务中，BotaCLIP表示相比DOFA和监督基线方法都取得了持续改进。

Conclusion: 这项工作展示了如何通过领域感知的基础模型适应，将专家知识注入数据稀缺场景，实现经济高效的表示学习。
...
Method: 使用区域特定语义激活模块，以判别性和文本约束的语义作为查询，在视频帧中捕获与动作最相关的区域响应；将捕获的区域响应组织成动作轨迹，通过多级轨迹对比约束优化空间和时间层面的区域响应。

Result: 在广泛使用的动作识别基准测试中表现出优于先前最先进基线的性能。

Conclusion: ART框架通过跟踪局部细节的动态变化，能够有效区分相似动作，在细粒度动作识别任务中取得了优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 比较三种不同生成建模范式的性能差异，特别是关注采样效率和生成质量，并探索条件流匹配在图像修复任务中的应用潜力。

Method: 使用统一的TinyUNet架构（<150万参数）在CIFAR-10数据集上实现DDPM、CFM和MeanFlow三种方法。对CFM进行了图像修复扩展，实现了四种掩码类型（中心、随机边界框、不规则、半图）的掩码引导采样。
...
Conclusion: CFM在生成质量上优于DDPM，MeanFlow在保持良好生成质量的同时实现了高效的单步采样。针对特定任务（如图像修复）进行微调可以显著提升模型性能，证明了任务感知训练的有效性。
...
TL;DR: FIELDS方法通过结合3D表情参数监督和情感识别分支，从单张图像重建保留微妙情感细节的高保真3D人脸模型。
...
Method: 扩展自监督2D图像一致性线索，加入直接3D表情参数监督和辅助情感识别分支，使用自发4D面部扫描的真实表情参数指导编码器。

Result: 产生情感丰富的3D人脸模型，显著提高野外面部表情识别性能，同时保持自然度。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基准仅限于DeepFake伪造和单粒度注释，无法捕捉真实世界伪造场景的多样性和复杂性。
...
Conclusion: AVFakeBench为音视频伪造检测提供了全面的基准，揭示了当前方法的局限性，为未来研究指明了方向。
...
TL;DR: 本文提出了LaGen框架，这是首个能够逐帧自回归生成长序列LiDAR场景的生成模型，解决了现有方法无法支持长时程交互式生成的问题。
...
Method: 提出LaGen框架，采用单帧LiDAR输入作为起点，利用边界框信息作为条件生成高保真4D场景点云；引入场景解耦估计模块增强对象级内容的交互生成能力，以及噪声调制模块缓解长时程生成中的误差累积。

Result: 在基于nuScenes构建的评估协议上，LaGen在长时程LiDAR场景生成任务中全面优于现有最先进的LiDAR生成和预测模型，特别是在后续帧上表现更佳。

Conclusion: LaGen是首个支持逐帧自回归生成长序列LiDAR场景的框架，通过创新的场景解耦和噪声调制机制，显著提升了长时程生成的性能和交互能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决基于学习的图像匹配对大规模、多样化且几何准确训练数据的依赖问题，同时克服3DGS在几何精度和深度渲染方面的局限性。
...
TL;DR: ConFu是一个多模态表示学习框架，通过对比学习联合嵌入单个模态及其融合组合，能够捕捉高阶依赖关系同时保持强配对对应性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要处理两模态对齐，但忽略了多模态间的高阶交互，且往往无法充分保持配对关系，限制了在单模态任务上的有效性。

Method: ConFu扩展传统配对对比目标，增加融合模态对比项，鼓励模态对与第三模态的联合嵌入，从而捕捉仅通过配对对齐无法恢复的高阶依赖关系。

Result: 在合成和真实多模态基准测试中，ConFu在检索和分类任务上表现出竞争力，支持统一的一对一和一对多检索。

Conclusion: ConFu能够有效利用跨模态互补性，捕捉高阶依赖关系，并随着多模态复杂性的增加而扩展，为多模态表示学习提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 随着Transformer在单个遥感任务上取得优异表现，研究者希望开发一个统一模型，通过多任务学习在多个任务上表现出色。视觉语言模型在遥感图像理解、定位和超高分辨率图像推理方面显示出潜力。

Method: 创建数据引擎处理复杂遥感数据环境，提出统一动态分辨率策略处理不同尺度图像，引入Zoom-in Chain机制处理超高分辨率图像，增强目标检测能力并建立公平评估协议。

Result: RSCoVLM在多样化任务上实现最先进性能，超越现有遥感视觉语言模型，甚至可与专业专家模型相媲美。所有工具、模型权重和数据集均已开源。

Conclusion: 该基线模型将推动通用遥感模型的进一步发展，为多任务学习提供了简单而灵活的解决方案。
...
TL;DR: Monet是一个训练框架，使多模态大语言模型能够在潜在视觉空间中直接推理，通过生成连续嵌入作为中间视觉思维。该工作解决了现有方法在抽象视觉推理方面的局限性，提出了三阶段蒸馏监督微调流程和VLPO强化学习方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉推理方法缺乏类人的抽象视觉思维能力，其灵活性受限于外部工具。作者希望让多模态大语言模型直接在潜在视觉空间中进行推理，生成中间视觉思维。

Method: 提出三阶段蒸馏监督微调(SFT)管道解决潜在视觉对齐的高计算成本和潜在嵌入监督不足的问题；提出VLPO(视觉潜在策略优化)强化学习方法，将潜在嵌入明确纳入策略梯度更新；构建包含12.5万条真实世界、图表、OCR和几何推理链的高质量数据集。
...
TL;DR: 提出HTTM方法加速VGGT模型，通过头级时间合并技术解决传统合并方法导致的特征同质化问题，实现7倍加速且性能损失可忽略。


<details>
  <summary>Details</summary>
Motivation: VGGT模型在重建大场景时，全局注意力机制导致显著延迟瓶颈，需要高效的加速方法。

Method: 提出头级时间合并(HTTM)，在多头粒度上合并token，保持特征独特性，并利用空间局部性和时间对应性实现高合并比。

Result: 在GPU推理中实现最高7倍加速，性能下降可忽略不计。

Conclusion: HTTM是首个无需训练的3D token合并方法，有效解决了VGGT模型在大场景重建中的延迟问题。
...
TL;DR: 本文提出了一种新的遥感变化描述方法，利用SAM基础模型提取区域级表示，结合知识图谱增强变化检测和描述能力，在多个基准数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有遥感变化描述方法通常使用CNN/Transformer提取视觉特征，但存在区域感知能力弱和时间对齐有限的问题。本文旨在通过SAM基础模型解决这些问题。
...
Conclusion: 通过集成SAM基础模型和知识图谱，提出的方法显著提升了遥感变化描述的性能，证明了区域级表示和领域知识注入的有效性。
...
TL;DR: 提出了一种从工业视频流中自动提取和组织视觉-语言-动作模型预训练数据的无监督框架，包括运动分词器和基于潜在动作能量的动作分割器。


<details>
  <summary>Details</summary>
Motivation: 解锁工业视频流中大量未标记的人类演示数据，为视觉-语言-动作模型预训练提供结构化数据，解决制造业中具身AI集成的可扩展性问题。

Method: 首先训练轻量级运动分词器编码运动动态，然后使用基于新型"潜在动作能量"指标的无监督动作分割器发现和分割语义一致的动作基元。

Result: 在公共基准和专有电机装配数据集上的评估显示能有效分割工作台中人类执行的关键任务，通过视觉语言模型的聚类和定量评估确认了发现动作基元的语义一致性。

Conclusion: 这是第一个从非结构化工业视频中完全自动端到端提取和组织VLA预训练数据的系统，为制造业具身AI集成提供了可扩展解决方案。
...
TL;DR: 提出了一种基于超图的时空事件流补全机制，通过超图连接不同时空的事件标记，利用上下文信息传递来补全稀疏事件，并能灵活融合RGB标记实现多模态信息补全。
...
Method: 提出超图引导的时空事件流补全机制，通过超图连接不同时空的事件标记，利用上下文信息传递补全稀疏事件；可灵活融合RGB标记实现多模态信息补全；通过自注意力聚合不同时间步的超图节点信息。

Result: 在单标签和多标签事件分类任务上的大量实验充分验证了所提框架的有效性。

Conclusion: 该方法能有效解决事件流空间稀疏性问题，实现多模态特征的有效学习和融合。
...
Method: 1. 几何引导先验蒸馏：将置信度门控的单目深度转换为监督，使用尺度不变深度和深度梯度损失，采用预热到上限的调度来软注入先验；2. 时间嵌入高斯场：在XYZT中表示动态，使用转子式旋转参数化，实现时间一致几何；3. 关键帧约束流式处理：在最大点数预算下进行关键帧优化，非关键帧进行轻量更新。

Result: 在EndoNeRF和StereoMIS-P1数据集上，Endo-G²T在单目重建基线中达到了最先进的结果。

Conclusion: Endo-G²T通过几何引导和时间感知的训练策略，有效解决了动态内窥镜场景中的几何漂移问题，实现了高质量的时间一致重建，同时保持了计算效率。
...
TL;DR: 提出一种频率感知的token缩减策略，通过将token分为高频和低频两类，选择性保留高频token并将低频token聚合成紧凑的直流token，在提高计算效率的同时缓解rank collapsing问题。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers的二次计算复杂度是主要挑战，现有token缩减方法往往忽略了自注意力的频率特性，如rank collapsing和过平滑现象。

Method: 频率感知token缩减策略：将token划分为高频token和低频token，选择性保留高频token，将低频token聚合成紧凑的直流token以保留必要的低频分量。

Result: 通过大量实验和分析表明，该方法在减少计算开销的同时显著提高了准确性，并缓解了rank collapsing和过平滑问题。

Conclusion: 该方法有效改善了Vision Transformers的计算效率，同时分析了先前方法的隐含频率特性和局限性。
...
TL;DR: 提出了一种名为Merge-and-Bound (M&B)的类增量学习新方法，通过在参数空间中直接操作模型权重进行优化，包含任务间和任务内权重合并，结合边界更新技术来减少灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 解决类增量学习中的灾难性遗忘问题，通过直接操作模型权重来优化模型性能，同时保留先前任务的知识。

Method: 使用两种权重合并：任务间权重合并（统一先前所有阶段模型的权重）和任务内权重合并（在当前阶段内组合模型参数）。结合边界更新技术，以最小累积更新优化目标模型。
...
TL;DR: DiverseVAR是一个无需重新训练或微调就能增强文本条件视觉自回归模型多样性的框架，通过文本嵌入噪声注入和尺度旅行精炼技术解决VAR模型多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: VAR模型在图像生成中表现出色，但存在多样性严重不足的问题，即使对于简单提示也经常生成几乎相同的图像，这一问题在关注图像质量的主流研究中被忽视。

Method: 采用两阶段方法：1）在文本嵌入中注入噪声以增强多样性；2）提出尺度旅行精炼技术，使用多尺度自编码器提取粗尺度标记，在中间阶段恢复生成以保持图像质量。

Result: 实验表明，文本嵌入噪声注入与尺度旅行精炼相结合能显著增强多样性，同时最小化图像质量下降，在多样性-质量权衡中实现了新的帕累托前沿。

Conclusion: DiverseVAR框架成功解决了VAR模型的多样性限制问题，无需额外训练成本就能在保持图像质量的同时显著提升生成多样性。
...
### [71] [E-M3RF: An Equivariant Multimodal 3D Re-assembly Framework](https://arxiv.org/abs/2511.21422)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于深度学习的3D重组方法主要依赖几何特征，当几何信息不足（如碎片小、侵蚀或对称）时表现不佳，且缺乏防止重叠组装的物理约束。

Method: 使用旋转等变编码器提取几何特征，Transformer提取颜色特征，将两者结合形成多模态表示，通过SE(3)流匹配预测碎片变换。

Result: 在四个数据集上的实验表明，在RePAIR数据集上相比竞争方法，旋转误差降低23.1%，平移误差降低13.2%，Chamfer距离减少18.4%。

Conclusion: E-M3RF通过多模态特征融合有效解决了仅依赖几何特征的局限性，在真实文化遗产数据集上显著提升了3D重组精度。
...
TL;DR: 提出了一种名为MRPD的多模态鲁棒提示蒸馏框架，通过教师-学生架构学习轻量级提示，将学生点云模型特征与来自深度投影视觉模型、高性能3D模型和文本编码器的鲁棒嵌入对齐，实现高效对抗防御。
...
Method: 采用教师-学生框架，通过置信度门控机制动态平衡三种教师模型（深度投影视觉模型、高性能3D模型、文本编码器）的知识传递，在训练阶段完成蒸馏，推理时无额外计算成本。

Result: 在广泛的白盒和黑盒攻击下显著优于最先进的防御方法，甚至在干净数据上也能获得更好的性能。

Conclusion: MRPD为构建鲁棒3D视觉系统提供了一种新的实用范式，通过高效利用多模态知识实现对抗防御。
...
TL;DR: MobileI2V是一个270M参数的轻量级扩散模型，用于在移动设备上实现实时图像到视频生成，通过线性混合架构、时间步蒸馏和移动特定注意力优化，将生成速度提升10倍，首次在移动设备上实现720p视频的快速生成。
...
Result: MobileI2V首次在移动设备上实现快速720p图像到视频生成，质量与现有模型相当，在单步条件下每帧720p视频生成时间小于100毫秒。
...
TL;DR: 提出基于交叉注意力的非局部知识蒸馏框架CanKD，通过交叉注意力机制实现学生特征图中每个像素动态考虑教师特征图所有像素的非局部知识迁移，提升特征表示学习效果。


<details>
  <summary>Details</summary>
Motivation: 传统自注意力蒸馏方法独立对齐师生特征图，无法充分捕捉像素间关系，需要更有效的知识迁移机制来改善特征表示学习。

Method: 引入基于交叉注意力的非局部知识蒸馏框架，仅通过额外损失函数实现师生特征图间的动态像素级关系建模。
...
Conclusion: 建立了鲁棒的、与架构无关的最佳实践，为未来深度伪造检测系统的设计和开发提供指导。
...
TL;DR: 本文提出了一种用于抗核抗体（ANA）检测的新框架，该框架处理多实例多标签（MIML）任务的复杂性，使用未经预处理的显微镜图像，无需手动预处理。


<details>
  <summary>Details</summary>
Motivation: ANA检测在临床环境中具有挑战性，因为它涉及多实例多标签学习，而传统的手动检测方法缓慢、劳动密集且需要多年培训。

Method: 该框架包括三个任务特定组件：实例采样器、概率伪标签分发器和自定步调权重学习率系数，模拟人类标记逻辑，识别一致的ANA子区域并分配聚合标签。

Result: 在ANA数据集上，该模型比先前最佳方法提高了+7.0% F1-Macro和+12.6% mAP，在公共数据集上，汉明损失和单错误分别降低了18.2%和26.9%。

Conclusion: 该框架克服了传统MIML方法的局限性，支持端到端优化，在ANA检测和公共医疗MIML基准测试中表现出优越性。
...
TL;DR: 本文提出了一种用于构建遥感基础模型(RSFMs)的专家集成框架，通过分解训练过程为轻量级、任务特定的ConvNeXtV2专家模型，解决了现有大规模模型在计算资源和环境可持续性方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型主要依赖扩大模型规模和数据集规模，需要巨大的计算和数据资源，限制了可访问性，且与可持续和环境友好AI原则相悖。需要一种更高效、可持续的替代方案。
...
Conclusion: 提出的专家集成框架为遥感基础模型的发展提供了更高效、可持续的路径，解决了当前大规模模型范式在资源消耗和环境影响方面的问题。
...
TL;DR: 本研究提出了一种基于定量指标的序列图像生成方法，结合年龄缩放因子生成年龄特异性MRI图像，用于阿尔茨海默病的长期预测。
...
Conclusion: 该方法通过定量指标引导和年龄缩放因子整合，有效提高了阿尔茨海默病长期预测中MRI图像合成的准确性和相似度。
...
TL;DR: 本文提出了PRFL框架，在潜在空间进行偏好优化，解决了视频生成中Reward反馈学习的内存消耗大、训练时间长的问题，显著提升了与人类偏好的对齐效果。
...
Method: 利用预训练视频生成模型在噪声潜在空间进行奖励建模，提出PRFL框架在潜在空间进行偏好优化，无需VAE解码即可在整个去噪链中实现高效梯度反向传播。

Result: 实验表明PRFL显著提高了与人类偏好的对齐效果，同时相比RGB ReFL大幅降低了内存消耗和训练时间。

Conclusion: 在潜在空间进行奖励反馈学习是视频生成对齐的有效方法，PRFL框架在保持性能的同时显著提升了训练效率。
...
TL;DR: UAVLight是一个用于光照鲁棒3D重建的基准数据集，通过在不同固定时间重复飞行捕获自然光照变化，在保持几何、校准和视点一致性的前提下解决多视角重建中的光照不一致问题。


<details>
  <summary>Details</summary>
Motivation: 多视角3D重建面临光照不一致的根本挑战，无人机长时间飞行和户外环境使光照变化不可避免。现有数据集要么缺乏有意义的照明多样性，要么因几何和语义变化而无法孤立研究光照鲁棒性。

Method: 引入UAVLight基准，每个场景在可重复的地理参考飞行路径上，在多个固定时间点进行捕获，在一致的几何、校准和视点下产生自然光照变化。

Result: 提供了标准化的跨光照条件评估协议，为开发在真实户外环境中一致、忠实和可重光照的重建方法提供了可靠基础。

Conclusion: UAVLight基准为光照鲁棒3D重建方法的发展和基准测试提供了受控但真实的评估环境，解决了现有数据集的局限性。
...
TL;DR: 提出了一种将2D/3D地标配准集成到U-Net地标预测模型训练中的新框架，用于处理骨盆透视术中患者姿态变化的问题。
...
Method: 将2D/3D地标配准集成到U-Net地标预测模型的训练中，比较基线U-Net、使用姿态估计损失训练的U-Net以及在姿态估计损失下微调的U-Net的性能差异。

Result: 在患者姿态变化的真实手术条件下评估了不同方法的性能，但具体结果未在摘要中明确说明。

Conclusion: 提出的框架能够更好地适应骨盆透视术中患者姿态变化的情况，提高了地标检测的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 开源模型在音频-视频联合生成中存在严重的同步对齐问题，主要源于联合扩散过程中的三个根本挑战：对应漂移、低效的全局注意力机制和传统CFG的模态内偏差。

Method: 提出Harmony框架：1)跨任务协同训练利用音频驱动视频和视频驱动音频生成任务的强监督信号；2)全局-局部解耦交互模块实现高效精确的时序风格对齐；3)同步增强CFG在推理时显式分离和增强对齐信号。

Result: 大量实验表明Harmony在生成保真度和细粒度音频-视频同步方面均显著优于现有方法，建立了新的最先进水平。

Conclusion: Harmony通过机制性地强制音频-视频同步，有效解决了联合扩散过程中的根本挑战，为音频-视频内容生成提供了更可靠的解决方案。
...
TL;DR: 该研究使用深度学习构建了一个16种不同口腔病变的多类分类器，通过分层数据分割、数据增强和过采样技术解决数据有限和不平衡问题，在口腔癌早期检测中取得了良好效果。
...
Method: 结合分层数据分割、高级数据增强和过采样技术来构建深度学习多类分类器，以克服有限和不平衡数据集的挑战。

Result: 实验结果显示模型达到了83.33%的准确率、89.12%的精确率和77.31%的召回率，优于现有最先进方法，特别是在少数类分类方面表现显著。

Conclusion: 该框架展示了过采样和增强策略在少数类分类中的有效性，是迈向可信赖的计算机辅助诊断系统用于临床口腔癌早期检测的重要第一步。
...
TL;DR: 论文主张LLM效率研究应从追求大规模复杂性转向强调在有限资源和专业知识下的稳健简单性，提出了包括模型架构改造、轻量级微调、经济推理、动态知识管理和基于开销感知效率基准的新研究议程。


<details>
  <summary>Details</summary>
Motivation: 当前主流的LLM效率方法（如MoE、推测解码、复杂RAG）主要服务于拥有庞大基础设施的科技巨头，导致大多数组织无法受益，加剧了数字鸿沟和碳浪费问题。

Method: 提出新的研究议程：无需重新训练的预训练模型架构改造、保持对齐的轻量级微调、长思维链的经济推理、无需重型RAG管道的动态知识管理，以及采用开销感知效率作为标准基准。

Result: 通过重新定义效率以包含采用成本、可持续性和公平性，可以民主化LLM部署，确保优化减少不平等和碳浪费而不是加剧它们。

Conclusion: LLM效率研究的下一前沿应是稳健简单性，即在适度资源和最少专业知识下仍能发挥作用的效率方法，这有助于实现更公平和可持续的AI部署。
...
TL;DR: 本文提出了Harmonic Token Projection (HTP)，一种无需训练、词汇表或随机参数的可逆确定性文本嵌入生成框架，通过Unicode整数表示生成谐波轨迹来编码标记。


<details>
  <summary>Details</summary>
Motivation: 传统神经嵌入依赖统计共现或优化方法，HTP旨在提供一种透明、高效且可解释的替代方案，证明有意义的语义关系可以从确定性几何中产生。

Method: HTP将每个标记分析性地编码为从其Unicode整数表示导出的谐波轨迹，建立离散符号与连续向量空间之间的双射可解释映射。

Result: 在语义文本相似性基准测试中，HTP在英语上达到Spearman相关性ρ=0.68，在十种语言中保持稳定性能，计算成本可忽略且每句对延迟低于毫秒。

Conclusion: HTP展示了确定性几何可以产生有意义的语义关系，为数据驱动嵌入提供了透明高效的替代方案。
...
TL;DR: PIRA是一种新的奖励模型训练范式，通过重构问答对为偏好指令、聚合多样化偏好任务奖励、以及使用dropout平均来稳定奖励，解决了传统奖励模型数据效率低和奖励过度优化的问题。


<details>
  <summary>Details</summary>
Motivation: 传统判别式奖励模型存在数据效率低和容易奖励过度优化的问题，需要更有效的训练方法来提升LLM与人类偏好的对齐效果。

Method: 采用三种策略：1) 将问答对重构为基于偏好的指令；2) 聚合多样化偏好任务的奖励；3) 在不同dropout率下平均价值头输出以稳定奖励。

Result: 大量实验证明了PIRA方法的有效性。

Conclusion: PIRA训练范式成功解决了奖励模型的两个关键挑战，为LLM与人类偏好的对齐提供了更有效的解决方案。
...
TL;DR: 本文分析大语言模型在法律任务中的表现，通过重组文档结构、定义法律修辞角色和模拟法庭推理来提升模型在零样本设置下的性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然在通用推理方面表现出色，但在法律等专业领域表现不佳，主要因为缺乏领域特定的预训练，且法律文档通常冗长复杂。

Method: 在三个印度法律判决预测数据集上进行零样本实验：(i)基于修辞角色重组文档结构，(ii)定义法律修辞角色以熟悉法律术语，(iii)模拟法庭对修辞角色的逐步推理过程。

Result: 重组数据或解释关键法律术语显著提升模型性能，F1分数相比基线最低提高约1.5%，最高提高4.36%。

Conclusion: 通过结构化信息处理和领域特定知识引导，可以有效提升大语言模型在法律任务中的表现，特别是在处理长文本和复杂法律概念时。
...
<details>
  <summary>Details</summary>
Motivation: 现有心理健康分析基准数据集已过时，存在数据量有限、清理不足、内容多样性不足等问题，需要构建更全面、高质量的数据集来支持社交媒体心理健康研究。

Method: 从Reddit收集自报告诊断数据，应用严格预处理步骤（语言过滤、NSFW内容移除、重复内容删除），使用LIWC进行语言分析，并通过微调语言模型和词袋特征进行二元分类实验。

Result: MindSET数据集规模是之前基准的两倍多，在诊断检测任务中表现优异，自闭症检测的F1分数提高了18个百分点，模型性能显著优于现有基准。
...
TL;DR: FlexCode是一个流行度感知的生成推荐框架，通过自适应分配固定token预算在协同过滤和语义编码本之间，解决现有方法使用单一编码本处理热门和长尾项目的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有生成推荐方法使用单一统一编码本编码所有项目，忽视了热门项目（富含协同信号）和长尾项目（依赖语义理解）之间的固有不平衡，这种统一处理限制了表示效率并阻碍泛化能力。

Method: 引入FlexCode框架，自适应分配固定token预算在协同过滤编码本和语义编码本之间；使用轻量级MoE动态平衡CF特定精度和语义泛化；通过对齐和平滑目标保持流行度谱系中的连贯性。
...
TL;DR: 本文介绍了首个阿拉伯语跨领域、上下文相关的文本到SQL数据集Ar-SParC，包含3,450个相关问题和10,225个问题-SQL对。使用GPT模型进行40个实验，并提出了GAT corrector方法提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前文本到SQL研究主要集中在英语和中文，阿拉伯语领域存在空白。作者旨在填补这一空白，为阿拉伯语用户提供与数据库自然语言交互的能力。

Method: 构建Ar-SParC数据集，使用GPT-3.5-turbo和GPT-4.5-turbo进行40个实验，应用10种提示工程技术，并提出GAT corrector方法进行性能优化。

Result: GAT corrector在所有实验中平均提升执行准确率1.9%和交互准确率1.9%（零样本），在上下文学习中提升1.72%执行准确率和0.92%交互准确率。

Conclusion: 成功创建了首个阿拉伯语文本到SQL数据集，提出的GAT corrector方法有效提升了模型性能，为阿拉伯语自然语言数据库交互奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在临床基准测试中表现良好，但它们可能通过错误的推理得出正确结论，这种失败模式在肿瘤学决策支持中具有安全影响，但基于准确性的评估无法捕捉到。
...
TL;DR: 提出了动态模板选择（DTS）方法，根据查询复杂度自适应选择响应模板，在保持响应质量的同时显著降低令牌成本，实现32.6%-33.9%的令牌减少。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型部署通常对所有查询类型采用统一的提示策略，导致令牌效率低下，特别是输出令牌成本比输入令牌高4-8倍。

Method: 使用两种路由方法：基于预计算嵌入的简单MLP和微调的RoBERTa变换器，通过查询复杂度匹配响应模板。

Result: MLP路由在1000个MMLU问题上达到90.5%的路由准确率，优于RoBERTa的89.5%，且参数少1.25亿。在三个主要LLM提供商上验证了模板选择的通用性。

Conclusion: DTS方法能有效降低令牌成本而不影响响应质量，提供了问题形式化、算法和实证验证的完整框架。
...
<details>
  <summary>Details</summary>
Motivation: 解决二维材料研究信息分散在不同论文中，难以系统获取其特性和制备方法的问题。

Method: 未在提供的摘要中明确说明具体方法。

Result: 未在提供的摘要中提供具体研究结果。

Conclusion: 二维材料研究信息分散的问题需要系统化解决方案。
...
TL;DR: 本文提出了一种新的记忆化框架——多前缀记忆化，通过计算能够引出记忆序列的不同前缀数量来量化LLM中的记忆化现象，相比传统方法能更可靠地区分记忆化和非记忆化数据。


<details>
  <summary>Details</summary>
Motivation: 现有记忆化定义在全面捕捉对齐模型中的记忆化现象方面存在不足，特别是无法准确衡量记忆的鲁棒性，这给隐私和版权保护带来了风险。

Method: 引入多前缀记忆化框架，核心观点是记忆序列被深度编码，可通过大量不同前缀检索。定义序列被记忆的条件是外部对抗搜索能找到足够数量的不同前缀来引出该序列。

Result: 在开源和对齐聊天模型上的实验表明，多前缀定义能可靠区分记忆化和非记忆化数据，为审计LLM中的数据泄露提供了稳健实用的工具。

Conclusion: 多前缀记忆化框架通过量化记忆检索路径的多样性，提供了更全面和实用的记忆化评估方法，有助于更好地理解和缓解LLM中的数据泄露风险。
...
TL;DR: SAGE是一个基于智能体的框架，将稀疏自编码器特征解释从被动的单次生成任务转变为主动的、解释驱动的过程，通过系统制定多个解释、设计针对性实验和基于经验激活反馈迭代优化解释。
...
Method: 提出SAGE框架，采用系统方法：为每个特征制定多个解释，设计针对性实验进行测试，基于经验激活反馈迭代优化解释。

Result: 在多种语言模型的SAE特征上的实验表明，SAGE产生的解释在生成和预测准确性方面显著优于最先进的基线方法。

Conclusion: SAGE通过将特征解释重构为主动的解释驱动过程，显著提高了稀疏自编码器特征解释的质量和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言模型基准测试框架（如HELM）使用固定提示，无法跨模型泛化，导致性能估计不准确，可能低估模型真实能力。需要估计每个语言模型的上限性能。

Method: 开发了可复现的DSPy+HELM框架，使用四种提示方法评估四个前沿语言模型在七个基准测试（通用/医学领域）上的表现，并与现有HELM基线进行比较。

Result: 研究发现：HELM低估语言模型性能（平均4%）；性能估计在基准间变化更大（标准差增加2%）；性能差距被错误表示（3/7基准测试中排名翻转）；引入推理（思维链）可减少模型对提示设计的敏感性。

Conclusion: 这是首个大规模实证研究语言模型在不同基准和提示方法下行为的论文，表明可扩展的性能上限估计能够提供更有决策价值的基准测试。相关代码已开源。
...
TL;DR: 提出了一种新的Length-MAX分词器，通过最小化平均字符标记数来减少训练和推理时的标记数量，相比BPE分词器在不同词汇量下减少13-18%的标记，显著提升训练效率和推理性能。
...
Result: 在FineWeb和多样化领域上，相比BPE减少14-18%的标记数；GPT-2模型训练步数减少17.2-18.5%，推理延迟降低12.7-13.7%，下游任务性能提升（LAMBADA困惑度降低11.7%，HellaSwag准确率提升4.3%）。

Conclusion: 优化平均标记长度而非仅频率是提高语言模型效率的有效方法，在不牺牲下游性能的情况下显著提升训练和推理效率，兼容生产系统并减少18%的内存使用。
...
TL;DR: Evo-Memory是一个评估LLM智能体自进化记忆能力的综合流式基准和框架，通过将数据集构建为顺序任务流，要求LLM在每次交互后搜索、适应和进化记忆。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注静态对话设置，忽略了在动态任务流中积累和重用经验的能力。现实环境中LLM需要处理连续任务流，但往往无法从累积交互中学习，丢失有价值的上下文洞察。

Method: 引入Evo-Memory基准框架，统一实现十多个代表性记忆模块，提出ExpRAG基线方法和ReMem行动-思考-记忆优化管道，紧密集成推理、任务行动和记忆更新。

Result: 在10个多样化多轮目标导向和单轮推理问答数据集上评估记忆模块，展示了持续改进的能力。

Conclusion: Evo-Memory填补了LLM智能体自进化记忆评估的空白，为测试时记忆演化提供了系统化框架。
...
TL;DR: 该论文研究跨语言论证挖掘方法，针对低资源语言波斯语，通过三种训练场景（零样本迁移、LLM增强、跨语言混合）进行实验，发现跨语言模型在波斯语上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言在论证挖掘任务中的数据短缺问题，探索跨语言方法的应用潜力。

Method: 构建三种训练场景：零样本迁移（仅用英语数据）、LLM增强（英语数据+合成样本）、跨语言混合（英语数据+人工翻译波斯语句子），在英语和波斯语语料上进行评估。

Result: 零样本迁移模型在英语和波斯语测试集上的F1分数分别为50.2%和50.7%；LLM增强模型提升至59.2%（英语）和69.3%（波斯语）；跨语言模型在波斯语测试集上达到74.8%的F1分数。

Conclusion: 轻量级跨语言混合方法优于资源密集的增强管道，为低资源语言的论证挖掘任务提供了实用解决方案。
...
TL;DR: 本文提出了一种结合角色交叉最小对、时间涌现分析和跨模型比较的方法，研究大型语言模型如何实现语义角色。研究发现LLMs形成了紧凑的、因果隔离的抽象语义结构机制，这些机制在不同规模和架构间存在部分迁移。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型显示出语义能力，但其内部实现抽象语义结构的机制仍缺乏充分表征。需要研究LLMs如何实现语义角色的内部机制。

Method: 采用角色交叉最小对、时间涌现分析和跨模型比较的集成方法，分析LLMs实现语义角色的机制。

Result: 发现：(i)高度集中的电路（89-94%归因在28个节点内）；(ii)渐进结构细化而非相变，较大模型有时绕过局部化电路；(iii)中等跨尺度保守性（24-59%组件重叠）和高度光谱相似性。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在知识图谱对话问答中的局限性：LLM缺乏对私有动态知识图谱的直接访问，RAG系统序列化结构且难以处理多轮上下文，传统KGQA系统仅支持单轮问答且延迟高。

Method: 采用模块化多智能体架构，结合RAG式检索与结构化执行，通过专门任务LLM智能体协作实现上下文解释、对话跟踪、实体关系链接和高效查询规划，将自然语言问题转换为可执行SPARQL查询。

Result: 在大型多样化知识图谱上的实验显示，Chatty-KG在单轮和多轮设置中显著优于最先进基线方法，获得更高的F1和P@1分数，模块化设计保持对话连贯性并支持演进知识图谱。

Conclusion: Chatty-KG统一了对话灵活性与结构化知识图谱基础，为可靠的多轮KGQA提供了可扩展和可扩展的方法，与商业和开源LLM兼容且性能稳定。
...
TL;DR: 本文评估了大型语言模型(LLMs)在不同类型查询中的表现差异，发现LLMs在定义类问题上表现最佳，而在举例类问题上表现最差，且对高频知识的定义更倾向于使用释义。


<details>
  <summary>Details</summary>
Motivation: 虽然人类能够轻松提供多种类型的答案（如例子和释义），但LLMs在处理除定义类查询外的其他类型问题时表现不佳，本研究旨在评估这种性能下降现象。

Method: 使用TrackList细粒度语言和统计分析管道，研究预训练数据对LLMs回答多样化语言查询的影响；引入RefoMed-EN数据集（6170个人工标注的医学术语及其对应定义、命名、举例、解释或释义）；通过句法和语义相似度指标、统计相关性和嵌入来评估LLM输出质量。

Result: LLMs在定义类问题上的任务表现最高，而在举例类问题上表现最低；对于定义类问题，大型语言模型更倾向于对流行和频繁的知识进行释义，而对尾部和技术性知识（特别是在专业文本中）的释义较少。

Conclusion: LLMs在处理不同类型查询时存在显著性能差异，定义类问题表现最佳而举例类问题表现最差，且模型对高频知识的处理方式与低频知识存在差异。
...
TL;DR: 本文研究了上下文学习(ICL)能否覆盖预训练的标签语义，还是仅仅优化现有的语义基础。通过将LLMs视为提示诱导分类器，对比自然演示和反转演示下的行为，发现ICL主要调整输入如何映射到预训练期间学习的稳定语义方向，而不是灵活地重新映射标签含义。


<details>
  <summary>Details</summary>
Motivation: 探究上下文学习是否能真正覆盖预训练的标签语义，还是仅仅在现有语义基础上进行微调，以理解ICL的基本限制。

Method: 将LLMs视为提示诱导分类器，使用自然演示和反转演示对比分析，引入三个对齐指标（真实对齐、先验对齐和提示对齐）和语义覆盖率。在8个分类任务和8个开源LLM上进行实验。

Result: 发现ICL改进准确性的同时保持强先验对齐；大多数正确预测与零样本行为一致；在反转演示下，模型无法学习连贯的反语义分类器；语义覆盖率在少样本设置中保持为零。

Conclusion: ICL主要调整输入如何映射到预训练期间学习的稳定语义方向，而不是灵活地重新映射标签含义，表明在这些规模上覆盖标签语义需要超越ICL的干预措施。
...
TL;DR: 本研究探索在缅甸语等低资源语言分类任务中使用Kolmogorov-Arnold Networks (KANs)作为替代MLP的分类头，评估了三种KAN变体在不同嵌入方法上的表现，发现KAN在准确性和效率方面具有竞争力或优于传统MLP。


<details>
  <summary>Details</summary>
Motivation: 在低资源语言分类任务中，通常只微调最终分类层而保持预训练编码器权重冻结。传统使用的MLP因其固定的非线性特性可能限制表达能力并增加计算成本，因此需要探索更有效的替代方案。

Method: 评估了三种KAN变体（FourierKAN、EfficientKAN、FasterKAN）作为分类头，在多种嵌入方法（TF-IDF、fastText、mBERT、Distil-mBERT）上进行实验比较。

Result: 实验结果显示：EfficientKAN与fastText结合获得最高F1分数（0.928）；FasterKAN在速度和准确性之间提供最佳平衡；在transformer嵌入上，EfficientKAN与mBERT匹配或略微优于MLP（0.917 F1）。

Conclusion: KANs作为MLP的表达性和高效替代方案，在低资源语言分类任务中展现出良好潜力，提供了更好的准确性和效率权衡。
...
TL;DR: 该研究评估了28种不同架构的大语言模型在58个字符级约束的字谜任务上的表现，发现架构差异比参数规模对约束满足能力的影响更大，并揭示了模型在拼写异常但约束有效的常见词上的系统性失败。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在受控文本生成中需要满足严格的拼写约束，但目前缺乏跨架构的系统性评估。研究旨在了解不同模型架构和参数规模对字符级约束满足能力的影响。

Method: 评估了三个模型家族（Qwen3、Claude Haiku-4.5、GPT-5-mini）共28种配置，在58个需要字符级约束满足的字谜任务上进行测试，并使用10,000名人类解谜者的难度评级进行校准分析。

Result: 架构差异产生的性能差距（2.0-2.2倍，F1=0.761 vs. 0.343）远大于同家族内参数规模扩展的收益（83%增益）。高容量模型在思考预算增加时表现提升显著，而中等规模模型则饱和或退化。模型在拼写异常的常见词上存在系统性失败（人类成功率86-95%，模型失败率89-96%）。

Conclusion: 约束满足能力可能需要专门的架构特征或训练目标，而不仅仅是参数规模或计算预算的扩展。模型过度依赖分布合理性，惩罚了拼写异常但约束有效的模式，表明需要超越标准语言模型扩展的架构创新。
...
TL;DR: MortgageLLM是一个专门针对抵押贷款金融领域的双专家大语言模型，通过双轨专业化框架解决通用LLM在专业领域应用时的知识增强与指令跟随保真度双重挑战。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型在专业领域如抵押贷款金融中需要领域特定知识增强，同时保持指令跟随能力。单一多任务模型存在性能权衡问题，优化结构化任务会降低对话保真度。

Method: 采用双轨专业化框架，从单一基础模型创建两个专家模型：对话问答模型和结构化任务模型。应用指令残差技术恢复领域适应后的指令跟随能力，无需监督微调。使用智能任务路由机制，由专家模型自身进行少样本分类。

Result: 在领域特定基准测试中，最终模型MLM v2显著优于基础模型：LLM-as-a-Judge摘要得分4.58（vs 3.99），问答得分4.09（vs 4.0），分类得分2.6（vs 1.2）。语义相似度方面，摘要BERTScore 0.77（vs 0.74），问答0.68（vs 0.58），分类0.75（vs 0.73）。

Conclusion: 双专家架构结合指令残差技术有效解决了专业领域LLM应用中的性能权衡问题，在抵押贷款金融领域显著提升了模型性能，为专业领域LLM开发提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 由于对抗性越狱提示具有隐蔽性和欺骗性，能够绕过内置安全机制生成有害内容，因此需要一种自适应安全对齐方法，使模型能够自主加强防御。

Method: SGASA框架包含两个关键阶段：数据预合成（生成安全指南和增强提示）和对齐微调（使用监督微调和直接偏好优化将指南嵌入模型）。

Result: 在多个数据集上的广泛实验表明，SGASA显著提高了模型安全性。

Conclusion: SGASA验证了其自适应和可扩展的有效性，为推理模型提供了一种有效的安全对齐方法。
...
TL;DR: 该研究探讨了在人类调查数据上进行微调是否能改善大型语言模型(LLM)模拟人类行为的准确性。研究发现微调确实提高了异质性、对齐度和信念-行动一致性，但即使最佳微调模型也无法复现原始研究的回归系数，表明LLM生成的数据仍不适合替代人类参与者进行正式推断分析。


<details>
  <summary>Details</summary>
Motivation: 解决关于LLM是否能替代人类参与者的持续争论，特别是研究在少量人类调查数据上微调是否能缓解LLM与真实人类行为不一致的问题，如多样性有限、少数群体系统性错位、组内方差不足以及信念与行动差异等。

Method: 使用信息披露行为实验，比较人类和LLM生成的反应，评估分布差异、子群体对齐、信念-行动一致性以及回归系数恢复等多个维度。在少量人类样本上对基础模型进行微调。

Result: 在少量人类样本上微调显著改善了异质性、对齐度和信念-行动一致性，但即使最佳微调模型也无法复现原始研究的回归系数。

Conclusion: 虽然微调改善了LLM模拟人类行为的某些方面，但LLM生成的数据仍不适合替代人类参与者进行正式推断分析，因为无法准确复现关键的统计关系。
...
TL;DR: 本文介绍了首个伊桑语开放式对话语音数据集的开发，该语言是泰国使用最广泛的地区方言。与现有的基于朗读或脚本语音的语料库不同，该数据集包含自然语音，捕捉了真实的语言现象。


<details>
  <summary>Details</summary>
Motivation: 开发伊桑语对话语音数据集，以支持对代表性不足语言的研究，促进包容性AI发展，并解决建模对话语音的语言和技术挑战。

Method: 建立了实用的转录协议，在表征准确性和计算处理需求之间取得平衡，解决了伊桑语缺乏标准化正字法的问题。

Result: 创建了首个包含自然伊桑语对话的开放数据集，捕捉了口语化表达、自发韵律、不流畅性和与泰语的频繁语码转换等真实语言现象。

Conclusion: 通过发布这个开放数据集，为代表性不足语言的研究做出贡献，为建模对话语音的语言和技术挑战提供基础，促进包容性AI发展。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在许多任务上达到最先进性能，但其大规模导致高计算和环境成本，限制了可访问性。参数高效微调方法通过减少可训练参数同时保持强大的下游性能来解决这一挑战，但当前评估仍有限且难以复现。

Method: 引入PEFT-Bench统一基准测试，评估6种PEFT方法在27个NLP数据集上的表现，并开发了PSCP指标综合考虑可训练参数、推理速度和训练内存使用。

Result: 建立了涵盖广泛数据集和PEFT方法的统一评估框架，提供了更全面的性能比较标准。

Conclusion: PEFT-Bench填补了当前PEFT方法评估的空白，为参数高效微调方法的系统评估提供了标准化工具和指标。
...
TL;DR: 该研究通过微调训练，使7B参数模型从几乎完全失败（0.4%准确率）转变为可靠检测注入的激活模式（85%准确率），展示了语言模型内省能力可以通过训练直接获得。


<details>
  <summary>Details</summary>
Motivation: 解决Lindsey研究中提出的开放性问题：是否可以通过训练来消除模型间内省能力的差异，探索语言模型内省能力是否可以直接训练获得而非等待自然涌现。

Method: 通过对瞬态单令牌注入进行微调训练，让模型学习检测和识别注入的激活模式，并保留信息在后续生成步骤中报告语义内容。

Result: 模型从0.4%准确率提升到85%准确率，假阳性率为0%，能够检测单令牌位置注入的瞬时"想法"，并在后续生成中报告语义内容。

Conclusion: 研究表明语言模型的至少一部分内省行为可以通过直接训练获得，为构建内置AI透明度提供了可行路径。
...
<details>
  <summary>Details</summary>
Motivation: 在线新闻文章下的用户评论中经常传播错误信息，需要有效的方法来检测事实不正确的信息。为了支持或反驳这些声明，需要识别相关文档并精确定位证明或反驳每个声明的确切文本片段。

Method: 创建了由付费标注人员标注的细粒度证据数据集，并评估了多种大型语言模型（LLMs）在此数据集上的表现，分析它们与人类标注的一致性。

Result: 结果显示LLMs经常无法从源文本中逐字复制证据，导致无效输出。llama3.1:8b模型尽管规模较小但正确输出比例高，而gpt-oss-120b模型参数虽多但表现不佳。qwen3:14b、deepseek-r1:32b和gpt-oss:20b模型在模型大小与人类标注对齐之间达到了有效平衡。

Conclusion: 大型语言模型在细粒度证据提取任务中经常无法准确复制源文本证据，模型大小与性能之间不存在简单正相关关系，某些中等规模模型在性能与效率之间达到了更好的平衡。
...
TL;DR: Odin是一个新颖的图文本集成架构，通过定向双模块机制在特定Transformer层注入图结构，避免了传统GNN的过平滑问题，同时解决了Transformer忽略图拓扑的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖GNN但受限于过平滑和跳数依赖扩散，要么使用Transformer但忽略图拓扑并将节点视为孤立序列。需要一种能有效结合文本理解和结构推理的方法。

Method: 提出Odin架构，在选定深度的Transformer层通过定向双模块机制注入图结构。不同于消息传递GNN，Odin不依赖多跳扩散，而是在特定层集成多跳结构，产生与语义层次对齐的低、中、高级结构抽象。聚合操作在全局[CLS]表示上进行，从根本上避免过平滑。
...
TL;DR: 本文对六种最先进的模型合并方法进行了大规模系统评估，发现在LLM中，最古老简单的Task Arithmetic方法是唯一能可靠提升性能的方法，其他方法通常会导致显著性能下降。


<details>
  <summary>Details</summary>
Motivation: 模型合并可以在不额外训练的情况下将多个微调检查点合并为单一模型，但尚不清楚这些方法是否适用于大型语言模型(LLM)。

Method: 在四个开源LLM、每个基础模型十二个微调检查点和十六个标准LLM基准上，系统评估了六种最先进的合并方法，包括最新的子空间方法。
...
TL;DR: 本文提出了一种双向可读性评估机制，通过捕捉上下文信息识别文本中语义丰富的区域来预测句子级别的可读性，并使用成对排序算法建模可读性级别间的序数关系。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习技术在可读性评估中的应用大多未能同时考虑文本长度和可读性标签的序数关系，因此需要一种能够解决这两个问题的机制。

Method: 提出双向可读性评估机制，通过上下文信息识别语义丰富区域预测句子可读性，使用句子级标签辅助文档级可读性预测，并引入成对排序算法通过标签减法建模序数关系。

Result: 在中文和英文数据集上的实验结果表明，所提模型取得了有竞争力的性能，并优于其他基线模型。

Conclusion: 该研究提出的双向可读性评估机制和成对排序算法能有效提升可读性评估性能，特别是在处理文本长度和序数关系方面表现出色。
...
TL;DR: 本文研究了语音翻译模型中性别分配的机制，发现模型不简单复制训练数据中的性别关联，而是学习更广泛的男性主导模式。模型能够基于声学输入覆盖内部语言模型的男性偏见，并通过第一人称代词将性别化术语与说话者链接，利用频率谱中分布的性别信息。


<details>
  <summary>Details</summary>
Motivation: 语音翻译中，从有概念性性别的语言翻译到语法性别语言时，说话者的声音特征可能在性别分配中起作用，这可能导致误判性别。目前对语音翻译模型如何做出这些决策的理解还很有限。

Method: 研究三个语言对（英-西/法/意）中语音翻译模型为说话者相关术语分配性别的机制，考察训练数据模式、内部语言模型偏见和声学信息的相互作用，使用对比性特征归因分析频谱图。

Result: 模型学习更广泛的男性主导模式而非简单复制训练数据中的特定术语性别关联。内部语言模型表现出强烈的男性偏见，但模型能基于声学输入覆盖这些偏好。高性别准确率的模型使用第一人称代词将性别化术语链接回说话者，利用频率谱中分布的性别信息而非集中在音高。

Conclusion: 语音翻译模型通过复杂的机制分配性别，涉及训练数据模式、内部语言模型偏见和声学信息的相互作用，发现了一个新的机制：使用第一人称代词链接性别化术语到说话者，利用分布式频率信息而非仅依赖音高。
...
Method: 创建了IsharaKhobor数据集及其两个子集，进行了词汇限制和规范化处理，并使用基于地标的原始嵌入和RQE嵌入进行基准测试。

Result: 开发了公开可用的数据集IsharaKhobor、IsharaKhobor_small和IsharaKhobor_canonical_small，并提供了基准性能。

Conclusion: 该数据集为孟加拉手语翻译研究提供了重要资源，通过词汇限制和规范化处理提升了数据质量，为未来研究奠定了基础。
...
TL;DR: RoParQ基准测试评估LLMs在转述问题中的一致性，提出XParaCon指标量化模型鲁棒性，并通过转述感知的SFT策略提升模型语义不变性。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs对转述问题回答不一致的问题，表明模型依赖表层模式而非真正语义理解。

Method: 构建RoParQ基准测试，生成转述问题并筛选不一致样本；提出XParaCon评估指标；实施基于推理的转述感知SFT策略。

Result: 针对性对齐显著提升模型鲁棒性，轻量级微调模型达到与大型预训练模型相当的一致性水平。

Conclusion: 该方法有效减轻表层记忆依赖，促进更稳健可靠的LLMs发展。
...
### [120] [MTTR-A: Measuring Cognitive Recovery Latency in Multi-Agent Systems](https://arxiv.org/abs/2511.20663)
*Barak Or*

Main category: cs.MA

TL;DR: 该论文将经典可靠性指标（MTTR、MTBF）引入认知领域，定义MTTR-A作为多智能体系统认知恢复延迟的运行时度量，通过基准模拟验证了不同恢复策略的恢复时间。


<details>
  <summary>Details</summary>
Motivation: 解决自主多智能体系统中认知稳定性的挑战，现有工具无法量化推理一致性丢失后的恢复速度。

Method: 使用AG News语料库和LangGraph编排框架进行基准模拟，建模多种反射模式下的恢复延迟。

Result: 自动反射平均约6秒恢复稳定性，人工干预约需12秒；200次运行中MTTR-A中位数为6.21±2.14秒，MTBF为6.7±2.14秒，NRR为0.08。

Conclusion: 通过将恢复延迟形式化为分布式推理的可量化属性，建立了运行时可靠性的基础，将认知恢复从临时过程转变为标准化、可解释的性能指标。
...
### [121] [Resilient Charging Infrastructure via Decentralized Coordination of Electric Vehicles at Scale](https://arxiv.org/abs/2511.20943)
*Chuhao Qin,Alexandru Sorici,Andrei Olaru,Evangelos Pournaras,Adina Magda Florea*

Main category: cs.MA

TL;DR: 提出了一种基于集体学习的电动汽车充电协调框架，能够在充电站故障或需求激增等紧急情况下平衡个体舒适度与系统效率，实现帕累托最优权衡。


<details>
  <summary>Details</summary>
Motivation: 现有分散式充电控制方法在严重突发事件（如充电站故障或充电需求激增）下表现不佳，导致长队列和驾驶员舒适度降低。需要解决有限充电槽竞争问题。

Method: 采用集体学习协调框架，推荐电动汽车采取自适应充电行为，在舒适度和效率之间动态调整优先级，适应变化的充电站容量和时空分布。

Result: 使用真实世界数据的实验表明，该方法优于基准方法，显著减少了行驶和排队时间。在不确定充电条件下，适时自私或利他的驾驶员比始终保持中等行为的驾驶员等待时间更短。

Conclusion: 该方法在高比例充电站故障和对抗性电动汽车情况下表现出更高的弹性和可信度，提升了分散式电动汽车充电基础设施的可靠性。
...
### [122] [Tool-RoCo: An Agent-as-Tool Self-organization Large Language Model Benchmark in Multi-robot Cooperation](https://arxiv.org/abs/2511.21510)
*Ke Zhang,Xiaoning Zhao,Ce Zheng,Jiahong Ning,Dandan Zhu,Wenqi Zhang,Chen Sun,Toshiharu Sugawara*

Main category: cs.MA

TL;DR: Tool-RoCo是一个基于RoCo多机器人协作基准的新型基准测试，用于评估大语言模型在长期多智能体协作中的表现，通过工具使用来评估协作和自组织能力。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM多智能体系统研究依赖预定义的编排，忽视了智能体的自主性。Tool-RoCo将其他智能体视为工具，通过工具使用来评估多智能体协作和自组织能力。

Method: 提出了四种LLM范式：集中式协作、集中式自组织、去中心化协作和自组织。包含SORT、PACK和CABINET三个多机器人任务，通过工具使用测量格式和参数准确性以及智能体协调。

Result: 结果显示协作工具仅占所有工具的7.09%，表明基于LLM的智能体很少将其他智能体作为助手调用。激活工具占96.42%，表明当前LLM倾向于保持活跃智能体，很少为自适应协调而停用它们。

Conclusion: Tool-RoCo提供了一个系统性的基准测试，用于评估LLM在多智能体任务中的自主性和协作能力。
...
### [123] [BAMAS: Structuring Budget-Aware Multi-Agent Systems](https://arxiv.org/abs/2511.21572)
...
<details>
  <summary>Details</summary>
Motivation: 现有工作很少在明确预算约束下构建多智能体系统，随着系统复杂性增加，成本成为实际部署的重要考虑因素。

Method: 首先通过整数线性规划选择最优LLM集合平衡性能和成本，然后使用强化学习方法确定交互拓扑，最后基于选择的智能体和协作拓扑实例化系统。

Result: 在三个代表性任务上的评估显示，BAMAS在达到可比性能的同时，成本降低了高达86%。
...
### [124] [Train While You Fight -- Technical Requirements for Advanced Distributed Learning Platforms](https://arxiv.org/abs/2511.20813)
*Simon Hacks*

Main category: cs.SE

TL;DR: 本文探讨了支持'边战边训'(TWYF)模式的先进分布式学习平台技术需求，通过设计科学研究方法识别了七个技术挑战，并提出了相应的软件工程模式解决方案。


<details>
  <summary>Details</summary>
Motivation: 推动在作战行动中持续学习，而不仅仅是在作战前后进行训练，这需要先进分布式学习平台具备特殊的技术能力。

Method: 采用设计科学研究方法：(i)从PfPC/北约文件和实践中提取挑战，(ii)定义解决方案目标，(iii)系统性地将挑战映射到已验证的模式。

Result: 识别出七个关键技术挑战：互操作性、弹性、多语言支持、数据安全与隐私、可扩展性、平台独立性和模块化，并通过德国武装部队的国家用例进行了模式说明。

Conclusion: 现有的软件工程模式能够满足TWYF模式的技术需求，为先进分布式学习平台的开发提供了可行的解决方案框架。
...
### [125] [Hierarchical Evaluation of Software Design Capabilities of Large Language Models of Code](https://arxiv.org/abs/2511.20933)
*Mootez Saad,Boqi Chen,José Antonio Hernández López,Dániel Varró,Tushar Sharma*

Main category: cs.SE

TL;DR: 本文通过实证研究评估大型语言模型对软件设计中内聚性和耦合性的理解，发现模型在理想条件下表现良好，但在实际应用中存在脆弱性和不对称性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在软件工程领域的广泛应用，需要系统评估其对核心软件设计概念（内聚性和耦合性）的理解鲁棒性。

Method: 通过程序化生成设计不良的代码片段，测试DeepSeek-R1模型系列在不同指导水平（验证、引导、开放式生成）和不同上下文噪声下的表现。

Result: 模型在理想条件下对两个概念都有良好理解，但实际知识脆弱且高度不对称。耦合性推理在噪声环境中性能下降超过50%，而内聚性分析在引导任务中表现稳健。

Conclusion: LLMs在识别设计缺陷方面能提供可靠帮助，但在噪声现实环境中自主推理能力有限，需要更可扩展和鲁棒的程序理解能力。
...
### [126] [SpaceX: Exploring metrics with the SPACE model for developer productivity](https://arxiv.org/abs/2511.20955)
*Sanchit Kaul,Kevin Nhu,Jason Eissayou,Ivan Eser,Victor Borup*

Main category: cs.SE

TL;DR: 本研究通过SPACE框架和统计方法揭示了传统单一生产力指标的局限性，提出了综合考虑情感状态、协作拓扑等多维度的复合生产力评分(CPS)。


<details>
  <summary>Details</summary>
Motivation: 传统确定性、单一维度的生产力启发式方法存在局限性，无法全面反映开发者的实际效能，需要更全面的多维度评估框架。

Method: 采用开源仓库数据集，运用广义线性混合模型(GLMM)和基于RoBERTa的情感分类等统计方法，通过仓库挖掘操作化SPACE框架。

Result: 发现负面情感状态与提交频率呈显著正相关，表明存在由挫败感驱动的迭代修复循环；同时证明分析贡献者交互拓扑比传统基于数量的指标能更准确地映射协作动态。

Conclusion: 研究提出了复合生产力评分(CPS)来解决开发者效能的异质性问题，为更全面评估软件开发生产力提供了新方法。
...
### [127] [Lightweight Model Editing for LLMs to Correct Deprecated API Recommendations](https://arxiv.org/abs/2511.21022)
*Guancheng Lin,Xiao Yu,Jacky Keung,Xing Hu,Xin Xia,Alex X. Liu*

Main category: cs.SE

TL;DR: 该研究首次系统性地应用10种最先进的模型编辑技术来更新LLMs中已弃用的API知识，提出了AdaLoRA-L方法，通过在特定API层进行编辑来显著提高特异性。


<details>
  <summary>Details</summary>
Motivation: LLMs在代码补全任务中表现出色，但其嵌入知识受限于训练数据的时效性，经常生成已弃用的API。重新训练LLMs成本高昂，因此需要轻量级的模型编辑方法来有效更新已弃用的API知识。

Method: 构建EDAPIBench基准测试，包含70多个已弃用API和3000多个编辑实例；应用10种模型编辑技术；提出AdaLoRA-L方法，定义"通用API层"和"特定API层"，仅在特定API层进行编辑。

Result: AdaLoRA在生成正确的最新API方面表现最佳，但在特异性方面不足；AdaLoRA-L显著提高了特异性，同时在其他评估指标上保持可比性能。

Conclusion: AdaLoRA-L方法通过分层编辑策略有效解决了模型编辑中的特异性问题，为LLMs中API知识的时效性更新提供了可行方案。
...
### [128] [Exploring Hidden Geographic Disparities in Android Apps](https://arxiv.org/abs/2511.21151)
*M. Alecci,P. Jiménez,J. Samhi,T. Bissyandé,J. Klein*

Main category: cs.SE

TL;DR: 本文研究了Android应用的地理差异现象，发现GeoTwins（功能相似但不同包名的跨国家应用）和Android App Bundle生态系统中base.apk文件的区域差异，这些差异对安全评估、隐私和公平性产生重要影响。


<details>
  <summary>Details</summary>
Motivation: 移动应用演化已被广泛研究，但应用行为的地理差异仍未被充分探索。本文旨在揭示地理位置对Android应用行为的影响，特别是安全性和公平性方面的潜在问题。

Method: 构建了跨多个区域的分布式应用收集管道，分析了数千个应用，并发布了包含81,963个GeoTwins的数据集。

Result: 发现GeoTwins在请求权限、第三方库和隐私披露方面存在差异；即使base.apk文件也存在区域差异，这可能导致同一应用在不同地区的安全评估结果不一致。

Conclusion: 移动软件存在系统性区域差异，这些差异破坏了研究的可重复性，引入了地理偏见，并引发了关于透明度和用户同意的伦理问题，对研究人员、开发者、平台架构师和政策制定者具有重要意义。
...
### [129] [Bug Detective and Quality Coach: Developers' Mental Models of AI-Assisted IDE Tools](https://arxiv.org/abs/2511.21197)
...
TL;DR: 本研究通过6场工作坊与58名开发者探讨了他们对AI辅助bug检测和代码可读性评估工具的心理模型，发现开发者将bug检测工具视为'bug侦探'，将可读性评估工具视为'质量教练'，并提出了以人为本的IDE设计原则。
...
Method: 进行了6场共同设计工作坊，涉及58名开发者，以获取他们对AI辅助bug检测和可读性功能的心理模型。

Result: 开发者将bug检测工具视为'bug侦探'，仅对关键问题发出警告，保证透明度、可操作反馈和信心提示；可读性评估工具被视为'质量教练'，提供情境化、个性化和渐进式指导。信任取决于解释清晰度、时机和用户控制。

Conclusion: 提炼了一套以人为本的IDE中AI设计原则，旨在平衡干扰与支持、简洁与深度、自动化与人类自主性。
...
### [130] [Multi-Agent Systems for Dataset Adaptation in Software Engineering: Capabilities, Limitations, and Future Directions](https://arxiv.org/abs/2511.21380)
*Jingyi Chen,Xiaoyan Guo,Songqiang Chen,Shing-Chi Cheung,Jiasi Shen*

Main category: cs.SE

TL;DR: 本文首次实证研究多智能体系统在数据集适应任务中的表现，评估了基于GPT-4.1和Claude Sonnet 4的Copilot在软件工程研究工件适应方面的能力，发现当前系统能识别关键文件并生成部分适应，但很少产生功能正确的实现。
...
Method: 通过五阶段评估管道（文件理解、代码编辑、命令生成、验证和最终执行），评估Copilot在ROCODE和LogHub2.0基准库上的表现，测量成功率、分析失败模式，并评估提示干预策略。

Result: 当前系统能识别关键文件并生成部分适应，但很少产生功能正确的实现。提示干预（特别是提供执行错误信息和参考代码）显著提高了与真实结果的结构相似性（从7.25%提升到67.14%）。
...
### [131] [Supporting Students in Navigating LLM-Generated Insecure Code](https://arxiv.org/abs/2511.20878)
*Jaehwan Park,Kyungchan Lim,Seonhye Park,Doowon Kim*

Main category: cs.CR

TL;DR: Bifröst是一个教育框架，旨在培养AI辅助开发中的安全意识，通过模拟不安全代码生成环境来训练学生识别和缓解安全漏洞。


<details>
  <summary>Details</summary>
Motivation: AI和大型语言模型在软件开发中的广泛应用带来了安全风险，当前教育方法忽视这些风险，导致学生无法有效识别AI生成代码中的安全问题。

Method: 开发了Bifröst框架，包含VS Code扩展模拟真实环境、对抗性配置的LLMs生成不安全代码，以及反馈系统突出显示漏洞。

Result: 课堂部署(n=61)显示学生对不安全代码的脆弱性，干预后调查(n=21)表明对LLM输出的怀疑态度增强。

Conclusion: Bifröst通过沉浸式任务和针对性安全分析，成功培养了学生批判性评估AI生成代码安全性的能力。
...
### [132] [A Taxonomy of Pix Fraud in Brazil: Attack Methodologies, AI-Driven Amplification, and Defensive Strategies](https://arxiv.org/abs/2511.20902)
*Glener Lanes Pizzolato,Brenda Medeiros Lopes,Claudio Schepke,Diego Kreutz*

Main category: cs.CR

TL;DR: 本文综述了针对巴西央行2020年推出的即时支付系统Pix的攻击方法，识别和分类了影响用户和金融机构的主要欺诈类型，揭示了这些技术从纯社会工程向混合策略的演变趋势。


<details>
  <summary>Details</summary>
Motivation: 研究旨在识别和分类影响Pix支付系统用户和金融机构的主要欺诈类型，揭示攻击技术的演变和日益复杂化趋势。

Method: 结合结构化文献综述与对银行业专业人士的探索性访谈。
...
### [133] [Readout-Side Bypass for Residual Hybrid Quantum-Classical Models](https://arxiv.org/abs/2511.20922)
*Guilin Zhang,Wulan Guo,Ziqi Tan,Hongyang He,Hailong Jiang*

Main category: cs.CR

TL;DR: 提出一种轻量级残差混合架构，通过将量子特征与原始输入在分类前连接来绕过测量瓶颈，在中心化和联邦学习设置中均优于纯量子模型和先前混合模型。
...
Method: 采用轻量级残差混合架构，在分类前将量子特征与原始输入进行连接，在不增加量子复杂度的前提下绕过测量瓶颈。

Result: 实验显示该模型在中心化和联邦设置中均优于纯量子模型和先前混合模型，相比量子基线准确率提升高达+55%，同时保持低通信成本和增强的隐私鲁棒性。

Conclusion: 该方法为将量子模型集成到隐私敏感、资源受限的环境（如联邦边缘学习）提供了一条实用、近期的路径。
...
### [134] [Road Network-Aware Personalized Trajectory Protection with Differential Privacy under Spatiotemporal Correlations](https://arxiv.org/abs/2511.21020)
*Minghui Min,Jiahui Liu,Mingge Cao,Shiyin Li,Hongliang Zhang,Miao Pan,Zhu Han*

Main category: cs.CR

TL;DR: 本文提出了一种个性化轨迹隐私保护机制(PTPPM)，通过结合地理不可区分性和失真隐私，允许用户自定义隐私预算和期望推断误差边界，有效保护移动用户在基于位置服务中的隐私。


<details>
  <summary>Details</summary>
Motivation: 基于位置服务(LBS)为用户带来便利的同时也带来隐私风险，攻击者可通过时空相关性推断敏感信息。由于用户对位置数据的敏感度因停留时间、访问频率和语义敏感性等因素而异，实施个性化隐私保护至关重要。

Method: 1. 建模攻击者对用户轨迹时空相关性的知识；2. 结合地理不可区分性和失真隐私构建保护位置集(PLS)；3. 提出个性化隐私预算分配算法(PPBA)，基于轨迹数据评估位置敏感度；4. 引入Permute-and-Flip机制生成扰动位置，最小化扰动距离。

Result: 仿真结果表明，该机制优于现有基准方法，在满足用户服务质量(QoS)要求的同时提供更优越的隐私保护。

Conclusion: PTPPM机制通过个性化隐私预算分配和位置扰动，有效平衡了隐私保护和服务质量，为基于位置服务中的轨迹隐私保护提供了有效解决方案。
...
### [135] [CAHS-Attack: CLIP-Aware Heuristic Search Attack Method for Stable Diffusion](https://arxiv.org/abs/2511.21180)
*Shuhan Xia,Jing Dai,Hui Ouyang,Yadong Shang,Dongxiao Zhao,Peipei Li*

Main category: cs.CR

TL;DR: CAHS-Attack是一种针对扩散模型的对抗性攻击方法，通过CLIP感知启发式搜索和蒙特卡洛树搜索进行细粒度后缀优化，在无需白盒访问的情况下实现高效攻击。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在面对对抗性提示时表现出明显的脆弱性，现有方法依赖白盒模型梯度或手工提示工程，在现实部署中不可行或效果不佳。

Method: 结合蒙特卡洛树搜索进行细粒度后缀优化，使用约束遗传算法预选高潜力对抗性提示作为根节点，并在每次模拟中保留最具语义破坏性的结果进行高效局部搜索。

Result: 在具有不同语义的短长提示上都实现了最先进的攻击性能，发现SD模型的脆弱性源于其基于CLIP的文本编码器的固有漏洞。

Conclusion: 当前文生图流水线存在基本安全风险，CAHS-Attack为揭示此类漏洞和构建更鲁棒的生成系统提供了有效方法。
...
### [136] [AuthenLoRA: Entangling Stylization with Imperceptible Watermarks for Copyright-Secure LoRA Adapters](https://arxiv.org/abs/2511.21216)
...
<details>
  <summary>Details</summary>
Motivation: 现有的水印技术要么针对基础模型，要么验证LoRA模块本身，但无法将水印传播到生成的图像中，存在可追踪性空白。此外，为基础模型设计的可追踪水印与风格化不紧密耦合，通常会导致视觉质量下降或高误检率。

Method: AuthenLoRA采用双目标优化策略，联合学习目标风格分布和水印引起的分布偏移，确保任何使用带水印LoRA生成的图像都可靠地携带水印。还设计了扩展的LoRA架构以增强多尺度适应，并引入零消息正则化机制来显著降低水印验证中的误检率。

Result: 大量实验表明，AuthenLoRA实现了高保真风格化、鲁棒的水印传播，与现有方法相比显著降低了误检率。

Conclusion: AuthenLoRA有效解决了LoRA定制扩散模型中的版权保护问题，通过统一的水印框架实现了高质量风格化和可靠的水印追踪能力。
...
### [137] [Empirical Assessment of the Code Comprehension Effort Needed to Attack Programs Protected with Obfuscation](https://arxiv.org/abs/2511.21301)
*Leonardo Regano,Daniele Canavese,Cataldo Basile,Marco Torchiano*

Main category: cs.CR

TL;DR: 本文通过控制实验评估软件混淆技术的有效性，研究混淆对代码理解任务的影响以及复杂度指标能否预测保护效果。


<details>
  <summary>Details</summary>
Motivation: 软件保护的有效性评估对于选择最有效的资产保护方法至关重要。虽然混淆技术被广泛采用，但其有效性仍缺乏深入研究和系统评估。

Method: 采用控制实验方法，让硕士生对经过混淆处理的应用程序执行代码理解任务，评估混淆在延迟代码理解方面的效果，并检验复杂度指标能否准确预测保护效果。

Result: 研究首次评估了在单一保护代码上叠加多种混淆技术的效果，提供了客观代码指标与成功攻击可能性之间相关性的实验证据。

Conclusion: 混淆技术能有效延迟代码理解，复杂度指标可以预测保护效果，研究为软件保护评估开辟了新方向，指出了需要进一步分析的重要方面。
...
### [138] [Minimizing Hyperbolic Embedding Distortion with LLM-Guided Hierarchy Restructuring](https://arxiv.org/abs/2511.20679)
...
<details>
  <summary>Details</summary>
Motivation: 双曲几何是嵌入层次数据结构的有效几何形式，但双曲嵌入的质量与输入层次结构密切相关。已有研究发现最优双曲嵌入需要高分支因子和单继承结构。本文旨在探索大型语言模型是否能够自动重构层次结构以满足这些标准，帮助知识工程师优化知识组织。

Method: 提出基于提示的方法，使用大型语言模型在已知双曲嵌入期望标准的指导下转换现有层次结构。在16个不同层次结构上进行实验验证。

Result: 实验结果表明，经过LLM重构的层次结构在多个标准嵌入质量指标上一致产生更高质量的双曲嵌入。同时，LLM引导的层次重构能够提供可解释的重组，为知识工程师提供合理性说明。

Conclusion: 大型语言模型能够有效自动重构层次结构以优化双曲嵌入质量，不仅提升了嵌入效果，还提供了可解释的重组过程，为知识工程师提供了实用的工具。
...
### [139] [AssurAI: Experience with Constructing Korean Socio-cultural Datasets to Discover Potential Risks of Generative AI](https://arxiv.org/abs/2511.20686)
...
TL;DR: AssurAI是一个针对韩语多模态生成AI安全评估的质量控制数据集，包含11,480个文本、图像、视频和音频实例，覆盖35种AI风险因素，专门设计用于韩国社会文化背景的安全评估。
...
### [140] [$A^2Flow:$ Automating Agentic Workflow Generation via Self-Adaptive Abstraction Operators](https://arxiv.org/abs/2511.20693)
...
Conclusion: A²Flow通过完全自动化的算子提取和工作流生成，显著提升了智能体工作流的性能和效率，证明了自适应抽象算子的有效性。
...
### [141] [Reasoning With a Star: A Heliophysics Dataset and Benchmark for Agentic Scientific Reasoning](https://arxiv.org/abs/2511.20694)
*Kevin Lee,Russell Spiewak,James Walsh*

Main category: cs.AI

TL;DR: 该论文提出了一个名为'Reasoning With a Star'的日球物理学数据集和基准测试方法，用于评估大型语言模型在科学推理中的表现，包括物理假设、单位一致性和科学格式要求。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在日球物理学领域进行科学推理时面临的挑战，包括需要整合物理假设、保持单位一致性和提供清晰科学格式的问题。

Method: 构建了基于NASA和UCAR Living With a Star暑期学校问题集的数据集，包含问题上下文、推理步骤、预期答案类型等要素，并开发了程序化评分器进行验证。

Result: 通过基准测试发现，基于系统工程原理分解工作流程的多智能体模式在需要演绎推理的问题上表现优于直接提示方法。

Conclusion: 在日球物理学科学推理任务中，采用系统工程方法分解工作流程的多智能体策略比直接提示更有效，特别是在需要演绎推理而非纯粹归纳回忆的问题上。
...
### [142] [A Brief History of Digital Twin Technology](https://arxiv.org/abs/2511.20695)
...
Method: 整合医学影像、生物传感器和计算模型创建患者特异性数字孪生，应用包括心脏数字孪生预测心律失常治疗结果、肿瘤数字孪生跟踪进展和优化放疗、药理学数字孪生加速药物发现。

Result: 数字孪生技术在医疗领域取得快速进展，代表性应用已证明其在改善诊断和治疗规划方面的潜力，但临床广泛集成仍受限于技术挑战。

Conclusion: 未来需要通过可解释AI、联邦学习和统一监管框架等解决方案，结合多器官数字孪生、基因组学整合和伦理治理，确保数字孪生技术实现医疗健康的根本性转型。
...
### [143] [Cross Domain Evaluation of Multimodal Chain-of-Thought Reasoning of different datasets into the Amazon CoT Framework](https://arxiv.org/abs/2511.20701)
*Nitya Tiwari,Parv Maheshwari,Vidisha Agarwal*

Main category: cs.AI

TL;DR: 本文对多模态思维链推理进行了综合分析，评估其在A-OKVQA、OKVQA和ChartQA数据集上的有效性，发现视觉整合显著减少推理幻觉，但CoT推理效果因问题类型而异。
...
Result: 视觉整合显著减少了推理生成中的幻觉，但CoT推理的有效性在不同问题类型间差异很大，常识推理面临特别挑战。
...
### [144] [Representation Interventions Enable Lifelong Unstructured Knowledge Control](https://arxiv.org/abs/2511.20892)
*Xuyuan Liu,Zhengzhang Chen,Xinshuai Dong,Yanchi Liu,Xujiang Zhao,Shengyu Chen,Haoyu Wang,Yujun Yan,Haifeng Chen*

Main category: cs.AI

TL;DR: RILKE是一种在大语言模型表示空间进行干预的知识控制方法，能够在冻结基础权重的情况下实现细粒度知识更新，避免大规模重新训练。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型产生错误或过时内容的问题，在终身学习设置中高效准确地更新知识，避免多个编辑之间的干扰。

Method: 在模型表示空间进行干预，学习具有抗释义性和编辑局部性的模块，将每个更新限制在低维子空间以减少交叉干扰；推理时通过查询自适应路由器选择适当模块指导生成。
...
### [145] [ENACT: Evaluating Embodied Cognition with World Modeling of Egocentric Interaction](https://arxiv.org/abs/2511.20937)
...
TL;DR: ENACT是一个评估视觉语言模型是否展现具身认知的基准，通过视觉问答形式评估从自我中心交互中的世界建模能力，包含正向和逆向世界建模两个任务。


<details>
  <summary>Details</summary>
Motivation: 研究现代视觉语言模型在主要以非具身方式训练的情况下，是否表现出具身认知的迹象。具身认知认为智能源于感知运动交互而非被动观察。

Method: 引入ENACT基准，将具身认知评估构建为部分可观察马尔可夫决策过程中的世界建模问题，包含正向世界建模（根据动作重新排序观察）和逆向世界建模（根据观察重新排序动作）两个互补任务。

Result: 实验显示前沿视觉语言模型与人类之间存在性能差距，且随着交互时间跨度增加而扩大。模型在逆向任务上表现优于正向任务，并表现出人类中心偏见，如偏好右手动作，以及当相机参数或视角偏离人类视觉时性能下降。

Conclusion: ENACT基准揭示了当前视觉语言模型在具身认知能力方面的局限性，为评估和改进模型的具身智能提供了重要工具。
...
### [146] [Improving Procedural Skill Explanations via Constrained Generation: A Symbolic-LLM Hybrid Architecture](https://arxiv.org/abs/2511.20942)
*Rahul Dass,Thomas Bowlin,Zebing Li,Xiao Jin,Ashok Goel*

Main category: cs.AI

TL;DR: Ivy是一个AI辅导系统，结合符号化TMK模型和生成式LLM，提供结构化、多步骤的解释，以改善程序技能学习中的因果、目标导向和组合逻辑传达。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通常产生流畅但浅层的响应，缺乏程序技能学习所需的因果、目标导向和组合逻辑结构。

Method: 结合符号化Task-Method-Knowledge（TMK）模型与生成式解释层，TMK编码因果转换、目标层次和问题分解，在明确结构边界内指导LLM构建解释。

Result: 与GPT和检索增强GPT基线相比，符号约束持续改善了针对"如何"和"为什么"问题的解释结构质量。

Conclusion: 这项研究展示了一种可扩展的AI教育方法，通过符号约束增强AI生成解释在智能辅导系统中的教学价值。
...
### [147] [ICPO: Intrinsic Confidence-Driven Group Relative Preference Optimization for Efficient Reinforcement Learning](https://arxiv.org/abs/2511.21005)
*Jinpeng Wang,Chao Li,Ting Ye,Mengyuan Zhang,Wei Liu,Jian Luan*

Main category: cs.AI

TL;DR: 本文提出了ICPO方法，通过结合语言模型生成概率的内在置信度和可验证奖励，解决现有RLVR方法中存在的粗粒度奖励、奖励噪声和低效探索等问题，显著提升大语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法存在粗粒度奖励、奖励噪声和低效探索等问题，导致训练不稳定和熵崩溃，限制了语言模型推理能力的进一步提升。

Method: 提出ICPO方法，基于语言模型生成不同响应的概率来反映其对推理过程的自我评估，通过计算多个响应在相同输入提示下的相对生成概率来获得偏好优势分数，并将该分数与可验证奖励结合指导探索过程。

Result: 在四个通用领域基准和三个数学基准上的综合实验表明，ICPO相比GRPO方法能够稳定提升推理能力。

Conclusion: ICPO方法通过偏好优势分数有效缓解了粗粒度奖励和奖励噪声问题，抑制了过度自信错误，增强了被低估高质量响应的相对优势，防止模型对特定策略的过拟合，促进更彻底的探索。
...
### [148] [Causality Without Causal Models](https://arxiv.org/abs/2511.21260)
...
<details>
  <summary>Details</summary>
Motivation: Halpern和Pearl的因果定义局限于因果模型，无法处理包含析取、否定、信念和嵌套反事实的复杂公式，需要一种更通用的定义框架。

Method: 通过抽象化Halpern-Pearl定义的关键特征，构建一个可在任何反事实模型中使用的一般化因果定义框架。

Result: 新定义不仅适用于更广泛的模型（包括允许回溯的模型），还能处理复杂逻辑公式，并扩展到解释的定义。

Conclusion: 抽象化方法扩展了因果定义的适用范围，提供了对因果模型更深入的理解，并为跨模型的解释概念奠定了基础。
...
### [149] [New Hybrid Heuristics for Pseudo-Boolean Propagation](https://arxiv.org/abs/2511.21417)
...
Method: 提出了用于混合决策的新启发式方法，结合了观察字面量方案和计数方法。

Result: 新启发式方法能够大幅优于RoundingSAT求解器中的当前方法。

Conclusion: 新的混合决策启发式方法在伪布尔求解中表现出显著优势，为单元传播策略提供了改进方向。
...
### [150] [MADRA: Multi-Agent Debate for Risk-Aware Embodied Planning](https://arxiv.org/abs/2511.21460)
*Junjian Wang,Lidan Zhao,Xi Sheryl Zhang*

Main category: cs.AI

TL;DR: 提出了MADRA框架，一种无需训练的多智能体辩论风险评估方法，通过集体推理增强安全意识，同时保持任务性能。该框架结合分层认知协作规划，在AI2-THOR和VirtualHome实验中实现了超过90%的危险任务拒绝率，且安全任务拒绝率低。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在具身AI智能体任务规划中的安全问题，现有方法存在计算成本高或过度拒绝的问题，特别是在家庭环境中危险指令带来重大风险。

Method: 使用基于LLM的多个智能体对给定指令的安全性进行辩论，由关键评估器根据逻辑合理性、风险识别、证据质量和清晰度对响应进行评分。通过迭代审议和共识投票，结合分层认知协作规划框架整合安全、记忆、规划和自我进化机制。

Result: 在AI2-THOR和VirtualHome上的广泛实验表明，该方法实现了超过90%的不安全任务拒绝率，同时确保安全任务拒绝率低，在安全性和执行效率方面优于现有方法。

Conclusion: 该工作为构建可信赖的具身智能体提供了一种可扩展、模型无关的解决方案，显著减少了误拒绝，同时保持对危险任务的高敏感性。
...
### [151] [SpatialBench: Benchmarking Multimodal Large Language Models for Spatial Cognition](https://arxiv.org/abs/2511.21471)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准往往将空间认知过度简化为单一维度指标，无法捕捉空间能力的层次结构和相互依赖性，需要建立更系统化的评估框架。

Method: 构建了分层空间认知框架（五个认知层次），创建了SpatialBench大规模细粒度基准（涵盖15个任务），并引入了高级能力导向的统一评估指标。
...
### [152] [Pessimistic Verification for Open Ended Math Questions](https://arxiv.org/abs/2511.21522)
*Yanxing Huang,Zihan Tang,Zejin Lin,Peng Li,Yang Liu*

Main category: cs.AI

TL;DR: 提出悲观验证方法，通过构建多个并行验证流程来检测数学证明错误，显著提升验证性能且计算资源消耗低。
...
Method: 设计悲观验证变体，对同一证明构建多个并行验证流程，只要任一验证报告错误即判定证明不正确。

Result: 该方法在多个数学验证基准测试中显著提升性能，token效率甚至超过扩展长链思维，且发现数据集标注错误是强模型假阴性的主要原因。

Conclusion: 悲观验证能有效提升语言模型数学输出的可靠性，对实现长视野数学任务具有关键作用，有助于增强语言模型在各种数学任务中的能力。
...
### [153] [Self-Transparency Failures in Expert-Persona LLMs: A Large-Scale Behavioral Audit](https://arxiv.org/abs/2511.21569)
*Alex Diep*

Main category: cs.AI

TL;DR: 该研究审计了16个开源模型在专业角色下的AI身份披露行为，发现模型在不同领域存在显著的透明度不一致性，某些模型在推理优化后反而降低了自我透明度。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估语言模型在专业角色下可靠披露AI身份的能力，因为在高风险领域，虚假专业知识可能对用户造成伤害，缺乏透明度会破坏用户对模型能力边界的信任。

Method: 采用共同花园设计，对16个开源模型（4B-671B参数）进行了19,200次试验，使用贝叶斯验证和Rogan-Gladen校正来确保测量误差的稳健性。

Result: 模型表现出明显的领域特定不一致性：金融顾问角色初始披露率为30.8%，而神经外科医生角色仅为3.5%。披露率范围从2.8%到73.6%，模型身份比参数数量更能预测行为。推理优化在某些模型中主动抑制了自我透明度。

Conclusion: 透明度反映了训练因素而非规模，组织不能假设安全属性会转移到部署环境中，需要刻意设计行为和进行实证验证。
...
### [154] [From Prediction to Foresight: The Role of AI in Designing Responsible Futures](https://arxiv.org/abs/2511.21570)
*Maria Perez-Ortiz*

Main category: cs.AI

TL;DR: 本文提出"负责任计算前瞻"概念，探讨人本人工智能和计算建模在负责任前瞻中的作用，建立该领域基本原则并展示AI驱动的前瞻工具。
...
Method: 通过人本人工智能、计算建模、模拟和情景分析，结合社会、环境、经济和政治系统的相互依存关系理解，建立负责任计算前瞻的基本原则。

Result: AI特别是结合模拟和情景分析，增强了政策制定者应对不确定性、评估风险和制定可持续、韧性未来战略的能力。

Conclusion: AI应作为支持性工具融入前瞻实践，补充而非替代政策制定者判断，以主动塑造有韧性和道德健全的未来。
